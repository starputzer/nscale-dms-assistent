...
# from modules.rag.engine import RAGEngine  # Moved to conditional import below
# from modules.rag.compatibility_layer import get_compatibility_layer  # Moved to conditional import
...
from api.rag_health_check import router as rag_health_router
# from api.doc_converter_endpoints import router as doc_converter_router
doc_converter_router = None
from api.background_endpoints import app as background_app
...
# Commented out - using direct implementation instead
# from api.missing_endpoints import router as missing_router
# app.include_router(missing_router, prefix="/api/v1")

# System endpoints without /v1 for backward compatibility
# Commented out - not needed
# try:
#     from api.system_endpoints_no_v1 import router as system_no_v1_router
#     app.include_router(system_no_v1_router, prefix="/api")
# except Exception as e:
#     logger.warning(f"Could not import system_endpoints_no_v1: {e}")

# Doc converter endpoints without auth for backward compatibility
try:
    from api.doc_converter_no_auth import router as doc_converter_no_auth_router
    app.include_router(doc_converter_no_auth_router, prefix="/api")
except Exception as e:
    logger.warning(f"Could not import doc_converter_no_auth: {e}")

# Advanced document processing endpoints (Phase 2.7)
try:
    from api.advanced_document_endpoints import router as advanced_doc_router
    app.include_router(advanced_doc_router)
    logger.info("Advanced document processing endpoints loaded")
except ImportError as e:
    logger.warning(f"Could not import advanced_document_endpoints: {e}")

# Include RAG health check endpoints if optimization is enabled
if Config.USE_OPTIMIZED_RAG:
    app.include_router(rag_health_router)

# Mount background processing API if available
if background_app:
    app.mount("/api/background", background_app)

# Include new admin API routers if available
# NOTE: These routers are registered with specific prefixes later in the file
# Commenting out duplicate registrations to avoid conflicts
# if knowledge_router:
#     app.include_router(knowledge_router, prefix="/api")
# if rag_settings_router:
#     app.include_router(rag_settings_router, prefix="/api") 
# if system_monitor_router:
#     app.include_router(system_monitor_router, prefix="/api")
# if document_upload_router:
#     app.include_router(document_upload_router, prefix="/api")
if admin_system_router:
    app.include_router(admin_system_router, prefix="/api")
# if admin_dashboard_router:
#     app.include_router(admin_dashboard_router, prefix="/api")
# if admin_system_comprehensive_router:
#     app.include_router(admin_system_comprehensive_router, prefix="/api")
# if admin_dashboard_standard_router:
#     app.include_router(admin_dashboard_standard_router, prefix="/api")
# if knowledge_manager_router:
#     app.include_router(knowledge_manager_router, prefix="/api")
# if admin_statistics_router:
#     app.include_router(admin_statistics_router, prefix="/api")
# if doc_converter_enhanced_router:
#     app.include_router(doc_converter_enhanced_router, prefix="/api")
# if background_processing_router:
#     app.include_router(background_processing_router, prefix="/api")
# if advanced_documents_router:
#     app.include_router(advanced_documents_router, prefix="/api")
# if admin_users_router:
#     app.include_router(admin_users_router, prefix="/api")
# if admin_feedback_router:
...
# Always use compatibility layer to avoid import issues
try:
    from modules.rag.compatibility_layer import get_compatibility_layer
    logger.info(f"Using RAG Compatibility Layer")
    rag_engine = get_compatibility_layer()
except ImportError as e:
    logger.warning(f"Could not import compatibility layer: {e}")
    # Create a mock RAG engine for testing
    class MockRAGEngine:
        async def generate_response(self, *args, **kwargs):
            return {"answer": "Mock response - RAG engine not available", "sources": []}
        async def process_query(self, *args, **kwargs):
            return "Mock response - RAG engine not available"
    rag_engine = MockRAGEngine()
    logger.warning("Using mock RAG engine")
...
@app.get("/api/v1/admin/feature-toggles/{toggle_id}/history")
async def get_admin_feature_toggle_history(
    toggle_id: str,
    user_data: Dict[str, Any] = Depends(get_admin_user)
):
    """Gibt die Historie eines Feature-Toggles zurück (nur für Admins)"""
    # Mock implementation
    history = [
        {
...
@app.get("/api/v1/admin/feature-toggles/metrics")
async def get_admin_feature_toggles_metrics(
    start_date: Optional[str] = Query(None),
    end_date: Optional[str] = Query(None),
    user_data: Dict[str, Any] = Depends(get_admin_user)
):
    """Gibt Metriken für Feature-Toggles zurück (nur für Admins)"""
    # Mock implementation
    metrics = {
        "features": {
            "enhanced-rag-search": {
                ...
                "trend": [
                    {"date": "2024-01-19", "count": 50},
                    {"date": "2024-01-20", "count": 60},
                    {"date": "2024-01-21", "count": 55},
                    {"date": "2024-01-22", "count": 70},
                    {"date": "2024-01-23", "count": 65},
                    {"date": "2024-01-24", "count": 80},
                    {"date": "2024-01-25", "count": 70}
                ]
            },
            "multi-llm-support": {
                "usageCount": 120,
...
@app.get("/api/v1/admin/feature-toggles/errors")
async def get_admin_feature_toggles_errors(
    start_date: Optional[str] = Query(None),
    end_date: Optional[str] = Query(None),
    limit: int = Query(10),
    user_data: Dict[str, Any] = Depends(get_admin_user)
):
    """Gibt Fehlerprotokolle für Feature-Toggles zurück (nur für Admins)"""
    # Mock implementation
    errors = [
        {
...
@app.post("/api/v1/admin/feature-toggles/import")
async def import_admin_feature_toggles(
    data: dict,
    user_data: Dict[str, Any] = Depends(get_admin_user)
):
    """Importiert Feature-Toggles (nur für Admins)"""
    features = data.get("features", [])
    options = data.get("options", {})
    
    logger.info(f"Admin {user_data['email']} importiert {len(features)} Feature-Toggles")
    
    return {
        "success": True,
        "message": f"{len(features)} Feature-Toggles erfolgreich importiert",
        "imported": len(features)
    }

@app.get("/api/v1/admin/feature-toggles/stats")
async def get_admin_feature_toggles_stats(
    user_data: Dict[str, Any] = Depends(get_admin_user)
):
    """Gibt Statistiken für Feature-Toggles zurück (nur für Admins)"""
    # Enhanced statistics for the component
    stats = {
        "total": 8,
        "enabled": 2,
        "disabled": 6,
        ...
        "history": [
            {"date": "2024-01-19", "enabled": 1},
            {"date": "2024-01-20", "enabled": 1},
            {"date": "2024-01-21", "enabled": 2},
            {"date": "2024-01-22", "enabled": 2},
            {"date": "2024-01-23", "enabled": 2},
            {"date": "2024-01-24", "enabled": 2},
            {"date": "2024-01-25", "enabled": 2}
        ]
    }
...
async def startup_event():
    ...
    try:
        ...
        logger.info("Preloading LLM model to avoid cold start delays...")
        # Use create_task to run preload in background without blocking
...
# Additional missing endpoints for compatibility

@app.get("/api/admin-dashboard/summary")
async def get_admin_dashboard_summary(user_data: Dict[str, Any] = Depends(get_admin_user)):
    ...
    try:
        ...
        # Calculate stats
        total_users = len(all_users)
        active_users = sum(1 for u in all_users if u.get('last_login', 0) > time.time() - 86400)
        
        return {
            "users": {
                "total": total_users,
                "active": active_users,
                "new": sum(1 for u in all_users if u.get('created_at', 0) > time.time() - 86400 * 30)
            },
            "sessions": {
                "total": chat_history.get_total_sessions(),
                "today": 10  # Mock value
            },
            "documents": {
                "total": 50,  # Mock value
                "processed": 45  # Mock value
            },
            "system": {
                "status": "healthy",
                "uptime": "99.9%"
...
# ===========================
# V1 Chat Session Endpoints - Map to existing session endpoints
# ===========================

@app.get("/api/v1/chat/sessions")
async def get_v1_chat_sessions(
    since: Optional[int] = Query(None),
    user_data: Dict[str, Any] = Depends(get_current_user)
):
    """Get all chat sessions for the current user (V1 endpoint)"""
    user_id = user_data['user_id']
    sessions = chat_history.get_user_sessions(user_id)
    ...
    for session in sessions:
        transformed_sessions.append({
            'id': session.get('uuid', str(session['id'])),
...
async def create_v1_chat_session(
    request: Dict[str, Any],
    user_data: Dict[str, Any] = Depends(get_current_user)
):
    """Create a new chat session (V1 endpoint)"""
    return await create_session_from_frontend(request, user_data)

@app.get("/api/v1/chat/sessions/{session_id}")
async def get_v1_chat_session(
    session_id: str,
    user_data: Dict[str, Any] = Depends(get_current_user)
):
    """Get a specific chat session (V1 endpoint)"""
    user_id = user_data['user_id']
...
async def delete_v1_chat_session(
    session_id: str,
    user_data: Dict[str, Any] = Depends(get_current_user)
):
    """Delete a chat session (V1 endpoint)"""
    return await delete_session_uuid(session_id, user_data)

@app.get("/api/v1/chat/sessions/{session_id}/messages")
async def get_v1_chat_session_messages(
    session_id: str,
    user_data: Dict[str, Any] = Depends(get_current_user)
):
    """Get messages for a session (V1 endpoint)"""
    return await get_session_messages(session_id, user_data)

@app.post("/api/v1/chat/message")
async def send_v1_chat_message(
    request: Dict[str, Any],
    user_data: Dict[str, Any] = Depends(get_current_user)
):
    """Send a chat message (V1 endpoint)"""
    # Map to existing question endpoint
    question_request = QuestionRequest(
        question=request.get('message', request.get('content', '')),
        session_id=request.get('session_id', request.get('sessionId'))
    )
    return await ask_question(question_request, BackgroundTasks(), user_data)
...
try:
    ...
    logger.warning(f"Could not import batch_handler_fastapi: {e}")
    batch_router = None

# Include the document converter router
if doc_converter_router:
    app.include_router(doc_converter_router, prefix="/api/doc-converter", tags=["Document Converter"])

# Include the documentation router
if documentation_router:
    app.include_router(documentation_router, prefix="/api/documentation", tags=["Documentation"])

# Include the RAG health check router
if rag_health_router:
    app.include_router(rag_health_router, prefix="/api/rag", tags=["RAG Health"])

# Include additional admin API routers if they were imported successfully
if knowledge_router:
...
if system_monitor_router:
    app.include_router(system_monitor_router, prefix="/api/system-monitor", tags=["System Monitor"])
    
if document_upload_router:
    app.include_router(document_upload_router, prefix="/api/document-upload", tags=["Document Upload"])

if admin_dashboard_router:
    app.include_router(admin_dashboard_router, prefix="/api/admin-dashboard", tags=["Admin Dashboard Enhanced"])

if admin_system_comprehensive_router:
    app.include_router(admin_system_comprehensive_router, prefix="/api/admin-system-comprehensive", tags=["Admin System Comprehensive"])

if admin_dashboard_standard_router:
    app.include_router(admin_dashboard_standard_router, prefix="/api/admin-dashboard-standard", tags=["Admin Dashboard Standard"])

if knowledge_manager_router:
    app.include_router(knowledge_manager_router, prefix="/api/knowledge-manager", tags=["Knowledge Manager"])

if admin_statistics_router:
    app.include_router(admin_statistics_router, prefix="/api/admin-statistics", tags=["Admin Statistics"])

if doc_converter_enhanced_router:
    app.include_router(doc_converter_enhanced_router, prefix="/api/doc-converter-enhanced", tags=["Document Converter Enhanced"])

if background_processing_router:
    app.include_router(background_processing_router, prefix="/api/background-processing", tags=["Background Processing"])
...
if admin_feedback_router:
    app.include_router(admin_feedback_router, prefix="/api/admin/feedback", tags=["Admin Feedback"])

# Test router for debugging (only in development)
if test_router:
    app.include_router(test_router, prefix="/api/test", tags=["Test"])
    logger.info("Test router registered for debugging")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        port=Config.PORT,
        reload=True
    )# Force reload Wed Jun  4 01:48:35 PM CEST 2025
