...
# Revert config.ts to use traditional imports temporarily
echo "Reverting config.ts to fix import errors..."

# Create a backup of the current config
cp src/services/api/config.ts src/services/api/config.ts.shared-backup

# Create a temporary fixed config that doesn't use shared routes yet
cat > src/services/api/config.ts.temp << 'EOF'
/**
...
# Replace the file
mv src/services/api/config.ts.temp src/services/api/config.ts

echo "✅ Config.ts has been reverted to working version"
echo "📝 Backup saved as config.ts.shared-backup"