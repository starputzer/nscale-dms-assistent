...
try {
  ...
  isSourceReferencesComposable =
    typeof window.__sourceReferencesComposable !== "undefined";
} catch (err) {
  console.error(
    "Fehler beim Prüfen der Quellen-Referenzen-Verfügbarkeit:",
    err,
  );
}
...
window.isSourceReferencesVisible = function (message) {
  // Wenn das Composable verfügbar ist, nutzen wir es
  if (isSourceReferencesComposable && window.__sourceReferencesComposable) {
    return window.__sourceReferencesComposable.isSourceReferencesVisible(
      message,
    );
  }
...
console.log(
  "[Source References Bridge] Bridge-Funktionen für Quellenreferenzen wurden global registriert",
);

// Event-Listener für Vue-Initialisierung
document.addEventListener("vue-initialized", function () {
  console.log(
    "[Source References Bridge] Vue wurde initialisiert, prüfe Quellenreferenz-Composable",
  );
  ...
  try {
    ...
    if (isSourceReferencesComposable) {
      console.log(
        "[Source References Bridge] Quellenreferenz-Composable erfolgreich gefunden",
      );
    } else {
      console.warn(
        "[Source References Bridge] Quellenreferenz-Composable nicht gefunden",
      );
    }
  } catch (err) {
    console.error(
      "Fehler beim Prüfen der Quellen-Referenzen nach Vue-Initialisierung:",
      err,
    );
  }
});
