...
export function setupSourceReferences(options) {
    ...
    const showSourcePopupHandler = (event, sourceId) => {
        ...
        const currentMessage = messages.value.find(m => !m.is_user && m.message.includes(`Quelle-${sourceId}`));
        ...
        if (currentMessage) {
            ...
            const loadSourceInfo = async () => {
                try {
                    ...
                    if (currentExplanation.value && currentExplanation.value.source_references) {
                    ...
                    if (currentMessage.id) {
                        ...
                        const response = await axios.get(`/api/explain/${currentMessage.id}`);
    ...
    const loadExplanation = async (message) => {
        ...
        if (!message || !message.id) {
            ...
            currentExplanation.value = {
                ...
                explanation_text: "Diese Nachricht kann nicht erklärt werden, da keine gültige ID vorhanden ist.",
                source_references: []
            };
            return;
        }
        ...
        try {
            ...
            currentExplanation.value = {
                ...
                source_references: [],
                explanation_text: "Es ist ein Fehler bei der Erklärung aufgetreten. Möglicherweise stehen für diese Antwort keine Quellen zur Verfügung."
            };
        } finally {
            explanationLoading.value = false;
        }
