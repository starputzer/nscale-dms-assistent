...
test.describe('Visual Regression Tests', () => {
  ...
  test('Session-Liste sollte visuell konsistent sein', async ({ page }) => {
    ...
    await expect(page.locator('.session-list')).toHaveScreenshot('session-list.png');
  });
  
  // Screenshot-Vergleich für die Dokumentenkonverter-Oberfläche
  test('Dokumentenkonverter-Oberfläche sollte visuell konsistent sein', async ({ page }) => {
  ...
  test('Admin-Panel-Oberfläche sollte visuell konsistent sein', async ({ page }) => {
    // Administrator-Anmeldedaten verwenden
    await page.context().storageState({ path: './e2e/fixtures/admin-auth.json' });
    await page.goto('/');
    ...
    // Screenshot erstellen und mit Baseline vergleichen
    await expect(page.locator('.admin-panel')).toHaveScreenshot('admin-panel.png');
  });
