...
test.describe("Systemeinstellungen im Admin-Bereich", () => {
  ...
  test("Ändern des System-Loglevels", async ({ page }) => {
    ...
    const initialLoglevel = await page
      .locator('select[name="loglevel"]')
      .inputValue();
  ...
  test("Konfiguration von API-Timeouts", async ({ page }) => {
    ...
    const initialApiTimeout = await page
      .locator('input[name="apiTimeout"]')
      .inputValue();
  ...
  test("Konfiguration der maximalen Dokumentgröße", async ({ page }) => {
    ...
    const initialMaxDocSize = await page
      .locator('input[name="maxDocumentSize"]')
      .inputValue();
    ...
    await expect(page.locator('input[name="maxDocumentSize"]')).toHaveValue(
      "20",
    );
  ...
  test("Aktivieren und Deaktivieren der erweiterten Fehlerprotokollierung", async ({
    page,
  }) => {
    const adminPage = new AdminPage(page);
    await page.goto("/");
    ...
    const enableDetailedLogging = await page
      .locator('input[name="enableDetailedLogging"]')
      .isChecked();

    // Status umkehren
    await page
      .locator('input[name="enableDetailedLogging"]')
      .setChecked(!enableDetailedLogging);
    ...
    await expect(
      page.locator('input[name="enableDetailedLogging"]'),
    ).toBeChecked({ checked: !enableDetailedLogging });
  ...
  test("Aktivieren und Deaktivieren des Wartungsmodus", async ({ page }) => {
    ...
    const maintenanceMode = await page
      .locator('input[name="maintenanceMode"]')
      .isChecked();
    ...
    await expect(
      page.locator('input[name="maintenanceMode"]'),
    ).not.toBeChecked();
