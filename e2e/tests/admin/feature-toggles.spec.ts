...
test.describe("Feature-Toggles im Admin-Bereich", () => {
  ...
  test("Auswirkung eines Feature-Toggles auf die Anwendung", async ({
    page,
  }) => {
    const adminPage = new AdminPage(page);
    await page.goto("/");
  ...
  test("Speichern und Anwenden mehrerer Feature-Toggles gleichzeitig", async ({
    page,
  }) => {
    const adminPage = new AdminPage(page);
    await page.goto("/");
    ...
    const sfcDocConverterInitialState = await page
      .locator('input[name="useSfcDocConverter"]')
      .isChecked();
    const sfcChatInitialState = await page
      .locator('input[name="useSfcChat"]')
      .isChecked();

    // Beide Feature-Toggles ändern
    await page
      .locator('input[name="useSfcDocConverter"]')
      .setChecked(!sfcDocConverterInitialState);
    await page
      .locator('input[name="useSfcChat"]')
      .setChecked(!sfcChatInitialState);
    ...
    await expect(page.locator('input[name="useSfcDocConverter"]')).toBeChecked({
      checked: !sfcDocConverterInitialState,
    });
    await expect(page.locator('input[name="useSfcChat"]')).toBeChecked({
      checked: !sfcChatInitialState,
    });
