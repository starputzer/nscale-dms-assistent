...
test.describe("Fehlerbehandlung bei Netzwerkproblemen", () => {
  ...
  test("Fehlerbehandlung bei API-Fehler während des Nachrichtensendens", async ({
    page,
  }) => {
    ...
    await expect(page.locator(".message-item.message-user")).toContainText(
      "Testnachricht mit Netzwerkfehler",
    );
  });

  // Test für die Fehlerbehandlung bei Timeout während des Nachrichtensendens
  test("Fehlerbehandlung bei Timeout während des Nachrichtensendens", async ({
    page,
  }) => {
    const chatPage = new ChatPage(page);
    await chatPage.goto();
    ...
    await expect(
      page.locator('.error-message:has-text("Timeout")'),
  ...
  test("Automatische Wiederherstellung nach einem Netzwerkfehler", async ({
    page,
  }) => {
    const chatPage = new ChatPage(page);
    await chatPage.goto();
    ...
    await expect(page.locator(".message-item.message-user")).toContainText(
      "Testnachricht im Offline-Modus",
    );
  ...
  test("Fehlerbehandlung bei Verbindungsverlust während des Streamings", async ({
    page,
  }) => {
    const chatPage = new ChatPage(page);
    await chatPage.goto();
    ...
    await expect(page.locator(".streaming-error")).toBeVisible({
      timeout: 30000,
    });
    ...
    await expect(
      page.locator(".message-item.message-assistant.streaming"),
    ).toBeVisible();
