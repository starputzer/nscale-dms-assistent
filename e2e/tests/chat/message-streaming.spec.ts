...
test.describe("Chat-Nachrichten Streaming", () => {
  ...
  test("Streaming von Assistentenantworten", async ({ page }) => {
    ...
    const testMessage =
      "Erkläre mir die gesamte Geschichte der künstlichen Intelligenz in mindestens 500 Wörtern";
  ...
  test("Unterbrechen des Streaming-Prozesses", async ({ page }) => {
    ...
    const testMessage =
      "Schreibe mir eine ausführliche Geschichte über Künstliche Intelligenz und deren Entwicklung";
    await chatPage.sendMessage(testMessage);
  ...
  test("Wiederherstellen der Verbindung nach einem Netzwerkfehler", async ({
    page,
  }) => {
    const chatPage = new ChatPage(page);
    await chatPage.goto();
    ...
    await chatPage.sendMessage(
      "Dies ist eine Nachricht während der Offline-Phase",
    );
    ...
    await chatPage.sendMessage(
      "Dies ist eine Nachricht nach der Wiederverbindung",
    );
  ...
  test("Automatisches Weiterschreiben bei unterbrochener Verbindung", async ({
    page,
  }) => {
    const chatPage = new ChatPage(page);
    await chatPage.goto();
    ...
    // Eine Nachricht senden, die eine lange Antwort erfordert
    const testMessage =
      "Erkläre mir die Geschichte der modernen Programmiersprachen";
    await chatPage.sendMessage(testMessage);
