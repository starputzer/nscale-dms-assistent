...
test.describe('Grundlegender Chat-Flow', () => {
  ...
  test('Senden einer Nachricht und Empfangen einer Antwort', async ({ page }) => {
    ...
    await expect(page.locator('.message-item.message-user').last()).toContainText(testMessage);
  ...
  test('Erstellen einer neuen Chat-Session', async ({ page }) => {
    ...
    await expect(page.locator('.session-item')).toHaveCount(initialSessionCount + 1);
  ...
  test('Umbenennen einer Chat-Session', async ({ page }) => {
    ...
    await expect(page.locator('.session-item').first()).toContainText(newSessionTitle);
  });
  ...
  test('Wechseln zwischen Chat-Sessions', async ({ page }) => {
    ...
    await expect(page.locator('.message-item.message-user')).toContainText(firstSessionMessage);
    ...
    await expect(page.locator('.message-item.message-user')).toContainText(secondSessionMessage);
  });
  ...
  test('Löschen einer Chat-Session', async ({ page }) => {
    ...
    await expect(page.locator('.session-item')).toHaveCount(initialSessionCount);
  });
});