 ...
 */
export async function setupMobileContext(
...
export async function swipe(
  ...
  for (let i = 1; i <= steps; i++) {
    await page.mouse.move(startX + stepX * i, startY + stepY * i, { steps: 1 });
  }
      ...
      const touchStartEvent = new TouchEvent("touchstart", {
        ...
        touches: [
          new Touch({
            identifier: 0,
            target:
              document.elementFromPoint(centerX - 50, centerY - 50) ||
              document.body,
            clientX: centerX - 50,
            clientY: centerY - 50,
            pageX: centerX - 50,
 ...
 */
export async function hasSufficientTouchTargetSize(
  locator: Locator,
): Promise<boolean> {
  const box = await locator.boundingBox();
...
export const devices = {
  ...
  "Galaxy S20": {
    userAgent:
      "Mozilla/5.0 (Linux; Android 11; SM-G980F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.50 Mobile Safari/537.36",
    viewport: {
      width: 360,
      height: 800,
    },
