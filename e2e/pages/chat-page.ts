...
export class ChatPage {
  ...
  constructor(page: Page) {
    ...
    // Chat-Elemente
    this.messageInput = page.locator('[data-testid="message-input"], textarea.message-input');
    this.sendButton = page.locator('[data-testid="send-button"], button.send-message-btn');
    this.messageList = page.locator('[data-testid="message-list"], .message-list');
    this.messageItems = page.locator('[data-testid="message-item"], .message-item');
    this.userMessages = page.locator('.message-item.user, .message-item[data-role="user"]');
    this.assistantMessages = page.locator('.message-item.assistant, .message-item[data-role="assistant"]');
    
    // Session-Elemente
    this.sessionListContainer = page.locator('[data-testid="session-list"], .session-list');
    this.newSessionButton = page.locator('[data-testid="new-session-button"], button.new-session-btn');
    this.sessionItems = page.locator('[data-testid="session-item"], .session-item');
    this.sessionNameDisplay = page.locator('.session-name, .current-session-title');
    
    // Navigation und UI-Elemente
    this.userProfileButton = page.locator('.user-avatar, .user-profile-button');
    this.logoutButton = page.locator('button[aria-label="Logout"], .logout-button');
    this.loadingIndicator = page.locator('.loading-indicator, .spinner');
    this.errorDisplay = page.locator('.error-message, .error-display');
    this.retryButton = page.locator('button[aria-label="Retry"], .retry-button');
    this.clearChatButton = page.locator('button[aria-label="Clear chat"], .clear-chat-button');
    this.settingsButton = page.locator('button[aria-label="Settings"], .settings-button');
    this.searchButton = page.locator('button[aria-label="Search"], .search-button');
  }
  ...
  async waitForAssistantResponse(timeout = 30000) {
    ...
    await Promise.race([
      ...
      this.page.waitForFunction(() => {
        ...
        const messages = document.querySelectorAll('.message-item.assistant, .message-item[data-role="assistant"]');
        return messages.length > 0 && messages[messages.length - 1].textContent?.trim().length > 0;
      }, { timeout })
    ]);
  ...
  async switchToSession(sessionIndex: number) {
    ...
    if (sessionIndex < 0 || sessionIndex >= count) {
      throw new Error(`Session mit Index ${sessionIndex} nicht gefunden. Verfügbare Sessions: ${count}`);
    }
  ...
  async openSettings() {
    ...
    await this.page.waitForSelector('.settings-panel, [data-testid="settings-panel"]', {
      state: 'visible',
      timeout: 5000
    });
  }
  ...
  async clearChat() {
    ...
    const confirmButton = this.page.locator('.confirm-dialog button.confirm, [data-testid="confirm-button"]');
