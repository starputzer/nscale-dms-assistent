...
export class LoginPage {
  ...
  constructor(page: Page) {
    ...
    // Tab-Elemente
    this.loginTab = page.locator('.auth-tabs .tab-item').filter({ hasText: 'Anmelden' });
    this.registerTab = page.locator('.auth-tabs .tab-item').filter({ hasText: 'Registrieren' });
    
    // Registrierungsspezifische Elemente
    this.passwordStrengthIndicator = page.locator('.password-strength-indicator .indicator-bar');
  ...
  async expectFormValidation() {
    ...
    const emailError = this.page.locator('input[name="email"] + .error-message, input[name="email"][aria-invalid="true"]');
    const passwordError = this.page.locator('input[name="password"] + .error-message, input[name="password"][aria-invalid="true"]');
   ...
   */
  async checkPasswordStrength(password: string, expectedStrength: 'weak' | 'medium' | 'strong' | 'very-strong') {
    // Zum Registrierungs-Tab wechseln
    await this.registerTab.click();
    ...
    await expect(this.passwordStrengthIndicator).toHaveClass(new RegExp(strengthClasses[expectedStrength]));
  }
  
  /**
   * Überprüft die Übereinstimmung der Passwörter.
   */
  async checkPasswordMatch(password: string, confirmPassword: string, shouldMatch: boolean) {
    // Zum Registrierungs-Tab wechseln
    await this.registerTab.click();
