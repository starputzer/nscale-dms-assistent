...
export class AdminPage {
  ...
  async deleteUser(username: string) {
    ...
    await expect(this.page.locator(this.userRows).filter({ hasText: username })).not.toBeVisible();
  }
  ...
  async toggleFeature(featureName: string, enable: boolean) {
    ...
    const featureRow = this.page.locator(this.featureToggleRows).filter({ hasText: featureName });
  ...
  async expectUserExists(username: string) {
    await this.switchToTab('users');
    await expect(this.page.locator(this.userRows).filter({ hasText: username })).toBeVisible();
  }
