...
export class DocumentConverterPage {
  ...
  constructor(page: Page) {
    ...
    this.desktopLayout = page.locator(
      ".doc-converter-container.desktop-layout",
    );
    ...
    this.desktopStepsVisualization = page.locator(
      ".conversion-progress__steps:not(.conversion-progress__steps--mobile)",
    );
    this.mobileStepsVisualization = page.locator(
  ...
  async openDocumentPreview(documentIndex: number) {
    ...
    if (documentIndex >= 0 && documentIndex < count) {
      ...
      throw new Error(
        `Dokument mit Index ${documentIndex} existiert nicht. Verfügbare Dokumente: ${count}`,
      );
    }
  }
  ...
  async expectDocumentExists(documentName: string) {
    await expect(
      this.page.locator(this.documentItems).filter({ hasText: documentName }),
    ).toBeVisible();
  }
