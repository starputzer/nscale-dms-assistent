...
describe("Theme Switching", () => {
  ...
  describe("Basic Theme Functionality", () => {
    ...
    it("sets correct CSS variables for each theme", async () => {
      ...
      const lightBg = getComputedCssVariable(
        document.documentElement,
        "--background-color",
      );
      const lightText = getComputedCssVariable(
        document.documentElement,
        "--text-color",
      );
      ...
      const darkBg = getComputedCssVariable(
        document.documentElement,
        "--background-color",
      );
      const darkText = getComputedCssVariable(
        document.documentElement,
        "--text-color",
      );
  ...
  describe("Theme Selector Component", () => {
    ...
    it("emits update event when selecting a theme", async () => {
      ...
      const themeOptions = themeSelector.findAll(".theme-option");
      const darkOption = themeOptions.find((option) =>
        option.text().includes("theme.dark"),
      );
  ...
  describe("Theme Integration in Complex Components", () => {
    it("applies theme correctly to ChatView", async () => {
      ...
      if (messageItems.length > 0) {
        ...
        const userMessage = messageItems.find((msg) =>
          msg.classes().includes("message-user"),
        );
        if (userMessage) {
        ...
        const assistantMessage = messageItems.find((msg) =>
          msg.classes().includes("message-assistant"),
        );
        if (assistantMessage) {
          expect(assistantMessage.classes()).toContain(
    ...
    it("applies theme correctly to AdminPanel", async () => {
      ...
      expect(adminPanel.find(".admin-panel__sidebar--dark").exists()).toBe(
        true,
      );
    ...
    it("applies theme correctly to SettingsPanel", async () => {
      ...
      const settingsPanel = createWrapper(
        SettingsPanel,
        {
          isVisible: true,
        },
  ...
  describe("Color Scheme Transitions", () => {
    it("applies transition styles when changing themes", async () => {
      ...
      const transitionBefore =
        document.documentElement.style.getPropertyValue("transition");
  ...
  describe("Theme-specific Images and Assets", () => {
    ...
    it("applies appropriate icon colors based on theme", async () => {
      ...
      if (lightIcons.length > 0) {
        const lightIconColor = getComputedCssVariable(
          lightIcons[0].element,
          "color",
        );
        ...
        if (darkIcons.length > 0) {
          const darkIconColor = getComputedCssVariable(
            darkIcons[0].element,
            "color",
          );
