    ...
    value: vi.fn().mockImplementation((query) => ({
      matches:
        ...
        (query.includes("max-width") &&
          parseInt(query.match(/max-width:\s*(\d+)/)?.[1] || "0") >= width),
      media: query,
      onchange: null,
...
describe("Responsive Behavior", () => {
  ...
  describe("App Layout", () => {
    it("adapts layout based on viewport size", async () => {
      // Test on desktop
      const cleanupDesktop = simulateViewport(
      ...
      expect(desktopWrapper.find(".app-container--desktop").exists()).toBe(
        true,
      );
      expect(desktopWrapper.find(".app-container--mobile").exists()).toBe(
        false,
      );
      ...
      // Test on mobile
      const cleanupMobile = simulateViewport(
      ...
      expect(mobileWrapper.find(".app-container--desktop").exists()).toBe(
        false,
      );
    ...
    it("handles orientation changes", async () => {
      ...
      const cleanupPortrait = simulateViewport(
      ...
      expect(portraitWrapper.find(".app-container--tablet").exists()).toBe(
        true,
      );
      expect(portraitWrapper.find(".app-container--landscape").exists()).toBe(
        false,
      );
      ...
      const cleanupLandscape = simulateViewport(
        viewports.tablet.height,
        viewports.tablet.width,
      );
      ...
      expect(landscapeWrapper.find(".app-container--landscape").exists()).toBe(
        true,
      );
  ...
  describe("Sidebar Component", () => {
    ...
    it("adjusts content based on available space", async () => {
      ...
      expect(desktopWrapper.find(".sidebar__menu-item-text").exists()).toBe(
        true,
      );
      ...
      const cleanupTablet = simulateViewport(
        viewports.tablet.width,
        viewports.tablet.height,
      );
      ...
      expect(tabletWrapper.find(".sidebar__menu-item-text").exists()).toBe(
        true,
      );
      ...
      if (mobileWrapper.find(".sidebar--collapsed").exists()) {
        expect(mobileWrapper.find(".sidebar__menu-item-text").isVisible()).toBe(
          false,
        );
        expect(mobileWrapper.find(".sidebar__menu-item-icon").exists()).toBe(
          true,
        );
      }
  ...
  describe("Chat Interface", () => {
    ...
    it("shows/hides additional UI elements based on viewport size", async () => {
      ...
      expect(mobileWrapper.find(".session-actions--collapsed").exists()).toBe(
        true,
      );
  ...
  describe("Admin Panel", () => {
    it("adapts layout for different viewport sizes", async () => {
      ...
      expect(mobileWrapper.find(".admin-panel__mobile-nav").exists()).toBe(
        true,
      );

      // Test mobile navigation toggle
      await mobileWrapper
        .find(".admin-panel__mobile-nav-toggle")
        .trigger("click");
      await flushPromises();

      // Navigation should expand
      expect(
        mobileWrapper.find(".admin-panel__sidebar--expanded").exists(),
      ).toBe(true);
  ...
  describe("Settings Panel", () => {
    it("adapts layout for different viewport sizes", async () => {
      ...
      // Desktop should have side-by-side layout
      expect(desktopWrapper.find(".settings-panel--desktop").exists()).toBe(
        true,
      );
      expect(desktopWrapper.find(".settings-panel__categories").exists()).toBe(
        true,
      );
      expect(desktopWrapper.find(".settings-panel__content").exists()).toBe(
        true,
      );
      ...
      expect(
        mobileWrapper
          .find(".settings-panel__mobile-category-selector")
          .exists(),
      ).toBe(true);
  ...
  describe("Navigation Elements", () => {
    it("collapses navigation menu on small viewports", async () => {
      ...
      expect(mobileWrapper.find(".nav-links--mobile-hidden").exists()).toBe(
        true,
      );
      ...
      // Menu should expand
      expect(mobileWrapper.find(".nav-links--mobile-visible").exists()).toBe(
        true,
      );
    ...
    it("shows different UI elements on different viewports", async () => {
      ...
      expect(mobileWrapper.find(".mobile-menu-user-profile").exists()).toBe(
        true,
      );
  ...
  describe("Document Viewer", () => {
    it("adapts document display for different viewport sizes", async () => {
      ...
      // Desktop should have multi-column layout
      expect(desktopWrapper.find(".documents-view--desktop").exists()).toBe(
        true,
      );
      ...
      if (mobileWrapper.findAll(".document-item").length > 0) {
        ...
        expect(mobileWrapper.find(".document-preview--mobile").exists()).toBe(
          true,
        );
        expect(
          mobileWrapper.find(".document-preview--mobile-visible").exists(),
        ).toBe(true);
        ...
        expect(
          mobileWrapper.find(".document-list--mobile-visible").exists(),
        ).toBe(true);
      }
    ...
    it("adjusts document controls for different viewport sizes", async () => {
      ...
      expect(
        desktopWrapper.findAll(".document-action-button").length,
      ).toBeGreaterThan(1);
      ...
      expect(mobileWrapper.find(".document-controls--mobile").exists()).toBe(
        true,
      );

      // Less used actions should be in overflow menu
      expect(mobileWrapper.find(".document-actions-overflow").exists()).toBe(
        true,
      );
