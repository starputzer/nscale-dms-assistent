...
describe("UI Components Visual Regression", () => {
  ...
  describe("Button Component", () => {
    it("renders in various variants and states", async () => {
      ...
      expect(secondaryButton.find(".nscale-btn--secondary").exists()).toBe(
        true,
      );
  ...
  describe("Input Component", () => {
    it("renders in various states", async () => {
      ...
      expect(errorInput.find(".nscale-input__error-message").text()).toBe(
  ...
  describe("Checkbox Component", () => {
    it("renders in various states", async () => {
      ...
      expect(
        disabledCheckbox.find("input").attributes("disabled"),
      ).toBeDefined();
      ...
      expect(
        disabledCheckedCheckbox.find("input").attributes("disabled"),
      ).toBeDefined();
  ...
  describe("Toggle Component", () => {
    it("renders in various states", async () => {
      ...
      expect(disabledToggle.find(".nscale-toggle--disabled").exists()).toBe(
        true,
      );
      ...
      expect(disabledActiveToggle.find(".nscale-toggle--active").exists()).toBe(
        true,
      );
      expect(
        disabledActiveToggle.find(".nscale-toggle--disabled").exists(),
      ).toBe(true);
    });
  ...
  describe("Modal Component", () => {
    it("renders when visible with various content", async () => {
      ...
      // Hidden modal
      const hiddenModal = createWrapper(
        Modal,
        {
          title: "Hidden Modal",
          isVisible: false,
      ...
      expect(closeableModal.find(".nscale-modal__close-button").exists()).toBe(
        true,
      );
  ...
  describe("Card Component", () => {
    it("renders with different content and styles", async () => {
      ...
      // Card without title
      const noTitleCard = createWrapper(
        Card,
        {},
        {
          slots: {
      ...
      expect(noTitleCard.find(".nscale-card__content").text()).toBe(
        "Content only",
      );

      // Card with different variants
      const borderedCard = createWrapper(
        Card,
        {
          title: "Bordered Card",
          variant: "bordered",
      ...
      const elevatedCard = createWrapper(
        Card,
        {
          title: "Elevated Card",
          variant: "elevated",
  ...
  describe("Tabs Component", () => {
    it("renders tabs with different states and selections", async () => {
      ...
      expect(tabsComponent.find(".nscale-tabs__tab--active").text()).toBe(
        "First Tab",
      );
      ...
      expect(tabsWithDisabled.find(".nscale-tabs__tab--disabled").text()).toBe(
        "Disabled Tab",
      );
  ...
  describe("Alert Component", () => {
    it("renders with different variants and content", async () => {
      ...
      expect(alert.find(".nscale-alert__message").text()).toBe(
      ...
      expect(dismissibleAlert.find(".nscale-alert__dismiss").exists()).toBe(
        true,
      );
  ...
  describe("ProgressBar Component", () => {
    it("renders with different values and variants", async () => {
      ...
      expect(
        progressBar.find(".nscale-progress__bar").attributes("style"),
      ).toContain("width: 50%");
      ...
      expect(
        successProgressBar.find(".nscale-progress--success").exists(),
      ).toBe(true);
      expect(
        successProgressBar.find(".nscale-progress__bar").attributes("style"),
      ).toContain("width: 75%");
      ...
      expect(
        warningProgressBar.find(".nscale-progress--warning").exists(),
      ).toBe(true);
      ...
      expect(errorProgressBar.find(".nscale-progress--error").exists()).toBe(
        true,
      );
      ...
      expect(
        indeterminateProgressBar
          .find(".nscale-progress--indeterminate")
          .exists(),
      ).toBe(true);
      ...
      expect(labeledProgressBar.find(".nscale-progress__label").exists()).toBe(
        true,
      );
      expect(
        labeledProgressBar.find(".nscale-progress__label").text(),
      ).toContain("40%");
    });
  });
  ...
  describe("StatusIndicator Component", () => {
    it("renders with different states", async () => {
      ...
      expect(successIndicator.find(".nscale-status-indicator").exists()).toBe(
        true,
      );
      expect(
        successIndicator.find(".nscale-status-indicator--success").exists(),
      ).toBe(true);
      expect(
        successIndicator.find(".nscale-status-indicator__label").text(),
      ).toBe("Completed");
      ...
      expect(
        warningIndicator.find(".nscale-status-indicator--warning").exists(),
      ).toBe(true);
      ...
      expect(
        errorIndicator.find(".nscale-status-indicator--error").exists(),
      ).toBe(true);
      ...
      expect(
        infoIndicator.find(".nscale-status-indicator--info").exists(),
      ).toBe(true);
      ...
      expect(
        loadingIndicator.find(".nscale-status-indicator--loading").exists(),
      ).toBe(true);
      expect(
        loadingIndicator.find(".nscale-status-indicator__spinner").exists(),
      ).toBe(true);
    });
  });
  ...
  describe("InlineMessage Component", () => {
    it("renders with different variants and content", async () => {
      ...
      expect(defaultMessage.find(".nscale-inline-message--info").exists()).toBe(
        true,
      );
      ...
      expect(
        successMessage.find(".nscale-inline-message--success").exists(),
      ).toBe(true);
      ...
      expect(
        warningMessage.find(".nscale-inline-message--warning").exists(),
      ).toBe(true);
      ...
      expect(errorMessage.find(".nscale-inline-message--error").exists()).toBe(
        true,
      );
      ...
      expect(titledMessage.find(".nscale-inline-message__title").exists()).toBe(
        true,
      );
      expect(titledMessage.find(".nscale-inline-message__title").text()).toBe(
        "Important",
      );
      ...
      expect(iconMessage.find(".nscale-inline-message__icon").exists()).toBe(
        true,
      );
    });
  });
  ...
  describe("Banner Component", () => {
    it("renders with different variants and content", async () => {
      ...
      expect(dismissibleBanner.find(".nscale-banner__dismiss").exists()).toBe(
        true,
      );
      ...
      expect(titledBanner.find(".nscale-banner__title").text()).toBe(
        "Announcement",
      );
      ...
      expect(actionBanner.find(".nscale-banner__action").text()).toBe(
        "Learn More",
      );
    });
  ...
  describe("Theme Integration", () => {
    it("applies theme classes correctly", async () => {
      ...
      const lightButton = createWrapper(
        Button,
        {
          variant: "primary",
      ...
      const darkButton = createWrapper(
        Button,
        {
          variant: "primary",
