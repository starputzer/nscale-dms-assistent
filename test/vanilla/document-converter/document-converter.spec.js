...
describe('Document Converter', () => {
  ...
  beforeEach(() => {
    ...
    documentConverterService = {
      ...
      // Conversion
      convertDocument: vi.fn().mockImplementation(async (documentId, settings, onProgress) => {
        ...
        return {
          ...
          metadata: {
            title: 'Test Document',
            pageCount: 5,
            ...(settings?.extractMetadata ? { author: 'Test Author', created: new Date() } : {})
          }
        };
      }),
      ...
      // Downloads
      downloadDocument: vi.fn().mockImplementation(async (documentId, filename, onProgress) => {
        if (documentId === 'doc-456') {
        ...
        return new Blob(['Fake document content'], { type: 'application/pdf' });
      })
    };
  ...
  describe('File Upload', () => {
    it('should upload a file and return a document ID', async () => {
      ...
      const documentId = await documentConverterService.uploadDocument(testFile, progressCallback);
      
      // Verify behavior
      expect(documentId).toBe('doc-123');
      expect(documentConverterService.uploadDocument).toHaveBeenCalledWith(testFile, progressCallback);
    ...
    it('should handle upload errors', async () => {
      // Set up error
      documentConverterService.uploadDocument.mockRejectedValueOnce(new Error('Upload failed'));
      
      // Create a test file
      const testFile = new File(['test content'], 'test-document.pdf', { type: 'application/pdf' });
      
      // Call upload function and expect error
      await expect(documentConverterService.uploadDocument(testFile)).rejects.toThrow('Upload failed');
    });
  });
  ...
  describe('Document Conversion', () => {
    it('should convert a document with default settings', async () => {
      ...
      expect(documentConverterService.convertDocument).toHaveBeenCalledWith('doc-123', undefined, undefined);
    });
    ...
    it('should convert a document with custom settings', async () => {
      ...
      // Call convert function
      const result = await documentConverterService.convertDocument('doc-123', settings, progressCallback);
      ...
      expect(documentConverterService.convertDocument).toHaveBeenCalledWith('doc-123', settings, progressCallback);
      expect(progressCallback).toHaveBeenCalledTimes(5);
      expect(progressCallback).toHaveBeenLastCalledWith(100, 'Conversion complete', 0);
    });
    
    it('should handle conversion errors', async () => {
      // Call convert function and expect error
      await expect(documentConverterService.convertDocument('doc-456')).rejects.toThrow('Document conversion failed');
    });
  });
  ...
  describe('Document Management', () => {
    ...
    it('should handle error when retrieving non-existent document', async () => {
      ...
      await expect(documentConverterService.getDocument('non-existent')).rejects.toThrow('Document non-existent not found');
    });
    ...
    it('should delete a document', async () => {
      ...
      // Verify behavior
      expect(documentConverterService.deleteDocument).toHaveBeenCalledWith('doc-123');
    });
  });
  ...
  describe('Conversion Status', () => {
    it('should retrieve conversion status for a successful document', async () => {
      ...
      const status = await documentConverterService.getConversionStatus('doc-123');
    ...
    it('should retrieve conversion status for a failed document', async () => {
      // Call get conversion status function
      const status = await documentConverterService.getConversionStatus('doc-456');
    ...
    it('should retrieve conversion status for a processing document', async () => {
      // Call get conversion status function
      const status = await documentConverterService.getConversionStatus('doc-789');
    ...
    it('should cancel a conversion', async () => {
      ...
      // Verify behavior
      expect(documentConverterService.cancelConversion).toHaveBeenCalledWith('doc-789');
    });
  });
  ...
  describe('Document Download', () => {
    it('should download a document', async () => {
      ...
      const blob = await documentConverterService.downloadDocument('doc-123', 'downloaded-document.pdf', progressCallback);
      
      // Verify behavior
      expect(blob).toBeInstanceOf(Blob);
      expect(blob.type).toBe('application/pdf');
      expect(documentConverterService.downloadDocument).toHaveBeenCalledWith('doc-123', 'downloaded-document.pdf', progressCallback);
      expect(progressCallback).toHaveBeenCalledTimes(3);
      expect(progressCallback).toHaveBeenLastCalledWith(100);
    });
    ...
    it('should handle download errors', async () => {
      ...
      await expect(documentConverterService.downloadDocument('doc-456')).rejects.toThrow('Document download failed');
    });
  });
  ...
  describe('Document Conversion Workflow', () => {
    it('should handle a full document conversion workflow', async () => {
      ...
      const documentId = await documentConverterService.uploadDocument(testFile, uploadProgress);
      expect(documentId).toBe('doc-123');
      ...
      const status = await documentConverterService.getConversionStatus(documentId);
      ...
      expect(documentConverterService.deleteDocument).toHaveBeenCalledWith(documentId);
    });
  });
});