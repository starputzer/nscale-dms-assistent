...
describe("Basis UI-Komponenten Barrierefreiheitstests", () => {
  ...
  describe("Button", () => {
    it("sollte barrierefrei sein (primär)", async () => {
      ...
      // Prüfe grundlegende a11y-Attribute
      const button = wrapper.find("button").element;
      expect(button).toBeTruthy();
      expect(button.textContent).toBe("Primärer Button");
    ...
    it("sollte barrierefrei sein (mit Icon)", async () => {
      ...
      // Prüfe, ob der Text vorhanden ist
      expect(button.textContent.trim()).toBe("Speichern");

      // Prüfe, ob das Icon aria-hidden hat
      const icon = wrapper.find(".icon") || wrapper.find(".button-icon");
      if (icon.exists()) {
