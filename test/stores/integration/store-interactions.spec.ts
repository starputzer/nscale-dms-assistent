...
import { mockDate, waitForPromises, mockLocalStorage } from '../__setup__/testSetup';
...
describe('Store Interaktionen', () => {
  ...
  describe('Auth und Sessions Interaktionen', () => {
    it('sollte pendente Nachrichten synchronisieren, wenn der Benutzer sich anmeldet', async () => {
      ...
      const syncSpy = vi.spyOn(sessionsStore, 'syncPendingMessages').mockResolvedValueOnce();
    ...
    it('sollte Tokens aktualisieren, wenn ein 401-Fehler bei Sessions-Anfragen auftritt', async () => {
      ...
      authStore.user = { id: 'user-1', email: 'test@example.com', roles: ['user'] };
  ...
  describe('DocumentConverter und UI Interaktionen', () => {
    it('sollte bei Dokumentenfehler eine Benachrichtigung anzeigen', async () => {
      ...
      // Act
      converterStore.setError('CONVERSION_FAILED', 'Dokument konnte nicht konvertiert werden');
      
      // UI-Benachrichtigung anzeigen
      if (converterStore.error) {
        uiStore.showError(`${converterStore.error.code}: ${converterStore.error.message}`);
      }
      
      // Assert
      expect(converterStore.error).not.toBeNull();
      expect(showErrorSpy).toHaveBeenCalledWith('CONVERSION_FAILED: Dokument konnte nicht konvertiert werden');
    });
    ...
    it('sollte während der Konvertierung einen Ladeindikator anzeigen', async () => {
      ...
      if (converterStore.isConverting) {
        uiStore.setLoading(true, `Konvertierung: ${converterStore.conversionStep} (${converterStore.conversionProgress}%)`);
      } else {
