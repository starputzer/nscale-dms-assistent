...
const createArrayOfSize = <T>(size: number, creator: (index: number) => T): T[] => 
  Array.from({ length: size }, (_, i) => creator(i));

/**
 * Performance-Messung mit High-Resolution-Timer
 */
const measurePerformance = async (fn: () => Promise<void> | void, iterations: number = 1): Promise<number> => {
 ...
 */
const countReactivityUpdates = (getter: () => any, callback: () => Promise<void> | void): Promise<number> => {
  return new Promise<number>(async (resolve) => {
    let updateCount = 0;
    
    // Watcher für den Getter einrichten
    const stopWatch = watch(getter, () => {
      updateCount++;
    }, { deep: true });
...
describe('Sessions Store Performance', () => {
  ...
  describe('Initialization', () => {
    it('sollte die optimierte Implementierung gleiche oder bessere Initialisierungszeiten haben', async () => {
      ...
      console.log(`Initialisierungszeit - Original: ${originalTime.toFixed(2)}ms, Optimiert: ${optimizedTime.toFixed(2)}ms`);
  ...
  describe('Reactivity Performance', () => {
    it('sollte bei der optimierten Version weniger Updates für Nachrichten auslösen', async () => {
      ...
      const originalUpdates = await countReactivityUpdates(
        ...
        async () => {
          ...
          for (let i = 0; i < 10; i++) {
            ...
            messagesCopy[session.id] = [
              ...(messagesCopy[session.id] || []),
              createMockMessage({ sessionId: session.id, content: `Message ${i}` })
            ];
      ...
      console.log(`Reaktivitäts-Updates - Original: ${originalUpdates}, Optimiert: ${optimizedUpdates}`);
    ...
    it('sollte bei der optimierten Version Cache-Hits für wiederholte Getter-Aufrufe haben', async () => {
      ...
      const sessions = createArrayOfSize(10, i => createMockSession({ title: `Session ${i}` }));
      ...
      console.log(`Getter-Aufrufzeit - Original: ${originalTime.toFixed(2)}ms, Optimiert: ${optimizedTime.toFixed(2)}ms`);
  ...
  describe('Large Dataset Performance', () => {
    it('sollte mit vielen Sessions effizient umgehen', async () => {
      ...
      const manySessions = createArrayOfSize(100, i => createMockSession({ title: `Session ${i}` }));
      ...
      console.log(`Sortierung von 100 Sessions - Original: ${originalTime.toFixed(2)}ms, Optimiert: ${optimizedTime.toFixed(2)}ms`);
    ...
    it('sollte mit vielen Nachrichten effizient umgehen', async () => {
      ...
      console.log(`Laden von 1000 Nachrichten - Original: ${originalTime.toFixed(2)}ms, Optimiert: ${optimizedTime.toFixed(2)}ms`);
      expect(optimizedTime).toBeLessThanOrEqual(originalTime);
    });
  });
  ...
  describe('Streaming Performance', () => {
    it('sollte beim Streaming effizient Updates durchführen', async () => {
      ...
      console.log(`Streaming-Updates - Original: ${originalUpdates}, Optimiert: ${optimizedUpdates}`);
      expect(optimizedUpdates).toBeLessThanOrEqual(originalUpdates);
    });
  });
  ...
  describe('Session Operations Performance', () => {
    it('sollte bei Session-CRUD-Operationen effizienter sein', async () => {
      ...
      console.log(`CRUD-Operationen - Original: ${originalStoreTime.toFixed(2)}ms, Optimiert: ${optimizedStoreTime.toFixed(2)}ms`);
  ...
  describe('Cache Invalidation', () => {
    it('sollte den Cache bei relevanten Änderungen zurücksetzen', async () => {
      // Optimierten Store einrichten
      const store = useOptimizedSessionsStore();
      const session = createMockSession();
      store.sessions = [session];
      ...
      originalStore.sessions = [session];
      originalStore.currentSessionId = session.id;
      originalStore.messages = { [session.id]: [createMockMessage({ sessionId: session.id })] };
      
      // Bei der optimierten Version sollten wiederholte Aufrufe nach der Cache-Invalidierung
      // neue Berechnungen auslösen
      const updatedSession = { ...session, isPinned: true, title: 'Updated Title' };
