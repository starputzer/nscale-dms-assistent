...
import { ExtractState, ExtractGetters, ExtractActions } from '@/utils/storeTypes';
...
describe('SessionsStore (Enhanced TypeScript)', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(axios.patch).mockImplementation(async (url: string, data?: any) => {
  ...
  describe('State Management', () => {
    ...
    it('should handle sending a message and receiving a reply', async () => {
      ...
      const userMessage = createMockMessage(mockSession.id, 'user', { content: messageContent });
      const assistantReply = createMockMessage(mockSession.id, 'assistant', { content: 'Test reply' });
    ...
    it('should add streaming message chunks', () => {
      ...
      const mockStreamingMessage = createMockMessage(mockSession.id, 'assistant', {
        content: '',
      ...
      const updatedMessage = store.currentMessages.find(msg => msg.id === 'streaming-message');
    ...
    it('should archive a session', async () => {
      ...
      expect(axios.delete).toHaveBeenCalledWith(`/api/sessions/${sessionToArchive.id}`);
    });
    ...
    it('should update a session title', async () => {
      ...
      // Assert
      const updatedSession = store.sessions.find(s => s.id === sessionToUpdate.id);
    ...
    it('should toggle pinned status of a session', async () => {
      ...
      // Assert
      const updatedSession = store.sessions.find(s => s.id === mockSession.id);
  ...
  describe('Type Safety', () => {
    it('should demonstrate strongly typed mocks', async () => {
      ...
      const successResponse: Result<ChatSession, APIError> = createSuccessResult(
