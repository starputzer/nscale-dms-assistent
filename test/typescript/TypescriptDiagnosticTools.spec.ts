...
describe('TypescriptDiagnosticTools TypeScript-Kompatibilität', () => {
  ...
  const createMockHealthyComponent = (): DiagnosticCapable => ({
    getDiagnostics: vi.fn(async () => createSuccessResult<ComponentDiagnostic>({
  ...
  const createMockFailingComponent = (): DiagnosticCapable => ({
    getDiagnostics: vi.fn(async () => createErrorResult<ComponentDiagnostic>({
  ...
  describe('Diagnose-Funktionen', () => {
    ...
    it('sollte Komponenten filtern können', async () => {
      ...
      if (result.success) {
        ...
        expect(Object.keys(result.data.components)).toEqual([BridgeComponentName.EVENT_BUS]);
    ...
    it('sollte mit fehlgeschlagenen Diagnosen umgehen können', async () => {
      ...
      if (result.success) {
        ...
        expect(result.data.components[BridgeComponentName.MEMORY_MANAGER].status)
  ...
  describe('Typ-Definitionen', () => {
    ...
    it('sollte BridgeComponentStatus als Enum korrekt definieren', () => {
      ...
      // Type-Check
      const componentStatus: BridgeComponentStatus = BridgeComponentStatus.HEALTHY;
