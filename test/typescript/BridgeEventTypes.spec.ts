...
describe("BridgeEventTypes TypeScript-Kompatibilität", () => {
  describe("Event Interface Typen", () => {
    it("sollte BridgeEventMap korrekt definieren", () => {
      ...
      const mockEventHandler = <T extends keyof BridgeEventMap>(
        eventName: T,
  ...
  describe("Event-Handler Types", () => {
    it("sollte BridgeEventHandler korrekt typisieren", () => {
      ...
      const initializedHandler: BridgeEventHandler<"bridge:initialized"> = (
        data,
      ) => {
        expect(data.timestamp).toBeDefined();
        expect(data.bridgeVersion).toBeDefined();
      ...
      const statusHandler: BridgeEventHandler<"bridge:statusChanged"> = (
        data,
      ) => {
      ...
      type InitializedHandlerAssert = AssertType<
        typeof initializedHandler,
        (data: BridgeInitializedEvent) => void
      >;
      type ErrorHandlerAssert = AssertType<
        typeof errorHandler,
        (data: BridgeErrorEvent) => void
      >;
      type StatusHandlerAssert = AssertType<
        typeof statusHandler,
        (data: BridgeStatusEvent) => void
      >;
    });
  });
  ...
  describe("Event-Validierung", () => {
    it("sollte isValidBridgeEvent korrekt funktionieren", () => {
      ...
      expect(isValidBridgeEvent("bridge:initialized", "not an object")).toBe(
        false,
      );
      ...
      expect(
        isValidBridgeEvent("bridge:initialized", invalidTimestampEvent),
      ).toBe(false);
    });
  });
  ...
  describe("Komplexe Event-Typen", () => {
    it("sollte Vue-Kommando-Events korrekt typisieren", () => {
      ...
      const sendMessageHandler: BridgeEventHandler<"vue:sendMessage"> = (
        data,
      ) => {
      ...
      const updateSessionHandler: BridgeEventHandler<"vue:updateSession"> = (
        data,
      ) => {
      ...
      type SendMessageAssert = AssertType<
        Parameters<typeof sendMessageHandler>[0],
        {
          requestId: string;
          timestamp: number;
          command: "sendMessage";
      ...
      type UpdateSessionAssert = AssertType<
        Parameters<typeof updateSessionHandler>[0],
        {
          requestId: string;
          timestamp: number;
    ...
    it("sollte Legacy-zu-Vue-Events korrekt typisieren", () => {
      ...
      // Laufzeit-Mock für spezifische Event-Handler
      const sessionCreatedHandler: BridgeEventHandler<
        "legacy:sessionCreated"
      > = (data) => {
        expect(data.sessionId).toBeDefined();
      ...
      const messageAddedHandler: BridgeEventHandler<"legacy:messageAdded"> = (
        data,
      ) => {
      ...
      // Diese Assertions würden bei der Kompilierung fehlschlagen, wenn die Typen falsch wären
      type SessionCreatedAssert = AssertType<
        Parameters<typeof sessionCreatedHandler>[0],
        {
          sessionId: string;
          title: string;
          timestamp: number;
      ...
      type MessageAddedAssert = AssertType<
        Parameters<typeof messageAddedHandler>[0],
        {
          messageId: string;
          sessionId: string;
          content: string;
