...
import { BridgeErrorCode, BridgeResult, isBridgeError } from '@/bridge/enhanced/bridgeErrorUtils';
...
describe('TypedBridgeComponent TypeScript-Kompatibilität', () => {
  ...
  describe('Initialisierung und Grundfunktionen', () => {
    ...
    it('sollte performOperation mit korrektem Rückgabetyp aufrufen', async () => {
      ...
      // Typprüfung
      type OpResultType = AssertType<typeof successResult, BridgeResult<string>>;
    ...
    it('sollte processData mit korrektem Rückgabetyp aufrufen', async () => {
      ...
      // Typprüfung
      type ProcessResultType = AssertType<typeof validResult, BridgeResult<{ processed: boolean, id: string }>>;
  ...
  describe('Legacy-Code-Integration', () => {
    it('sollte getProcessedDataForLegacyCode mit korrektem Rückgabetyp aufrufen', async () => {
      ...
      const result = await component.getProcessedDataForLegacyCode({ id: 'legacy-id' });
      ...
      await expect(component.getProcessedDataForLegacyCode(null)).rejects.toThrow();
    });
  });
  ...
  describe('Fehlerverhalten', () => {
    ...
    it('sollte mehrfache Wiederherstellungsversuche verarbeiten', async () => {
      ...
      if (!fourthError.success) {
        expect(fourthError.error.code).toBe(BridgeErrorCode.RETRY_LIMIT_EXCEEDED);
        expect(fourthError.error.message).toContain('Maximale Anzahl an Wiederholungsversuchen');
      }
    });
  });
});