...
import { AssertType, assertType, createSuccessResult, createErrorResult } from '../utils/typescript-test-utils';
...
describe('BridgeErrorUtils TypeScript-Kompatibilität', () => {
  ...
  describe('Type Assertions', () => {
    ...
    it('sollte BridgeError als Interface-Typ haben', () => {
      ...
      type ErrorAssignment = AssertType<typeof error, {
    ...
    it('sollte BridgeResult als Union-Typ haben', () => {
      ...
      if (result.success) {
        ...
        type SuccessAssignment = AssertType<typeof result, { success: true; data: string }>;
      } else {
        expect(result.error).toBeDefined();
        type ErrorAssignment = AssertType<typeof result, { success: false; error: BridgeError }>;
      }
  ...
  describe('Funktionen', () => {
    ...
    it('sollte executeBridgeOperation korrekt typisieren', async () => {
      ...
      const successOperation = await executeBridgeOperation(
        () => 'Success',
        {
          component: 'TestComponent',
      ...
      // Ergebnistyp überprüfen
      type SuccessOpTypeCheck = AssertType<typeof successOperation, BridgeResult<string>>;
      ...
      // Ergebnistyp überprüfen
      type FailOpTypeCheck = AssertType<typeof failOperation, BridgeResult<number>>;
    ...
    it('sollte withRecovery korrekt typisieren', async () => {
      ...
      const recoveredResult = await withRecovery(
        failureResult,
        () => success<TestDataType>({ value: 'recovered', count: 456 })
      );
      
      // Ergebnistyp überprüfen
      type RecoveryTypeCheck = AssertType<typeof recoveredResult, BridgeResult<TestDataType>>;
      ...
      const notRecoveredResult = await withRecovery(
        nonRecoverableResult,
  ...
  describe('Type Guards', () => {
    it('sollte isBridgeError korrekt typisieren', () => {
      ...
      expect(processError(validError)).toBe(BridgeErrorCode.COMMUNICATION_ERROR);
    ...
    it('sollte hasErrorCode korrekt typisieren', () => {
      // Mit korrektem Code
      expect(hasErrorCode(testError, BridgeErrorCode.COMMUNICATION_ERROR)).toBe(true);
      
      // Mit falschem Code
      expect(hasErrorCode(testError, BridgeErrorCode.INITIALIZATION_FAILED)).toBe(false);
      
      // Mit Null/Undefined
      expect(hasErrorCode(null, BridgeErrorCode.COMMUNICATION_ERROR)).toBe(false);
      expect(hasErrorCode(undefined, BridgeErrorCode.COMMUNICATION_ERROR)).toBe(false);
    });
  });
});