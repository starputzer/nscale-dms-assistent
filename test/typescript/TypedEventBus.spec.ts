...
describe("TypedEventBus TypeScript-Kompatibilität", () => {
  ...
  describe("Listener-Registrierung", () => {
    ...
    it("sollte .once mit korrekten Event-Typen funktionieren", async () => {
      ...
      const subscription = await eventBus.once(
        "bridge:initialized",
        initHandler,
      );
      expect(subscription.success).toBe(true);
    ...
    it("sollte .priority mit korrekten Event-Typen funktionieren", async () => {
      ...
      expect(highPriorityHandler.mock.invocationCallOrder[0]).toBeLessThan(
        mediumPriorityHandler.mock.invocationCallOrder[0],
      );

      expect(mediumPriorityHandler.mock.invocationCallOrder[0]).toBeLessThan(
  ...
  describe("Event-Emission und -Verarbeitung", () => {
    it("sollte ungültige Events ablehnen", async () => {
      ...
      const emitResult = await eventBus.emit(
        "bridge:initialized",
        invalidEvent as any,
      );
    ...
    it("sollte processTypedEvent korrekt verwenden", async () => {
      ...
      const validResult = await eventBus.processTypedEvent(
        "bridge:initialized",
        validEvent,
        handler,
      );
      ...
      handler.mockClear();
      const invalidEvent = { timestamp: "not a number" };
      const invalidResult = await eventBus.processTypedEvent(
        "bridge:initialized",
        invalidEvent,
        handler,
      );
  ...
  describe("Batching-Funktionalität", () => {
    ...
    it("sollte Events in Batches verarbeiten", async () => {
      // Handler für ein typisiertes Event
      const handler = vi.fn(
        (data: BridgeInitializedEvent | BridgeInitializedEvent[]) => {
