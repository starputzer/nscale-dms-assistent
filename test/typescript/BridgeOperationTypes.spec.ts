...
describe("BridgeOperationTypes TypeScript-Kompatibilität", () => {
  describe("Typ-Definitionen", () => {
    ...
    it("sollte BridgeOperation korrekt typisieren", () => {
      ...
      // Async Operation
      const asyncOp: BridgeOperation<number> = async () =>
    ...
    it("sollte ParameterizedBridgeOperation korrekt typisieren", () => {
      const paramOp: ParameterizedBridgeOperation<
        number,
        [string, boolean]
  ...
  describe("makeCancellable", () => {
    it("sollte eine abbrechbare Operation erzeugen", async () => {
      ...
      type OpType = AssertType<
        typeof cancellable,
        CancellableBridgeOperation<string>
      >;
  ...
  describe("chain", () => {
    it("sollte eine verkettbare Operation erzeugen", async () => {
      ...
      const secondOp = vi.fn(async (val: number) =>
        createSuccessResult(val * 2),
      );
      const finalOp = vi.fn(async () => {});

      const chainable = chain(firstOp).then(secondOp).finally(finalOp);

      // Type check - sollte eine ChainableBridgeOperation<void, number> sein
      type ChainType = AssertType<
        typeof chainable,
        ChainableBridgeOperation<void, number>
      >;
    ...
    it("sollte Fehler abfangen können", async () => {
      const errorOp = vi.fn(async () =>
        createErrorResult<number>({ message: "Test error" }),
      );
      const recoveryOp = vi.fn(async () => createSuccessResult(42));

      const chainable = chain(errorOp).catch(recoveryOp);
  ...
  describe("withEventHandler", () => {
    it("sollte einen Event-Handler mit Aufräumfunktion registrieren", async () => {
      ...
      const mockOn = vi.fn(async () =>
        createSuccessResult({ unsubscribe: mockUnsubscribe }),
      );
      ...
      expect(mockOn).toHaveBeenCalledWith("bridge:initialized", mockHandler, {
        priority: 10,
      });
  ...
  describe("makePausable", () => {
    it("sollte eine pausierbare Operation erzeugen", async () => {
      ...
      type PausableType = AssertType<
        typeof pausable,
        PausableOperation<string>
      >;
  ...
  describe("SelfHealingComponent", () => {
    it("sollte einen SelfHealingComponent-Typ korrekt definieren", () => {
      ...
      expect(selfHealingComponent.isHealthy()).resolves.toEqual(
