...
import { useLocalStorage, LocalStorageOptions, LocalStorageRef } from "@/composables/useLocalStorage";
...
describe("useLocalStorage (Enhanced TypeScript)", () => {
  ...
  it("should support the hasItem method", () => {
    ...
    const { hasItem: hasNonExistingItem } = createUserStorage("non-existing-key");
  ...
  it("should apply a prefix to the storage key when provided", () => {
    ...
    // Act
    const { value } = createUserStorage(key, {
      id: "prefixed-user",
      name: "Prefixed User",
  ...
  it("should support the option to not watch value changes", async () => {
    // Arrange
    const { value } = createUserStorage("no-watch-user", {
      id: "no-watch-user",
      name: "No Watch User",
