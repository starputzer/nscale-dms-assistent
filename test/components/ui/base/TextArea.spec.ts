...
describe("TextArea.vue", () => {
  ...
  describe("Rendering", () => {
    ...
    it("renders helper text when provided", () => {
      ...
      expect(wrapper.find(".n-textarea-helper-text").text()).toBe(
        "Helper text",
      );
    });
    ...
    it("renders error message when provided", () => {
      ...
      expect(wrapper.find(".n-textarea-error-text").text()).toBe(
        "Error message",
      );
    });
    ...
    it("shows character count when enabled with maxlength", () => {
      ...
      expect(wrapper.find(".n-textarea-character-count").text()).toBe(
        "5 / 100",
      );
    });
  });
  ...
  describe("Auto-resize functionality", () => {
    it("calls resizeTextarea when content changes", async () => {
      ...
      await wrapper.setProps({
        modelValue: "New content with multiple lines\nSecond line\nThird line",
      });
      await nextTick();
  ...
  describe("Markdown preview", () => {
    it("toggles markdown preview when the toggle button is clicked", async () => {
      ...
      expect(wrapper.find(".n-textarea-preview-content").html()).toContain(
        "<p># Heading</p>",
      );
    });
