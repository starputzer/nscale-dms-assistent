...
describe("Toggle.vue", () => {
  ...
  describe("Sizes", () => {
    it("applies small size correctly", () => {
      ...
      const toggleStyle = window.getComputedStyle(
        wrapper.find(".n-toggle-switch").element,
      );
  ...
  describe("Accessibility", () => {
    ...
    it("sets aria-describedby attribute when there is helper text or error", () => {
      ...
      expect(
        wrapper.find('input[type="checkbox"]').attributes("aria-describedby"),
      ).toBe("test-toggle-helper");
    });
  });
});
