...
describe('PrivacySettings.vue', () => {
  ...
  describe('Rendering', () => {
    it('rendert die Komponente korrekt', () => {
      ...
      expect(wrapper.find('.privacy-settings__title').text()).toBe('settings.privacy.title');
    });
    ...
    it('zeigt den Datenverwaltungs-Bereich an', () => {
      ...
      expect(section.find('.privacy-settings__subtitle').text()).toBe('settings.privacy.dataManagement');
  ...
  describe('Datenverwaltung', () => {
    it('löst Aktionen aus, wenn auf Buttons geklickt wird', async () => {
      ...
      await wrapper.findAll('.privacy-settings__action-button')[0].trigger('click');
      
      // Wir können nicht direkt auf den gemockten useDialog() zugreifen
      // Daher prüfen wir nur, ob der Button vorhanden ist
      expect(wrapper.findAll('.privacy-settings__action-button')[0].exists()).toBe(true);
    });
  });
});