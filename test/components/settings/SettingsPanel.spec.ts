  ...
  return mount(SettingsPanel, {
    ...
    global: {
      ...
      stubs: {
        ...
        AppearanceSettings: {
          template:
            '<div class="mocked-appearance-settings">AppearanceSettings</div>',
          methods: {
        ...
        NotificationSettings: {
          template:
            '<div class="mocked-notification-settings">NotificationSettings</div>',
          methods: {
            emits: ["apply-settings", "reset"],
          },
        },
        PrivacySettings: {
          template:
            '<div class="mocked-privacy-settings">PrivacySettings</div>',
          methods: {
            emits: ["apply-settings", "reset"],
          },
        },
        AccessibilitySettings: {
          template:
            '<div class="mocked-accessibility-settings">AccessibilitySettings</div>',
          methods: {
            emits: ["apply-settings", "reset"],
          },
        },
...
describe("SettingsPanel.vue", () => {
  ...
  describe("Rendering", () => {
    it("renders the component when visible", () => {
      ...
      expect(wrapper.find(".settings-panel__title").text()).toBe(
        "settings.title",
      );
    });
    ...
    it("renders all category buttons", () => {
      ...
      expect(categoryButtons[0].text()).toContain(
        "settings.categories.appearance",
      );
      expect(categoryButtons[1].text()).toContain(
        "settings.categories.notifications",
      );
      expect(categoryButtons[2].text()).toContain(
        "settings.categories.privacy",
      );
      expect(categoryButtons[3].text()).toContain(
  ...
  describe("Interactions", () => {
    it("switches between categories when clicking category buttons", async () => {
      ...
      expect(wrapper.find(".mocked-accessibility-settings").exists()).toBe(
        true,
      );
    });
  ...
  describe("Settings Handling", () => {
    ...
    it("calls saveSettings when handleSave is called", async () => {
      ...
      const saveSettingsSpy = vi
        .spyOn(settingsStore, "saveSettings")
    ...
    it("calls resetToDefault when confirmRestoreDefaults is called", async () => {
      ...
      const resetToDefaultSpy = vi
        .spyOn(settingsStore, "resetToDefault")
        .mockResolvedValue();
  ...
  describe("Mobile View", () => {
    it("has responsive styles for mobile screens", () => {
      ...
      styleElement.textContent = wrapper.vm.$.type.__scopeId
