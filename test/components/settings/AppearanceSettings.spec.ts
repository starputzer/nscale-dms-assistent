...
describe('AppearanceSettings.vue', () => {
  ...
  describe('Rendering', () => {
    ...
    it('markiert das aktuell ausgewählte Theme', () => {
      const wrapper = createWrapper();
      const store = useSettingsStore();
      const activeThemeCard = wrapper.find('.appearance-settings__theme-card--active');
    ...
    it('zeigt alle Schriftgrößen-Optionen an', () => {
      const wrapper = createWrapper();
      const fontSizeButtons = wrapper.findAll('.appearance-settings__font-size-button');
    ...
    it('zeigt alle Schriftart-Optionen an', () => {
      const wrapper = createWrapper();
      const fontFamilyOptions = wrapper.find('.appearance-settings__select').findAll('option');
    ...
    it('zeigt alle Zeilenhöhe-Optionen an', () => {
      const wrapper = createWrapper();
      const lineHeightButtons = wrapper.findAll('.appearance-settings__line-height-button');
  ...
  describe('Benutzerdefiniertes Theme', () => {
    it('zeigt den benutzerdefinierten Theme-Editor an wenn der Toggle-Button geklickt wird', async () => {
      ...
      expect(wrapper.find('.appearance-settings__custom-theme').exists()).toBe(false);
      ...
      expect(wrapper.find('.appearance-settings__custom-theme').exists()).toBe(true);
    });
    ...
    it('setzt das benutzerdefinierte Theme zurück wenn auf den Reset-Button geklickt wird', async () => {
      ...
      const resetButton = wrapper.find('.appearance-settings__button--secondary');
      await resetButton.trigger('click');
      
      // Prüfe, ob der Name zurückgesetzt wurde
      expect((nameInput.element as HTMLInputElement).value).toBe('Benutzerdefiniertes Thema');
    });
    ...
    it('speichert und wendet das benutzerdefinierte Theme an', async () => {
      ...
      // Zeige den Editor an
      await wrapper.find('.appearance-settings__toggle-button').trigger('click');
