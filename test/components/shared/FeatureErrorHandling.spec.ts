...
const ConditionalErrorComponent = {
  ...
  template: '<div class="conditional-component">{{ shouldThrow ? "Error" : "No Error" }}</div>',
};
...
describe("Feature Error Handling Tests", () => {
  ...
  describe("Basic Error Handling", () => {
    it("should handle synchronous errors and activate fallback", () => {
      ...
      expect(store.reportFeatureError.mock.calls[0][0]).toBe("testErrorFeature");
      expect(store.reportFeatureError.mock.calls[0][1]).toContain("Simulated component error");
    ...
    it("should handle errors in document converter components", () => {
      ...
      const store = useFeatureTogglesStore();
      expect(store.reportFeatureError).toHaveBeenCalled();
      expect(store.reportFeatureError.mock.calls[0][0]).toBe("useSfcDocConverterContainer");
    ...
    it("should emit feature-error event when captureErrors is true", () => {
      ...
      expect(wrapper.emitted("feature-error")?.[0][0].message).toBe("Simulated component error");
  ...
  describe("Feature Toggle Store Error Handling", () => {
    it("should correctly set fallback mode for features with errors", () => {
      ...
      store.reportFeatureError("useSfcDocConverterProgress", "Test error", null, true);
  ...
  describe("Error reporting to composable", () => {
    it("should report errors to useFeatureToggles composable", () => {
      ...
      expect(mockReportError.mock.calls[0][1]).toContain("Simulated component error");
