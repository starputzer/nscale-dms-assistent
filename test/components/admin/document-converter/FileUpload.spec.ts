...
describe("FileUpload.vue", () => {
  ...
  describe("Rendering", () => {
    ...
    it("applies the dragging class when isDragging is true", async () => {
      ...
      expect(wrapper.find(".file-upload").classes()).toContain(
    ...
    it("removes the dragging class on dragleave", async () => {
      ...
      expect(wrapper.find(".file-upload").classes()).not.toContain(
        "file-upload--dragging",
      );
    });
    ...
    it("displays progress bar when uploading", () => {
      ...
      expect(
        wrapper.find(".file-upload__progress-bar").attributes("style"),
      ).toContain("width: 42%");
    ...
    it("shows selected file info when a file is selected", async () => {
      ...
      expect(wrapper.find(".file-upload__selected-name").text()).toBe(
        "document.pdf",
      );
      expect(wrapper.find(".file-upload__selected-size").text()).toContain(
        "MB",
      );
    });
  });
  ...
  describe("Edge cases", () => {
    ...
    it("handles files with no extension", async () => {
      ...
      const fileWithoutExtension = createTestFile(
        "document",
        "application/pdf",
      );
