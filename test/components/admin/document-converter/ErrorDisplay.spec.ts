...
describe("ErrorDisplay.vue", () => {
  ...
  describe("Rendering", () => {
    ...
    it("renders correctly with an ErrorObject", () => {
      ...
      expect(wrapper.find(".error-message").text()).toBe(
        "Test error from ErrorObject",
      );
      expect(wrapper.find(".error-display").classes()).toContain(
    ...
    it("conditionally renders fallback button based on showFallbackOption prop", () => {
      ...
      const wrapperWithoutFallback = createWrapper({
  ...
  describe("Props", () => {
    ...
    it("correctly determines error type from error message content", () => {
      ...
      expect(networkError.find(".error-display").classes()).toContain(
        "error-display--network",
      );
      expect(formatError.find(".error-display").classes()).toContain(
        "error-display--format",
      );
      expect(serverError.find(".error-display").classes()).toContain(
        "error-display--server",
      );
      expect(permissionError.find(".error-display").classes()).toContain(
  ...
  describe("Edge cases", () => {
    it("handles null or undefined error gracefully", () => {
      // Use an empty string instead of null for the test
      const wrapperWithEmptyString = createWrapper({ error: "" });
      expect(wrapperWithEmptyString.find(".error-message").text()).toBe(
        "errorDisplay.defaultMessage",
      );

      // Create a default wrapper with the standard error message
      const wrapperWithDefaultMessage = createWrapper();
      expect(wrapperWithDefaultMessage.find(".error-message").text()).toBe(
    ...
    it("handles non-standard error objects", () => {
      ...
      // Should use the string representation of the object
      expect(wrapper.find(".error-message").text()).toBe(
    ...
    it("sanitizes error details for display", () => {
      // Create a simple error with details for testing
      const wrapper = createWrapper({
        error: {
          message: "Error with details",
          details: "This is a detailed error message that should be displayed",
        },
        showDetails: true,
      });
      ...
      // The component should display the error details
      expect(wrapper.find(".error-details-content").exists()).toBe(true);
      expect(wrapper.find(".error-details-content").text()).toContain(
