...
describe("DocConverterContainer.vue", () => {
  ...
  describe("Functionality", () => {
    it("starts conversion when file is uploaded", async () => {
      ...
      const documentConverterMock = useDocumentConverter();
      const uploadDocumentSpy = vi.spyOn(
        documentConverterMock,
        "uploadDocument",
      );
      const convertDocumentSpy = vi.spyOn(
        documentConverterMock,
        "convertDocument",
      );
      ...
      expect(convertDocumentSpy).toHaveBeenCalledWith(
        "doc2",
        expect.any(Function),
      );
    });
    ...
    it("handles conversion cancellation", async () => {
      ...
      const documentConverterMock = useDocumentConverter();
      const cancelConversionSpy = vi.spyOn(
        documentConverterMock,
        "cancelConversion",
      );
    ...
    it("handles document viewing", async () => {
      ...
      const documentConverterMock = useDocumentConverter();
      const selectDocumentSpy = vi.spyOn(
        documentConverterMock,
        "selectDocument",
      );
  ...
  describe("Error handling", () => {
    it("handles conversion errors gracefully", async () => {
      ...
      const documentConverterMock = useDocumentConverter();
      vi.spyOn(documentConverterMock, "uploadDocument").mockRejectedValue(
    ...
    it("enables fallback when specific errors occur", async () => {
      ...
      const documentConverterMock = useDocumentConverter();
      vi.spyOn(documentConverterMock, "uploadDocument").mockRejectedValue(
        new Error("SERVER_UNREACHABLE: Cannot connect to server"),
      );
  ...
  describe("Edge cases", () => {
    ...
    it("handles download for special characters in document name", () => {
      ...
      expect(window.location.href).toBe(
        "https://api.example.com/api/documents/doc-with-special-chars!@#/download",
      );
