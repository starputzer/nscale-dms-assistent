...
describe("DocumentList.vue", () => {
  ...
  describe("Rendering", () => {
    ...
    it("displays document details correctly", () => {
      ...
      expect(firstDocument.find(".document-name").text()).toBe(
        "Document 1.pdf",
      );
    ...
    it("shows different icon for each document type", () => {
      ...
      expect(items[0].find(".document-icon").classes()).toContain(
        "document-icon--pdf",
      );
      expect(items[1].find(".document-icon").classes()).toContain(
        "document-icon--docx",
      );
      expect(items[2].find(".document-icon").classes()).toContain(
        "document-icon--xlsx",
      );
      expect(items[3].find(".document-icon").classes()).toContain(
        "document-icon--pptx",
      );
    });
    ...
    it("displays appropriate status indicators", () => {
      ...
      expect(items[0].find(".document-status").classes()).toContain(
        "document-status--success",
      );
      expect(items[1].find(".document-status").classes()).toContain(
        "document-status--error",
      );
      expect(items[2].find(".document-status").classes()).toContain(
        "document-status--pending",
      );
      expect(items[3].find(".document-status").classes()).toContain(
  ...
  describe("Interactions", () => {
    ...
    it("filters documents by status", async () => {
      ...
      expect(wrapper.findAll(".document-item").length).toBe(1);
      expect(wrapper.find(".document-status").classes()).toContain(
        "document-status--error",
      );
    });
    ...
    it("sorts documents by name", async () => {
      ...
      const documentNames = wrapper
        .findAll(".document-name")
        .map((el) => el.text());
  ...
  describe("Edge cases", () => {
    it("handles documents with missing properties gracefully", () => {
      ...
      expect(wrapper.find(".document-name").text()).toBe(
        "Document with minimal properties.pdf",
      );
    });
