...
describe("AdminFeedback Component", () => {
  ...
  it("should filter feedback by date range", async () => {
    ...
    store.filteredFeedback = vi.fn().mockImplementation(() => {
      const startDate = store.filter.startDate
        ? new Date(store.filter.startDate)
        : null;
      const endDate = store.filter.endDate
        ? new Date(store.filter.endDate)
        : null;
    ...
    expect(wrapper.vm.displayedFeedback.some((item) => item.id === "2")).toBe(
      true,
    );
    expect(wrapper.vm.displayedFeedback.some((item) => item.id === "3")).toBe(
      true,
    );
  });
