...
describe("AdminStatistics.vue", () => {
  ...
  it("displays correct stats data", () => {
    ...
    const usersValue = wrapper.find(
      ".admin-statistics__card--users .admin-statistics__card-value",
    );
    const sessionsValue = wrapper.find(
      ".admin-statistics__card--sessions .admin-statistics__card-value",
    );
    const messagesValue = wrapper.find(
      ".admin-statistics__card--messages .admin-statistics__card-value",
    );
    const feedbackValue = wrapper.find(
      ".admin-statistics__card--feedback .admin-statistics__card-value",
    );
  ...
  it("switches time range when clicking range buttons", async () => {
    ...
    const newActiveButton = wrapper.find(
      ".admin-statistics__range-button--active",
    );
  ...
  it("correctly displays resource usage metrics", () => {
    const cpuUsage = wrapper.find(
      ".admin-statistics__resource-card:nth-child(1) .admin-statistics__meter-value",
    );
    const memoryUsage = wrapper.find(
      ".admin-statistics__resource-card:nth-child(2) .admin-statistics__meter-value",
    );
