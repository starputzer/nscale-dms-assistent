...
describe("Feedback Charts", () => {
  ...
  it("should render charts when stats data is available", async () => {
    ...
    expect(wrapper.find('[data-test="source-chart-container"]').exists()).toBe(
      true,
    );
    expect(wrapper.find('[data-test="trend-chart-container"]').exists()).toBe(
      true,
    );
  ...
  it("should display loading state when store is loading", async () => {
    ...
    expect(wrapper.find('[data-test="loading-indicator"]').exists()).toBe(
      false,
    );
  ...
  it("should format dates correctly in trend chart", () => {
    ...
    expect(formattedDate).toMatch(
      /^(0?[1-9]|[12][0-9]|3[01])[\/\-\.](0?[1-9]|1[012])[\/\-\.]\d{4}$|^\d{4}[\/\-\.](0?[1-9]|1[012])[\/\-\.](0?[1-9]|[12][0-9]|3[01])$/,
    );
  });
});
