...
describe('TabPanel.vue', () => {
  const defaultTabs = [
    ...
    { id: 'tab4', label: 'Tab 4', icon: 'icon-chart', badge: '5', badgeColor: '#ff0000' },
  ...
  it('renders correctly with default props', () => {
    ...
    expect(wrapper.findAll('.n-tab-panel__tab').length).toBe(defaultTabs.length);
  ...
  it('switches tabs when clicking on a tab', async () => {
    ...
    expect(wrapper.findAll('.n-tab-panel__tab').at(1).classes()).toContain('n-tab-panel__tab--active');
    expect(wrapper.find('.n-tab-panel__pane--active .tab-content').text()).toBe('Tab 2 Content');
  });
  ...
  it('supports closing tabs when closable is true', async () => {
    ...
    expect(wrapper.findAll('.n-tab-panel__tab-close').length).toBe(defaultTabs.length);
  ...
  it('supports pinned tabs when allowPinning is true', async () => {
    ...
    expect(wrapper.findAll('.n-tab-panel__tab').at(4).classes()).toContain('n-tab-panel__tab--pinned');
    
    // Should have pin indicators and buttons
    expect(wrapper.find('.n-tab-panel__tab-pin-indicator').exists()).toBe(true);
    expect(wrapper.findAll('.n-tab-panel__tab-pin').length).toBe(defaultTabs.length);
  ...
  it('supports different display types', async () => {
    ...
    for (const type of displayTypes) {
      ...
      expect(wrapper.find('.n-tab-panel__content').classes()).toContain(`n-tab-panel__content--${type}`);
    }
  });
