...
describe('Footer.vue', () => {
  ...
  it('renders copyright and version when provided', async () => {
    ...
    expect(wrapper.find('.n-footer__copyright').text()).toContain('Test Company');
    expect(wrapper.find('.n-footer__version').text()).toContain('Version 1.2.3');
  });
  ...
  it('applies size class based on size prop', async () => {
    ...
    for (const size of sizes) {
      ...
      expect(wrapper.find('.n-footer').classes()).toContain(`n-footer--size-${size}`);
    }
  });
  ...
  it('applies variant class based on variant prop', async () => {
    ...
    for (const variant of variants) {
      ...
      expect(wrapper.find('.n-footer').classes()).toContain(`n-footer--variant-${variant}`);
    }
  });
  ...
  it('applies full-width class when fullWidth is true', async () => {
    ...
    expect(wrapper.find('.n-footer__container').classes()).toContain('n-footer__container--full-width');
  });

  it('applies custom height when provided', async () => {
    const wrapper = createWrapper({ height: '80px' });
    
    expect(wrapper.find('.n-footer').attributes('style')).toContain('--n-footer-height: 80px');
  });
  ...
  it('calculates the current year correctly', async () => {
    ...
    expect(wrapper.find('.n-footer__copyright').text()).toContain(currentYear.toString());
  });
