...
describe("ChatContainer.vue", () => {
  ...
  it("handles session renaming", async () => {
    ...
    expect(uiStore.showSuccess).toHaveBeenCalledWith(
  ...
  it("handles session clearing", async () => {
    ...
    expect(sessionsStore.refreshSession).toHaveBeenCalledWith(
      "test-session-id",
    );
  });
  ...
  it("shows archive badge when session is archived", async () => {
    ...
    expect(wrapper.find(".n-chat-container__archive-badge").exists()).toBe(
      true,
    );

    // Der Archive-Button sollte nun "Sitzung aus Archiv wiederherstellen" als aria-label haben
    const archiveButton = wrapper.findAll(".n-chat-container__action-btn")[2];
    expect(archiveButton.attributes("aria-label")).toBe(
  ...
  it("uses proper ARIA attributes for accessibility", () => {
    ...
    ).toBe("chat-container-title");
    expect(wrapper.find(".n-chat-container").attributes("aria-live")).toBe(
      "polite",
    );

    // Test for proper ARIA attributes on toolbar
    expect(wrapper.find(".n-chat-container__actions").attributes("role")).toBe(
      "toolbar",
    );
