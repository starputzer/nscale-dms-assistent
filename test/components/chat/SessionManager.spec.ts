...
describe('SessionManager', () => {
  ...
  it('renders sessions when provided', () => {
    const wrapper = mount(SessionManager, {
      ...
      global: {
        stubs: {
          VueDraggable: {
            template: '<div class="draggable-stub"><slot v-if="$slots.item" name="item" v-for="element in elements" :element="element"></slot></div>',
            props: ['modelValue'],
            setup(props) {
              return {
    ...
    expect(wrapper.find('.n-session-manager__session-list').exists()).toBe(true);
  });
  ...
  it('filters sessions by search query', async () => {
    ...
    // Öffne die Suche
    await wrapper.find('.n-session-manager__action-btn[aria-label="Suche anzeigen"]').trigger('click');
  ...
  describe('Category System', () => {
    it('shows category information in session items when available', () => {
      ...
      expect(sessions[1].find('.n-session-manager__category').exists()).toBe(true);
      expect(sessions[1].find('.n-session-manager__category-name').text()).toBe('Dokumentation');
    });
  });
  ...
  describe('Multi-Select functionality', () => {
    it('enables multi-select mode when the multi-select button is clicked', async () => {
      ...
      const multiSelectButton = wrapper.find('.n-session-manager__multi-select-btn');
      await multiSelectButton.trigger('click');
      
      // Prüfe, ob der Multi-Select-Modus aktiv ist
      expect(wrapper.find('.n-session-manager__multi-select-btn--active').exists()).toBe(true);
  ...
  describe('Tag System', () => {
    it('shows tag information in session items when available', () => {
      ...
      expect(sessions[2].find('.n-session-manager__tag').text()).toBe('Wichtig');
    });
  });
});