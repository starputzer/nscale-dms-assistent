...
describe('Loading Time and Time-to-Interactive Performance Tests', () => {
    ...
    if (typeof PerformanceObserver !== 'undefined') {
      const observer = new PerformanceObserver((list) => {
        ...
        entries.forEach((entry) => {
          console.log(`Performance Entry: ${entry.name} - ${entry.startTime.toFixed(2)}ms to ${(entry.startTime + entry.duration).toFixed(2)}ms (${entry.duration.toFixed(2)}ms)`);
        });
      });
  ...
  describe('Initial Application Loading', () => {
    ...
    it('measures time to interactive', async () => {
      ...
      if (button.exists()) {
        ...
        expect(clickDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.USER_INTERACTION);
      }
    });
  ...
  describe('View Component Loading', () => {
    ...
    it('measures AdminView mounting performance', async () => {
      ...
      expect(duration).toBeLessThan(PERFORMANCE_THRESHOLDS.COMPONENT_MOUNT * 1.5); // Admin view is more complex
  ...
  describe('Data Loading Performance', () => {
    it('measures chat history loading performance', async () => {
      ...
      expect(loadDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.DATA_LOADING * 2); // Adjusted for large dataset
    ...
    it('measures document list loading performance', async () => {
      ...
      // Log and assert
      console.log(`Large document list load time: ${loadDuration.toFixed(2)}ms`);
      expect(loadDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.DATA_LOADING * 3); // Adjusted for large dataset
  ...
  describe('User Interaction Performance', () => {
    it('measures input field responsiveness', async () => {
      ...
      const longText = 'This is a long message that a user might type into the chat interface. It contains multiple sentences and should be long enough to test the performance of the input field when handling larger amounts of text input. This simulates a user typing quickly or pasting content.';
      ...
      expect(inputDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.USER_INTERACTION);
    ...
    it('measures theme switching performance', async () => {
      ...
      expect(switchDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.USER_INTERACTION * 2); // Theme switching is more expensive
  ...
  describe('Animation Frame Performance', () => {
    it('measures performance during scrolling', async () => {
      ...
      expect(avgFrameTime).toBeLessThan(PERFORMANCE_THRESHOLDS.ANIMATION_FRAME * 2); // Some flexibility for test environment
    });
  });
  ...
  describe('Memory Usage', () => {
    it('measures memory usage during component mounting', async () => {
      ...
      // Log memory usage
      console.log(`Memory usage for App mount: ${(memoryDiff / 1024 / 1024).toFixed(2)} MB`);
  ...
  describe('Performance Report Generation', () => {
    it('generates a performance summary report', async () => {
      ...
      const dataLoadingTime = await measureFunctionTime(async () => {
        ...
        wrapper.vm.$store.documents.items = Array.from({ length: 20 }, (_, i) => ({
          id: `doc-${i}`,
          title: `Document ${i}`,
          type: 'pdf',
        }));
