...
test.describe("Document Converter", () => {
  ...
  test("should display document converter interface", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="file-upload-area"]'),
    ).toBeVisible();
  ...
  test("should upload a document via drag and drop", async ({ page }) => {
    ...
    await page.dispatchEvent('[data-testid="file-upload-area"]', "drop", {
      dataTransfer,
    });

    // Verify file appears in list
    await expect(page.locator(".document-item")).toBeVisible();
    await expect(page.locator(".document-item")).toContainText(
      "test-document.pdf",
    );
  });
  ...
  test("should convert a document", async ({ page }) => {
    ...
    await expect(page.locator(".conversion-complete")).toBeVisible({
      timeout: 60000,
    });

    // Verify download link appears
    await expect(
      page.locator('[data-testid="download-converted"]'),
    ).toBeVisible();
  });
