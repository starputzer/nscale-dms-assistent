...
test.describe('Chat Functionality', () => {
  ...
  test('should send a message', async ({ page }) => {
    ...
    await expect(page.locator('.message-item.ai-message')).toBeVisible({ timeout: 30000 });
  });
  ...
  test('should show typing indicator while AI responds', async ({ page }) => {
    await page.locator('.message-input textarea').fill('Test message for typing indicator');
    await page.locator('[data-testid="send-button"]').click();
  ...
  test('should switch between chat sessions', async ({ page }) => {
    ...
    await page.locator('[data-testid="new-session-button"]').click();
    await page.locator('.message-input textarea').fill('Second session message');
    await page.locator('[data-testid="send-button"]').click();
    ...
    await expect(page.locator('.message-item')).toContainText('First session message');
  });
  ...
  test('should handle message streaming', async ({ page }) => {
    ...
    // Get initial content
    const initialContent = await page.locator('.message-item.ai-message').last().textContent();
    
    // Wait a bit for more content to stream
    await page.waitForTimeout(1000);
    
    // Get updated content
    const updatedContent = await page.locator('.message-item.ai-message').last().textContent();
  ...
  test('should provide feedback on messages', async ({ page }) => {
    // Send a message
    await page.locator('.message-input textarea').fill('Test message for feedback');
    await page.locator('[data-testid="send-button"]').click();
    ...
    await page.locator('[data-testid="feedback-comment"]').fill('Good response!');
