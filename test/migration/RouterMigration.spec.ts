...
describe('RouterMigrator', () => {
  ...
  it('should identify legacy URLs', () => {
    ...
    expect(routerMigrator.isLegacyUrl('http://example.com/chat/123')).toBe(true);
    expect(routerMigrator.isLegacyUrl('http://example.com/convert')).toBe(true);
    expect(routerMigrator.isLegacyUrl('http://example.com/unknown')).toBe(false);
  });
  
  it('should convert legacy URLs to Vue URLs', () => {
    // Act & Assert
    expect(routerMigrator.legacyUrlToVueUrl('http://example.com/admin')).toBe('/admin');
    expect(routerMigrator.legacyUrlToVueUrl('http://example.com/chat/123')).toBe('/chat/123');
    expect(routerMigrator.legacyUrlToVueUrl('http://example.com/convert')).toBe('/documents/convert');
    expect(routerMigrator.legacyUrlToVueUrl('http://example.com/convert?type=pdf')).toBe('/documents/convert?type=pdf');
    expect(routerMigrator.legacyUrlToVueUrl('http://example.com/unknown')).toBe(null);
  });
  ...
  it('should setup event listeners for legacy navigation', () => {
    // Assert
    expect(mockEventBus.on).toHaveBeenCalledWith('legacy:navigation', expect.any(Function));
  });
