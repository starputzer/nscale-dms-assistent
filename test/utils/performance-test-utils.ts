...
export function endMeasurement(name: string): number {
  ...
  const duration =
    measurements.length > 0
      ? measurements[measurements.length - 1].duration
      : 0;
...
export function formatPerformanceReport(
  ...
  const passedTests = Object.values(report).filter(
    (data) => data.status === "pass",
  ).length;
  const warnTests = Object.values(report).filter(
    (data) => data.status === "warn",
  ).length;
  const failedTests = Object.values(report).filter(
 ...
 */
export async function measureTimeToFirstPaint(
  componentMountFn: Function,
): Promise<number> {
...
export function generatePerformanceProfile(
  ...
  return {
    ...
    summary: {
      ...
      passingTests: Object.values(testResults).filter(
        (result: any) => result.status === "pass",
      ).length,
      warningTests: Object.values(testResults).filter(
        (result: any) => result.status === "warn",
      ).length,
      failingTests: Object.values(testResults).filter(
        (result: any) => result.status === "fail",
      ).length,
    },
  };
}
