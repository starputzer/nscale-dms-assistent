...
describe("Document Converter Integration", () => {
  ...
  describe("End-to-End Workflows", () => {
    it("completes full document conversion workflow", async () => {
      ...
      for (let progress = 0; progress <= 100; progress += 25) {
        await wrapper.vm.updateProgress(progress, "Processing page " + Math.floor(progress / 25));
        await flushPromises();
        
        // Update the store to match the component's state
        docConverterStore.conversionProgress = progress;
        docConverterStore.conversionStep = "Processing page " + Math.floor(progress / 25);
      }
    ...
    it("handles document deletion", async () => {
      ...
      expect(docConverterStore.documents.find(d => d.id === "doc1")).toBeUndefined();
    });
  });
    ...
    it("handles conversion errors", async () => {
      ...
      ApiService.post.mockImplementation((url) => {
        if (url.includes("/api/documents/upload/finalize")) {
          ...
          return Promise.reject(new Error("Conversion failed: Unsupported format"));
        }
  ...
  describe("Performance and Large Files", () => {
    it("handles large documents with progress tracking", async () => {
      ...
      Object.defineProperty(uploadProgressEvent, "loaded", { value: 33 * 1024 * 1024 });
      Object.defineProperty(uploadProgressEvent, "total", { value: 100 * 1024 * 1024 });
      ...
      for (let i = 0; i <= 100; i += 20) {
        await wrapper.vm.updateProgress(i, `Processing page ${i / 20 + 1} of 6`);
          ...
          return Promise.resolve({
            data: {
              document: {
                ...
                content: "<html><body><h1>HTML Preview</h1><p>This is HTML content</p></body></html>",
                slides: 10,
              },
            },
          });
  ...
  describe("Batch Processing", () => {
    it("supports batch upload and conversion", async () => {
        ...
        if (url.includes("/api/documents/batch")) {
          ...
          return Promise.resolve({
            data: {
              ...
              totalFiles: 3,
              results: [
                { id: "batch-doc1", status: "processed", originalName: "file1.pdf" },
                { id: "batch-doc2", status: "processed", originalName: "file2.docx" },
                { id: "batch-doc3", status: "processed", originalName: "file3.xlsx" },
              ],
            },
          });
        }
      ...
      expect(ApiService.post).toHaveBeenCalledWith("/api/documents/batch", expect.any(Object));
