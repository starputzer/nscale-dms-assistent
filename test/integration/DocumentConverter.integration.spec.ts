...
describe('DocumentConverter Integration Tests', () => {
  ...
  describe('Document Management', () => {
    it('shows document list when documents exist', async () => {
      ...
      store.convertedDocuments = [
        { id: 'doc-1', originalName: 'test1.pdf', status: 'success', originalFormat: 'pdf' },
        { id: 'doc-2', originalName: 'test2.pdf', status: 'success', originalFormat: 'pdf' }
      ];
  ...
  describe('Error Handling', () => {
    it('shows error display when upload fails', async () => {
      ...
      store.error = { code: 'UPLOAD_FAILED', message: 'Upload failed', timestamp: new Date() };
      await wrapper.vm.$nextTick();
    ...
    it('shows error display when conversion fails', async () => {
      ...
      // Set error in store to simulate the error being caught
      store.error = { code: 'CONVERSION_FAILED', message: 'Conversion failed', timestamp: new Date() };
      await wrapper.vm.$nextTick();
    ...
    it('can retry after an error', async () => {
      // Set error in store
      store.error = { code: 'INITIALIZATION_FAILED', message: 'Init failed', timestamp: new Date() };
      await wrapper.vm.$nextTick();
