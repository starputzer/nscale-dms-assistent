...
import transactionManager, { TransactionStatus } from '../../../src/bridge/enhanced/sync/TransactionManager';
...
describe('TransactionManager', () => {
  ...
  it('should capture snapshots for state changes', () => {
    ...
    expect(transactionManager['stateTransactions'].get(pathStr)?.has(txId)).toBe(true);
  });
  ...
  it('should check if a path is in an active transaction', () => {
    ...
    const isActiveExcluded = transactionManager.isPathInActiveTransaction(statePath, [txId]);
  ...
  it('should handle transaction conflicts', () => {
    ...
    // Assert
    const isPathActive = transactionManager.isPathInActiveTransaction(sharedPath);
    expect(isPathActive).toBe(true);
    
    // Verify we can check if path is in a transaction excluding tx1
    const isPathActiveExcludingTx1 = transactionManager.isPathInActiveTransaction(sharedPath, [tx1Id]);
