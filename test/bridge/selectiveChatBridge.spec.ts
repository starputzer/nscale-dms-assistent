...
describe('SelectiveChatBridge', () => {
  ...
  describe('Initialisierung', () => {
    it('sollte erfolgreich initialisiert werden', async () => {
      // Arrange
      const initSpy = vi.spyOn(chatBridge as any, 'connectToVue').mockResolvedValue(true);
    ...
    it('sollte einen Fehler zurückgeben, wenn die Verbindung fehlschlägt', async () => {
      // Arrange
  ...
  describe('Nachrichtenverwaltung', () => {
    it('sollte Nachrichten selektiv synchronisieren', async () => {
      // Arrange
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        (chatBridge as any).messages.value.get(sessionId).set(messageId, message);
      ...
      const emitCalls = emitSpy.mock.calls.filter(call => call[0] === 'vueChat:messagesUpdated');
    ...
    it('sollte streaming-Nachrichten priorisieren', async () => {
      // Arrange
      vi.spyOn(chatBridge as any, 'connectToVue').mockResolvedValue(true);
      await chatBridge.initialize();
      ...
      (chatBridge as any).messages.value.get(sessionId).set(streamingMessageId, streamingMessage);
  ...
  describe('Ereignisverarbeitung', () => {
    it('sollte Ereignisse von Vue korrekt verarbeiten', async () => {
      // Arrange
      vi.spyOn(chatBridge as any, 'connectToVue').mockResolvedValue(true);
      await chatBridge.initialize();
      ...
      expect((chatBridge as any).messages.value.get(sessionId).has(messageId)).toBe(true);
    });
    
    it('sollte Nachrichten an Vue senden können', async () => {
      // Arrange
      vi.spyOn(chatBridge as any, 'connectToVue').mockResolvedValue(true);
      await chatBridge.initialize();
      ...
      // Assert
      expect(result).toBe(true);
      expect(emitSpy).toHaveBeenCalledWith('vanillaChat:sendMessage', expect.objectContaining({
        content: 'Hello from test',
        sessionId
      }));
    });
    
    it('sollte einen Timeout zurückgeben, wenn keine Antwort von Vue kommt', async () => {
      // Arrange
      vi.spyOn(chatBridge as any, 'connectToVue').mockResolvedValue(true);
      await chatBridge.initialize();
      ...
      const sendPromise = chatBridge.sendMessage('Message with timeout', sessionId);
  ...
  describe('Wiederherstellung', () => {
    it('sollte bei Verbindungsverlust Wiederverbindung versuchen', async () => {
      // Arrange
      const connectSpy = vi.spyOn(chatBridge as any, 'connectToVue')
      ...
      await vi.advanceTimersByTimeAsync((chatBridge as any).config.retryDelay * 2);
    ...
    it('sollte nach der maximalen Anzahl von Versuchen aufgeben', async () => {
      // Arrange
      ...
      for (let i = 0; i < (chatBridge as any).config.maxRetryAttempts; i++) {
        await vi.advanceTimersByTimeAsync((chatBridge as any).config.retryDelay * (i + 1));
      }
  ...
  describe('Ressourcenverwaltung', () => {
    it('sollte alle Ressourcen beim Dispose freigeben', () => {
      // Arrange
  ...
  describe('Integration mit Legacy-Code', () => {
    it('sollte Legacy-Funktionen aufrufen, wenn Ereignisse von Vue empfangen werden', async () => {
      // Arrange
      vi.spyOn(chatBridge as any, 'connectToVue').mockResolvedValue(true);
      await chatBridge.initialize();
      ...
      eventBus.emit('vueChat:sessionsUpdated', {
        sessions: [
          { id: 'session-1', title: 'Session 1', lastActivity: Date.now(), messageCount: 5 }
        ],
      ...
      // Assert
      expect(onStatusUpdatedSpy).toHaveBeenCalledWith(expect.objectContaining({
        isLoading: true,
        isSending: false,
        hasStreamingMessage: true
      }));
      
      expect(onSessionsUpdatedSpy).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('sollte Vue-Ereignisse auslösen, wenn Legacy-Funktionen aufgerufen werden', async () => {
      // Arrange
      vi.spyOn(chatBridge as any, 'connectToVue').mockResolvedValue(true);
      await chatBridge.initialize();
