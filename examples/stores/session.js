...
export const useSessionStore = defineStore('session', () => {
  ...
  const setupEventSource = (question, assistantMessageId) => {
    ...
    const source = new EventSource(`/api/chat/stream?session_id=${currentSessionId.value}&question=${encodeURIComponent(question)}&token=${authStore.token}`);
    ...
    source.onmessage = (event) => {
      ...
      if (data.message_id) {
        ...
        if (index !== -1) {
          const updatedMessage = { ...messages.value[index], id: data.message_id };
  ...
  const updateSessionTitle = async () => {
    ...
    try {
      const response = await axios.post(`/api/sessions/${currentSessionId.value}/title`, {}, {
        headers: { Authorization: `Bearer ${authStore.token}` }
      });
