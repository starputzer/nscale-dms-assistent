  ...
  <div class="conversion-example">
    ...
    <div class="demo-controls">
      ...
      <div class="control-group">
        <label for="progress-slider">Fortschritt: {{ conversionState.progress }}%</label>
        <input 
          id="progress-slider" 
          type="range" 
          min="0" 
      ...
      <div class="control-group">
        <label for="time-slider">Verbleibende Zeit: {{ conversionState.timeRemaining }} Sekunden</label>
        <input 
          id="time-slider" 
          type="range" 
          min="0" 
      ...
      <div class="control-group">
        ...
        <select id="step-select" v-model="conversionState.currentStep">
          <option value="Dokument wird konvertiert...">Dokument wird konvertiert...</option>
          <option value="Extrahiere Text aus PDF...">Extrahiere Text aus PDF...</option>
          <option value="Analysiere Dokumentstruktur...">Analysiere Dokumentstruktur...</option>
          <option value="Tabellen werden erkannt...">Tabellen werden erkannt...</option>
          <option value="Wandle Inhalt in Textformat um...">Wandle Inhalt in Textformat um...</option>
          <option value="Optimiere Ausgabe...">Optimiere Ausgabe...</option>
          <option value="Finalisiere Konvertierung...">Finalisiere Konvertierung...</option>
        </select>
      </div>
  ...
  demoInterval = window.setInterval(() => {
    ...
    if (conversionState.progress < 100) {
      ...
      if (conversionState.timeRemaining > 0) {
        conversionState.timeRemaining = Math.max(0, 120 - Math.floor(conversionState.progress * 1.2));
      }
      
      // Aktualisiert den Schritt basierend auf dem Fortschritt
      if (currentStepIndex < steps.length && conversionState.progress >= steps[currentStepIndex].progress) {
      ...
      if (conversionState.progress >= 100) {
        ...
        conversionState.details = 'Alle Seiten erfolgreich konvertiert. Dokument bereit.';
        clearInterval(demoInterval!);
        demoInterval = null;
      }
    } else {
