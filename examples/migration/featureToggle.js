...
export const useFeatureTogglesStore = defineStore('featureToggles', {
  ...
  getters: {
    ...
    isUsingFullSfcUI: (state) => {
      return state.useSfcAuth && state.useSfcChat && state.useSfcAdmin 
          && state.useSfcSettings && state.useSfcFeedback;
    },
    ...
    logFeatureChange(feature, enabled) {
      ...
      if (this.isDebugMode) {
        try {
          ...
          }).catch(err => console.error('Telemetrie konnte nicht gesendet werden:', err));
        } catch (error) {
