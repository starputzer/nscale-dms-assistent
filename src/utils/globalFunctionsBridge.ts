...
export interface TelemetryFunctions {
  ...
  trackPerformance: (metricName: string, durationMs: number, context?: Record<string, any>) => void
...
export interface ABTestingFunctions {
  ...
  registerTest: (testName: string, variants: string[], weights?: number[]) => void
...
function initializeSourceReferencesFunctions(): void {
  ...
  window.isSourceReferencesVisible = (message?: any) => {
    try {
      ...
      if (typeof window.__sourceReferencesComposable !== 'undefined') {
        return message ? window.__sourceReferencesComposable.isSourceReferencesVisible(message) : false;
      }
  ...
  window.showSourceReferencesForMessage = (messageId: string, sources?: SourceReference[]) => {
    const store = getStore()
...
function initializeTelemetryFunctions(): void {
  ...
  window.telemetry = {
    ...
    trackPerformance: (metricName: string, durationMs: number, context = {}) => {
...
function initializeABTestingFunctions(): void {
  ...
  const activeTests: Record<string, {
    variants: string[],
    weights: number[],
    active: boolean
  }> = {}
...
function initializeChatFunctions(): void {
  ...
  window.sendQuestion = async (question: string, sessionId?: string) => {
    info(`Sende Frage: ${question}${sessionId ? ` (Session: ${sessionId})` : ''}`)
  ...
  window.sendQuestionStream = async (
    ...
    info(`Sende Streaming-Frage: ${question}${sessionId ? ` (Session: ${sessionId})` : ''}`)
...
declare global {
  interface Window {
    ...
    showSourceReferencesForMessage: (messageId: string, sources?: SourceReference[]) => void
