...
function setupFallbackFunctions() {
  ...
  if (typeof (window as any).isSourceReferencesVisible !== 'function') {
    (window as any).isSourceReferencesVisible = (message: any) => {
      console.warn('[sourceReferenceAdapter] Fallback-Funktion isSourceReferencesVisible verwendet');
      try {
        // Verwende das sichere Composable
        return safeComposable.isSourceReferencesVisible(message);
      } catch (error) {
        console.error('[sourceReferenceAdapter] Fehler in Fallback-Funktion isSourceReferencesVisible:', error);
        return false;
      }
    };
  }
  ...
  if (typeof (window as any).isLoadingSourceReferences !== 'function') {
    (window as any).isLoadingSourceReferences = (message: any) => {
      console.warn('[sourceReferenceAdapter] Fallback-Funktion isLoadingSourceReferences verwendet');
      try {
        return safeComposable.isLoadingSourceReferences(message);
      } catch (error) {
        console.error('[sourceReferenceAdapter] Fehler in Fallback-Funktion isLoadingSourceReferences:', error);
        return false;
      }
    };
  }
  ...
  if (typeof (window as any).getSourceReferences !== 'function') {
    (window as any).getSourceReferences = (message: any) => {
      console.warn('[sourceReferenceAdapter] Fallback-Funktion getSourceReferences verwendet');
      try {
        return safeComposable.getSourceReferences(message);
      } catch (error) {
        console.error('[sourceReferenceAdapter] Fehler in Fallback-Funktion getSourceReferences:', error);
        return [];
      }
    };
  }
  ...
  if (typeof (window as any).toggleSourceReferences !== 'function') {
    (window as any).toggleSourceReferences = (message: any) => {
      console.warn('[sourceReferenceAdapter] Fallback-Funktion toggleSourceReferences verwendet');
      try {
        return safeComposable.toggleSourceReferences(message);
      } catch (error) {
        console.error('[sourceReferenceAdapter] Fehler in Fallback-Funktion toggleSourceReferences:', error);
      }
    };
  }
...
export function initializeSourceReferenceAdapter() {
  console.log('[sourceReferenceAdapter] Initialisierung des Source Reference Adapters...');
  ...
  try {
    ...
    if (typeof (window as any).__sourceReferencesState !== 'undefined') {
      console.log('[sourceReferenceAdapter] Globales Source-References-System gefunden, synchronisiere...');
      
      // Synchronisieren des initialen Zustands
      try {
        // Für jedes Objekt im globalen State...
        const globalVisibleRefs = (window as any).__sourceReferencesState.visibleSourceReferences || {};
        ...
        Object.keys(globalVisibleRefs).forEach(messageId => {
          if (globalVisibleRefs[messageId] && visibleSourceReferences.value) {
            visibleSourceReferences.value[messageId] = globalVisibleRefs[messageId];
          }
        });
        
        console.log('[sourceReferenceAdapter] Zustand synchronisiert:', {
          globalState: (window as any).__sourceReferencesState.visibleSourceReferences,
          localState: visibleSourceReferences.value
        });
        
        // Event-Listener für Synchronisation einrichten
        window.addEventListener('sourceReferencesToggled', ((event: CustomEvent) => {
          try {
            const { messageId, visible } = event.detail;
            if (messageId && visibleSourceReferences.value) {
              console.log('[sourceReferenceAdapter] Event sourceReferencesToggled empfangen:', messageId, visible);
              visibleSourceReferences.value[messageId] = visible;
            }
          } catch (error) {
            console.error('[sourceReferenceAdapter] Fehler bei Event-Verarbeitung:', error);
          }
        }) as EventListener);
      } catch (error) {
        console.error('[sourceReferenceAdapter] Fehler bei der Synchronisierung mit globalem Zustand:', error);
      }
    }
    // Sonst: Unsere eigenen Funktionen registrieren, aber nur wenn sie nicht schon existieren
      ...
      if (typeof (window as any).isSourceReferencesVisible !== 'function') {
        (window as any).isSourceReferencesVisible = (message: any) => {
          ...
          if (!message) {
            console.warn('[sourceReferenceAdapter] isSourceReferencesVisible: message is undefined or null');
            return false;
          }
    
          try {
            console.log('[sourceReferenceAdapter] isSourceReferencesVisible aufgerufen', message);
            ...
            console.error('[sourceReferenceAdapter] Fehler in isSourceReferencesVisible:', error);
      ...
      if (typeof (window as any).isLoadingSourceReferences !== 'function') {
        (window as any).isLoadingSourceReferences = (message: any) => {
          console.log('[sourceReferenceAdapter] isLoadingSourceReferences aufgerufen', message);
      ...
      if (typeof (window as any).getSourceReferences !== 'function') {
        (window as any).getSourceReferences = (message: any) => {
          console.log('[sourceReferenceAdapter] getSourceReferences aufgerufen', message);
          return getSourceReferences(message);
        };
      }
    
      if (typeof (window as any).isSourceDetailOpen !== 'function') {
        (window as any).isSourceDetailOpen = (message: any, sourceIndex: number) => {
          console.log('[sourceReferenceAdapter] isSourceDetailOpen aufgerufen', message, sourceIndex);
          return isSourceDetailOpen(message, sourceIndex);
        };
      }
    
      if (typeof (window as any).toggleSourceDetail !== 'function') {
        (window as any).toggleSourceDetail = (message: any, sourceIndex: number) => {
          console.log('[sourceReferenceAdapter] toggleSourceDetail aufgerufen', message, sourceIndex);
      ...
      if (typeof (window as any).toggleSourceReferences !== 'function') {
        (window as any).toggleSourceReferences = async (message: any) => {
          console.log('[sourceReferenceAdapter] toggleSourceReferences aufgerufen', message);
          toggleSourceReferences(message);
        };
      }
    
      if (typeof (window as any).showSourcePopupHandler !== 'function') {
        (window as any).showSourcePopupHandler = (event: MouseEvent, sourceId: string) => {
          console.log('[sourceReferenceAdapter] showSourcePopupHandler aufgerufen', sourceId);
    ...
    functionsToTrack.forEach(key => {
      ...
      if (typeof originalFunction === 'function' && !originalFunction.__isTracked) {
    ...
    // Erfolgsmeldung
    console.log('[sourceReferenceAdapter] Source Reference Adapter erfolgreich initialisiert!');
    
    // Debug-Info ausgeben
    let functionStatus: Record<string, boolean> = {};
    functionsToTrack.forEach(fn => {
      functionStatus[fn] = typeof (window as any)[fn] === 'function';
    });
    console.log('[sourceReferenceAdapter] Verfügbare Funktionen:', functionStatus);
    
  } catch (error) {
    console.error('[sourceReferenceAdapter] Fehler bei der Initialisierung des Source Reference Adapters:', error);
