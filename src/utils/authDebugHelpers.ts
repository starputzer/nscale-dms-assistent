  ...
  syncAll: () => {
    ...
    if (token) {
      ...
      if (window.apiService?.axiosInstance) {
        window.apiService.axiosInstance.defaults.headers.common['Authorization'] = `Bearer ${token}`;
...
console.log('%c🔐 Auth Debug Commands Available:', 'color: #00a550; font-weight: bold;');
console.log('%c  window.authDebug.checkStatus() - Check auth status across all sources', 'color: #666;');
console.log('%c  window.authDebug.fixStatus() - Attempt to fix inconsistent auth status', 'color: #666;');
console.log('%c  window.authDebug.forceLogin(token) - Force authentication with token', 'color: #666;');
console.log('%c  window.authDebug.showReport() - Show detailed auth sync report', 'color: #666;');
console.log('%c  window.authDebug.syncAll() - Force sync auth everywhere and reload', 'color: #666;');
