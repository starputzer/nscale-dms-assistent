 ...
 */
export function formatApiError(error: any, defaultMessage: string = 'Ein unbekannter Fehler ist aufgetreten'): APIError {
  ...
  if (error.response && error.response.status) {
    ...
    const errorCode = HTTP_STATUS_TO_ERROR_CODE[statusCode] || APIErrorCode.UNKNOWN_ERROR;
 ...
 */
export function hasErrorCode(error: Nullable<APIError>, code: APIErrorCode | string): boolean {
 ...
 */
export function toErrorResult<T>(error: any, defaultMessage: string = 'Ein unbekannter Fehler ist aufgetreten'): Result<T, APIError> {
  return {
    success: false,
...
export const HTTP_STATUS_MESSAGES: Record<number, string> = {
  ...
  [StatusCode.CONFLICT]: 'Die Anfrage konnte wegen eines Konflikts nicht durchgeführt werden',
  [StatusCode.TOO_MANY_REQUESTS]: 'Zu viele Anfragen, bitte versuchen Sie es später erneut',
  [StatusCode.INTERNAL_SERVER_ERROR]: 'Ein Server-Fehler ist aufgetreten',
  [StatusCode.NOT_IMPLEMENTED]: 'Diese Funktionalität ist noch nicht implementiert',
  ...
  [StatusCode.GATEWAY_TIMEOUT]: 'Zeitüberschreitung bei der Verbindung zum Server'
};
...
export function getStatusCodeMessage(statusCode: number): string {
  return HTTP_STATUS_MESSAGES[statusCode] || 'Ein unbekannter Fehler ist aufgetreten';
}
