...
export class AuthStatusSync {
  ...
  checkAuthConsistency() {
    ...
    const localStorageToken = localStorage.getItem('nscale_access_token') || 
    ...
    if (window.apiService) {
      ...
      const authHeader = headers?.common?.['Authorization'] || headers?.['Authorization'];
      sources.push({
        source: 'apiService',
    ...
    if (!isConsistent) {
      ...
      this.logger.debug('Auth status is consistent:', allAuthenticated ? 'authenticated' : 'unauthenticated');
    }
  ...
  fixAuthStatus() {
    ...
    try {
      ...
      if (window.apiService?.axiosInstance) {
        if (isAuthenticated && localStorageToken) {
          window.apiService.axiosInstance.defaults.headers.common['Authorization'] = `Bearer ${localStorageToken}`;
        } else {
          delete window.apiService.axiosInstance.defaults.headers.common['Authorization'];
        }
      }
      ...
      window.dispatchEvent(new CustomEvent('authStatusSynced', { 
