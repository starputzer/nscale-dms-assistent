...
import { useFallbackManager, type FallbackError, type FallbackErrorSeverity } from './fallbackManager';
...
const DEFAULT_OPTIONS: Required<ErrorReportingOptions> = {
  ...
  privacy: {
    ...
    sensitiveFields: [
      ...
      'email', 'phone', 'address', 'name', 'credit', 'card'
    ]
  }
};
...
export class ErrorReportingService {
  ...
  private user = reactive<Required<ErrorReportingOptions['userInfo']>>(DEFAULT_OPTIONS.userInfo);
   ...
   */
  private isDuplicate(report: ErrorReport): { isDuplicate: boolean, index?: number } {
  ...
  private collectUIState(): ErrorReport['uiState'] {
    ...
    try {
      ...
      const featureKeys = Object.keys(this.featureToggles.$state)
        .filter(key => typeof this.featureToggles.$state[key] === 'boolean');
      ...
      this.logger.warn('[ErrorReportingService] Fehler beim Erfassen des UI-States', { error });
      return undefined;
    }
  }
  ...
  private createErrorReport(
    error: Error | string,
    options: Partial<Omit<ErrorReport, 'id' | 'timestamp' | 'handled' | 'reported' | 'message' | 'originalError'>> = {}
  ): ErrorReport {
  ...
  public captureError(
    ...
    if (this.options.consoleLogLevel !== 'none') {
      const method = report.severity === 'critical' || report.severity === 'high' 
        ? 'error'
        : report.severity === 'medium' ? 'warn' : 'debug';
    ...
    if (this.options.onError) {
      try {
        ...
        this.logger.error('[ErrorReportingService] Fehler im onError-Callback', { error: callbackError });
      }
    }
  ...
  private reportToRemoteService(report: ErrorReport): void {
    ...
    fetch(this.options.remoteEndpoint, {
      ...
      this.logger.warn('[ErrorReportingService] Fehler beim Senden des Berichts', { error });
    });
  }
  ...
  public captureSourceError(
    ...
    name: string, 
    error: Error | string, 
    options: Partial<Omit<ErrorReport, 'id' | 'timestamp' | 'handled' | 'reported' | 'message' | 'originalError' | 'source'>> = {}
  ): string {
   ...
   */
  public setUserInfo(userInfo: Partial<ErrorReportingOptions['userInfo']>): void {
 ...
 */
export function useErrorReporting(options: ErrorReportingOptions = {}): ErrorReportingService {
