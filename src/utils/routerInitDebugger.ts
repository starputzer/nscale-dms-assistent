...
export class RouterInitDebugger {
   ...
   */
  markStage(stage: string, status: 'started' | 'completed' | 'failed', error?: Error): void {
    if (status === 'started') {
  ...
  private analyzeRootCause(initReport: any, routerReport: any): string {
    ...
    if (piniaStage && routerStage && routerStage.timestamp < piniaStage.timestamp) {
      return 'ROUTER_BEFORE_PINIA';
    }
    
    // Prüfe auf fehlende Pinia
    if (routerReport.issues.some((i: any) => i.type === 'pinia-initialization')) {
      return 'PINIA_NOT_INITIALIZED';
    }
    
    // Prüfe auf currentRoute-Problem
    if (routerReport.issues.some((i: any) => i.type === 'currentRoute-undefined')) {
      return 'CURRENT_ROUTE_UNDEFINED';
    }
    
    // Prüfe auf Race Conditions
    if (initReport.warnings.some((w: string) => w.includes('fehlende Abhängigkeiten'))) {
  ...
  private generateSolution(rootCause: string): string {
    const solutions: Record<string, string> = {
      ROUTER_BEFORE_PINIA: 'Router wird vor Pinia initialisiert. Ändere die Initialisierungsreihenfolge in main.ts.',
      PINIA_NOT_INITIALIZED: 'Pinia ist nicht verfügbar wenn Router-Guards ausgeführt werden. Stelle sicher, dass Pinia vor dem Router initialisiert wird.',
      CURRENT_ROUTE_UNDEFINED: 'Router.currentRoute ist undefined. Warte auf router.isReady() bevor die App gemountet wird.',
      RACE_CONDITION: 'Race Condition zwischen verschiedenen Initialisierungsphasen. Verwende async/await für sequentielle Initialisierung.',
  ...
  private generateCodeChanges(rootCause: string): string[] {
    ...
    return changes[rootCause] || ['// Keine spezifischen Code-Änderungen verfügbar'];
  }
  ...
  private installDebugCommands(): void {
    ...
    const commands = {
      ...
      showCodeFixes: () => {
        ...
        console.log('\nCode:');
        report.combinedAnalysis.codeChanges.forEach(line => console.log(line));
        console.log('================================');
      },
