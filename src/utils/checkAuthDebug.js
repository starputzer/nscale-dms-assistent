...
function checkAuth() {
  ...
  console.log(
    `  nscale_access_token: ${localStorage.getItem("nscale_access_token")?.substring(0, 20) || "null"}`,
  );
  console.log(
    `  nscale_refresh_token: ${localStorage.getItem("nscale_refresh_token")?.substring(0, 20) || "null"}`,
  );
  ...
  if (window.__PINIA__) {
    ...
    console.log(
      `  refreshToken: ${authStore?.refreshToken?.substring(0, 20) || "null"}`,
    );
  ...
  if (window.axios) {
    ...
    console.log(
      `  Authorization: ${window.axios.defaults.headers.common["Authorization"] || "not set"}`,
    );
  }
