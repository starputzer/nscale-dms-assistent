<template>
  <div class="dialog-example">
    ...
    <div class="button-group">
      ...
      <button @click="showConfirmDialog" class="example-btn">
        Bestätigungsdialog
      </button>
      <button @click="showInfoDialog" class="example-btn">Info-Dialog</button>
      <button @click="showWarningDialog" class="example-btn">
        Warnungs-Dialog
      </button>
      <button @click="showErrorDialog" class="example-btn">
        Fehler-Dialog
      </button>
      <button @click="showSuccessDialog" class="example-btn">
        Erfolgs-Dialog
      </button>
    </div>
    ...
    <div class="button-group">
      ...
      <button @click="showPromptDialog" class="example-btn">
        Text-Eingabe
      </button>
      <button @click="showPasswordDialog" class="example-btn">
        Passwort-Eingabe
      </button>
      <button @click="showValidatedInput" class="example-btn">
        Validierte Eingabe
      </button>
    </div>
    ...
    <div class="button-group">
      ...
      <button @click="useWindowConfirm" class="example-btn">
        window.confirm()
      </button>
      <button @click="useWindowPrompt" class="example-btn">
        window.prompt()
      </button>
      <button @click="useWindowAlert" class="example-btn">
        window.alert()
      </button>
    </div>
...
const showInfoDialog = async () => {
  const confirmed = await dialog.info({
    title: "Information",
    message:
      "Ihre Sitzung wird in 5 Minuten automatisch beendet. Bitte speichern Sie Ihre Arbeit.",
...
const showWarningDialog = async () => {
  const confirmed = await dialog.warning({
    title: "Warnung",
    message:
      "Diese Aktion kann zu Datenverlust führen. Möchten Sie trotzdem fortfahren?",
  });
...
const useWindowConfirm = async () => {
  const confirmed = await window.confirm(
    "Möchten Sie fortfahren? (Ersetzt window.confirm)",
  );
...
const useWindowPrompt = async () => {
  const name = await window.prompt(
    "Geben Sie Ihren Namen ein: (Ersetzt window.prompt)",
    "Max Mustermann",
  );
