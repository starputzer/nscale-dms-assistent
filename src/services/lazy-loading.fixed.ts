...
const componentRegistry = {
  // Admin Komponenten
  AdminPanel: () => import("@/components/admin/AdminPanel.simplified.vue"),
...
export async function preloadComponents(
  ...
  // First, preload required base components if not already in cache
  const baseComponents = ["Input", "Toast", "Dialog"];

  // Preload base components first if needed and any aren't cached yet
  for (const baseName of baseComponents) {
    if (!componentCache.has(baseName)) {
      try {
        const loader =
          componentRegistry[baseName as keyof typeof componentRegistry];
        if (loader) {
          console.log(`Preloading base component: ${baseName}`);
          const module = await loader();
          // Create the component and cache it
          const component = defineAsyncComponent({
          ...
          componentCache.set(baseName, component);
        }
      } catch (err) {
        console.error(`Error preloading base component ${baseName}:`, err);
      }
    }
  }

  // Then, preload the requested components
  const promises = componentNames.map((name) => {
    ...
    if (loader) {
      ...
      return loader().catch((err) => {
        console.error(`Failed to preload component ${name}:`, err);
      });
    }
    return Promise.resolve();
  });
...
export function setupIntelligentPreloading(): void {
  // Always preload the essential UI components first
  preloadComponents(["Input", "Toast", "Dialog"]);
  ...
  try {
    ...
    console.warn("Error accessing auth store for preloading:", error);
  }
