...
export class AuthTokenManager {
  ...
  public loadTokens(): StoredTokenData | null {
    ...
    if (!data) {
      console.log('[AuthTokenManager] Keine Daten in localStorage, versuche sessionStorage');
      data = this.loadFromSessionStorage();
    }
    
    if (!data) {
      console.log('[AuthTokenManager] Keine Daten in sessionStorage, versuche Legacy-Keys');
  ...
  public validateToken(token: string): boolean {
    try {
      ...
      console.error('[AuthTokenManager] Token-Validierung fehlgeschlagen:', error);
      return false;
    }
  }
   ...
   */
  public extractUserFromToken(token: string): StoredTokenData['userInfo'] | null {
    try {
      const payload = jwtDecode<TokenPayload>(token);
      ...
      console.error('[AuthTokenManager] Fehler beim Extrahieren der Benutzerdaten:', error);
      return null;
    }
  }
  ...
  private saveToLocalStorage(data: StoredTokenData): void {
    try {
      ...
      if (data.userInfo) {
        localStorage.setItem(this.TOKEN_KEYS.user, JSON.stringify(data.userInfo));
      }
    } catch (error) {
      console.error('[AuthTokenManager] Fehler beim Speichern in localStorage:', error);
    }
  }
  ...
  private saveToSessionStorage(data: StoredTokenData): void {
    try {
      ...
      sessionStorage.setItem(this.SESSION_KEYS.expires, data.expiresAt.toString());
      
      if (data.userInfo) {
        sessionStorage.setItem(this.SESSION_KEYS.user, JSON.stringify(data.userInfo));
      }
    } catch (error) {
      console.error('[AuthTokenManager] Fehler beim Speichern in sessionStorage:', error);
    }
  }
  ...
  private loadFromLocalStorage(): StoredTokenData | null {
    try {
      ...
      return {
        ...
        expiresAt: expires ? parseInt(expires, 10) : Date.now() + 24 * 60 * 60 * 1000,
        userInfo: userStr ? JSON.parse(userStr) : undefined
      };
    } catch (error) {
      console.error('[AuthTokenManager] Fehler beim Laden aus localStorage:', error);
      return null;
    }
  }
  ...
  private loadFromSessionStorage(): StoredTokenData | null {
    try {
      ...
      console.error('[AuthTokenManager] Fehler beim Laden aus sessionStorage:', error);
      return null;
    }
  }
  ...
  private loadFromLegacyStorage(): StoredTokenData | null {
    try {
      ...
      console.error('[AuthTokenManager] Fehler beim Laden aus Legacy-Storage:', error);
      return null;
    }
  }
