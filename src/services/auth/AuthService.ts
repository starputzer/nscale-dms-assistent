...
export class AuthError extends Error {
  ...
  static fromAxiosError(error: AxiosError): AuthError {
    ...
    if (!error.response) {
      type = 'network'
      message = 'Netzwerkfehler: Keine Verbindung zum Authentifizierungsserver möglich'
    } 
    // Basierend auf HTTP-Statuscodes
      ...
      switch (status) {
        ...
        case 401:
          ...
          message = 'Ungültige Anmeldedaten. Bitte überprüfen Sie Ihre E-Mail und Passwort.'
          break
        case 403:
          type = 'account_locked'
    ...
    if (error.code === 'ECONNABORTED' || message.includes('timeout')) {
      type = 'timeout'
      message = 'Zeitüberschreitung bei der Verbindung zum Authentifizierungsserver.'
    }
...
export class AuthService {
  ...
  public async login(credentials: LoginCredentials): Promise<AuthResponse> {
    try {
      ...
      // Generische Fehler
      throw new AuthError(
        error instanceof Error ? error.message : 'Unbekannter Fehler beim Login',
        'unknown',
        undefined,
        { originalError: error }
      )
  ...
  public async register(credentials: RegisterCredentials): Promise<AuthResponse> {
    try {
      ...
      // Generische Fehler
      throw new AuthError(
        error instanceof Error ? error.message : 'Unbekannter Fehler bei der Registrierung',
        'unknown',
        undefined,
        { originalError: error }
      )
  ...
  public async logout(): Promise<void> {
    ...
    if (token) {
      try {
        ...
        console.warn('Fehler beim Server-Logout - Token wird lokal gelöscht', error)
      }
    }
  ...
  public async refreshToken(): Promise<string> {
    ...
    if (!token) {
      throw new AuthError('Kein Token zum Aktualisieren vorhanden', 'token_invalid')
    }
    ...
    this.tokenRefreshPromise = new Promise<string>(async (resolve, reject) => {
      try {
        ...
        if (!data.token) {
          throw new AuthError('Kein neues Token vom Server erhalten', 'token_invalid')
        }
  ...
  public async validateToken(): Promise<boolean> {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && !error.response) {
        console.warn('Netzwerkfehler bei Token-Validierung - setze Token als gültig voraus')
