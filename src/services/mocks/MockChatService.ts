...
const MOCK_MESSAGES: Record<string, ChatMessage[]> = {
  '1': [
    ...
    },
    {
      id: '103',
      sessionId: '1',
      content: 'Die Dokumentensuche können Sie über das Menü "Suchen" aufrufen. Dort können Sie nach Dokumenten suchen, indem Sie Stichwörter, Dateinamen oder Dokumenteninhalte eingeben.',
      role: 'assistant',
  ...
  '2': [
    ...
    },
    {
      id: '203',
      sessionId: '2',
      content: 'Um Dokumente hochzuladen, klicken Sie auf den Button "Hochladen" in der oberen Menüleiste. Sie können dann Dateien von Ihrem Computer auswählen oder per Drag & Drop in das Uploadfenster ziehen.',
      role: 'assistant',
...
export class MockChatService {
    ...
    return new Promise(async (resolve) => {
      ...
      const sendNextChunk = async () => {
        if (!this.activeStreams.has(streamId) || currentIndex >= words.length) {
          ...
          const messageIndex = this.messages[sessionId].findIndex(m => m.id === assistantMessageId);
          if (messageIndex !== -1) {
        ...
        const chunkSize = Math.min(1 + Math.floor(Math.random() * 3), words.length - currentIndex);
  ...
  private generateResponse(userMessage: string): string {
    ...
    if (lowercaseMessage.includes('hallo') || lowercaseMessage.includes('hi') || lowercaseMessage.includes('guten tag')) {
      return 'Hallo! Wie kann ich Ihnen heute helfen?';
    }
    
    if (lowercaseMessage.includes('dokument') && (lowercaseMessage.includes('hochladen') || lowercaseMessage.includes('upload'))) {
      return 'Um Dokumente hochzuladen, klicken Sie auf den Button "Hochladen" in der oberen Menüleiste. ' +
             'Sie können dann Dateien von Ihrem Computer auswählen oder per Drag & Drop in das Uploadfenster ziehen.';
    }
    
    if (lowercaseMessage.includes('suche') || lowercaseMessage.includes('finden')) {
      return 'Die Dokumentensuche finden Sie im Hauptmenü unter "Suchen". Dort können Sie Volltext- oder ' +
             'Metadatensuchen durchführen. Sie können auch erweiterte Suchfilter verwenden, um Ihre Ergebnisse einzugrenzen.';
    }
    
    if (lowercaseMessage.includes('einstellung') || lowercaseMessage.includes('konfiguration') || lowercaseMessage.includes('config')) {
      return 'Die Einstellungen können Sie über das Zahnradsymbol in der oberen rechten Ecke aufrufen. ' +
             'Dort können Sie verschiedene Aspekte der Anwendung anpassen, darunter Darstellung, Sprache und Benachrichtigungen.';
    }
    
    if (lowercaseMessage.includes('fehler') || lowercaseMessage.includes('problem') || lowercaseMessage.includes('funktioniert nicht')) {
      return 'Es tut mir leid, dass Sie ein Problem haben. Hier sind einige allgemeine Schritte zur Fehlerbehebung:\n' +
    ...
    return 'Vielen Dank für Ihre Nachricht. Ich bin ein einfacher Mock-Service ohne echte KI-Funktionalität. ' +
