...
export class MockBatchService {
  async sendBatch(requests: any[]): Promise<any> {
    console.log('[MockBatchService] Handling batch request with', requests.length, 'requests');
    
    // Return mock responses for each request
    const responses = requests.map(request => {
      const { endpoint, method } = request;
      
      console.log('[MockBatchService] Processing request:', { endpoint, method });
        ...
        return {
          ...
          data: {
            messages: [
              {
                id: uuidv4(),
                content: 'Welcome to the demo version of the Digital File Assistant! This is a demonstration environment.',
                role: 'assistant',
