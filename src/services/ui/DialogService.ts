 ...
 * Arten von Dialogen
 */
export type DialogType = 'confirm' | 'alert' | 'prompt' | 'info' | 'warning' | 'error' | 'success' | 'custom';
...
export interface ConfirmDialogOptions extends DialogBaseOptions {
   ...
   */
  confirmButtonVariant?: 'primary' | 'secondary' | 'tertiary' | 'danger' | 'success' | 'warning';
  
  /**
   * Variante des Abbrechen-Buttons
   */
  cancelButtonVariant?: 'primary' | 'secondary' | 'tertiary' | 'danger' | 'success' | 'warning';
...
function createDialog<T = any>(options: DialogBaseOptions): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    ...
    if (typeof document !== 'undefined') {
      state.value.previouslyFocusedElement = document.activeElement as HTMLElement;
    }
 ...
 */
function closeDialog(id: string, options: { trigger?: 'button' | 'mask' | 'esc' | 'auto' | 'api', result?: any } = {}): void {
...
function removeDialog(id: string, result?: any): void {
  ...
  if (state.value.activeDialog && state.value.activeDialog.id === id) {
    state.value.activeDialog = state.value.dialogs.length > 0 
      ? state.value.dialogs[state.value.dialogs.length - 1] 
      : null;
  }
  
  // Fokus zurücksetzen, wenn keine Dialoge mehr geöffnet sind
  if (state.value.dialogs.length === 0 && state.value.previouslyFocusedElement) {
    setTimeout(() => {
...
export const dialogService = {
  ...
  confirm(options: string | Partial<ConfirmDialogOptions>): Promise<boolean> {
    ...
    if (options.showCancelButton === false) {
      dialogOptions.buttons = dialogOptions.buttons.filter(b => b.returnValue !== false);
    }
  ...
  prompt(options: string | PromptDialogOptions): Promise<string | null> {
    ...
    const validateInput = () => {
      ...
      if (result === true || result === undefined) {
        ...
        inputValid.value = false;
        inputError.value = options.validationMessage || i18n.t('dialog.invalidInput');
        return false;
      }
    };
    ...
    if (options.showCancelButton === false) {
      dialogOptions.buttons = dialogOptions.buttons.filter(b => b.returnValue !== null);
    }
