...
function updateGroups(): void {
  ...
  groupIds.forEach(groupId => {
    ...
    if (groupNotifications.length > 0) {
      ...
      const highestPriority = groupNotifications.reduce((highest, notification) => {
        const currentWeight = PRIORITY_WEIGHTS[notification.priority || 'medium'] || 0;
        const highestWeight = PRIORITY_WEIGHTS[highest] || 0;
        return currentWeight > highestWeight ? notification.priority || 'medium' : highest;
      }, 'low' as NotificationPriority);
      
      // Zähle ungelesene Benachrichtigungen
      const unreadCount = groupNotifications.filter(notification => !notification.read).length;
      ...
      groups[groupId] = {
        ...
        notificationIds: groupNotifications.map(notification => notification.id)
      };
    }
  });
...
function removeExpiredNotifications(): void {
  ...
  state.notifications = state.notifications.filter(notification => {
    if (notification.expires && notification.expires > 0) {
      const expirationTime = new Date(notification.timestamp.getTime() + notification.expires);
...
export const notificationService = {
  ...
  add(message: string, options: NotificationOptions = {}): string {
    ...
    const notification: Notification = {
      ...
      expires: options.expires !== undefined ? options.expires : DEFAULT_EXPIRATION
    };
   ...
   * @returns ID der erstellten Benachrichtigung
   */
  system(message: string, options: Omit<NotificationOptions, 'type'> = {}): string {
    return this.add(message, {
      ...options,
      type: 'system',
      priority: options.priority || 'high',
