...
export class CacheService {
  ...
  public set<T>(key: string, data: T, options: {
    ...
    if (ttl > 60) { // Nur längerlebige Einträge persistieren
      try {
  ...
  public cleanup(): void {
    ...
    if (this.config.debug && expiredCount > 0) {
      this.logger.debug(`Cache: ${expiredCount} abgelaufene Einträge bereinigt`);
    }
  }
...
export const cacheService = new CacheService({
  ...
  storageType: (import.meta.env.VITE_CACHE_STORAGE || 'localStorage') as 'localStorage' | 'sessionStorage' | 'memory',
