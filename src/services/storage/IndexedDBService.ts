...
export class IndexedDBService {
    ...
    this.initPromise = new Promise<boolean>((resolve, reject) => {
      try {
        ...
        request.onupgradeneeded = (event) => {
          ...
          if (this.debug) {
            this.logger.info(`Datenbank-Upgrade von Version ${event.oldVersion} zu ${event.newVersion}`, {
              stores: this.stores
            });
          }
          ...
          for (const storeConfig of this.stores) {
            ...
            if (existingStoreNames.includes(storeConfig.name)) {
              ...
              if (this.debug) {
                this.logger.debug(`Store '${storeConfig.name}' existiert bereits`);
              }
            } else {
              ...
              if (this.debug) {
                this.logger.debug(`Store '${storeConfig.name}' erstellt`, storeConfig);
              }
            }
            ...
            if (storeConfig.indices) {
              ...
              for (const indexConfig of storeConfig.indices) {
                ...
                if (!existingIndexNames.includes(indexConfig.name)) {
                  ...
                  if (this.debug) {
                    this.logger.debug(`Index '${indexConfig.name}' erstellt in Store '${storeConfig.name}'`, indexConfig);
                  }
                }
              }
            }
        ...
        request.onsuccess = (event) => {
          ...
          // Debug-Informationen
          if (this.debug) {
            this.logger.info(`Datenbank '${this.dbName}' (v${this.version}) erfolgreich geöffnet`, {
        ...
        request.onerror = (event) => {
          ...
          this.logger.error(`Fehler beim Öffnen der Datenbank '${this.dbName}'`, error);
          reject(error);
        };
      } catch (error) {
        this.logger.error(`Ausnahme beim Initialisieren der Datenbank '${this.dbName}'`, error);
        reject(error);
      }
    }).catch((error) => {
      this.logger.error(`Initialisierung der Datenbank '${this.dbName}' fehlgeschlagen`, error);
      return false;
    });
  ...
  private setupDatabaseEventHandlers(): void {
    ...
    this.db.onversionchange = (event) => {
      this.logger.warn(`Eine andere Registerkarte hat ein Datenbank-Upgrade durchgeführt. Verbindung wird geschlossen.`);
      this.db?.close();
      this.initialized = false;
      this.initPromise = null;
      
      // Benutzer über die Änderung informieren
      window.dispatchEvent(new CustomEvent('indexeddb:versionchange', {
        detail: {
          dbName: this.dbName,
    ...
    this.db.onerror = (event) => {
      this.logger.error(`Datenbankfehler in '${this.dbName}'`, (event.target as IDBDatabase).error);
    };
  ...
  public close(): void {
    if (this.db) {
      ...
      if (this.debug) {
        this.logger.info(`Verbindung zur Datenbank '${this.dbName}' geschlossen`);
      }
    }
  }
  ...
  public async deleteDatabase(): Promise<boolean> {
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      request.onerror = (event) => {
        this.logger.error(`Fehler beim Löschen der Datenbank '${this.dbName}'`, (event.target as IDBOpenDBRequest).error);
  ...
  private getTransaction(
    ...
    if (!this.db) {
      this.logger.error(`Keine Datenbankverbindung für Transaktion in '${this.dbName}'`);
      return null;
    }
    ...
    try {
      ...
      this.logger.error(`Fehler beim Erstellen der Transaktion in '${this.dbName}'`, {
        stores: storeNames,
        mode,
        error
      });
  ...
  public async add<T = any>(storeName: string, item: T): Promise<any> {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      if (!transaction) {
        reject(new Error(`Transaktion für Store '${storeName}' konnte nicht erstellt werden`));
        return;
      }
      ...
      request.onerror = (event) => {
        this.logger.error(`Fehler beim Hinzufügen des Objekts in '${storeName}'`, (event.target as IDBRequest).error);
  ...
  public async addBulk<T = any>(storeName: string, items: T[]): Promise<any[]> {
    ...
    return new Promise<any[]>((resolve, reject) => {
      ...
      transaction.oncomplete = () => {
        if (this.debug) {
          this.logger.debug(`${items.length} Objekte in '${storeName}' hinzugefügt`);
        }
        resolve(keys);
      };
      
      transaction.onerror = (event) => {
        this.logger.error(`Fehler beim Hinzufügen mehrerer Objekte in '${storeName}'`, (event.target as IDBTransaction).error);
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        request.onsuccess = (event) => {
          ...
          if (completed === items.length) {
            if (this.debug) {
              this.logger.debug(`Alle ${items.length} Objekte in '${storeName}' hinzugefügt`);
            }
          }
        };
      }
  ...
  public async put<T = any>(storeName: string, item: T): Promise<any> {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      request.onerror = (event) => {
        this.logger.error(`Fehler beim Aktualisieren des Objekts in '${storeName}'`, (event.target as IDBRequest).error);
        reject((event.target as IDBRequest).error);
      };
    });
  }
   ...
   */
  public async upsert<T = any>(storeName: string, item: T, key?: any): Promise<any> {
    // Sicherstellen, dass die Datenbank initialisiert ist
    if (!this.initialized) {
      await this.init();
    }
  ...
  public async delete(storeName: string, key: any): Promise<boolean> {
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      request.onsuccess = () => {
        if (this.debug) {
          this.logger.debug(`Objekt mit Schlüssel '${key}' aus '${storeName}' gelöscht`);
        }
      ...
      request.onerror = (event) => {
        this.logger.error(`Fehler beim Löschen des Objekts aus '${storeName}'`, (event.target as IDBRequest).error);
        reject((event.target as IDBRequest).error);
      };
    });
  }
  ...
  public async deleteBulk(storeName: string, keys: any[]): Promise<boolean> {
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      transaction.oncomplete = () => {
        if (this.debug) {
          this.logger.debug(`${keys.length} Objekte aus '${storeName}' gelöscht`);
        }
        resolve(true);
      };
      
      transaction.onerror = (event) => {
        this.logger.error(`Fehler beim Löschen mehrerer Objekte aus '${storeName}'`, (event.target as IDBTransaction).error);
        reject((event.target as IDBTransaction).error);
      };
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        request.onsuccess = () => {
          ...
          if (completed === keys.length) {
            if (this.debug) {
              this.logger.debug(`Alle ${keys.length} Objekte aus '${storeName}' gelöscht`);
            }
          }
        };
      }
  ...
  public async clear(storeName: string): Promise<boolean> {
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      request.onerror = (event) => {
        this.logger.error(`Fehler beim Löschen aller Objekte aus '${storeName}'`, (event.target as IDBRequest).error);
        reject((event.target as IDBRequest).error);
      };
    });
  }
  ...
  public async get<T = any>(storeName: string, key: any): Promise<T | null> {
    ...
    return new Promise<T | null>((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (this.debug) {
          this.logger.debug(`Objekt mit Schlüssel '${key}' aus '${storeName}' geladen`, {
      ...
      request.onerror = (event) => {
        this.logger.error(`Fehler beim Laden des Objekts aus '${storeName}'`, (event.target as IDBRequest).error);
        reject((event.target as IDBRequest).error);
      };
    });
  }
   ...
   */
  public async getBulk<T = any>(storeName: string, keys: any[]): Promise<(T | null)[]> {
    // Sicherstellen, dass die Datenbank initialisiert ist
    if (!this.initialized) {
      await this.init();
    }
   ...
   */
  public async getAll<T = any>(storeName: string, options: QueryOptions = {}): Promise<T[]> {
    // Sicherstellen, dass die Datenbank initialisiert ist
    if (!this.initialized) {
      await this.init();
    }
    ...
    return new Promise<T[]>((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (this.debug) {
          this.logger.debug(`${limitedResults.length} Objekte aus '${storeName}' geladen`);
        }
      ...
      request.onerror = (event) => {
        this.logger.error(`Fehler beim Laden aller Objekte aus '${storeName}'`, (event.target as IDBRequest).error);
        reject((event.target as IDBRequest).error);
      };
    });
  }
   ...
   */
  public addChangeHandler<T = any>(storeName: string, handler: ChangeHandler<T>): void {
    ...
    if (this.changeHandlers.has(storeName)) {
      ...
      for (const handler of handlers) {
        try {
          ...
          this.logger.error(`Fehler beim Ausführen eines Änderungshandlers für '${storeName}'`, error);
        }
      }
    }
  }
   ...
   */
  public async count(storeName: string, options: QueryOptions = {}): Promise<number> {
    // Sicherstellen, dass die Datenbank initialisiert ist
    if (!this.initialized) {
      await this.init();
    }
    ...
    return new Promise<number>((resolve, reject) => {
      ...
      request.onerror = (event) => {
        this.logger.error(`Fehler beim Zählen der Objekte in '${storeName}'`, (event.target as IDBRequest).error);
        reject((event.target as IDBRequest).error);
      };
    });
  }
    ...
    return new Promise<T[]>((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (!cursor || (options.limit && results.length >= options.limit)) {
          if (this.debug) {
            this.logger.debug(`${results.length} Objekte aus '${storeName}' abgefragt`);
          }
      ...
      request.onerror = (event) => {
        this.logger.error(`Fehler bei der Abfrage in '${storeName}'`, (event.target as IDBRequest).error);
        reject((event.target as IDBRequest).error);
      };
    });
  }
 ...
 */
export function createIndexedDBService(options: IndexedDBOptions): IndexedDBService {
