...
export class ChatStorageService {
  ...
  private initializeStorageInfo(): void {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (
        key &&
        key.startsWith(ChatStorageService.STORAGE_PREFIX) &&
        key.endsWith(ChatStorageService.META_SUFFIX)
      ) {
        try {
    ...
    console.log(
      `Chat storage initialized. Using ${this.formatBytes(this.totalStoredSize)} of ${this.formatBytes(ChatStorageService.MAX_LOCAL_STORAGE_SIZE)}`,
    );
  }
    ...
    try {
      ...
      const messagesToStore = keepLatest
        ? messages.slice(
            ...
            Math.max(
              0,
              messages.length - ChatStorageService.MESSAGE_LIMIT_PER_SESSION,
            ),
          )
        : messages;
      ...
      for (
        let i = 0;
        i < messagesToStore.length;
        i += ChatStorageService.CHUNK_SIZE
      ) {
        chunks.push(
          messagesToStore.slice(i, i + ChatStorageService.CHUNK_SIZE),
        );
      }
      ...
      while (
        this.totalStoredSize + estimatedSize >
        ChatStorageService.MAX_LOCAL_STORAGE_SIZE
      ) {
  ...
  public updateSessionMetadata(
    sessionId: string,
    updates: Partial<StorageMetadata>,
  ): boolean {
    if (!sessionId) {
      return false;
    }
  ...
  public optimizeSessionStorage(sessionId: string): boolean {
    ...
    try {
      ...
      console.error(
        `Error optimizing storage for session ${sessionId}:`,
        error,
      );
      return false;
    }
  ...
  public getStorageStats(): {
    ...
    return {
      ...
      usedPercentage:
        (this.totalStoredSize / ChatStorageService.MAX_LOCAL_STORAGE_SIZE) *
        100,
