...
> {
  ...
  private eventHandlers: Map<SessionServiceEvent, Set<SessionEventHandler>> = new Map();
  ...
  private async getOfflineSessions(
    ...
    try {
      ...
      const filterFn = (session: ChatSession): boolean => {
        ...
        if (params?.archived !== undefined && session.isArchived !== params.archived) {
          return false;
        }
        
        // Nach angehefteten filtern
        if (params?.pinned !== undefined && session.isPinned !== params.pinned) {
          return false;
        }
   ...
   */
  public on(event: SessionServiceEvent, handler: SessionEventHandler): () => void {
