...
class DocumentConverterService implements IDocumentConverterService {
  ...
  public async uploadDocument(file: File, onProgress?: (progress: number) => void): Promise<string> {
    this.logService.info(`Starte Upload für Datei: ${file.name} (${file.size} Bytes)`);
    ...
    try {
      ...
      this.logService.info(`Upload erfolgreich, Dokument-ID: ${response.data.documentId}`);
  ...
  public async convertDocument(
    ...
    try {
      ...
      this.logService.info(`Konvertierung erfolgreich abgeschlossen für Dokument: ${documentId}`);
      return result;
    } catch (error) {
      this.logService.error(`Konvertierung fehlgeschlagen für Dokument: ${documentId}`, error);
  ...
  private pollConversionProgress(
    ...
    const intervalId = window.setInterval(async () => {
      try {
        ...
        if (progress.progress >= 100 || progress.step.includes('fehlgeschlagen') || progress.step.includes('abgeschlossen')) {
          this.stopPollingConversionProgress(documentId);
        }
      } catch (error) {
        this.logService.error(`Fehler beim Abrufen des Konvertierungsfortschritts für Dokument ${documentId}`, error);
      }
    }, 1000);
    
    // Intervall-ID speichern
    this.conversionPollingIntervals.set(documentId, intervalId);
    this.logService.debug(`Polling für Konvertierungsfortschritt gestartet für Dokument ${documentId}`);
  }
  ...
  private stopPollingConversionProgress(documentId: string): void {
    ...
    if (intervalId) {
      ...
      this.logService.debug(`Polling für Konvertierungsfortschritt gestoppt für Dokument ${documentId}`);
    }
  }
  ...
  public async getDocuments(): Promise<ConversionResult[]> {
    ...
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.message || 'Fehler beim Abrufen der Dokumente');
      }
  ...
  public async getDocument(documentId: string): Promise<ConversionResult> {
    ...
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.message || `Dokument ${documentId} nicht gefunden`);
      }
      
      return this.mapDocumentResponse(response.data);
    } catch (error) {
      this.logService.error(`Fehler beim Abrufen des Dokuments ${documentId}`, error);
      throw error;
    }
  }
  ...
  public async deleteDocument(documentId: string): Promise<void> {
    ...
    try {
      ...
      if (!response.success) {
        throw new Error(response.message || `Fehler beim Löschen des Dokuments ${documentId}`);
      }
      
      // Laufende Polling-Prozesse beenden
      this.stopPollingConversionProgress(documentId);
    } catch (error) {
      this.logService.error(`Fehler beim Löschen des Dokuments ${documentId}`, error);
      throw error;
    }
  }
  ...
  public async downloadDocument(
    ...
    try {
      ...
      this.logService.error(`Fehler beim Herunterladen des Dokuments ${documentId}`, error);
      throw error;
    }
  }
  ...
  public async getDocumentContent(documentId: string): Promise<string> {
    ...
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.message || `Inhalt für Dokument ${documentId} nicht verfügbar`);
      }
      
      return response.data.content;
    } catch (error) {
      this.logService.error(`Fehler beim Abrufen des Inhalts für Dokument ${documentId}`, error);
      throw error;
    }
  }
  ...
  public async getConversionStatus(documentId: string): Promise<ConversionProgress> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.message || `Status für Dokument ${documentId} nicht verfügbar`);
      }
      ...
      this.logService.error(`Fehler beim Abrufen des Konvertierungsstatus für Dokument ${documentId}`, error);
      throw error;
    }
  }
  ...
  public async cancelConversion(documentId: string): Promise<void> {
    ...
    try {
      ...
      if (!response.success) {
        throw new Error(response.message || `Fehler beim Abbrechen der Konvertierung für Dokument ${documentId}`);
      }
      
      // Polling stoppen
      this.stopPollingConversionProgress(documentId);
    } catch (error) {
      this.logService.error(`Fehler beim Abbrechen der Konvertierung für Dokument ${documentId}`, error);
      throw error;
    }
  }
...
class MockDocumentConverterService implements IDocumentConverterService {
  ...
  private mockConversionProcesses: Map<string, {
    progress: number;
    step: string;
    ...
    this.mockDocuments = [
      {
        ...
        convertedAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 3 + 1000 * 60 * 2), // 2 Minuten nach Upload
        status: 'success',
      ...
      },
      {
        ...
        status: 'error',
        error: 'Das Dokument ist passwortgeschützt und konnte nicht konvertiert werden.'
      },
      {
        id: 'mock-doc-4',
   ...
   */
  public async uploadDocument(file: File, onProgress?: (progress: number) => void): Promise<string> {
    this.logService.info(`[MOCK] Starte Upload für Datei: ${file.name} (${file.size} Bytes)`);
    ...
    this.logService.info(`[MOCK] Upload erfolgreich, Dokument-ID: ${documentId}`);
  ...
  public async convertDocument(
    ...
    this.logService.info(`[MOCK] Starte Konvertierung für Dokument: ${documentId}`);
    ...
    if (this.mockDocuments[docIndex].size > 1024 * 1024) {
      conversionTime += this.mockDocuments[docIndex].size / 1024 / 1024 * 1000;
    }
    ...
    if (Math.random() < 0.05) {
      ...
      this.mockDocuments[docIndex].error = 'Bei der Konvertierung ist ein Fehler aufgetreten.';
    ...
    this.mockDocuments[docIndex].content = `Dies ist der konvertierte Inhalt des Dokuments ${this.mockDocuments[docIndex].originalName}...`;
      ...
      this.mockDocuments[docIndex].metadata.tables = [
        {
          ...
          headers: ['Spalte 1', 'Spalte 2', 'Spalte 3', 'Spalte 4', 'Spalte 5'].slice(0, (this.mockDocuments[docIndex].metadata?.tables?.[0]?.columnCount || 3))
        }
      ];
    }
    
    this.logService.info(`[MOCK] Konvertierung erfolgreich abgeschlossen für Dokument: ${documentId}`);
  ...
  private startMockConversionProcess(
    ...
    const intervalId = window.setInterval(() => {
      ...
      // Callback aufrufen
      onProgress(Math.floor(progress), this.CONVERSION_STEPS[stepIndex], timeRemaining);
    }, 500);
  ...
  public async downloadDocument(
    ...
    if (document.status !== 'success') {
      throw new Error(`Dokument ${documentId} ist nicht verfügbar für Download (Status: ${document.status})`);
    }
    ...
    // Mock-Blob erstellen
    const content = document.content || `Inhalt des Dokuments ${document.originalName}`;
    ...
    const mimeType = mimeTypes[document.originalFormat] || 'application/octet-stream';
    ...
    switch (document.status) {
      ...
      case 'processing':
        return {
          ...
          step: this.CONVERSION_STEPS[Math.floor(Math.random() * this.CONVERSION_STEPS.length)],
   ...
   */
  public async cancelConversion(documentId: string): Promise<void> {
    this.logService.info(`[MOCK] Breche Konvertierung ab für Dokument: ${documentId}`);
    ...
    if (docIndex !== -1) {
      this.mockDocuments[docIndex].status = 'error';
      this.mockDocuments[docIndex].error = 'Konvertierung wurde vom Benutzer abgebrochen';
    }
  }
}
...
const useMockService = (isDevelopment || isTest) && 
...
export { IDocumentConverterService, DocumentConverterService, MockDocumentConverterService };
