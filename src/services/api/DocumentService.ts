...
export class DocumentService {
  ...
  public async uploadDocument(
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        await defaultIndexedDBService.upsert('documents', response.data, response.data.id);
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Hochladen der Datei ${file.name}`,
        error: {
  ...
  public async convertDocument(
    ...
    try {
      ...
      this.logger.error(`Fehler bei der Konvertierung des Dokuments ${request.documentId}`, error);
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler bei der Konvertierung des Dokuments ${request.documentId}`,
        error: {
    ...
    const timerId = window.setInterval(async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          // Im Offline-Speicher aktualisieren
          await defaultIndexedDBService.upsert('documents', document, document.id);
          ...
          if (document.status === 'completed' || document.status === 'failed') {
            ...
            // Event auslösen
            this.emitEvent(
              document.status === 'completed' ? 'conversionCompleted' : 'conversionFailed', 
              document
            );
          }
        }
      } catch (error) {
        this.logger.error(`Fehler beim Abrufen des Konvertierungsstatus für Dokument ${documentId}`, error);
        ...
        if (state) {
          ...
          if (state.errorCount >= 5) {
            ...
            this.emitEvent('conversionFailed', {
              id: documentId,
              status: 'failed',
              error: error instanceof Error ? error.message : 'Zu viele fehlgeschlagene Statusabfragen'
            });
          }
        }
      }
  ...
  public async downloadDocument(
    ...
    try {
      ...
      this.logger.error(`Fehler beim Herunterladen des Dokuments ${documentId}`, error);
      throw error;
    }
  }
   ...
   */
  private async getDocumentFromCache(documentId: string): Promise<Document | null> {
    try {
      return await defaultIndexedDBService.get<Document>('documents', documentId);
    } catch (error) {
      this.logger.error(`Fehler beim Laden des Dokuments ${documentId} aus dem Cache`, error);
      return null;
    }
  }
  ...
  public async getDocuments(
    ...
    try {
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : 'Fehler beim Abrufen der Dokumente',
        error: {
  ...
  private async getOfflineDocuments(
    ...
    try {
      ...
      // Aus IndexedDB laden
      const documents = await defaultIndexedDBService.getAll<Document>('documents', {
        limit: pageSize,
      ...
      this.logger.error('Fehler beim Abrufen der Dokumente aus dem Offline-Speicher', error);
      return {
        success: false,
   ...
   */
  private async syncDocumentsToOfflineStorage(documents: Document[]): Promise<void> {
    try {
      for (const document of documents) {
        await defaultIndexedDBService.upsert('documents', document, document.id);
      }
      this.logger.debug(`${documents.length} Dokumente im Offline-Speicher synchronisiert`);
    } catch (error) {
  ...
  public async getDocument(documentId: string): Promise<ApiResponse<Document>> {
    try {
      ...
      this.logger.error(`Fehler beim Abrufen des Dokuments ${documentId}`, error);
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Abrufen des Dokuments ${documentId}`,
        error: {
   ...
   */
  private async getOfflineDocument(documentId: string): Promise<ApiResponse<Document>> {
    try {
      const document = await defaultIndexedDBService.get<Document>('documents', documentId);
      ...
      this.logger.error(`Fehler beim Abrufen des Dokuments ${documentId} aus dem Offline-Speicher`, error);
      return {
        success: false,
  ...
  public async deleteDocument(documentId: string): Promise<ApiResponse<void>> {
    try {
      ...
      this.logger.error(`Fehler beim Löschen des Dokuments ${documentId}`, error);
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Löschen des Dokuments ${documentId}`,
        error: {
  ...
  public async updateDocument(
    ...
    try {
      ...
      if (this.offlineMode) {
        ...
        // Im Offline-Speicher aktualisieren
        await defaultIndexedDBService.upsert('documents', updatedDocument, updatedDocument.id);
      ...
      this.logger.error(`Fehler beim Aktualisieren des Dokuments ${documentId}`, error);
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Aktualisieren des Dokuments ${documentId}`,
        error: {
   ...
   */
  public async cancelConversion(documentId: string): Promise<ApiResponse<void>> {
    try {
      // Polling stoppen
      this.stopStatusPolling(documentId);
      ...
      if (this.offlineMode) {
        return {
          success: false,
          message: 'Im Offline-Modus können keine Konvertierungen abgebrochen werden',
          error: {
      ...
      this.logger.error(`Fehler beim Abbrechen der Konvertierung ${documentId}`, error);
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Abbrechen der Konvertierung ${documentId}`,
        error: {
