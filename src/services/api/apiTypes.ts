...
import { AxiosRequestConfig, AxiosResponse, AxiosError, CancelTokenSource as AxiosCancelTokenSource } from 'axios';
import { 
  ApiServiceInterface, 
...
export interface NScaleCachedApiService extends NScaleApiService {
  ...
  setCacheEntry<T = any>(key: string, value: APIResponse<T>, ttl?: number): void;
}
...
export class NScaleApiServiceAdapter implements NScaleApiService {
  ...
  async get<T = any>(url: string, params?: any, options?: ApiRequestOptions): Promise<APIResponse<T>> {
  ...
  async getPaginated<T = any>(url: string, pagination: PaginationParams, options?: ApiRequestOptions): Promise<APIResponse<T>> {
  ...
  async downloadFile(url: string, options?: ApiRequestOptions & { filename?: string }): Promise<Blob> {
...
export class NScaleSessionServiceAdapter implements SessionServiceInterface {
  ...
  async getSessions(pagination?: PaginationParams): Promise<APIResponse<any[]>> {
  ...
  async updateSession(sessionId: string, updates: any): Promise<APIResponse<any>> {
    return this.originalService.updateSession(sessionId, updates);
  }
  
  async getMessages(sessionId: string, pagination?: PaginationParams): Promise<APIResponse<any[]>> {
 ...
 */
export class NScaleStreamingServiceAdapter implements StreamingServiceInterface {
  private originalService: any;
