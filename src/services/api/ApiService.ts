...
export class ApiService {
  ...
  private setupInterceptors(): void {
    ...
    this.axiosInstance.interceptors.request.use(
      (config) => {
        ...
        if (apiConfig.DEBUG.LOG_REQUESTS) {
          this.logService.debug(`📤 Request [${config.method?.toUpperCase()}] ${config.url}`, {
            requestId,
        ...
        if (apiConfig.DEFAULT_PARAMS.includeVersion && !config.url?.includes(`/${apiConfig.API_VERSION}/`)) {
    ...
    this.axiosInstance.interceptors.response.use(
      (response) => {
        // Debug-Logging
        if (apiConfig.DEBUG.LOG_REQUESTS) {
          this.logService.debug(`📥 Response [${response.status}] ${response.config.url}`, {
      ...
      async (error: AxiosError) => {
        ...
        if (originalRequest && this.retryHandler.shouldRetry(error)) {
          return this.retryHandler.retryRequest(originalRequest, this.axiosInstance);
        }
   ...
   */
  private async handleUnauthorizedError(originalRequest: AxiosRequestConfig): Promise<AxiosResponse> {
    ...
    try {
      ...
      this.logService.warn('🔑 Token refresh failed, clearing auth data', refreshError);
      this.clearAuthData();
    ...
    // HTTP-Statusfehler
      ...
      switch (error.response.status) {
        ...
        default:
          ...
          apiError.message = error.response.statusText || 'Unbekannter HTTP-Fehler';
      }
  ...
  private async refreshToken(): Promise<string> {
    ...
    this.refreshTokenPromise = new Promise<string>(async (resolve, reject) => {
      try {
        const refreshToken = this.storageService.getItem(apiConfig.AUTH.STORAGE_KEYS.REFRESH_TOKEN);
        ...
        this.storageService.setItem(apiConfig.AUTH.STORAGE_KEYS.ACCESS_TOKEN, accessToken);
        this.storageService.setItem(apiConfig.AUTH.STORAGE_KEYS.REFRESH_TOKEN, newRefreshToken);
        this.storageService.setItem(apiConfig.AUTH.STORAGE_KEYS.TOKEN_EXPIRY, expiresAt);
  ...
  private async executeRequest<T = any>(options: ApiRequestOptions): Promise<ApiResponse<T>> {
    try {
      ...
      const response = await this.axiosInstance.request<ApiResponse<T>>(options);
  ...
  public async uploadFile<T = any>(
    url: string,
    file: File | Blob,
    options?: ApiRequestOptions & { fieldName?: string, metadata?: Record<string, any> }
  ): Promise<ApiResponse<T>> {
  ...
  public async login(credentials: LoginRequest): Promise<ApiResponse<LoginResponse>> {
    ...
    if (response.success && response.data) {
      ...
      this.storageService.setItem(apiConfig.AUTH.STORAGE_KEYS.ACCESS_TOKEN, accessToken);
      this.storageService.setItem(apiConfig.AUTH.STORAGE_KEYS.REFRESH_TOKEN, refreshToken);
      this.storageService.setItem(apiConfig.AUTH.STORAGE_KEYS.TOKEN_EXPIRY, expiresAt);
      this.storageService.setItem(apiConfig.AUTH.STORAGE_KEYS.USER, JSON.stringify(user));
  ...
  public isAuthenticated(): boolean {
    const token = this.storageService.getItem(apiConfig.AUTH.STORAGE_KEYS.ACCESS_TOKEN);
    const expiryStr = this.storageService.getItem(apiConfig.AUTH.STORAGE_KEYS.TOKEN_EXPIRY);
  ...
  public getUserInfo(): any {
    const userJson = this.storageService.getItem(apiConfig.AUTH.STORAGE_KEYS.USER);
