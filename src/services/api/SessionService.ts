...
import { createStreamingConnection, StreamingService } from './StreamingService';
...
export class SessionService {
  ...
  private async initializeOfflineStorage(): Promise<void> {
    try {
      ...
      this.logger.error('Fehler bei der Initialisierung des Offline-Speichers', error);
    }
  }
  ...
  public async getSessions(
    ...
    try {
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : 'Fehler beim Abrufen der Sessions',
        error: {
  ...
  private async getOfflineSessions(
    ...
    try {
      ...
      // Aus IndexedDB laden
      const sessions = await defaultIndexedDBService.getAll<ChatSession>('sessions', {
        limit: pageSize,
      ...
      this.logger.error('Fehler beim Abrufen der Sessions aus dem Offline-Speicher', error);
      return {
        success: false,
   ...
   */
  private async syncSessionsToOfflineStorage(sessions: ChatSession[]): Promise<void> {
    try {
      for (const session of sessions) {
        await defaultIndexedDBService.upsert('sessions', session, session.id);
      }
      this.logger.debug(`${sessions.length} Sessions im Offline-Speicher synchronisiert`);
    } catch (error) {
   ...
   */
  public async getSession(sessionId: string): Promise<ApiResponse<ChatSession>> {
    try {
      // Im Offline-Modus aus IndexedDB laden
      if (this.offlineMode) {
      ...
      if (response.success && response.data) {
        ...
        await defaultIndexedDBService.upsert('sessions', response.data, response.data.id);
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Abrufen der Session ${sessionId}`,
        error: {
   ...
   */
  private async getOfflineSession(sessionId: string): Promise<ApiResponse<ChatSession>> {
    try {
      const session = await defaultIndexedDBService.get<ChatSession>('sessions', sessionId);
      ...
      this.logger.error(`Fehler beim Abrufen der Session ${sessionId} aus dem Offline-Speicher`, error);
      return {
        success: false,
  ...
  public async createSession(
    ...
    try {
      ...
      if (this.offlineMode) {
        return {
          success: false,
          message: 'Im Offline-Modus können keine neuen Sessions erstellt werden',
          error: {
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : 'Fehler beim Erstellen einer neuen Session',
        error: {
  ...
  public async deleteSession(sessionId: string): Promise<ApiResponse<void>> {
    try {
      ...
      if (this.offlineMode) {
        ...
        const messages = await defaultIndexedDBService.query<ChatMessage>('messages', {
          index: 'sessionId',
          equalTo: sessionId
        });
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Löschen der Session ${sessionId}`,
        error: {
  ...
  public async updateSession(
    ...
    try {
      ...
      this.logger.error(`Fehler beim Aktualisieren der Session ${sessionId}`, error);
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Aktualisieren der Session ${sessionId}`,
        error: {
  ...
  private async updateOfflineSession(
    ...
    try {
      ...
      // Im Offline-Speicher aktualisieren
      await defaultIndexedDBService.upsert('sessions', updatedSession, updatedSession.id);
      ...
      this.logger.error(`Fehler beim Aktualisieren der Session ${sessionId} im Offline-Modus`, error);
      return {
        success: false,
  ...
  public async getMessages(
    ...
    try {
      ...
      this.logger.error(`Fehler beim Abrufen der Nachrichten für Session ${sessionId}`, error);
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Abrufen der Nachrichten für Session ${sessionId}`,
        error: {
  ...
  private async getOfflineMessages(
    ...
    try {
      ...
      this.logger.error(`Fehler beim Abrufen der Nachrichten für Session ${sessionId} aus dem Offline-Speicher`, error);
      return {
        success: false,
  ...
  private async syncMessagesToOfflineStorage(
    ...
    try {
      ...
      this.logger.debug(`${messages.length} Nachrichten im Offline-Speicher synchronisiert`);
    } catch (error) {
  ...
  public async sendMessage(
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        await defaultIndexedDBService.upsert('messages', response.data, response.data.id);
      ...
      this.logger.error(`Fehler beim Senden einer Nachricht an Session ${sessionId}`, error);
      return {
        success: false,
        message: error instanceof Error ? error.message : `Fehler beim Senden einer Nachricht an Session ${sessionId}`,
        error: {
  ...
  private async sendStreamingMessage(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // Im Offline-Speicher speichern
        defaultIndexedDBService.upsert('messages', userMessage, userMessage.id)
          .catch(err => this.logger.error('Fehler beim Speichern der Benutzernachricht', err));
        ...
        // Im Offline-Speicher speichern
        defaultIndexedDBService.upsert('messages', assistantMessage, assistantMessage.id)
          .catch(err => this.logger.error('Fehler beim Speichern der temporären Assistentennachricht', err));
        ...
        this.currentStream = createStreamingConnection({
          ...
          onMessage: (event: StreamingEvent) => {
            if (event.type === 'content' && event.content) {
              ...
              const error = new Error(event.error.message || 'Streaming-Fehler');
          ...
          onComplete: (response) => {
            ...
            // Im Offline-Speicher aktualisieren
            defaultIndexedDBService.upsert('messages', receivedMessage, receivedMessage.id)
              .catch(err => this.logger.error('Fehler beim Aktualisieren der Assistentennachricht', err));
            
            // Temporäre Nachricht löschen
            defaultIndexedDBService.delete('messages', assistantMessage.id)
              .catch(err => this.logger.error('Fehler beim Löschen der temporären Nachricht', err));
        ...
        this.logger.error(`Fehler beim Streaming einer Nachricht an Session ${sessionId}`, error);
        reject(error);
      }
    });
  }
  ...
  private handleOnlineEvent = (): void => {
    ...
    this.processSyncQueue().catch(error => {
      this.logger.error('Fehler bei der Synchronisierung nach Online-Wechsel', error);
    });
  };
  ...
  private async processSyncQueue(): Promise<void> {
    try {
      ...
      const pendingRequests = await defaultIndexedDBService.query('offlineRequests', {
        index: 'status',
        equalTo: 'pending'
      });
      
      if (pendingRequests.length === 0) {
        this.logger.debug('Keine ausstehenden Anfragen in der Synchronisationsqueue');
        return;
      }
      
      this.logger.info(`${pendingRequests.length} ausstehende Anfragen in der Synchronisationsqueue gefunden`);
      ...
      for (const request of pendingRequests) {
        try {
          ...
          this.logger.error(`Fehler bei der Synchronisierung von Anfrage ${request.id}`, error);
      ...
      const completedRequests = await defaultIndexedDBService.query('offlineRequests', {
        index: 'status',
        equalTo: 'completed'
      });
      ...
      if (completedRequests.length > 0) {
        ...
        this.logger.debug(`${completedRequests.length} abgeschlossene Anfragen aus der Queue entfernt`);
      }
    } catch (error) {
      this.logger.error('Fehler bei der Verarbeitung der Synchronisationsqueue', error);
      throw error;
    }
  }
