...
export class RateLimitHandler {
  ...
  constructor(options: RateLimitHandlerOptions = {}) {
    this.remainingHeader =
      options.remainingHeader ?? apiConfig.RATE_LIMITING.REMAINING_HEADER;
    this.resetHeader =
      options.resetHeader ?? apiConfig.RATE_LIMITING.RESET_HEADER;
    this.limitHeader =
      options.limitHeader ?? apiConfig.RATE_LIMITING.LIMIT_HEADER;
    this.throttleThreshold =
      options.throttleThreshold ?? apiConfig.RATE_LIMITING.THROTTLE_THRESHOLD;
    this.minThrottleDelay =
      options.minThrottleDelay ?? apiConfig.RATE_LIMITING.MIN_THROTTLE_DELAY;
  ...
  public updateFromResponse(response: AxiosResponse): void {
    ...
    const remaining = this.getHeaderValue(
      response.headers,
      this.remainingHeader,
    );
    ...
    // Debug-Logging
    if (
      apiConfig.DEBUG.VERBOSE &&
      (remaining !== null || reset !== null || limit !== null)
    ) {
      this.logService.debug(
        `Rate-Limit für ${baseEndpoint}: ${remaining}/${limit}, Reset: ${new Date(reset || 0).toISOString()}`,
      );
    }
   ...
   */
  public getResetTimeFromResponse(
    ...
    const resetHeader =
      response.headers[this.resetHeader.toLowerCase()] ||
      response.headers[this.resetHeader];
  ...
  private updateThrottleDelay(): void {
    ...
    if (lowestRemaining <= this.throttleThreshold) {
      ...
      this.logService.warn(
        `Rate-Limit fast erreicht (${lowestRemaining} verbleibend). Drosselung mit ${this.currentThrottleDelay}ms`,
      );
    } else {
  ...
  public async applyThrottling(): Promise<void> {
    ...
    const actualDelay = Math.max(
      0,
      this.currentThrottleDelay - timeSinceLastRequest,
    );
    ...
    if (actualDelay > 0) {
      this.logService.debug(
        `Anfrage wird um ${actualDelay}ms verzögert (Rate-Limiting)`,
      );
