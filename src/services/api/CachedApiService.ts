...
export class CachedApiService {
  ...
  public async get<T = any>(
    ...
    const useCache = options.cache ?? (import.meta.env.VITE_API_CACHE_ENABLED !== 'false');
    ...
    try {
      ...
      if (response.success) {
        apiCacheService.cacheResponse<T>(
          url, 'GET', params, response, {
  ...
  public async uploadFile<T = any>(
    url: string,
    file: File | Blob,
    options?: CachedApiRequestOptions & { fieldName?: string, metadata?: Record<string, any> }
  ): Promise<ApiResponse<T>> {
      ...
      setTimeout(async () => {
        ...
        if (response.success) {
          ...
          if (import.meta.env.VITE_ENV !== 'production') {
            this.logger.debug('Hintergrundaktualisierung abgeschlossen', { url });
          }
        }
      }, 0);
    } catch (error) {
  ...
  private invalidateRelatedCaches(url: string, data?: any): void {
    try {
      ...
      if (urlParts.length > 0) {
        ...
        if (import.meta.env.VITE_ENV !== 'production') {
          this.logger.debug(`Cache invalidiert für Ressourcentyp: ${resourceType}`);
        }
      }
    } catch (error) {
