 ...
 * Event-Typen für SSE
 */
type StreamingEventType = 'message' | 'error' | 'open' | 'close' | 'complete' | 'progress';
...
export class StreamingService {
  ...
  /** Event-Handler-Map */
  private eventHandlers: Map<StreamingEventType, Array<(...args: any[]) => void>> = new Map();
   ...
   */
  public on(eventType: StreamingEventType, handler: (...args: any[]) => void): void {
   ...
   */
  public off(eventType: StreamingEventType, handler: (...args: any[]) => void): void {
    if (!this.eventHandlers.has(eventType)) {
      return;
    }
    ...
    if (handlers) {
      handlers.forEach(handler => {
        try {
          ...
          this.logService.error(`Fehler in Event-Handler für ${eventType}`, error);
        }
      });
    }
  }
  ...
  public connect(): void {
    ...
    try {
      ...
      const token = this.storageService.getItem(apiConfig.AUTH.STORAGE_KEYS.ACCESS_TOKEN);
      if (token) {
      ...
      this.state.eventSource.addEventListener('content', this.handleContentEvent);
      this.state.eventSource.addEventListener('metadata', this.handleMetadataEvent);
      this.state.eventSource.addEventListener('progress', this.handleProgressEvent);
  ...
  private handleConnectionTimeout(): void {
    if (!this.state.connected) {
      const error = new Error('Verbindungs-Timeout: Konnte keine Verbindung zum Server herstellen');
  ...
  private handleMaxDuration(): void {
    this.logService.warn(`Maximale Streaming-Dauer von ${this.options.maxStreamDuration}ms erreicht, beende Stream`);
    ...
    if (!this.state.completed) {
      ...
      this.emit('complete', { content: this.state.fullResponse, truncated: true });
    }
  ...
  private handleError = (event: Event): void => {
    ...
    // Verbindung automatisch wiederherstellen?
    if (this.options.autoReconnect && !this.state.manuallyClosed && !this.state.completed) {
      this.attemptReconnect();
    } else {
      // EventSource schließen
      if (this.state.eventSource) {
  ...
  private handleContentEvent = (event: MessageEvent): void => {
    try {
      ...
      this.logService.error('Fehler beim Verarbeiten eines Content-Events', error);
    }
  };
  ...
  private handleMetadataEvent = (event: MessageEvent): void => {
    try {
      ...
      this.logService.error('Fehler beim Verarbeiten eines Metadata-Events', error);
    }
  };
  ...
  private handleProgressEvent = (event: MessageEvent): void => {
    try {
      ...
      this.logService.error('Fehler beim Verarbeiten eines Progress-Events', error);
    }
  };
  ...
  private handleErrorEvent = (event: MessageEvent): void => {
    try {
      ...
      this.state.error = new Error(data.error?.message || 'Unbekannter Streaming-Fehler');
      ...
      this.logService.error('Fehler beim Verarbeiten eines Error-Events', error);
    }
  };
  ...
  private attemptReconnect(): void {
    ...
    if (this.state.reconnectAttempts >= (this.options.maxReconnectAttempts || 0)) {
      this.logService.warn(`Maximale Anzahl von Wiederverbindungsversuchen (${this.options.maxReconnectAttempts}) erreicht`);
      
      if (!this.state.completed) {
        // Als abgeschlossen markieren und Complete-Event auslösen
        this.state.completed = true;
        this.emit('complete', { content: this.state.fullResponse, error: this.state.error });
      }
    ...
    this.state.currentReconnectDelay = Math.min(
      this.state.currentReconnectDelay * (this.options.reconnectBackoffFactor || 1.5),
      this.options.maxReconnectDelay || 10000
    );
    
    this.logService.info(`Wiederverbindung in ${this.state.currentReconnectDelay}ms (Versuch ${this.state.reconnectAttempts}/${this.options.maxReconnectAttempts})`);
 ...
 */
export function createStreamingConnection(options: StreamingOptions): StreamingService {
