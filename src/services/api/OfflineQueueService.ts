...
export class OfflineQueueService {
      ...
      while (this.isOnline && this.hasProcessableOperations()) {
        ...
        try {
          ...
          // Zurück auf "Pending" setzen für den nächsten Versuch
            ...
            this.emit('operation-retry', { operation, error, retryCount: operation.retryCount });
          }
        }
  ...
  private async executeOperation(operation: QueuedOperation): Promise<void> {
    ...
    switch (operation.type) {
      ...
      case OperationType.DELETE_MESSAGE:
        // Nachricht löschen
        await fetch(`/api/sessions/${operation.sessionId}/messages/${operation.data.messageId}`, {
          method: 'DELETE',
          headers
        }).then(response => {
          if (!response.ok) {
      ...
      case OperationType.ARCHIVE_SESSION:
        ...
        await fetch(`/api/sessions/${operation.sessionId}`, {
          ...
          if (!response.ok) {
            throw new Error(`Failed to archive session: ${response.status} ${response.statusText}`);
          }
          return response.json();
        });
        break;
      ...
      case OperationType.PIN_SESSION:
        ...
        await fetch(`/api/sessions/${operation.sessionId}`, {
          ...
          if (!response.ok) {
            throw new Error(`Failed to pin/unpin session: ${response.status} ${response.statusText}`);
          }
          return response.json();
        });
        break;
   ...
   */
  public getPendingMessagesForSession(sessionId: string): { id: string, content: string, timestamp: string }[] {
    return this.queue
      .filter(op => 
    ...
    if (this.handlers.has(event)) {
      this.handlers.get(event)!.forEach(callback => {
        try {
          ...
          console.error(`Error in offline queue event handler for ${event}:`, error);
        }
      });
    }
  }
