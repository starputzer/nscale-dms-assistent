...
export class ChatService {
  ...
  public async getSession(sessionId: string): Promise<ChatSession> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(
          response.message || `Fehler beim Laden der Session ${sessionId}`,
        );
      }
      ...
      this.logService.error(
        `Fehler beim Laden der Session ${sessionId}`,
        error,
      );
      throw error;
    }
  ...
  public async createSession(
    ...
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(
          response.message || "Fehler beim Erstellen der Session",
        );
      }
  ...
  public async deleteSession(sessionId: string): Promise<ApiResponse<void>> {
    try {
      ...
      if (!response.success) {
        throw new Error(
          response.message || `Fehler beim Löschen der Session ${sessionId}`,
        );
      }
  ...
  public async getMessages(sessionId: string): Promise<ChatMessage[]> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(
          response.message ||
            `Fehler beim Laden der Nachrichten für Session ${sessionId}`,
        );
      }
