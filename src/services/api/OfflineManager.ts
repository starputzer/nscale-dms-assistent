 ...
 */
export type SyncRequestStatus = 'pending' | 'processing' | 'completed' | 'failed';
...
export class OfflineManager {
  ...
  public startSynchronization(): void {
    ...
    this.logger.info(`Datensynchronisierung alle ${this.syncInterval / 1000} Sekunden gestartet`);
  }
  ...
  public async synchronizeData(): Promise<void> {
    ...
    try {
      ...
      this.logger.info(`Synchronisiere ${requests.length} ausstehende Anfragen`);
      ...
      for (const request of requests) {
        try {
          ...
          if (success) {
            ...
            await this.updateSyncRequestStatus(request.id, 'failed', 'Simulierter Fehler');
            failCount++;
          }
        } catch (error) {
          // Fehlerbehandlung
      ...
      // Event auslösen
      this.emitEvent('syncComplete', { successCount, failCount, totalCount: requests.length });
      
      this.logger.info(`Synchronisierung abgeschlossen: ${successCount} erfolgreich, ${failCount} fehlgeschlagen`);
    } catch (error) {
   ...
   */
  public async addSyncRequest(request: Omit<SyncRequest, 'id' | 'status' | 'timestamp'>): Promise<number | string> {
    ...
    try {
      const id = await defaultIndexedDBService.add('offlineRequests', syncRequest);
      this.logger.debug(`Neue Synchronisationsanfrage ${id} hinzugefügt`, request);
      ...
      this.logger.error('Fehler beim Hinzufügen der Synchronisationsanfrage', error);
      throw error;
    }
  }
  ...
  private async getPendingSyncRequests(): Promise<SyncRequest[]> {
    try {
      return await defaultIndexedDBService.query<SyncRequest>('offlineRequests', {
        index: 'status',
        equalTo: 'pending',
        direction: 'next' // Aufsteigend nach Zeitstempel
      });
    } catch (error) {
      this.logger.error('Fehler beim Laden ausstehender Synchronisationsanfragen', error);
      return [];
    }
  }
  ...
  private async updateSyncRequestStatus(
    ...
    try {
      ...
      const request = await defaultIndexedDBService.get<SyncRequest>('offlineRequests', id);
      ...
      this.logger.error(`Fehler beim Aktualisieren des Status für Anfrage ${id}`, error);
      throw error;
    }
  }
  ...
  private async cleanupCompletedRequests(): Promise<void> {
    try {
      ...
      const completedRequests = await defaultIndexedDBService.query<SyncRequest>('offlineRequests', {
        index: 'status',
        equalTo: 'completed'
      });
      ...
      this.logger.debug(`${completedRequests.length} abgeschlossene Anfragen entfernt`);
    } catch (error) {
      this.logger.error('Fehler beim Entfernen abgeschlossener Anfragen', error);
    }
  }
  ...
  public async deleteSyncRequest(id: string | number): Promise<void> {
    try {
      ...
      this.logger.error(`Fehler beim Löschen der Synchronisationsanfrage ${id}`, error);
      throw error;
    }
  }
  ...
  public destroy(): void {
    ...
    if (this.offlineIndicator && document.body.contains(this.offlineIndicator)) {
