...
export class ApiCacheService {
  ...
  public cacheResponse<T>(
    ...
    if (this.config.immutableEndpoints.some(endpoint => url.includes(endpoint))) {
  ...
  public invalidate(urlPattern: string | RegExp): void {
    if (typeof urlPattern === 'string') {
      ...
      this.logger.warn('Muster-basierte Cache-Invalidierung nicht implementiert');
    }
  }
  ...
  public invalidateEndpointType(endpointType: string): void {
    ...
    if (this.config.debug) {
      this.logger.debug(`API-Cache: Alle Einträge vom Typ ${endpointType} invalidiert`);
    }
  }
