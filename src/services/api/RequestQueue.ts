...
export class RequestQueue {
  ...
  constructor(options: RequestQueueOptions = {}) {
    this.maxConcurrent = options.maxConcurrent ?? apiConfig.QUEUE.MAX_CONCURRENT_REQUESTS;
   ...
   */
  public async enqueue<T>(fn: () => Promise<T>, priority: number = 0): Promise<T> {
    ...
    if (this.queue.length >= this.maxQueueSize) {
      this.logService.warn(`Warteschlange ist voll (${this.queue.length}), verwerfe älteste Anfrage mit niedriger Priorität`);
      
      // Älteste Anfrage mit niedrigster Priorität finden und verwerfen
      const lowestPriorityIndex = this.findLowestPriorityIndex();
      if (lowestPriorityIndex >= 0 && this.queue[lowestPriorityIndex].priority < priority) {
        const removedItem = this.queue.splice(lowestPriorityIndex, 1)[0];
        removedItem.reject(new Error('Anfrage wurde aus der Warteschlange entfernt, da eine höherpriorisierte Anfrage einging'));
      } else {
        throw new Error('Die Warteschlange ist voll und die Anfrage hat nicht ausreichend Priorität');
      }
    }
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      if (this.queueTimeout > 0) {
        const timeoutId = window.setTimeout(() => {
          ...
          reject(new Error(`Die Anfrage wurde nach ${this.queueTimeout}ms Wartezeit in der Warteschlange abgebrochen`));
        }, this.queueTimeout);
      ...
      this.logService.debug(`Anfrage mit Priorität ${priority} zur Warteschlange hinzugefügt. Aktuelle Größe: ${this.queue.length}`);
   ...
   */
  public clear(): void {
    const error = new Error('Die Warteschlange wurde geleert und alle Anfragen abgebrochen');
