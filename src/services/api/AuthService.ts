...
export class AuthService {
  ...
  constructor() {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.addEventListener('auth:unauthorized', this.handleUnauthorizedEvent);
  ...
  private loadAuthDataFromStorage(): void {
    try {
      // Access-Token laden
      this.accessToken = this.storageService.getItem(apiConfig.AUTH.STORAGE_KEYS.ACCESS_TOKEN);
      
      // Refresh-Token laden
      this.refreshToken = this.storageService.getItem(apiConfig.AUTH.STORAGE_KEYS.REFRESH_TOKEN);
      ...
      if (this.accessToken && this.currentUser) {
        this.logger.info(`Authentifizierungsdaten für Benutzer ${this.currentUser.email} geladen`);
      }
    } catch (error) {
  ...
  private saveAuthDataToStorage(): void {
    try {
      if (this.accessToken) {
        this.storageService.setItem(apiConfig.AUTH.STORAGE_KEYS.ACCESS_TOKEN, this.accessToken);
      } else {
        this.storageService.removeItem(apiConfig.AUTH.STORAGE_KEYS.ACCESS_TOKEN);
      }
      
      if (this.refreshToken) {
        this.storageService.setItem(apiConfig.AUTH.STORAGE_KEYS.REFRESH_TOKEN, this.refreshToken);
      } else {
      ...
      this.logger.error('Fehler beim Speichern der Authentifizierungsdaten', error);
    }
  }
  ...
  private startTokenExpirationMonitor(): void {
    ...
    setInterval(() => {
      if (this.accessToken && this.expiresAt) {
        ...
        if (status === 'expiring' && !this.tokenRefreshInProgress && this.refreshToken) {
          this.logger.info('Token läuft bald ab, wird automatisch aktualisiert');
          this.refreshAccessToken().catch(error => {
            this.logger.error('Automatische Token-Aktualisierung fehlgeschlagen', error);
          });
        }
  ...
  public async refreshAccessToken(): Promise<boolean> {
    ...
    if (!this.refreshToken) {
      this.logger.warn('Kein Refresh-Token vorhanden, Token-Aktualisierung nicht möglich');
      return false;
    }
  ...
  public async getCurrentUser(): Promise<User | null> {
    ...
    if (!this.currentUser && this.isAuthenticated()) {
      try {
        ...
        this.logger.error('Fehler beim Abrufen der Benutzerinformationen', error);
      }
    }
  ...
  private handleStorageEvent = (event: StorageEvent): void => {
    ...
    if (event.key === apiConfig.AUTH.STORAGE_KEYS.ACCESS_TOKEN ||
        event.key === apiConfig.AUTH.STORAGE_KEYS.USER) {
      
      // Wenn Token gelöscht wurde (Logout in anderem Tab)
      if (event.key === apiConfig.AUTH.STORAGE_KEYS.ACCESS_TOKEN && !event.newValue) {
        if (this.accessToken) {
          this.clearAuthData();
          this.emitEvent('logout');
        }
  ...
  public destroy(): void {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.removeEventListener('auth:unauthorized', this.handleUnauthorizedEvent);
