...
> {
  ...
  constructor(options: BaseApiServiceOptions) {
    ...
    this.defaultCachePolicy = options.defaultCachePolicy || CachePolicy.CACHE_FIRST;
   ...
   */
  protected handleError<T>(error: any, defaultMessage: string): AsyncAPIResult<T> {
  ...
  protected async safeRequest<T>(
    ...
    try {
      const [response, requestError] = await catchAsync<APIResponse<T>>(apiFunc(...args));
