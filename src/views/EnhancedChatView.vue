  ...
  <div class="enhanced-chat-view">
    <div class="sidebar-container" :class="{ 'sidebar-collapsed': isSidebarCollapsed }">
      <button 
        ...
        @click="toggleSidebar" 
        :aria-label="isSidebarCollapsed ? 'Sitzungsliste öffnen' : 'Sitzungsliste schließen'"
...
import { VirtualMessageList, EnhancedMessageInput, SessionManager } from '@/components/chat/enhanced';
...
const isEnhancedChatEnabled = computed(() => isFeatureEnabled('enhancedChatComponents'));
...
const activeSessionId = computed(() => sessionIdFromRoute.value || activeSession.value?.id || '');

// Bridge-Events registrieren
onMounted(() => {
  // Bridge-Events für Chat-Aktionen
  bridge.on('chat:sendMessage', (data: { content: string, sessionId?: string }) => {
