...
const mockMessages = [
  ...
  {
    id: '3',
    content: 'Natürlich! Die Indexierung in nscale DMS ermöglicht die schnelle Suche und das Auffinden von Dokumenten. Was genau möchtest du darüber wissen?',
    role: 'assistant',
...
const createMockStores = () => {
  return {
    ...
    // Mock für Bridge
    useBridge: () => ({
      on: (event: string, callback: Function) => action(`bridge.on(${event})`),
      off: (event: string, callback: Function) => action(`bridge.off(${event})`),
      emit: (event: string, data: any) => action(`bridge.emit(${event})`)(data),
      setState: (path: string, value: any) => action(`bridge.setState(${path})`)(value),
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'Die Hauptkomponente für die verbesserte Chat-Benutzeroberfläche mit Bridge-Integration.'
      }
    }
  },
  decorators: [
    (story) => ({
      ...
      setup() {
        ...
        const { useSessionsStore, useChat, useFeatureToggles, useBridge } = createMockStores();
