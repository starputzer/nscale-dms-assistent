...
export class Router404Diagnostics {
  ...
  private setupGlobalMonitoring(): void {
    ...
    console.error = (...args) => {
      ...
      if (message.includes('currentRoute') || 
    ...
    window.addEventListener('error', (event) => {
      if (event.message.includes('router') || 
  ...
  generateReport(): RouterHealthReport {
    ...
    const report: RouterHealthReport = {
      ...
      timing: {
        ...
        lastError: this.lastError ? this.events
          .filter(e => e.type === 'error')
          .slice(-1)[0]?.timestamp : undefined,
  ...
  private analyzeIssues(): RouterIssue[] {
    ...
    const error404Count = this.events.filter(e => 
      e.details.message?.includes('404') || 
      e.details.to?.name === 'NotFound'
    ).length;
    ...
    const currentRouteErrors = this.events.filter(e =>
    ...
    const piniaErrors = this.events.filter(e =>
  ...
  enableRealTimeMonitoring(): void {
    setInterval(() => {
      const report = this.generateReport();
      if (!report.healthy) {
        console.warn('[Router404Diag] Gesundheitsprobleme erkannt:', report.issues);
      }
    }, 5000);
  }
}
