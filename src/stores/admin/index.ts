...
export type AdminSection = 'dashboard' | 'users' | 'feedback' | 'motd' | 'system' | 'logs' | 'featureToggles';
...
export const useAdminStore = defineStore('admin', () => {
  ...
  const sectionStatus = computed(() => ({
    ...
    featureToggles: {
      ...
      enabled: Object.values(featureTogglesStore.allFeatureConfigs).filter(f => f.status.isActive).length
    }
  }));
  ...
  const overallStatus = computed(() => {
    ...
    const loading = Object.values(sectionStatus.value).some(section => section.loading);
    
    // Prüfen, ob irgendein Bereich einen Fehler hat
    const hasError = Object.values(sectionStatus.value).some(section => section.error !== null && section.error !== undefined);
  ...
  async function refreshAllData() {
    ...
    try {
      ...
      error.value = err.message || 'Fehler beim Aktualisieren aller Admin-Daten';
    } finally {
      isLoading.value = false;
    }
  }
