...
import type { FeedbackStats, FeedbackEntry, FeedbackFilter } from '@/types/admin'
...
export const useFeedbackStore = defineStore('admin-feedback', () => {
  ...
  const stats = computed<FeedbackStats>(() => {
    ...
    const withComments = feedback.value.filter(f => f.comment && f.comment.trim() !== '').length
    ...
    return feedback.value
      .filter(f => {
        ...
        if (currentFilter.value.searchTerm) {
          ...
          const commentMatches = f.comment && f.comment.toLowerCase().includes(term)
