...
export const useUIStore = defineStore('ui', () => {
  ...
  const isCompactMode = computed(() => layoutConfig.value.density === 'compact');
  ...
  const cssVariables = computed(() => {
    ...
    return {
      '--sidebar-width': sidebar.value.collapsed ? '60px' : `${sidebar.value.width}px`,
      ...
      '--ui-spacing': config.density === 'compact' ? '0.5rem' : 
                      config.density === 'spacious' ? '1.5rem' : '1rem',
      '--ui-element-padding': config.density === 'compact' ? '0.25rem 0.5rem' : 
                             config.density === 'spacious' ? '0.75rem 1.5rem' : '0.5rem 1rem'
    };
  });
   ...
   * UI-Dichte anpassen
   */
  function setUIDensity(density: 'compact' | 'comfortable' | 'spacious'): void {
   ...
   */
  function showSuccess(message: string, options?: Partial<NotificationOptions>): string {
    return showToast({
      type: 'success',
      message,
      ...(options || {})
   ...
   */
  function showError(message: string, options?: Partial<NotificationOptions>): string {
    return showToast({
      type: 'error',
      message,
   ...
   */
  function showWarning(message: string, options?: Partial<NotificationOptions>): string {
    return showToast({
      type: 'warning',
      message,
   ...
   */
  function showInfo(message: string, options?: Partial<NotificationOptions>): string {
    return showToast({
      type: 'info',
      message,
      ...(options || {})
  ...
  function checkViewport(): void {
    const newIsMobile = window.innerWidth < layoutConfig.value.sidebarBreakpoint;
    ...
    if (newIsMobile !== isMobile.value) {
      ...
      if (isMobile.value && sidebar.value.isOpen && !sidebar.value.collapsed) {
        sidebar.value.isOpen = false;
        requestUIUpdate('sidebar');
      }
    }
  ...
  const systemEvents = {
    emit(event: string, data?: any) {
      window.dispatchEvent(new CustomEvent(`nscale:ui:${event}`, { detail: data }));
    },
    
    on<T = any>(event: string, callback: (data: T) => void) {
      const handler = (e: CustomEvent) => callback(e.detail);
      window.addEventListener(`nscale:ui:${event}`, handler as EventListener);
      return () => window.removeEventListener(`nscale:ui:${event}`, handler as EventListener);
    }
  };
