...
export const useSessionsStore = defineStore('sessions', () => {
        ...
        try {
          ...
          if (legacyMessages) {
            ...
            Object.keys(parsedMessages).forEach(sessionId => {
              newMessages[sessionId] = parsedMessages[sessionId].map((msg: any) => ({
                ...msg,
  ...
  const currentSession = computed(() => {
    ...
    const result = sessions.value.find(s => s.id === currentSessionId.value) || null;
  ...
  const allCurrentMessages = computed(() => {
    ...
    const result = [...sessionMessages, ...pendingSessionMessages].sort((a, b) => 
    ...
    try {
      ...
      if (serverSessions.length > 0) {
        ...
        serverSessions.forEach(serverSession => {
          const existingIndex = updatedSessions.findIndex(s => s.id === serverSession.id);
          
          if (existingIndex !== -1) {
            // Aktualisieren, aber nur, wenn der Server eine neuere Version hat
            const localUpdatedAt = new Date(updatedSessions[existingIndex].updatedAt).getTime();
        ...
        if (hasChanges || filteredSessions.length !== updatedSessions.length) {
    ...
    if (authStore.isAuthenticated) {
      try {
        ...
        if (messagesClone[sessionId]) {
          messages.value = { ...messagesClone, [sessionId]: messagesClone[sessionId] };
        }
  ...
  async function syncPendingMessages(): Promise<void> {
    ...
    for (const sessionId of sessionIdsWithPending) {
      ...
      if (pendingForSession && pendingForSession.length > 0) {
        ...
        const messagesBackup = messages.value[sessionId] ? [...messages.value[sessionId]] : [];
        ...
        for (const message of pendingForSession) {
          try {
            ...
            const messagesUpdated = { ...messages.value, [sessionId]: messagesForSession };
            messages.value = messagesUpdated;
            ...
            pendingUpdated[sessionId] = [...pendingUpdated[sessionId], message];
            pendingMessages.value = pendingUpdated;
          }
        }
  ...
  async function sendMessage({ sessionId, content, role = 'user' }: SendMessageParams): Promise<void> {
    ...
    if (!authStore.isAuthenticated) {
      ...
      pendingUpdated[sessionId] = [...pendingUpdated[sessionId], {
        ...userMessage,
        status: 'pending'
      }];
      ...
      messagesWithResponse[sessionId] = [...messagesWithResponse[sessionId], assistantMessage];
    ...
    try {
      ...
      messagesWithAssistant[sessionId] = [...messagesWithAssistant[sessionId], assistantMessage];
      messages.value = messagesWithAssistant;
      
      resetGetterCache(); // Cache zurücksetzen bei Änderungen
      
      // Funktion zum batchen Aktualisieren des Nachrichteninhalts
      const updateMessageContent = (content: string, force: boolean = false) => {
        const now = Date.now();
        ...
        if (force || now - lastUpdateTime >= updateInterval) {
          ...
          if (index !== -1) {
            ...
            const messagesUpdated = { ...messages.value, [sessionId]: sessionMessages };
            messages.value = messagesUpdated;
    ...
    if (currentSessionId.value) {
      ...
      if (streamingMessageIndex !== -1) {
        ...
        sessionMessages[streamingMessageIndex] = {
          ...sessionMessages[streamingMessageIndex],
          isStreaming: false,
          content: sessionMessages[streamingMessageIndex].content + ' [abgebrochen]',
          status: 'error'
        };
        
        // Update mit selektiver Immutability
        const messagesUpdated = { ...messages.value, [currentSessionId.value]: sessionMessages };
        messages.value = messagesUpdated;
  ...
  async function deleteMessage(sessionId: string, messageId: string): Promise<void> {
    ...
    if (messages.value[sessionId]) {
      const sessionMessages = messages.value[sessionId].filter(m => m.id !== messageId);
      const messagesUpdated = { ...messages.value, [sessionId]: sessionMessages };
      messages.value = messagesUpdated;
    ...
    if (authStore.isAuthenticated) {
      try {
        ...
        // Bei Fehler lokale Änderung rückgängig machen
        const messagesUpdated = { ...messages.value, [sessionId]: messagesBackup };
        messages.value = messagesUpdated;
  ...
  function cleanupStorage() {
    ...
    Object.keys(messages.value).forEach(sessionId => {
      ...
      if (sessionMessages.length > messageLimit) {
        ...
        const messagesUpdated = { ...messages.value, [sessionId]: recentMessages };
        messages.value = messagesUpdated;
        ...
        try {
          ...
          for (let i = 0; i < chunks; i++) {
            ...
            sessionStorage.setItem(`session_${sessionId}_older_messages_${i}`, JSON.stringify(chunk));
          }
          
          // Metadata für die Chunks speichern
          sessionStorage.setItem(`session_${sessionId}_older_messages_meta`, JSON.stringify({
            chunks,
  ...
  function loadOlderMessages(sessionId: string): ChatMessage[] {
    ...
    try {
      ...
      const meta = JSON.parse(sessionStorage.getItem(`session_${sessionId}_older_messages_meta`) || 'null');
      ...
      if (!meta) {
        ...
        if (olderMessages.length > 0) {
          ...
          const combinedMessages = [...olderMessages, ...messages.value[sessionId]].sort(
            (a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
          );
          
          const messagesUpdated = { ...messages.value, [sessionId]: combinedMessages };
          messages.value = messagesUpdated;
      ...
      if (allOlderMessages.length > 0) {
        ...
        // Zusammenführen und nach Zeitstempel sortieren
        const combinedMessages = [...allOlderMessages, ...messages.value[sessionId]].sort(
          (a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
        );
   ...
   */
  const previewUpdateQueue = new Map<string, { text: string, count: number, timer: number | null }>();
  ...
  function updateSessionPreview(sessionId: string, previewText: string, messageCount: number): void {
    ...
    if (previewUpdateQueue.has(sessionId) && previewUpdateQueue.get(sessionId)!.timer !== null) {
    ...
    previewUpdateQueue.set(sessionId, {
      ...
      timer: window.setTimeout(() => {
        ...
        const currentIndex = sessions.value.findIndex(s => s.id === sessionId);
