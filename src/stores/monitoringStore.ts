...
export const useMonitoringStore = defineStore('monitoring', () => {
  ...
  function loadFromStorage() {
    try {
      ...
      const storedPerformance = StorageService.getItem(STORAGE_KEYS.PERFORMANCE);
      if (storedPerformance) performanceMetrics.value = JSON.parse(storedPerformance);
  ...
  function saveToStorage() {
    try {
      if (settings.value.dataCollection.errors) {
        StorageService.setItem(STORAGE_KEYS.ERRORS, JSON.stringify(errors.value));
      }
      
      if (settings.value.dataCollection.performance) {
        StorageService.setItem(STORAGE_KEYS.PERFORMANCE, JSON.stringify(performanceMetrics.value));
      }
      
      if (settings.value.dataCollection.usage) {
        StorageService.setItem(STORAGE_KEYS.USAGE, JSON.stringify(usageStats.value));
      }
      
      StorageService.setItem(STORAGE_KEYS.ALERTS, JSON.stringify(alerts.value));
      StorageService.setItem(STORAGE_KEYS.SETTINGS, JSON.stringify(settings.value));
  ...
  function setupWatchers() {
    ...
    watch([errors, performanceMetrics, usageStats, alerts, settings], () => {
      saveToStorage();
    }, { deep: true });
  }
  ...
  function cleanupOldData() {
    ...
    // Filter out old data
    errors.value = errors.value.filter(error => (now - error.timestamp) < maxAge);
    performanceMetrics.value = performanceMetrics.value.filter(metric => (now - metric.timestamp) < maxAge);
    usageStats.value = usageStats.value.filter(stat => (now - stat.timestamp) < maxAge);
    
    // Keep acknowledged alerts regardless of age
    alerts.value = alerts.value.filter(alert => 
      alert.acknowledged || (now - alert.timestamp) < maxAge
    );
  }
  ...
  function trackError(
    ...
    const existingErrorIndex = errors.value.findIndex(e => 
    ...
    if (existingErrorIndex >= 0) {
      ...
      const newError: FeatureError = {
        ...
        fallbackTriggered,
        count: 1,
        userAgent: settings.value.privacySettings.collectUserAgents ? userAgent : 'disabled',
        lastOccurrence: now
      };
  ...
  function checkErrorAlerts(featureId: string) {
    ...
    const featureErrors = errors.value.filter(e => 
      e.featureId === featureId && 
      !e.resolved &&
      (Date.now() - e.lastOccurrence) < 24 * 60 * 60 * 1000 // last 24 hours
    );
    
    const featureUsage = usageStats.value.filter(s => 
    ...
    if (featureUsage.length > 10) {
      ...
      if (errorRate >= settings.value.alertThresholds.errorRate) {
        ...
        createAlert(
          ...
          featureId,
          errorRate >= settings.value.alertThresholds.errorRate * 2 ? 'critical' : 'high',
          'errorRate',
    ...
    const criticalErrors = featureErrors.filter(e => 
      e.severity === 'critical' && 
      !e.resolved
    );
    
    if (criticalErrors.length >= settings.value.alertThresholds.criticalErrors) {
      createAlert(
        'error',
  ...
  function checkPerformanceAlerts(featureId: string, metricType: string, currentValue: number) {
    // Get historical metrics for this feature and type
    const historicalMetrics = performanceMetrics.value.filter(m => 
    ...
    const avgValue = historicalMetrics.reduce((sum, metric) => sum + metric.value, 0) / historicalMetrics.length;
    
    // Calculate percentage deviation
    const deviation = ((currentValue - avgValue) / avgValue) * 100;
    
    // Check if deviation exceeds threshold
    if (Math.abs(deviation) >= settings.value.alertThresholds.performanceDeviation) {
      ...
      if (isBad) {
        createAlert(
          ...
          featureId,
          Math.abs(deviation) >= settings.value.alertThresholds.performanceDeviation * 2 ? 'high' : 'medium',
          metricType,
  ...
  function checkUsageAlerts(featureId: string) {
    ...
    // Get today's usage
    const todayUsage = usageStats.value.filter(s => 
      s.featureId === featureId && 
      s.timestamp > oneDayAgo
    ).length;
    
    // Get yesterday's usage
    const yesterdayUsage = usageStats.value.filter(s => 
      s.featureId === featureId && 
      s.timestamp <= oneDayAgo &&
    ...
    if (usageDrop >= settings.value.alertThresholds.usageDropPercent) {
      createAlert(
        ...
        featureId,
        usageDrop >= settings.value.alertThresholds.usageDropPercent * 2 ? 'high' : 'medium',
        'usageDrop',
  ...
  const totalCriticalErrors = computed(() => {
    return errors.value.filter(e => !e.resolved && e.severity === 'critical').length;
  });
  ...
  function getErrorsByTimeRange(startTime: number, endTime: number = Date.now()) {
  ...
  function getPerformanceByTimeRange(startTime: number, endTime: number = Date.now()) {
    return performanceMetrics.value.filter(m => 
      m.timestamp >= startTime && 
      m.timestamp <= endTime
    );
  }

  function getUsageByTimeRange(startTime: number, endTime: number = Date.now()) {
    return usageStats.value.filter(s => 
      s.timestamp >= startTime && 
      s.timestamp <= endTime
    );
  }

  function getAlertsByTimeRange(startTime: number, endTime: number = Date.now()) {
