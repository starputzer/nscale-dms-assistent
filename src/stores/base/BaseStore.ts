  ...
  return defineStore(
    ...
    () => {
      ...
      async function initialize(): Promise<void> {
        ...
        try {
          ...
          if (options.debug) {
            console.error(
              `[BaseStore:${options.name}] Fehler bei Initialisierung:`,
              err,
            );
          }
        } finally {
    ...
    },
    {
      ...
      persist: options.persistence?.enabled
        ? {
            storage:
              options.persistence.storage === "sessionStorage"
                ? sessionStorage
                : localStorage,
            ...
            serializer: options.persistence.customSerializer || {
              deserialize: (value) => {
                try {
                  ...
                  console.error(
                    `[BaseStore:${options.name}] Fehler beim Deserialisieren:`,
                    err,
                  );
                  return {};
                }
              ...
              serialize: (state) => {
                try {
                  ...
                  console.error(
                    `[BaseStore:${options.name}] Fehler beim Serialisieren:`,
                    err,
                  );
                  return "{}";
                }
