 ...
 */
export interface UserStoreOptions<U extends UserData = UserData>
  extends BaseStoreOptions<UserStoreState<U>> {
  /**
...
export function createUserStore<U extends UserData = UserData>(
  ...
  return defineStore(`${options.name}-user`, () => {
    ...
    const isAdmin = computed((): boolean => {
      ...
      return base.state.user.roles.some(
    ...
    const tokenStatus = computed(
      (): "valid" | "expiring" | "expired" | "invalid" | "missing" => {
    ...
    const tokenExpiresIn = computed((): number => {
      ...
      return Math.max(
        0,
        Math.floor((base.state.token.expiresAt - Date.now()) / 1000),
      );
    });
    ...
    async function login(credentials: LoginCredentials): Promise<boolean> {
      ...
      try {
        ...
        */
        ...
        console.warn(
          "UserStore login: Diese Methode sollte in einer konkreten Implementierung überschrieben werden",
        );
        return false;
      } catch (err) {
    ...
    async function refreshToken(): Promise<boolean> {
      if (
        !base.state.token?.refreshToken ||
        base.state.tokenRefreshInProgress
      ) {
        return false;
      }
      ...
      try {
        ...
        */
        ...
        // Dummy-Implementierung
        console.warn(
          "UserStore refreshToken: Diese Methode sollte in einer konkreten Implementierung überschrieben werden",
        );
        return false;
      } catch (err) {
    ...
    function resetAutoLogoutTimer(): void {
      ...
      if (base.state.autoLogoutEnabled) {
        base.state.autoLogoutTimeoutId = window.setTimeout(() => {
          ...
          if (
            Date.now() - base.state.authStatus.lastActivity >
 ...
 */
export type UserStoreReturn<U extends UserData = UserData> = BaseStoreReturn<
  UserStoreState<U>
> & {
