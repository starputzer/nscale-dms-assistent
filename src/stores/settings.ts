...
export const useSettingsStore = defineStore('settings', () => {
  ...
  const currentTheme = computed(() => {
    ...
    return allThemes.find(t => t.id === theme.value.currentTheme) || DEFAULT_THEMES[0];
  });
  ...
  async function fetchSettings(): Promise<void> {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (response.data.notifications) notifications.value = response.data.notifications;
      }
    } catch (err: any) {
  ...
  function setTheme(themeId: string): void {
    ...
    const themeExists = [...DEFAULT_THEMES, ...theme.value.customThemes].some(t => t.id === themeId);
   ...
   */
  function updateCustomTheme(themeId: string, updatedTheme: Partial<ColorTheme>): void {
  ...
  function deleteCustomTheme(themeId: string): void {
    ...
    if (!isDefaultTheme) {
      theme.value.customThemes = theme.value.customThemes.filter(t => t.id !== themeId);
  ...
  function applyFontSettings(): void {
    ...
    document.documentElement.style.setProperty('--line-height', currentLineHeight.value.toString());
   ...
   */
  function updateMessageSettings(newSettings: Partial<MessageSettings>): void {
    messages.value = {
      ...messages.value,
      ...newSettings
    };
   ...
   */
  function updateNotificationSettings(newSettings: Partial<NotificationSettings>): void {
    notifications.value = {
      ...notifications.value,
      ...newSettings
    };
  ...
  watch(() => theme.value.currentTheme, () => {
    applyTheme();
  });
