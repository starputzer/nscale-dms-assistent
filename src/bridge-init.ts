...
export function initializeBridge(app: any) {
  ...
  try {
    ...
    if (!app._context?.provides?.pinia) {
      logger.warn('Pinia scheint nicht vor der Bridge-Initialisierung eingebunden zu sein. Es wird versucht, trotzdem fortzufahren.');
      ...
      logger.info('Pinia-Warnung behandelt, fahre mit Bridge-Initialisierung fort');
    }
    ...
    try {
      ...
      if (typeof installBridge === 'function') {
        ...
        logger.error('Bridge-Installation nicht möglich: installBridge ist keine Funktion');
      }
    } catch (bridgeError) {
      logger.error('Fehler beim Installieren der Bridge (wird übersprungen):', bridgeError);
      ...
      try {
        ...
        logger.error('Auch Fallback-Mechanismus fehlgeschlagen:', fallbackError);
      }
    }
    ...
    try {
      ...
      if (typeof initializeGlobalFunctions === 'function') {
        ...
        logger.error('Globale Funktionen nicht initialisiert: initializeGlobalFunctions ist keine Funktion');
      }
    } catch (globalFunctionsError) {
      logger.error('Fehler beim Initialisieren globaler Funktionen (wird übersprungen):', globalFunctionsError);
      ...
      try {
        ...
        logger.error('Auch Fallback für globale Funktionen fehlgeschlagen:', fallbackError);
      }
    }
    ...
    logger.error('Schwerwiegender Fehler bei der Bridge-Initialisierung:', error);
    
    // Trotzdem ein Event auslösen, damit die Anwendung weiß, dass versucht wurde, die Bridge zu initialisieren
    window.dispatchEvent(new CustomEvent('nscale-bridge-error', { 
      detail: {
        error,
        timestamp: Date.now()
      }
