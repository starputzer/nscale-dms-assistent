...
export function arrayProp<T>(options: {
  required?: boolean;
  default?: () => T[];
  validator?: (value: T[]) => boolean;
  minLength?: number;
...
export function objectProp<T extends Record<string, any>>(options: {
  required?: boolean;
  default?: () => T;
  validator?: (value: T) => boolean;
  shape?: {
  ...
  if (options.shape) {
    validators.push(value => {
      return Object.entries(options.shape!).every(([key, validator]: [string, any]) => {
...
// Function-Validator
export function functionProp<T extends (...args: any[]) => any>(options: {
  required?: boolean;
  default?: T;
} = {}): PropValidator<T> {
  return {
...
// Date-Validator
export function dateProp(options: {
  required?: boolean;
  default?: Date | (() => Date);
...
export function composedProp<T>(...validators: Array<PropValidator<T>>): PropValidator<T> {
  const composed: PropValidator<T> = {};
  
  for (const validator of validators) {
    if (validator.type) composed.type = validator.type;
    if (validator.required !== undefined) composed.required = validator.required;
