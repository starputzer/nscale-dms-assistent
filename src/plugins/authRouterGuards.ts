...
export function setupAuthRouterGuards(router: Router): void {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    if (from.path !== '/login' && sessionContinuityService.getState().isAuthenticated) {
    ...
    if (to.meta.guest && authResult.isAuthenticated) {
      logger.info('[AuthGuard] Bereits authentifiziert, Umleitung von Guest-Route');
 ...
 * Auth-Check mit Session-Wiederherstellung
 */
async function checkAuthWithSessionRecovery(): Promise<{ isAuthenticated: boolean }> {
  // Cache-Check
  if (authCheckCache && Date.now() - authCheckCache.timestamp < AUTH_CACHE_DURATION) {
...
function isPublicRoute(path: string): boolean {
  const publicRoutes = ['/login', '/register', '/forgot-password', '/reset-password', '/error', '/404'];
