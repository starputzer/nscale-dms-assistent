...
export function useAuthentication(): UseAuthenticationReturn {
  ...
  try {
    ...
    console.warn(
      "Error Reporting Service konnte nicht initialisiert werden, verwende Dummy-Implementation",
      e,
    );
  ...
  const { value: storedUser, setValue: setStoredUser } =
    useLocalStorage<User | null>("auth_user", null);

  // Reaktiver State
  const user = ref<User | null>(storedUser.value);
  const isAuthenticated = computed(() =>
    Boolean(user.value && authService.isAuthenticated()),
  );
  ...
  async function login(credentials: LoginCredentials): Promise<boolean> {
    try {
      ...
      if (response.token && response.user) {
        ...
        throw new AuthError(
          "Ungültige Antwort vom Authentifizierungsserver",
          "unknown",
        );
      }
    } catch (error) {
  ...
  async function refreshSession(): Promise<boolean> {
    ...
    try {
      ...
      if (
        ...
        (error.type === "token_expired" || error.type === "token_invalid")
      ) {
        clearSession();
        router.push({ name: "Login" });
      }
  ...
  function handleAuthError(error: unknown): void {
    ...
    if (error instanceof AuthError) {
      ...
      authError.value = new AuthError(error.message, "unknown", undefined, {
        stack: error.stack,
      });
    } else {
    ...
    try {
      if (errorReporting && typeof errorReporting.captureError === "function") {
        errorReporting.captureError(authError.value, {
        ...
        console.error(
          "[useAuthentication] Auth Error:",
          authError.value,
          "Typ:",
          authError.value.type,
          "User:",
          user.value?.id,
        );
      }
