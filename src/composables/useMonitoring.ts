...
export function useMonitoring(featureId: string, componentName: string) {
  ...
  const isMonitoringEnabled = computed(() => {
    return feature.value?.enabled && monitoringStore.settings.dataCollection.errors;
  });
  ...
  function endMeasure(actionName: string = 'render', type: 'renderTime' | 'totalTime' = 'renderTime') {
  ...
  function trackError(
    ...
    if (process.env.NODE_ENV === 'development') {
      LogService.error(`[${featureId}/${componentName}] ${errorMessage}`, error);
    }
  }
