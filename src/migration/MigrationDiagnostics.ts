...
import { MigrationStatus, ApplicationMigrationReport, StateMigrationReport } from './types';
...
export class MigrationDiagnostics {
  ...
  private migrationState = reactive({
    ...
    // Aufgetretene Fehler
    errors: [] as { component: string; message: string; timestamp: Date; stackTrace?: string }[],
  ...
  public registerComponent(
    ...
    const report: StateMigrationReport = {
      ...
      startedAt: now,
      migratedParts: [],
      pendingParts: Array.from({ length: totalSteps }, (_, i) => `step-${i + 1}`)
    };
    ...
    logger.debug(`Component ${componentName} (${componentId}) registered for migration with ${totalSteps} steps`);
  }
  ...
  public reportProgress(
    ...
    if (!this.migrationState.reports.has(componentId)) {
      logger.warn(`Unknown component ${componentId} reported migration progress`);
      return;
    }
    ...
    const pendingIndex = report.pendingParts ? report.pendingParts.indexOf(completedStep) : -1;
    ...
    if (pendingIndex >= 0 && report.pendingParts) {
      ...
      if (successful) {
        ...
        report.errors.push(`Error in step ${completedStep}: ${details?.message || 'Unknown error'}`);
    ...
    this.migrationState.progress = this.migrationState.totalSteps > 0
      ? (this.migrationState.completedSteps / this.migrationState.totalSteps) * 100
      : 0;
    
    // Zeitstatistiken aktualisieren
    if (report.startedAt && (report.status === MigrationStatus.COMPLETED || report.status === MigrationStatus.PARTIALLY_COMPLETED)) {
      const migrationTime = now.getTime() - report.startedAt.getTime();
      
      this.migrationState.statistics.totalMigrationTime += migrationTime;
      this.migrationState.statistics.averageMigrationTime = this.migrationState.statistics.totalMigrationTime / 
        (this.migrationState.statistics.migratedComponents + this.migrationState.statistics.partiallyMigratedComponents);
    }
    ...
    logger.debug(`Component ${componentId} reported progress: step ${completedStep} ${successful ? 'succeeded' : 'failed'}`);
  }
  ...
  public reportLegacyCodeUsage(source: string, count: number = 1): void {
    const currentCount = this.migrationState.statistics.legacyCodeUsage.get(source) || 0;
    this.migrationState.statistics.legacyCodeUsage.set(source, currentCount + count);
  ...
  public generateReport(): ApplicationMigrationReport {
    ...
    const partiallyMigratedComponents = this.migrationState.statistics.partiallyMigratedComponents;
    const migratedComponents = this.migrationState.statistics.migratedComponents;
    ...
    if (migratedComponents + failedComponents + partiallyMigratedComponents === totalComponents && totalComponents > 0) {
    ...
    if (overallStatus === MigrationStatus.COMPLETED && !this.migrationState.completionTime) {
    ...
    if (partiallyMigratedComponents > 0) {
      criticalIssues.push(`${partiallyMigratedComponents} component(s) partially migrated`);
    }
    ...
    if (failedComponents > 0) {
      recommendedActions.push('Review error logs for failed components and fix issues');
    }
    
    if (partiallyMigratedComponents > 0) {
      recommendedActions.push('Complete migration for partially migrated components');
    }
    
    if (this.migrationState.statistics.legacyCodeUsage.size > 0) {
      const topLegacyUsage = [...this.migrationState.statistics.legacyCodeUsage.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3);
    ...
    return {
      ...
      warnings: this.migrationState.warnings.length > 0 
        ? this.migrationState.warnings.map(w => `${w.component}: ${w.message}`)
        : undefined,
  ...
  public logMigrationError(
    ...
    // Ereignis loggen
    logger.error(`Migration error in ${componentId} (step ${step}): ${errorMessage}`, error);
  }
   ...
   */
  public setDashboardConfig(config: Partial<typeof this.dashboardConfig>): void {
