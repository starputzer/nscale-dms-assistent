...
export class RouterMigrator {
  ...
  public registerRouteConfig(config: RouterMigrationConfig): void {
    ...
    if (existingIndex >= 0) {
      ...
      logger.debug(`Updated route migration config: ${config.legacyPath} <-> ${config.vuePath}`);
    } else {
      // Neue Konfiguration hinzufügen
      this.routeConfigs.push(config);
      logger.debug(`Registered route migration config: ${config.legacyPath} <-> ${config.vuePath}`);
    }
  }
  ...
  private setupVueRouterGuards(): void {
    ...
    this.vueRouter.beforeEach((to: any, from: any, next: Function) => {
      ...
      if (matchingConfig) {
        logger.debug(`Vue route ${to.path} matched with legacy path ${matchingConfig.legacyPath}`);
  ...
  private setupLegacyNavigationHandlers(): void {
    if (!this.eventBus) {
      logger.warn('Cannot setup legacy navigation handlers: Event bus not initialized');
      return;
    }
    ...
    this.eventBus.on('legacy:navigation', (data: any) => {
      ...
      if (matchingConfig) {
        logger.debug(`Legacy navigation to ${path} redirected to Vue route ${matchingConfig.vuePath}`);
   ...
   */
  private findConfigByVuePath(vuePath: string): RouterMigrationConfig | undefined {
    return this.routeConfigs.find(config => {
      // Exakte Übereinstimmung
      if (config.vuePath === vuePath) return true;
      
      // Dynamische Route mit Parametern
      if (config.vuePath.includes(':') && this.pathsMatchPattern(vuePath, config.vuePath)) {
        return true;
      }
   ...
   */
  private findConfigByLegacyPath(legacyPath: string): RouterMigrationConfig | undefined {
    return this.routeConfigs.find(config => {
      // Exakte Übereinstimmung
      if (config.legacyPath === legacyPath) return true;
      
      // Dynamische Route mit Parametern
      if (config.legacyPath.includes(':') && this.pathsMatchPattern(legacyPath, config.legacyPath)) {
        return true;
      }
  ...
  private navigateToVueRoute(
    ...
    if (config.paramMapping && params) {
      ...
      for (const [legacyParam, vueParam] of Object.entries(config.paramMapping)) {
  ...
  public isLegacyUrl(url: string): boolean {
    ...
    return this.routeConfigs.some(config => 
      config.legacyPath === path || 
      (config.legacyPath.includes(':') && this.pathsMatchPattern(path, config.legacyPath))
    );
  }
  ...
  public legacyUrlToVueUrl(url: string): string | null {
    ...
    if (matchingConfig.paramMapping) {
      ...
      // Benannte Parameter gemäß Mapping transformieren
      for (const [legacyParam, vueParam] of Object.entries(matchingConfig.paramMapping)) {
        if (params[legacyParam] !== undefined) {
          transformedParams[vueParam] = params[legacyParam];
        }
      }
    ...
    if (matchingConfig.additionalParams) {
      transformedParams = { ...transformedParams, ...matchingConfig.additionalParams };
    }
    
    // Vue-URL erstellen
    const vueUrl = this.buildPathWithParams(matchingConfig.vuePath, transformedParams);
