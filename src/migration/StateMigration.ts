...
import { StateVersion, StateMigrationStrategy, StateMigrationResult } from './types';
...
export class StateMigrator {
  ...
  private migrationStrategies: Map<string, StateMigrationStrategy[]> = new Map();
  ...
  public registerMigrationStrategy(
    ...
    logger.info(`Migrationsstrategie für ${stateKey} registriert: ${JSON.stringify(fromVersion)} -> ${JSON.stringify(toVersion)}`);
  }
  ...
  public migrateForward(
    ...
    if (strategies.length === 0) {
      return {
        success: true,
        data: deepClone(legacyData),
        warnings: ['Keine Migrationsstrategien definiert, Daten unverändert übernommen']
      };
    }
    ...
    try {
      ...
      for (const strategy of strategies) {
        ...
        if (this.versionsMatch(currentFromVersion, strategy.fromVersion)) {
          logger.debug(`Migriere ${stateKey} von ${JSON.stringify(currentFromVersion)} nach ${JSON.stringify(strategy.toVersion)}`);
    ...
    try {
      ...
      for (let i = strategies.length - 1; i >= 0; i--) {
        ...
        if (this.versionsMatch(strategy.toVersion, currentToVersion)) {
          logger.debug(`Rückwärtsmigration für ${stateKey} von ${JSON.stringify(strategy.toVersion)} nach ${JSON.stringify(strategy.fromVersion)}`);
      ...
      logger.error(`Unerwarteter Fehler bei Rückwärtsmigration von ${stateKey}:`, error);
      return {
        success: false,
        data: deepClone(vueData),
        errors: [`Unerwarteter Fehler: ${error instanceof Error ? error.message : String(error)}`]
  ...
  public repairState(
    ...
    try {
      ...
      if (!validationResult.success) {
        logger.warn(`Schemavalidierung für ${stateKey} fehlgeschlagen, versuche Reparatur`);
        ...
        // Erneut validieren
        const revalidation = this.validateSchema(stateKey, repairedData, version);
      ...
      return {
        success: false,
        data,
        errors: [`Reparatur fehlgeschlagen: ${error instanceof Error ? error.message : String(error)}`]
      };
    }
  }
