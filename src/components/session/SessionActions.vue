  ...
  >
    ...
    >
      <button 
        ...
        :disabled="disabled"
      >
        <svg v-if="action.icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" v-html="action.icon"></svg>
        <span v-if="showLabels" class="n-session-actions__btn-text">{{ action.label }}</span>
      </button>
    </div>
    ...
    <div v-if="moreActions.length > 0" class="n-session-actions__more">
      ...
      >
        <button 
          ...
          :class="{ 'n-session-actions__dropdown-item--danger': action.variant === 'danger' }"
          @click="handleAction(action.name)"
        >
          <svg v-if="action.icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" v-html="action.icon"></svg>
    ...
    <div 
      v-if="selectedSessionIds.length > 1" 
      class="n-session-actions__bulk"
    >
      <span class="n-session-actions__selection-info">{{ selectedSessionIds.length }} ausgewählt</span>
...
const props = withDefaults(defineProps<Props>(), {
  ...
  showLabels: true,
  display: 'inline',
  availableActions: () => ['rename', 'delete', 'pin', 'export', 'share', 'duplicate'],
...
const enabledSessionActions = computed(() => {
  return sessionActions.filter(action => {
    ...
    const isFeatureEnabled = !action.requiredFeature || props.enabledFeatures.includes(action.requiredFeature);
    
    // Filtern nach Bedingung (wenn vorhanden)
    const meetsCondition = !action.condition || action.condition(props.activeSessionId);
