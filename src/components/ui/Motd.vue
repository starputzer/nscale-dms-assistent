<template>
  <div v-if="showMotd && motd" class="motd-container" :class="{ 'motd-minimized': isMinimized }">
    ...
    <div v-if="!isMinimized" class="motd-content">
      ...
      <div class="motd-footer">
        ...
        <span v-if="motd.date" class="motd-date">{{ formatDate(motd.date) }}</span>
        <div class="motd-controls">
          <label class="motd-checkbox">
            <input type="checkbox" v-model="dontShowAgain" @change="updateDontShowAgain">
            Nicht mehr anzeigen
          </label>
        </div>
      </div>
...
const updateDontShowAgain = () => {
  if (dontShowAgain.value && props.motd && props.motd.id) {
    ...
    const dismissedMotds = JSON.parse(getItem('dismissedMotds') || '[]')
    const updatedDismissedMotds = dismissedMotds.filter(id => id !== props.motd.id)
