<template>
  ...
  >
    ...
    <template v-else-if="type === 'circular'">
      ...
      <div v-if="showLabel || $slots.center" class="n-progress__circular-center">
        <slot name="center">
          {{ displayLabel }}
        </slot>
      </div>
...
const currentStepIndex = computed((): number => {
  ...
  const currentStepByStatus = props.steps.findIndex(step => step.status === 'current');
...
function getSegmentActive(index: number): boolean {
  ...
  const activeSegments = Math.round((normalizedValue.value / 100) * props.segmentCount);
