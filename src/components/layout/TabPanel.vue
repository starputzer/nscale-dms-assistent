<template>
  ...
  >
      ...
      <div class="n-tab-panel__tabs" :class="{ 'n-tab-panel__tabs--scrollable': scrollable }">
        ...
        >
          <div
            ...
            @keydown.delete="props.closable && !tab.permanent && !tab.disabled && closeTab(tab.id)"
            @dragstart="draggable && !tab.disabled && handleDragStart($event, index)"
          ...
          >
            <slot name="tab" :tab="tab" :index="index">
              <div class="n-tab-panel__tab-inner">
                <span v-if="tab.pinned && props.allowPinning" class="n-tab-panel__tab-pin-indicator" :title="'Pinned tab'">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <span
                  ...
                  :style="{ backgroundColor: tab.badgeColor || 'var(--n-primary-color, #3182ce)' }"
                >
                  {{ tab.badge }}
                </span>
                <button
    ...
    <div class="n-tab-panel__content" :class="{ [`n-tab-panel__content--${props.displayType}`]: true }">
      <div
      ...
      >
        <div
          v-if="contextMenuTab && props.allowPinning && !contextMenuTab.disabled"
        ...
        <div
          v-if="contextMenuTab && props.closable && !contextMenuTab.permanent && !contextMenuTab.disabled"
          class="n-tab-panel__context-menu-item"
      ...
      <div
        v-if="props.showTabPreview && showPreview && previewTab && previewTab.preview"
        class="n-tab-panel__preview"
        :style="{ left: previewPosition.x + 'px', top: previewPosition.y + 'px' }"
      >
        <div class="n-tab-panel__preview-content">
          <img v-if="typeof previewTab.preview === 'string'" :src="previewTab.preview" alt="Tab preview" />
...
const visibleTabs = computed(() => {
  ...
  const activeTab = sortedTabs.value.find(tab => tab.id === activeTabId.value && !tab.pinned);
  ...
  const normalTabs = sortedTabs.value.filter(tab => !tab.pinned && tab.id !== activeTabId.value);

  // Berechne, wie viele normale Tabs wir anzeigen können
  const remainingSlots = props.maxTabs - pinnedTabs.length - (activeTab ? 1 : 0);
...
function handleDrop(event: DragEvent, dropIndex: number) {
  ...
  const actualDraggedIndex = props.tabs.findIndex(tab => tab.id === draggedTabId);
