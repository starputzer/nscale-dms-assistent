...
function prepareChartData(): ChartData {
  ...
  features.forEach((feature, index) => {
    ...
    const errorRates = timeIntervals.map(interval => {
      ...
      const intervalErrors = monitoringStore.getErrorsByTimeRange(
        interval.start,
        interval.end
      ).filter(e => e.featureId === featureId);
      
      // Get usage in this interval
      const intervalUsage = monitoringStore.getUsageByTimeRange(
        interval.start,
        interval.end
    ...
    chartInstance.value = new Chart.Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text: t('monitoring.charts.errorRateTitle', 'Fehlerrate im Zeitverlauf'),
            font: {
              size: 16
            }
          },
...
watch(() => [props.timeRange, props.featureId], async () => {
  await loadChart();
});

// Check for store updates to refresh chart
watch(() => [monitoringStore.errors.length, monitoringStore.usageStats.length], async () => {
  if (!loading.value) {
    await loadChart();
  }
});
