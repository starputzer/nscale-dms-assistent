...
function prepareChartData(): ChartData {
  ...
  features.forEach((feature, index) => {
    ...
    const metricValues = timeIntervals.map(interval => {
      ...
      const intervalMetrics = monitoringStore.getPerformanceByTimeRange(
        interval.start,
        interval.end
      ).filter(m => 
        m.featureId === featureId && 
        m.type === metricType.value
      );
      ...
      return intervalMetrics.reduce((sum, metric) => sum + metric.value, 0) / intervalMetrics.length;
    });
    ...
    chartInstance.value = new Chart.Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text: t('monitoring.charts.performanceTitle', 'Performance-Verlauf'),
            font: {
              size: 16
            }
          },
          ...
          tooltip: {
            callbacks: {
              label: function(context) {
                if (context.raw === null) return `${context.dataset.label}: Keine Daten`;
                
                let unit = '';
                if (metricType.value === 'renderTime' || metricType.value === 'networkTime' || metricType.value === 'totalTime') {
                  unit = 'ms';
...
watch(() => [props.timeRange, props.featureId, props.metricType], async () => {
  await loadChart();
});

// Check for store updates to refresh chart
watch(() => monitoringStore.performanceMetrics.length, async () => {
  if (!loading.value) {
    await loadChart();
  }
});
