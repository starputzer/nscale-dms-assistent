...
async function loadChart() {
  ...
  try {
    ...
    const chartData =
      props.chartType === "pie" ? preparePieData() : prepareTimeSeriesData();
...
watch(
  () => [props.timeRange, props.chartType, props.featureId],
  async () => {
    await loadChart();
  },
);
...
watch(
  () => monitoringStore.usageStats.length,
  async () => {
    if (!loading.value) {
      await loadChart();
    }
