<template>
  ...
  >
    ...
    <div v-if="documentName" class="current-document">
      ...
      <span
        v-if="documentIndex !== undefined && totalDocuments > 1"
    ...
    <div class="progress-stages" aria-hidden="true">
      ...
      >
        ...
        <div
          v-if="stage.description && stageIndex === index"
    ...
    <div class="progress-container">
      ...
      <div class="progress-info">
        ...
        <div class="progress-details">
          ...
          <span
            v-else-if="estimatedCompletionTime"
            class="estimated-completion"
          >
            {{ t("documentConverter.estimatedCompletion", "Fertig um") }}:
            {{ formatTime(estimatedCompletionTime) }}
          </span>
          <span v-else-if="estimatedTimeRemaining > 0" class="estimated-time">
            {{ t("documentConverter.estimatedTimeRemaining", "Verbleibend") }}:
    ...
    <div class="step-details">
      <div class="current-step-container">
        <div class="current-step-label">
          {{ t("documentConverter.currentStep", "Aktueller Schritt") }}:
        </div>
        <div class="current-step" data-testid="current-step">
          {{ currentStep }}
        </div>
      </div>
    ...
    <div class="actions">
      <button
        ...
        :aria-label="
          t('documentConverter.cancelConversion', 'Konvertierung abbrechen')
        "
      ...
      <button
        ...
        :aria-label="
          t('documentConverter.pauseConversion', 'Konvertierung pausieren')
        "
      ...
      <button
        ...
        @click="handleResume"
        :aria-label="
          t('documentConverter.resumeConversion', 'Konvertierung fortsetzen')
        "
      ...
      >
        ...
        {{
          showDebugInfo
            ? t("documentConverter.hideDebug", "Debug ausblenden")
            : t("documentConverter.showDebug", "Debug anzeigen")
        }}
      </button>
    </div>
    ...
    <transition name="slide">
      <div v-if="showDebugInfo" class="debug-info" data-testid="debug-info">
        <h4 class="debug-info-title">
          {{ t("documentConverter.debugInfo", "Debug-Informationen") }}
        </h4>
        <div class="debug-info-content">
          <div class="debug-info-section">
            <h5>
              {{
                t("documentConverter.conversionPhases", "Konvertierungsphasen")
              }}
            </h5>
            <ul class="debug-phase-list">
              <li
                ...
                :class="{
                  'debug-phase-item--current': idx === currentPhaseIndex,
                }"
              >
          ...
          <div class="debug-info-section">
            <h5>
              {{
                t("documentConverter.performanceMetrics", "Leistungsmetriken")
              }}
            </h5>
            <div class="debug-metrics">
              <div class="debug-metric">
                <span class="debug-metric-label"
                  >{{
                    t(
                      "documentConverter.avgProcessingSpeed",
                      "Durchschn. Verarbeitungsgeschwindigkeit",
                    )
                  }}:</span
                >
              ...
              <div class="debug-metric">
                <span class="debug-metric-label"
                  >{{ t("documentConverter.startTime", "Startzeit") }}:</span
                >
                <span class="debug-metric-value">{{
              ...
              <div class="debug-metric">
                <span class="debug-metric-label"
                  >{{
                    t("documentConverter.elapsedTime", "Verstrichene Zeit")
                  }}:</span
                >
                <span class="debug-metric-value">{{
                  formatDuration(elapsedTimeSeconds)
                }}</span>
              </div>
              <div class="debug-metric">
                <span class="debug-metric-label"
                  >{{
                    t("documentConverter.timeRemaining", "Verbleibende Zeit")
                  }}:</span
                >
                <span class="debug-metric-value">{{
                  formatDuration(estimatedTimeRemaining)
    ...
    <div
      ...
      class="conversion-logs"
      data-testid="conversion-logs"
    >
      <details class="logs-details">
        <summary class="logs-summary">
          ...
          <span
            class="logs-count"
            :class="{
              'logs-count--warning': hasWarningLogs,
              'logs-count--error': hasErrorLogs,
            }"
          >
            {{ logs.length }}
          </span>
...
const stages = [
  {
    ...
    description: t(
      "documentConverter.stage.preparationDesc",
      "Dokumentanalyse und Vorbereitung der Konvertierung",
    ),
  },
  {
    name: t("documentConverter.stage.extraction", "Extraktion"),
    description: t(
      "documentConverter.stage.extractionDesc",
      "Inhaltsextraktion und Metadatensammlung",
    ),
  },
  {
    name: t("documentConverter.stage.processing", "Verarbeitung"),
    description: t(
      "documentConverter.stage.processingDesc",
      "Strukturerkennung und Inhaltsverarbeitung",
    ),
  },
  {
    name: t("documentConverter.stage.formatting", "Formatierung"),
    description: t(
      "documentConverter.stage.formattingDesc",
      "Anwendung von Formaten und Strukturen",
    ),
  },
  {
    name: t("documentConverter.stage.finalization", "Fertigstellung"),
    description: t(
      "documentConverter.stage.finalizationDesc",
      "Ergebniszusammenstellung und Finalisierung",
    ),
  },
];
...
const stageIndex = computed(() => {
  ...
  return Math.min(
    Math.floor(props.progress / (100 / stages.length)),
    stages.length - 1,
  );
});
...
const combinedLogs = computed(() => {
  ...
  return [...internalLogs.value, ...externalLogs].sort(
...
function updateTimeEstimation(): void {
  ...
  if (avgSpeed.value !== null && currentProgress < 100) {
    ...
    const adjustedEstimatedRemainingMs =
      estimatedRemainingMs * timeEstimationFactor.value;

    // Geschätzten Abschlusszeitpunkt berechnen
    estimatedCompletionTime.value = new Date(
      currentTime.getTime() + adjustedEstimatedRemainingMs,
    );
...
function updatePhaseStatus(): void {
  ...
  if (newPhaseIndex !== currentPhaseIndex.value) {
    ...
    if (currentPhaseIndex.value < phaseTimings.value.length) {
      ...
      addLog(
        "info",
        `Phase "${phaseTimings.value[currentPhaseIndex.value].name}" abgeschlossen`,
      );
    }
    ...
    if (newPhaseIndex < phaseTimings.value.length) {
      ...
      addLog(
        "info",
        `Phase "${phaseTimings.value[newPhaseIndex].name}" gestartet`,
      );
    }
...
watch(
  ...
  (newProgress) => {
    ...
    if (newProgress >= 100) {
      ...
      if (
        phaseTimings.value[lastPhaseIndex].endTime === null &&
        phaseTimings.value[lastPhaseIndex].startTime !== null
      ) {
        const now = new Date();
        phaseTimings.value[lastPhaseIndex].endTime = now;
        phaseTimings.value[lastPhaseIndex].duration =
          (now.getTime() -
            phaseTimings.value[lastPhaseIndex].startTime!.getTime()) /
          1000;
      }
    }
  },
  ...
  (newWarnings) => {
    if (newWarnings && newWarnings.length > 0) {
      ...
      for (const warning of newWarnings) {
        if (
          !internalLogs.value.some(
            (log) => log.level === "warning" && log.message === warning,
          )
        ) {
