<template>
  <div class="batch-upload">
    <div class="batch-upload__header">
      <h3 class="batch-upload__title">
        {{ t("documentConverter.batchUpload.title", "Batch-Upload") }}
      </h3>
      <div class="batch-upload__description">
        {{
          t(
            "documentConverter.batchUpload.description",
            "Laden Sie mehrere Dokumente gleichzeitig hoch und konvertieren Sie sie als Stapel.",
          )
        }}
      </div>
    </div>
    ...
    <div
      ...
      :aria-label="
        t(
          'documentConverter.batchUpload.dropzone',
          'Dateien für Batch-Upload hier ablegen',
        )
      "
    >
      <div class="batch-upload__dropzone-content">
        ...
        <div class="batch-upload__text">
          <p class="batch-upload__prompt">
            {{
              t(
                "documentConverter.batchUpload.dropFiles",
          ...
          <p class="batch-upload__hint">
            ...
            <br />
            {{
              t("documentConverter.batchUpload.maxFiles", "Maximale Anzahl:")
            }}
            {{ maxFiles }}
            {{ t("documentConverter.batchUpload.files", "Dateien") }}
          </p>
        </div>
    ...
    <div v-if="isLoading || uploadInProgress" class="batch-upload__loading">
      <div class="batch-upload__loader">
        ...
        <span>{{
          t(
            "documentConverter.batchUpload.preparingFiles",
    ...
    <div v-if="batchFiles.length > 0" class="batch-upload__overview">
      <div class="batch-upload__stats">
        <div class="batch-upload__stat-item">
          <span class="batch-upload__stat-label">{{
            t("documentConverter.batchUpload.totalFiles", "Dateien gesamt:")
          }}</span>
          <span class="batch-upload__stat-value">{{ batchFiles.length }}</span>
        </div>
        <div class="batch-upload__stat-item">
          <span class="batch-upload__stat-label">{{
            t("documentConverter.batchUpload.totalSize", "Gesamtgröße:")
          }}</span>
          <span
            class="batch-upload__stat-value"
            :class="{ 'batch-upload__stat-value--error': isTotalSizeExceeded }"
          >
            {{ totalSizeFormatted }}
            <span
              v-if="isTotalSizeExceeded"
              class="batch-upload__error-icon"
              aria-hidden="true"
              >⚠️</span
            >
          </span>
        ...
        <div class="batch-upload__stat-item">
          <span class="batch-upload__stat-label">{{
            t("documentConverter.batchUpload.validFiles", "Gültige Dateien:")
          }}</span>
          <span
            class="batch-upload__stat-value"
            :class="{
              'batch-upload__stat-value--success': validFilesCount > 0,
            }"
          >
            {{ validFilesCount }}
          </span>
        ...
        <div v-if="invalidFilesCount > 0" class="batch-upload__stat-item">
          <span class="batch-upload__stat-label">{{
            t(
              "documentConverter.batchUpload.invalidFiles",
              "Ungültige Dateien:",
            )
          }}</span>
          <span
            class="batch-upload__stat-value batch-upload__stat-value--error"
      ...
      <div class="batch-upload__settings">
        <div class="batch-upload__priority">
          ...
          >
            <option value="high">
              {{ t("documentConverter.batchUpload.priorityHigh", "Hoch") }}
            </option>
            <option value="normal">
              {{ t("documentConverter.batchUpload.priorityNormal", "Normal") }}
            </option>
            <option value="low">
              {{ t("documentConverter.batchUpload.priorityLow", "Niedrig") }}
            </option>
          </select>
        </div>
        ...
        <div class="batch-upload__option">
          ...
          <label for="auto-convert" class="batch-upload__checkbox-label">
            {{
              t(
                "documentConverter.batchUpload.autoConvert",
                "Automatisch konvertieren nach Upload",
              )
            }}
          </label>
        </div>
      ...
      <div
        v-if="isTotalSizeExceeded"
        class="batch-upload__error-message"
        role="alert"
      >
        {{
          t(
            "documentConverter.batchUpload.totalSizeExceeded",
      ...
      <div class="batch-upload__file-list">
        <h4 class="batch-upload__subheading">
          {{
            t(
              "documentConverter.batchUpload.filesToUpload",
              "Dateien für den Stapel-Upload:",
            )
          }}
        </h4>
        ...
        <div class="batch-upload__files">
          ...
          >
            ...
            <div
              v-if="fileSwiping === file.id"
              class="batch-upload__touch-actions"
            >
              <button
                ...
                :aria-label="
                  t(
                    'documentConverter.batchUpload.uploadNow',
                    `Datei '${file.file.name}' jetzt hochladen`,
                  )
                "
              >
              ...
              <button
                ...
                :aria-label="
                  t(
                    'documentConverter.batchUpload.removeFile',
              ...
              >
                ...
                <span>{{
                  t("documentConverter.batchUpload.remove", "Entfernen")
                }}</span>
              </button>
            </div>
            ...
            <div class="batch-upload__file-header">
              ...
              <div class="batch-upload__file-info">
                <span class="batch-upload__file-name" :title="file.file.name">{{
                  file.file.name
                }}</span>
              ...
              <div class="batch-upload__file-actions">
                ...
                <div
                  v-if="!isProcessing && file.validationStatus === 'valid'"
                  class="batch-upload__file-priority"
                >
                  <select
                    ...
                    :aria-label="
                      t(
                        'documentConverter.batchUpload.filePriority',
                        'Dateiprioritä',
                      )
                    "
                  >
                    ...
                    <option value="normal">
                      {{
                        t(
                          "documentConverter.batchUpload.priorityNormal",
                          "Normal",
                        )
                      }}
                    </option>
                    <option value="low">
                      {{
                        t(
                          "documentConverter.batchUpload.priorityLow",
                          "Niedrig",
                        )
                      }}
                    </option>
                ...
                <button
                  ...
                  :aria-label="
                    t(
                      ...
                      `Datei '${file.file.name}' einzeln hochladen`,
                    )
                  "
                >
                  <i class="fa fa-upload" aria-hidden="true"></i>
            ...
            <div
              ...
              class="batch-upload__file-progress"
            >
              <div
                class="batch-upload__progress"
                role="progressbar"
              ...
              >
                <div
                  class="batch-upload__progress-bar"
                  :style="{ width: `${file.progress}%` }"
                ></div>
                <span class="batch-upload__progress-text"
                  >{{ file.progress }}%</span
                >
              </div>
            </div>
            ...
            <div
              ...
              class="batch-upload__validating"
            >
              <i class="fa fa-spinner fa-spin" aria-hidden="true"></i>
            ...
            <div v-if="file.canResume" class="batch-upload__resume">
              ...
              >
                ...
                {{
                  t(
                    "documentConverter.batchUpload.resumeUpload",
                    "Upload fortsetzen",
                  )
                }}
              </button>
      ...
      <div class="batch-upload__actions">
        ...
        >
          ...
          {{
            t(
              "documentConverter.batchUpload.startBatch",
        ...
        >
          ...
          {{
            t(
              "documentConverter.batchUpload.cancelBatch",
      ...
      <div v-if="isProcessing" class="batch-upload__batch-progress">
        ...
        <div class="batch-upload__batch-stats">
          <div class="batch-upload__batch-stat">
            <span class="batch-upload__batch-label">{{
              t(
                "documentConverter.batchUpload.filesProcessed",
                "Dateien verarbeitet:",
              )
            }}</span>
            <span class="batch-upload__batch-value"
              >{{ completedFilesCount }} / {{ totalFilesToProcess }}</span
            >
          </div>
          <div class="batch-upload__batch-stat">
            <span class="batch-upload__batch-label">{{
              t("documentConverter.batchUpload.currentFile", "Aktuelle Datei:")
            }}</span>
            <span class="batch-upload__batch-value">{{
              currentFileName || "-"
            }}</span>
          ...
          <div class="batch-upload__batch-stat">
            <span class="batch-upload__batch-label">{{
              t(
                "documentConverter.batchUpload.estimatedTimeRemaining",
                "Geschätzte Restzeit:",
              )
            }}</span>
            <span class="batch-upload__batch-value">{{
        ...
        <div
          class="batch-upload__total-progress"
          role="progressbar"
        ...
        >
          <div
            class="batch-upload__total-progress-bar"
            :style="{ width: `${batchProgress}%` }"
          ></div>
          <span class="batch-upload__total-progress-text"
            >{{ batchProgress }}%</span
          >
        </div>
      </div>
    ...
    <div v-if="!batchFiles.length && !isLoading" class="batch-upload__empty">
      <p>
        {{
          t(
            "documentConverter.batchUpload.empty",
            "Fügen Sie Dateien hinzu, um einen Stapel-Upload zu starten.",
          )
        }}
      </p>
    </div>
    ...
    <div
      v-if="batchFiles.length > 0 && !isProcessing"
      class="batch-upload__quick-actions"
    >
      <button
      ...
      >
        {{
          t(
            "documentConverter.batchUpload.setAllHigh",
            "Alle auf hohe Priorität setzen",
          )
        }}
      </button>
      <button
      ...
      >
        {{
          t(
            "documentConverter.batchUpload.setAllNormal",
            "Alle auf normale Priorität setzen",
          )
        }}
      </button>
      <button class="batch-upload__quick-action" @click="removeInvalidFiles">
        {{
          t(
            "documentConverter.batchUpload.removeInvalid",
...
const validFilesCount = computed<number>(() => {
  return batchFiles.value.filter((file) => file.validationStatus === "valid")
    .length;
});

const invalidFilesCount = computed<number>(() => {
  return batchFiles.value.filter((file) => file.validationStatus === "invalid")
    .length;
});
...
function processFiles(files: File[]): void {
  ...
  if (remainingSlots <= 0) {
    toast.error(
      t(
        "documentConverter.batchUpload.tooManyFiles",
  ...
  Promise.all(processPromises).then(() => {
    ...
    if (files.length > remainingSlots) {
      ...
      toast.warning(
        t(
          "documentConverter.batchUpload.filesIgnored",
    ...
    if (filesToProcess.length > 0) {
      setScreenReaderMessage(
        t(
          "documentConverter.batchUpload.filesAdded",
...
function checkValidationComplete(): void {
  ...
  const stillValidating = batchFiles.value.some(
    (file) =>
...
function removeFile(index: number): void {
  if (isProcessing.value && batchFiles.value[index].status === "uploading") {
    ...
    toast.warning(
      t(
        "documentConverter.batchUpload.cannotRemoveDuringUpload",
  ...
  if (index >= 0 && index < batchFiles.value.length) {
    ...
    setScreenReaderMessage(
      t(
        "documentConverter.batchUpload.fileRemoved",
...
function clearBatch(): void {
  if (isProcessing.value) {
    toast.warning(
      t(
        "documentConverter.batchUpload.cannotClearDuringProcessing",
  ...
  if (batchFiles.value.length > 0) {
    dialog.confirm({
      title: t(
        "documentConverter.batchUpload.clearBatchConfirmTitle",
        "Stapel leeren",
      ),
      message: t(
        "documentConverter.batchUpload.clearBatchConfirm",
      ...
      onConfirm: () => {
        ...
        setScreenReaderMessage(
          t(
            "documentConverter.batchUpload.batchCleared",
            "Stapel wurde geleert.",
          ),
        );
      },
    });
...
function removeInvalidFiles(): void {
  ...
  batchFiles.value = batchFiles.value.filter(
    (file) => file.validationStatus === "valid",
  );

  setScreenReaderMessage(
    t(
      "documentConverter.batchUpload.invalidFilesRemoved",
...
function setPriorityForAll(priority: FilePriority): void {
  ...
  setScreenReaderMessage(
    t(
      "documentConverter.batchUpload.allPrioritySet",
...
async function uploadSingleFile(index: number): Promise<void> {
  ...
  try {
    ...
    toast.success(
      t(
        "documentConverter.batchUpload.singleFileUploaded",
    ...
    batchFiles.value[index].error =
      error instanceof Error
        ? error.message
        : t(
            "documentConverter.batchUpload.uploadFailed",
            "Upload fehlgeschlagen",
          );

    toast.error(
      t(
        "documentConverter.batchUpload.singleUploadFailed",
...
async function startBatchUpload(): Promise<void> {
  ...
  if (validFilesCount.value > 10) {
    dialog.confirm({
      title: t(
        "documentConverter.batchUpload.largeBatchTitle",
        "Großer Stapel",
      ),
      message: t(
        "documentConverter.batchUpload.largeBatchConfirm",
...
async function executeBatchUpload(): Promise<void> {
  try {
    ...
    const filesToUpload = batchFiles.value.filter(
      (file) => file.validationStatus === "valid",
    );
    ...
    for (let i = 0; i < sortedFiles.length; i++) {
      ...
      const fileIndex = batchFiles.value.findIndex(
        (f) => f.id === sortedFiles[i].id,
      );
      ...
      setScreenReaderMessage(
        t(
          "documentConverter.batchUpload.uploadingFile",
      ...
      try {
        ...
        batchProgress.value = Math.round(
          (completedFilesCount.value / totalFilesToProcess.value) * 100,
        );
        ...
        batchFiles.value[fileIndex].error =
          error instanceof Error
            ? error.message
            : t(
                "documentConverter.batchUpload.uploadFailed",
        ...
        if (
          props.enableResume &&
          isResumeSupported(batchFiles.value[fileIndex].file)
        ) {
    ...
    if (isProcessing.value) {
      ...
      toast.success(
        t(
          "documentConverter.batchUpload.batchCompleted",
    ...
    toast.error(
      t(
        "documentConverter.batchUpload.batchFailed",
...
function cancelBatchUpload(): void {
  ...
  dialog.confirm({
    title: t(
      "documentConverter.batchUpload.cancelUploadTitle",
      "Upload abbrechen",
    ),
    message: t(
      "documentConverter.batchUpload.cancelUploadConfirm",
      "Möchten Sie den laufenden Stapel-Upload wirklich abbrechen?",
    ),
    confirmButtonText: t("common.yes", "Ja"),
    cancelButtonText: t("common.no", "Nein"),
    onConfirm: () => {
      ...
      toast.info(
        t(
          "documentConverter.batchUpload.uploadCanceled",
          "Stapel-Upload wurde abgebrochen.",
        ),
      );
      setScreenReaderMessage(
        t(
...
function convertBatch(results: any[]): void {
  ...
  toast.info(
    t(
      "documentConverter.batchUpload.startingConversion",
      "Starte Konvertierung des Stapels...",
    ),
  );
}
...
function convertUploadedFile(file: File): void {
  ...
  toast.info(
    t(
      "documentConverter.batchUpload.startingFileConversion",
      `Starte Konvertierung von "${file.name}"...`,
    ),
  );
}
...
function handleSwipeLeft(fileId: string, index: number): void {
  ...
  if (
    ...
    setScreenReaderMessage(
      t(
        "documentConverter.batchUpload.swipeLeftForUpload",
...
function handleSwipeRight(fileId: string, index: number): void {
  ...
  setScreenReaderMessage(
    t(
      "documentConverter.batchUpload.swipeRightForDelete",
      "Nach rechts gewischt für Lösch-Option von Datei: " +
        batchFiles.value[index].file.name,
    ),
  );
...
function saveResumeState(): void {
  ...
  const incompleteUploads = batchFiles.value.filter(
    (file) =>
      file.status === "failed" ||
      (file.status === "canceled" && file.canResume),
  );
  ...
  try {
    ...
    if (resumeData) {
      ...
      if (validResumeData.length > 0) {
        toast.info(
          t(
            "documentConverter.batchUpload.resumeDataFound",
