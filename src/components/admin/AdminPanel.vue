<template>
  <div class="admin-panel" :class="[themeClass]" v-if="canAccessAdmin">
    <header class="admin-panel__header">
      <h1 class="admin-panel__title">{{ t('admin.title', 'nscale DMS Assistent Administration') }}</h1>
      <div class="admin-panel__user-info">
        <span class="admin-panel__user-email">{{ currentUser?.email }}</span>
        <span class="admin-panel__role" :class="`admin-panel__role--${currentUser?.role}`">
    ...
    <div class="admin-panel__main">
      ...
      <aside class="admin-panel__sidebar">
        <nav class="admin-panel__nav">
          ...
          >
            <i :class="['admin-panel__nav-icon', tab.icon]" aria-hidden="true"></i>
            <span class="admin-panel__nav-label">{{ tab.label }}</span>
          </button>
        </nav>
        
        <div class="admin-panel__sidebar-footer">
          <p class="admin-panel__version">{{ t('admin.version', 'Version') }}: {{ appVersion }}</p>
          <button 
            class="admin-panel__theme-toggle" 
            @click="toggleTheme"
            :aria-label="isDarkMode ? t('admin.lightMode', 'Zum hellen Modus wechseln') : t('admin.darkMode', 'Zum dunklen Modus wechseln')"
          >
            <i :class="['admin-panel__theme-icon', isDarkMode ? 'fas fa-sun' : 'fas fa-moon']" aria-hidden="true"></i>
            <span>{{ isDarkMode ? t('admin.lightMode', 'Hell') : t('admin.darkMode', 'Dunkel') }}</span>
          </button>
        </div>
      </aside>
  ...
  <div v-else class="admin-panel__no-access">
    ...
    <p>{{ t('admin.noAccess.message', 'Sie haben keine Berechtigung, auf den Admin-Bereich zuzugreifen.') }}</p>
...
const AdminDashboard = defineAsyncComponent(() => import('./tabs/AdminDashboard.vue'));
...
const availableTabs = computed(() => {
  return allTabs.filter(tab => {
    ...
    // Check feature flags
    if (tab.featureFlag && !featureTogglesStore.isFeatureEnabled(tab.featureFlag)) {
      return false;
    }
...
function setActiveTab(tabId: string) {
  if (availableTabs.value.some(tab => tab.id === tabId)) {
    ...
    if (router) {
      router.replace({ query: { ...router.currentRoute.value.query, tab: tabId } });
    }
  }
}
