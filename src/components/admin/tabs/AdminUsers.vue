<template>
  <div class="admin-users">
    <h2 class="admin-users__title">{{ t('admin.users.title', 'Benutzerverwaltung') }}</h2>
    <p class="admin-users__description">
      {{ t('admin.users.description', 'Verwalten Sie Benutzerkonten und Berechtigungen.') }}
    </p>
    ...
    <div class="admin-users__stats">
      <div class="admin-users__stat-card">
        ...
        <div class="admin-users__stat-content">
          <h3 class="admin-users__stat-label">{{ t('admin.users.totalUsers', 'Gesamtzahl der Benutzer') }}</h3>
      ...
      <div class="admin-users__stat-card">
        ...
        <div class="admin-users__stat-content">
          <h3 class="admin-users__stat-label">{{ t('admin.users.adminUsers', 'Administratoren') }}</h3>
      ...
      <div class="admin-users__stat-card">
        ...
        <div class="admin-users__stat-content">
          <h3 class="admin-users__stat-label">{{ t('admin.users.standardUsers', 'Standardbenutzer') }}</h3>
    ...
    <div class="admin-users__controls">
      ...
      <div class="admin-users__search">
        <div class="admin-users__search-input-container">
          <i class="fas fa-search admin-users__search-icon" aria-hidden="true"></i>
          <input
            v-model="searchQuery"
            type="text"
            class="admin-users__search-input"
            :placeholder="t('admin.users.search.placeholder', 'Benutzer suchen...')"
          />
          <button 
            v-if="searchQuery"
            @click="searchQuery = ''"
    ...
    <div class="admin-users__table-container">
      <table v-if="filteredUsers.length > 0" class="admin-users__table">
        <thead>
          <tr>
            <th @click="sortBy('email')" class="admin-users__table-header" :class="{ 'admin-users__table-header--sorted': sortField === 'email' }">
              {{ t('admin.users.table.email', 'E-Mail') }}
              <i v-if="sortField === 'email'" class="fas" :class="sortDirection === 'asc' ? 'fa-sort-up' : 'fa-sort-down'" aria-hidden="true"></i>
            </th>
            <th @click="sortBy('role')" class="admin-users__table-header" :class="{ 'admin-users__table-header--sorted': sortField === 'role' }">
              {{ t('admin.users.table.role', 'Rolle') }}
              <i v-if="sortField === 'role'" class="fas" :class="sortDirection === 'asc' ? 'fa-sort-up' : 'fa-sort-down'" aria-hidden="true"></i>
            </th>
            <th @click="sortBy('created_at')" class="admin-users__table-header" :class="{ 'admin-users__table-header--sorted': sortField === 'created_at' }">
              {{ t('admin.users.table.created', 'Erstellt am') }}
              <i v-if="sortField === 'created_at'" class="fas" :class="sortDirection === 'asc' ? 'fa-sort-up' : 'fa-sort-down'" aria-hidden="true"></i>
            </th>
            <th @click="sortBy('last_login')" class="admin-users__table-header" :class="{ 'admin-users__table-header--sorted': sortField === 'last_login' }">
              {{ t('admin.users.table.lastLogin', 'Letzter Login') }}
              <i v-if="sortField === 'last_login'" class="fas" :class="sortDirection === 'asc' ? 'fa-sort-up' : 'fa-sort-down'" aria-hidden="true"></i>
            </th>
            <th class="admin-users__table-header admin-users__table-header--actions">
        ...
        <tbody>
          <tr v-for="user in filteredUsers" :key="user.id" class="admin-users__table-row">
            ...
            <td class="admin-users__table-cell">{{ formatDate(user.created_at) }}</td>
            <td class="admin-users__table-cell">{{ user.last_login ? formatDate(user.last_login) : t('admin.users.table.never', 'Nie') }}</td>
            <td class="admin-users__table-cell admin-users__table-cell--actions">
              <div class="admin-users__actions">
                ...
                <button
                  ...
                  :disabled="isCurrentUser(user) || user.role === 'admin' || isLoading"
      ...
      <div v-else class="admin-users__empty-state">
        <i class="fas fa-users" aria-hidden="true"></i>
        <p v-if="searchQuery">
          {{ t('admin.users.noSearchResults', 'Keine Benutzer gefunden, die mit Ihrer Suche übereinstimmen.') }}
        </p>
        <p v-else>
    ...
    >
      <div class="admin-users__modal-content">
        <div class="admin-users__modal-header">
          <h3>{{ t('admin.users.modal.createTitle', 'Neuen Benutzer anlegen') }}</h3>
          <button
        ...
        <form @submit.prevent="createUser" class="admin-users__form">
          <div class="admin-users__form-group">
            ...
            <div class="admin-users__form-input-container">
              <input
                ...
                required
              />
              <span v-if="validationErrors.email" class="admin-users__form-error">
          ...
          <div class="admin-users__form-group">
            ...
            <div class="admin-users__form-input-container">
              ...
              <span v-if="validationErrors.password" class="admin-users__form-error">
          ...
          <div class="admin-users__form-group">
            ...
            <div class="admin-users__form-input-container">
              <select
                ...
                required
              >
                <option value="user">{{ t('admin.users.roles.user', 'Benutzer') }}</option>
                <option value="admin">{{ t('admin.users.roles.admin', 'Administrator') }}</option>
              </select>
            </div>
          </div>
    ...
    >
      <div class="admin-users__modal-content">
        <div class="admin-users__modal-header">
          <h3>{{ t('admin.users.modal.editRoleTitle', 'Benutzerrolle bearbeiten') }}</h3>
          <button
        ...
        <form v-if="selectedUser" @submit.prevent="updateUserRole" class="admin-users__form">
    ...
    >
      <div class="admin-users__modal-content">
        ...
        <div v-if="selectedUser" class="admin-users__confirm-delete">
          <i class="fas fa-exclamation-triangle admin-users__confirm-icon" aria-hidden="true"></i>
          
          <p class="admin-users__confirm-message">
            {{ t('admin.users.modal.deleteConfirmation', 'Sind Sie sicher, dass Sie den folgenden Benutzer löschen möchten?') }}
          </p>
          ...
          <p class="admin-users__delete-warning">
            {{ t('admin.users.modal.deleteWarning', 'Diese Aktion kann nicht rückgängig gemacht werden. Alle Daten des Benutzers werden gelöscht.') }}
          </p>
...
// Store state
const { users, loading: storeLoading, error: storeError } = storeToRefs(adminUsersStore);
...
function validateForm(): boolean {
  ...
  if (!newUser.value.email) {
    validationErrors.value.email = t('admin.users.validation.emailRequired', 'E-Mail-Adresse ist erforderlich');
    isValid = false;
  } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(newUser.value.email)) {
    validationErrors.value.email = t('admin.users.validation.emailInvalid', 'Ungültige E-Mail-Adresse');
    isValid = false;
  }
  
  // Validate password
  if (!newUser.value.password) {
    validationErrors.value.password = t('admin.users.validation.passwordRequired', 'Passwort ist erforderlich');
    isValid = false;
  } else if (newUser.value.password.length < 8) {
    validationErrors.value.password = t('admin.users.validation.passwordTooShort', 'Passwort muss mindestens 8 Zeichen lang sein');
    isValid = false;
  }
...
async function createUser() {
  ...
  try {
    ...
    showToast({
      ...
      message: t('admin.users.toast.createSuccessMessage', 'Der Benutzer wurde erfolgreich erstellt')
    });
    closeCreateUserModal();
  } catch (error: any) {
    const errorMessage = error?.response?.data?.message || t('admin.users.error.createFailed', 'Benutzer konnte nicht erstellt werden');
...
async function updateUserRole() {
  ...
  try {
    await adminUsersStore.updateUserRole(selectedUser.value.id, selectedRole.value);
    showToast({
      type: 'success',
      title: t('admin.users.toast.updateSuccess', 'Rolle aktualisiert'),
      message: t('admin.users.toast.updateSuccessMessage', 'Die Benutzerrolle wurde erfolgreich aktualisiert')
    });
    closeEditRoleModal();
  } catch (error: any) {
    const errorMessage = error?.response?.data?.message || t('admin.users.error.updateFailed', 'Rolle konnte nicht aktualisiert werden');
...
async function deleteUser() {
  ...
  try {
    ...
    showToast({
      ...
      message: t('admin.users.toast.deleteSuccessMessage', 'Der Benutzer wurde erfolgreich gelöscht')
    });
    closeConfirmDeleteModal();
  } catch (error: any) {
    const errorMessage = error?.response?.data?.message || t('admin.users.error.deleteFailed', 'Benutzer konnte nicht gelöscht werden');
...
onMounted(async () => {
  ...
  try {
    ...
    showToast({
      ...
      message: t('admin.users.error.loadFailed', 'Benutzer konnten nicht geladen werden')
    });
  } finally {
