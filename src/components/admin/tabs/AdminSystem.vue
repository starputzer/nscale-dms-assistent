<template>
  <div class="admin-system">
    <h2 class="admin-system__title">{{ t('admin.system.title', 'Systemeinstellungen') }}</h2>
    <p class="admin-system__description">
      {{ t('admin.system.description', 'Überwachen und konfigurieren Sie Systemeinstellungen und -ressourcen.') }}
    </p>
    ...
    <div class="admin-system__status-card" :class="systemHealthClass">
      ...
      <div class="admin-system__status-content">
        <h3 class="admin-system__status-title">{{ t('admin.system.healthStatus', 'Systemstatus') }}</h3>
        <p class="admin-system__status-value">{{ systemHealthText }}</p>
        <p class="admin-system__status-details" v-if="systemHealthDetails">{{ systemHealthDetails }}</p>
      </div>
    </div>

    <!-- System Resource Metrics -->
    <div class="admin-system__metrics">
      <h3 class="admin-system__section-title">{{ t('admin.system.resourceMetrics', 'Ressourcennutzung') }}</h3>
      ...
      <div class="admin-system__metrics-grid">
        ...
        <div class="admin-system__metric-card">
          ...
          <div class="admin-system__meter-container">
            ...
            <div class="admin-system__meter-value">{{ stats.cpu_usage_percent || 0 }}%</div>
          </div>
        </div>
        ...
        <div class="admin-system__metric-card">
          ...
          <div class="admin-system__meter-container">
            ...
            <div class="admin-system__meter-value">{{ stats.memory_usage_percent || 0 }}%</div>
          </div>
        </div>
    ...
    <div class="admin-system__info">
      <h3 class="admin-system__section-title">{{ t('admin.system.information', 'Systeminformationen') }}</h3>
      ...
      <div class="admin-system__info-grid">
        <div class="admin-system__info-card">
          ...
          <div class="admin-system__info-value">{{ stats.uptime_days || 0 }} {{ t('admin.system.days', 'Tage') }}</div>
          <div class="admin-system__info-details">
            {{ t('admin.system.info.startTime', 'Start:') }} {{ formatDate(stats.start_time) }}
          </div>
        </div>
        ...
        <div class="admin-system__info-card">
          <div class="admin-system__info-title">
            ...
            <span>{{ t('admin.system.info.activeModel', 'Aktives Modell') }}</span>
          </div>
          <div class="admin-system__info-value">{{ stats.active_model || '-' }}</div>
          <div class="admin-system__info-details">
            {{ t('admin.system.info.avgResponseTime', 'Antwortzeit:') }} {{ stats.avg_response_time_ms || 0 }} ms
          </div>
        </div>
        ...
        <div class="admin-system__info-card">
          ...
          <div class="admin-system__info-value">{{ stats.document_count || 0 }}</div>
        </div>

        <div class="admin-system__info-card">
          <div class="admin-system__info-title">
            <i class="fas fa-comments" aria-hidden="true"></i>
            <span>{{ t('admin.system.info.interactions', 'Interaktionen') }}</span>
          </div>
          <div class="admin-system__info-values">
            ...
            <div class="admin-system__info-value-item">
              <span>{{ t('admin.system.info.messagesPerSession', 'Ø pro Sitzung:') }}</span>
    ...
    <div class="admin-system__settings">
      <div class="admin-system__settings-header">
        <h3 class="admin-system__section-title">{{ t('admin.system.settings.title', 'Systemeinstellungen') }}</h3>
        <button 
          @click="toggleEditMode"
          class="admin-system__toggle-edit-button"
          :class="{ 'admin-system__toggle-edit-button--active': isEditMode }"
        >
          <i class="fas" :class="isEditMode ? 'fa-times' : 'fa-edit'" aria-hidden="true"></i>
          <span>{{ isEditMode ? t('admin.system.settings.cancelEdit', 'Abbrechen') : t('admin.system.settings.edit', 'Bearbeiten') }}</span>
        </button>
      </div>
      ...
      <form @submit.prevent="saveSettings" class="admin-system__settings-form">
        <div class="admin-system__settings-grid">
          ...
          <fieldset class="admin-system__settings-group">
            <legend>{{ t('admin.system.settings.models', 'Modelleinstellungen') }}</legend>
            ...
            <div class="admin-system__setting-item">
              ...
              <select 
                ...
                :disabled="!isEditMode"
              >
                <option v-for="model in systemSettings.availableModels" :key="model" :value="model">
                  {{ model }}
                </option>
              </select>
            </div>
            ...
            <div class="admin-system__setting-item">
              <label for="max-tokens" class="admin-system__setting-label">
                {{ t('admin.system.settings.maxTokens', 'Max. Tokens pro Anfrage') }}
              </label>
              <input
                id="max-tokens"
          ...
          <fieldset class="admin-system__settings-group">
            <legend>{{ t('admin.system.settings.rateLimiting', 'Rate-Limiting') }}</legend>
            
            <div class="admin-system__setting-item admin-system__setting-item--switch">
              <div class="admin-system__setting-switch-label">
                <label for="enable-rate-limit">
                  {{ t('admin.system.settings.enableRateLimit', 'Rate-Limiting aktivieren') }}
                </label>
              </div>
            ...
            <div class="admin-system__setting-item" v-if="systemSettings.enableRateLimit">
              <label for="rate-limit" class="admin-system__setting-label">
                {{ t('admin.system.settings.rateLimitPerMinute', 'Anfragen pro Minute') }}
              </label>
              <input
                id="rate-limit"
          ...
          <!-- Session Settings -->
          <fieldset class="admin-system__settings-group">
            <legend>{{ t('admin.system.settings.sessions', 'Sitzungen') }}</legend>

            <div class="admin-system__setting-item">
              <label for="max-sessions" class="admin-system__setting-label">
                {{ t('admin.system.settings.maxSessionsPerUser', 'Max. Sitzungen pro Benutzer') }}
              </label>
              <input
                id="max-sessions"
            ...
            <div class="admin-system__setting-item">
              <label for="session-timeout" class="admin-system__setting-label">
                {{ t('admin.system.settings.sessionTimeout', 'Sitzungs-Timeout (Minuten)') }}
              </label>
              <input
                id="session-timeout"
          ...
          <!-- Feedback Settings -->
          <fieldset class="admin-system__settings-group">
            <legend>{{ t('admin.system.settings.feedback', 'Feedback') }}</legend>
            
            <div class="admin-system__setting-item admin-system__setting-item--switch">
              <div class="admin-system__setting-switch-label">
                <label for="enable-feedback">
                  {{ t('admin.system.settings.enableFeedback', 'Feedback aktivieren') }}
                </label>
              </div>
              <label class="admin-system__toggle-switch">
                <input
          ...
          <fieldset class="admin-system__settings-group">
            <legend>{{ t('admin.system.settings.maintenance', 'Wartung') }}</legend>
            ...
            <div class="admin-system__setting-item" v-if="systemSettings.maintenanceMode">
              <label for="maintenance-message" class="admin-system__setting-label">
                {{ t('admin.system.settings.maintenanceMessage', 'Wartungsmeldung') }}
              </label>
              <textarea
    ...
    <div class="admin-system__actions">
      <h3 class="admin-system__section-title">{{ t('admin.system.actions.title', 'Systemaktionen') }}</h3>
      ...
      <div class="admin-system__actions-grid">
        ...
        >
          <div class="admin-system__action-content">
            ...
            <p class="admin-system__action-description">{{ action.description }}</p>
          </div>
          <button
          ...
          >
            <i v-if="isActionPending && pendingAction === action.type" class="fas fa-spinner fa-spin" aria-hidden="true"></i>
    ...
    >
      <div class="admin-system__dialog-content">
        ...
        <div class="admin-system__dialog-body">
          <i class="fas fa-exclamation-triangle admin-system__dialog-icon" aria-hidden="true"></i>
        ...
        <div class="admin-system__dialog-actions">
          ...
          <button
            ...
            :disabled="isActionPending"
          >
            <i v-if="isActionPending" class="fas fa-spinner fa-spin" aria-hidden="true"></i>
...
// Store state
const { stats, availableActions, memoryStatus, cpuStatus, systemHealthStatus } = storeToRefs(adminSystemStore);
...
const originalSettings = ref({
  ...
  maintenanceMessage: 'System wird gewartet und ist vorübergehend nicht verfügbar. Bitte versuchen Sie es später erneut.'
});
...
const systemHealthDetails = computed(() => {
  if (systemHealthStatus.value === 'critical') {
    if (memoryStatus.value === 'critical') {
      return t('admin.system.healthDetailMemoryCritical', 'Arbeitsspeicherauslastung kritisch hoch');
    }
    if (cpuStatus.value === 'critical') {
      return t('admin.system.healthDetailCpuCritical', 'CPU-Auslastung kritisch hoch');
    }
  }
  
  if (systemHealthStatus.value === 'warning') {
    if (memoryStatus.value === 'warning') {
      return t('admin.system.healthDetailMemoryWarning', 'Arbeitsspeicherauslastung erhöht');
    }
...
async function saveSettings() {
  ...
  setTimeout(() => {
    ...
    showToast({
      ...
      message: t('admin.system.toast.settingsSavedMessage', 'Die Systemeinstellungen wurden erfolgreich aktualisiert')
    });
...
async function runAction(action: SystemAction) {
  ...
  try {
    ...
    showToast({
      ...
      message: t('admin.system.toast.actionSuccessMessage', `Die Aktion "${action.name}" wurde erfolgreich ausgeführt`)
    });
  } catch (error: any) {
    const errorMessage = error?.message || t('admin.system.error.actionFailed', 'Die Aktion konnte nicht ausgeführt werden');
