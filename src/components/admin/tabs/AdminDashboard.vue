<template>
  <div class="admin-dashboard">
    <h2 class="admin-dashboard__title">{{ t('admin.dashboard.title', 'Dashboard') }}</h2>
    ...
    <div class="admin-dashboard__status-card" :class="systemStatusClass">
      ...
      <div class="admin-dashboard__status-content">
        <h3 class="admin-dashboard__status-title">{{ t('admin.dashboard.systemStatus', 'Systemstatus') }}</h3>
    ...
    <div class="admin-dashboard__actions">
      <h3 class="admin-dashboard__section-title">{{ t('admin.dashboard.quickActions', 'Schnellaktionen') }}</h3>
    ...
    <div class="admin-dashboard__activity">
      ...
      <div v-if="recentActivity.length > 0" class="admin-dashboard__activity-list">
        <div 
        ...
        >
          <div class="admin-dashboard__activity-icon">
            <i :class="['fas', getActivityIcon(activity.type)]" aria-hidden="true"></i>
          </div>
          <div class="admin-dashboard__activity-content">
            <p class="admin-dashboard__activity-text">
              <span class="admin-dashboard__activity-user">{{ activity.user }}</span>
              {{ activity.text }}
            </p>
            <p class="admin-dashboard__activity-time">{{ formatTime(activity.timestamp) }}</p>
          </div>
        </div>
      </div>
      <p v-else class="admin-dashboard__no-activity">
        {{ t('admin.dashboard.noRecentActivity', 'Keine aktuellen Aktivitäten vorhanden') }}
      </p>
    </div>
  </div>
</template>
...
const quickActions = computed(() => {
  return [
    ...
    {
      id: 'exportStats',
      label: t('admin.dashboard.actions.exportStats', 'Statistiken exportieren'),
      icon: 'fa-file-export'
    },
    {
      id: 'systemCheck',
...
function getActivityIcon(type: string): string {
  switch (type) {
    ...
    case 'cache': return 'fa-trash-alt';
