...
export interface DiagnosticsAPI {
  ...
  registerComponent(
    name: string,
    initialStatus?: Partial<BridgeComponentStatus>,
  ): void;

  /** Update component status */
  updateComponentStatus(
    name: string,
    status: Partial<BridgeComponentStatus>,
  ): void;
...
export function initDiagnostics(
  ...
  function setupPerformanceMonitoring() {
    ...
    const bridgeEventCompleteSub = eventBus.on(
      "bridge:eventComplete",
      (payload) => {
    ...
    const stateUpdateCompleteSub = eventBus.on(
      "bridge:stateUpdateComplete",
      (payload) => {
  ...
  function setupHealthCheck() {
    ...
    state.intervals.healthCheck = window.setInterval(() => {
      ...
      updateComponentStatus("diagnostics", {
        ...
        metrics: {
          memoryUsage:
            state.metrics.memoryUsage[state.metrics.memoryUsage.length - 1],
          eventProcessingTime: calculateAverage(
            state.metrics.eventProcessingTimes,
          ),
    ...
    getMetrics() {
      return {
        eventProcessing: {
          ...
          max:
            state.metrics.eventProcessingTimes.length > 0
        ...
        stateUpdate: {
          ...
          max:
            state.metrics.stateUpdateTimes.length > 0
        ...
        memory: {
          ...
          trend:
            state.metrics.memoryUsage.length > 10
              ? state.metrics.memoryUsage[
                  state.metrics.memoryUsage.length - 1
  ...
  function updateComponentStatus(
    ...
    if (currentStatus.status !== newStatus.status) {
      if (newStatus.status === "error") {
        logger.error(
          `Component ${name} status changed to error`,
          newStatus.error,
        );
