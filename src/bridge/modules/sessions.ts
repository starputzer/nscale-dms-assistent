...
export interface SessionsBridgeAPI {
  ...
  createSession(sessionData?: Partial<ChatSession>): Promise<BridgeResult<ChatSession>>;
  
  /** Update a session */
  updateSession(sessionId: string, sessionData: Partial<ChatSession>): Promise<BridgeResult<ChatSession>>;
  ...
  /** Delete a message */
  deleteMessage(sessionId: string, messageId: string): Promise<BridgeResult<void>>;
  
  /** Check if a session has streaming message */
  isSessionStreaming(sessionId: string): boolean;
  
  /** Get streaming status information */
  getStreamingStatus(): { isActive: boolean; progress: number; sessionId: string | null };
...
export function initSessionsBridge(
  ...
  function setupLegacySynchronization() {
    ...
    const sessionsUnwatch = sessionsStore.$subscribe((mutation, store) => {
      if (mutation.type === 'direct' || mutation.type === 'patch function') {
        ...
        if (mutation.payload && 'streaming' in mutation.payload) {
          eventBus.emit('message:streamingStatusUpdated', sessionsStore.streaming);
        }
      }
    });
    
    // Listen for legacy session events
    const sessionCreatedSub = eventBus.on('vanillaChat:sessionCreated', async (payload) => {
      if (payload && payload.session) {
        logger.debug('Received legacy session created event', { sessionId: payload.session.id });
        
        // Check if session already exists
        const existingSession = sessionsStore.sessions.find(s => s.id === payload.session.id);
        if (!existingSession) {
    ...
    const sessionUpdatedSub = eventBus.on('vanillaChat:sessionUpdated', async (payload) => {
      if (payload && payload.session) {
        logger.debug('Received legacy session updated event', { sessionId: payload.session.id });
        
        // Update session in store
        await sessionsStore.updateSession(payload.session.id, payload.session);
      }
    });
    
    const sessionDeletedSub = eventBus.on('vanillaChat:sessionDeleted', async (payload) => {
      if (payload && payload.sessionId) {
        logger.debug('Received legacy session deleted event', { sessionId: payload.sessionId });
    ...
    const sessionSelectedSub = eventBus.on('vanillaChat:sessionSelected', async (payload) => {
      if (payload && payload.sessionId) {
        logger.debug('Received legacy session selected event', { sessionId: payload.sessionId });
    ...
    const messageReceivedSub = eventBus.on('vanillaChat:messageReceived', async (payload) => {
      if (payload && payload.message && payload.sessionId) {
        ...
        const session = sessionsStore.sessions.find(s => s.id === payload.sessionId);
        if (!session) {
          logger.warn('Received message for non-existent session', { sessionId: payload.sessionId });
          return;
        }
        
        // Add message to store
        const sessionMessages = sessionsStore.messages[payload.sessionId] || [];
        const existingMessage = sessionMessages.find(m => m.id === payload.message.id);
        
        if (!existingMessage) {
          // If streaming is enabled and the message is from the assistant, it may be streaming
          if (mergedConfig.enableStreaming && payload.message.role === 'assistant') {
    ...
    const messageUpdatedSub = eventBus.on('vanillaChat:messageUpdated', async (payload) => {
      if (payload && payload.message && payload.sessionId) {
        ...
        if (
          ...
          (!payload.message.isStreaming || payload.message.isStreaming === false)
        ) {
    ...
    const messageDeletedSub = eventBus.on('vanillaChat:messageDeleted', async (payload) => {
      if (payload && payload.messageId && payload.sessionId) {
        ...
        await sessionsStore.deleteMessage(payload.sessionId, payload.messageId);
    ...
    const streamingProgressSub = eventBus.on('vanillaChat:streamingProgress', (payload) => {
      if (payload && typeof payload.progress === 'number') {
        logger.debug('Received streaming progress event', { progress: payload.progress });
  ...
  function synchronizeState() {
    ...
    state.dirtyMessageIds.forEach((messageIds, sessionId) => {
      ...
      const messagesToSync = sessionMessages.filter(m => messageIds.has(m.id));
  ...
  const api: SessionsBridgeAPI = {
    ...
    async updateSession(sessionId: string, sessionData: Partial<ChatSession>) {
      try {
        ...
        const updatedSession = await sessionsStore.updateSession(sessionId, sessionData);
    ...
    async sendMessage(params: SendMessageParams) {
      try {
        ...
        if (message) {
          ...
          logger.warn('Failed to send message', { sessionId: params.sessionId });
    ...
    isSessionStreaming(sessionId: string) {
      return sessionsStore.streaming.isActive && 
