...
export interface AuthBridgeAPI {
  ...
  login(credentials: { username: string, password: string }): Promise<BridgeResult<any>>;
...
export function initAuthBridge(
  ...
  try {
    ...
    if (!authStore || typeof authStore.login !== 'function' || typeof authStore.logout !== 'function') {
      logger.error('Auth store improperly initialized or missing required methods');
  ...
  function setupLegacySynchronization() {
        ...
        if (mutation.payload && (
          ...
          try {
            if (authStore.isAuthenticated) {
              ...
              const userToEmit = authStore.user || { id: '0', email: 'unknown@example.com', username: 'unknown', roles: ['user'] };
            ...
            logger.error('Error emitting auth events during store subscription', err);
          }
        }
      }
    });
    ...
    const loginSub = eventBus.on('vanillaAuth:login', (payload) => {
      ...
      if (payload && payload.user && payload.token) {
        logger.debug('Received legacy login event', { userId: payload.user.id });
