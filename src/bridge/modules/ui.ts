  ...
  function setupLegacySynchronization() {
    ...
    const uiUnwatch = uiStore.$subscribe((mutation, store) => {
      if (mutation.type === "direct" || mutation.type === "patch function") {
        ...
        if (
          mergedConfig.syncTheme &&
          mutation.payload &&
          "darkMode" in mutation.payload
        ) {
        ...
        if (mutation.payload && "sidebar" in mutation.payload) {
          eventBus.emit("ui:sidebarToggled", {
        ...
        // If toasts change
        if (
          mergedConfig.syncToasts &&
          mutation.payload &&
          "toasts" in mutation.payload
        ) {
          // Only emit for toasts that weren't created through the bridge to avoid loops
          const newToasts = uiStore.toasts.filter(
            (toast) => !state.bridgeCreatedToasts.has(toast.id),
          );
        ...
        // If modals change
        if (
          mergedConfig.syncModals &&
          mutation.payload &&
          "activeModals" in mutation.payload
        ) {
          // Only emit for modals that weren't created through the bridge to avoid loops
          const newModals = uiStore.activeModals.filter(
            (modal) => !state.bridgeCreatedModals.has(modal.id),
          );
    ...
    const themeChangedSub = eventBus.on("vanillaUI:themeChanged", (payload) => {
      if (payload && typeof payload.isDarkMode === "boolean") {
        logger.debug("Received legacy theme changed event", {
    ...
    const sidebarToggledSub = eventBus.on(
      "vanillaUI:sidebarToggled",
      (payload) => {
        if (payload && typeof payload.isOpen === "boolean") {
          logger.debug("Received legacy sidebar toggled event", {
    ...
    const viewChangedSub = eventBus.on("vanillaUI:viewChanged", (payload) => {
      if (payload && payload.view) {
        logger.debug("Received legacy view changed event", {
          view: payload.view,
        });
    ...
    const dismissToastSub = eventBus.on("vanillaUI:dismissToast", (payload) => {
      if (payload && payload.toastId) {
        logger.debug("Received legacy dismiss toast event", {
    ...
    const closeModalSub = eventBus.on("vanillaUI:closeModal", (payload) => {
      if (payload && payload.modalId) {
        logger.debug("Received legacy close modal event", {
