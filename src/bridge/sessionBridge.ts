...
export function initSessionBridge() {
  ...
  if (!featureTogglesStore.useSfcSessionManagement) {
    console.info('SFC Session-Management ist deaktiviert, Bridge wird nicht initialisiert');
    return;
  }
  ...
  const eventHandlers: Record<string, ((event: CustomEvent<LegacySessionEvent>) => void)[]> = {};
  
  /**
   * Registriert einen Event-Handler
   */
  function addEventHandler(type: string, handler: (event: CustomEvent<LegacySessionEvent>) => void) {
   ...
   */
  function removeEventHandler(type: string, handler: (event: CustomEvent<LegacySessionEvent>) => void) {
    if (eventHandlers[type]) {
  ...
  function dispatchEvent(type: string, detail: LegacySessionEvent) {
    const event = new CustomEvent<LegacySessionEvent>(`nscale:session:${type}`, { detail });
    window.dispatchEvent(event);
    
    // Lokale Handler aufrufen
    if (eventHandlers[type]) {
      eventHandlers[type].forEach(handler => handler(new CustomEvent<LegacySessionEvent>('', { detail })));
    }
  }
  ...
  const legacyAPI = {
    ...
    createSession: async (title?: string): Promise<LegacySessionData> => {
      ...
      if (!session) {
        throw new Error('Session wurde erstellt, konnte aber nicht gefunden werden');
      }
    ...
    on: (type: string, callback: (event: LegacySessionEvent) => void): void => {
      const handler = (event: CustomEvent<LegacySessionEvent>) => callback(event.detail);
      addEventHandler(type, handler);
      window.addEventListener(`nscale:session:${type}`, handler as EventListener);
    },
    
    /**
     * Entfernt Event-Listener für Session-Events
     */
    off: (type: string, callback: (event: LegacySessionEvent) => void): void => {
      const handlers = eventHandlers[type];
      if (handlers) {
        const handler = handlers.find(h => h.toString() === callback.toString());
        if (handler) {
          removeEventHandler(type, handler);
          window.removeEventListener(`nscale:session:${type}`, handler as EventListener);
        }
      }
    }
  };
