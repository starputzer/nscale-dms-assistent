...
export class BridgeStatusManager {
  ...
  updateStatus(newStatus: Partial<BridgeStatusInfo>): void {
    ...
    if (previousState !== this.currentStatus.state) {
      if (this.currentStatus.state === BridgeErrorState.HEALTHY) {
        this.logger.info(`Bridge-Status: HEALTHY - ${this.currentStatus.message}`);
      } else if (this.currentStatus.state === BridgeErrorState.DEGRADED_PERFORMANCE) {
        this.logger.warn(`Bridge-Status: DEGRADED - ${this.currentStatus.message}`, {
          components: this.currentStatus.affectedComponents
        });
      } else {
        this.logger.error(`Bridge-Status: ${BridgeErrorState[this.currentStatus.state]} - ${this.currentStatus.message}`, {
          components: this.currentStatus.affectedComponents,
  ...
  private showUserNotification(): void {
    ...
    if (window.nscaleUI?.showToast) {
      ...
      console.error(`[Bridge] KRITISCHER FEHLER: ${this.currentStatus.message}`);
