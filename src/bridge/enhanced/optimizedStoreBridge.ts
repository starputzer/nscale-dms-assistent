...
import { offlineQueueService, OperationType, OperationPriority } from '@/services/api/OfflineQueueService';
...
class OptimizedEventBus {
  ...
  on(event: string, callback: (data?: any) => void): () => void {
    ...
    window.addEventListener(`nscale:${event}`, wrappedCallback as EventListener);
  ...
  off(event: string, callback: (data?: any) => void): void {
    ...
    if (wrappedCallback) {
      window.removeEventListener(`nscale:${event}`, wrappedCallback as EventListener);
...
class DiagnosticsManager {
  ...
  toggleMonitoring(enable: boolean): void {
    if (enable && !this.isMonitoring) {
      ...
      this.updateMetrics();
      this.metricsInterval = window.setInterval(this.updateMetrics, this.updateInterval);
...
export function setupOptimizedStoreBridge(): OptimizedBridgeAPI {
  ...
  const startCacheUpdates = () => {
    if (cacheUpdateTimer === null) {
      cacheUpdateTimer = window.setInterval(updateStateCache, updateCacheInterval);
    }
  };
  ...
  const optimizedSessionsBridge: OptimizedSessionsBridgeAPI = {
    async createSession(title?: string): Promise<string> {
      ...
      if (!navigator.onLine) {
        offlineQueueService.addToQueue(
          ...
          { title: title || 'Neue Unterhaltung', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
          sessionId,
    ...
    isMessageStreaming(messageId: string): boolean {
      ...
      const message = sessionsStore.currentMessages.find(m => m.id === messageId);
  ...
  const unwatchAuth = watch(() => authStore.isAuthenticated, (isAuthenticated) => {
    ...
    optimizedBus.batchEmit([
      ...
      { event: isAuthenticated ? 'auth:login' : 'auth:logout', data: { user: authStore.user } }
    ]);
  ...
  const unwatchSessions = watch(() => sessionsStore.sessions.length, () => {
 ...
 */
export function initializeOptimizedStoreBridge(): { bridge: OptimizedBridgeAPI, cleanup: () => void } {
  // Bridge einrichten
  ...
  window.dispatchEvent(new CustomEvent('nscale:optimized-bridge:ready', {
