  ...
  public async executeWithRetry<T>(
    ...
    while (attempt <= opts.maxRetries) {
      ...
      try {
        ...
        logger.debug(`${operationName} succeeded after ${attempt} attempt(s) in ${elapsedMs}ms`);
        ...
        if (shouldRetry) {
          ...
          logger.debug(`${operationName} failed (${isTimeout ? 'timeout' : 'error'}), retrying in ${delay}ms (attempt ${attempt}/${opts.maxRetries})`);
        ...
        logger.warn(`${operationName} failed after ${attempt} attempt(s) in ${totalTimeMs}ms`);
    ...
    return {
      ...
      attempts: attempt,
      totalTimeMs,
      status: finalError instanceof Error && finalError.message.includes('timeout') 
...
export const bridgeTimeoutRetry = new TimeoutRetry({
  ...
  onRetry: (attempt, delay, error) => {
    logger.debug(`Bridge operation retry ${attempt}, waiting ${delay}ms. Error: ${error?.message || 'Unknown error'}`);
  }
});
