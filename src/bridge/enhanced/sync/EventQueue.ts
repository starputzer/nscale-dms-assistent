...
export type EventHandler<T = any> = (event: QueuedEvent<T>) => Promise<void> | void;
...
export class EventQueue {
  ...
  public enqueue<T>(
    ...
    if (this.queue.length >= this.config.maxQueueSize) {
      logger.warn(`Event queue is full (${this.queue.length} items). Dropping event: ${name}`);
    ...
    if (this.config.enableDependencies && options.dependsOn && options.dependsOn.length > 0) {
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < this.queue.length && eventsToProcess.length < batchSize; i++) {
        ...
        if (this.config.enableDependencies && event.dependsOn && event.dependsOn.length > 0) {
  ...
  private async processEvent(event: QueuedEvent): Promise<void> {
    ...
    if (this.config.logEventProcessing) {
      logger.debug(`Processing event ${event.name} [${event.id}] (priority: ${event.priority})`);
    }
    ...
    try {
      ...
      const timeoutPromise = this.config.dropAfterTimeout
        ? new Promise<void>((_, reject) => {
            setTimeout(() => {
              reject(new Error(`Event processing timed out after ${this.config.timeoutMs}ms`));
            }, this.config.timeoutMs);
          })
        : null;
      
      // Handler für das Event suchen
      const handlers = (this.handlers.get(event.name) || []).concat(this.globalHandlers);
      ...
      this.metrics.avgProcessingTime = (
        this.metrics.avgProcessingTime * (this.metrics.totalProcessed - 1) + event.processingTime
      ) / this.metrics.totalProcessed;
      ...
      if (this.config.enableDependencies && this.dependencies.has(event.id)) {
        ...
        if (dependentEvents.size > 0) {
          logger.debug(`Event ${event.id} completed. Notifying ${dependentEvents.size} dependent events.`);
        }
        this.dependencies.delete(event.id);
      }
      
      if (this.config.logEventProcessing) {
        logger.debug(`Event ${event.name} [${event.id}] processed in ${event.processingTime.toFixed(2)}ms`);
      }
    } catch (error) {
      ...
      logger.error(`Error processing event ${event.name} [${event.id}]:`, error);
      
      // Wiederholung planen, falls konfiguriert
      if (this.config.retryFailed && (event.retryCount ?? 0) < (event.maxRetries ?? this.config.maxRetries)) {
        ...
        logger.debug(`Retrying event ${event.name} [${event.id}] in ${delay}ms (attempt ${event.retryCount}/${event.maxRetries})`);
  ...
  public async shutdown(processRemaining: boolean = true): Promise<void> {
    ...
    if (processRemaining && this.queue.length > 0) {
      logger.debug(`Processing ${this.queue.length} remaining events before shutdown`);
