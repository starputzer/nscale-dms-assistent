...
export class TransactionManager {
  ...
  private changeListeners: Set<
    (transactionId: string, status: TransactionStatus) => void
  > = new Set();
  ...
  public beginTransaction(options?: TransactionOptions): string {
    ...
    if (mergedOptions.parentTransactionId) {
      if (!this.childTransactions.has(mergedOptions.parentTransactionId)) {
        this.childTransactions.set(
          mergedOptions.parentTransactionId,
          new Set(),
        );
      }
      this.childTransactions
        .get(mergedOptions.parentTransactionId)!
        .add(transactionId);
    }
  ...
  public captureSnapshot(
    ...
    if (!transaction || transaction.status !== TransactionStatus.PENDING) {
      logger.warn(
        `Cannot capture snapshot: Invalid transaction ${transactionId}`,
      );
      return;
    }
   ...
   */
  public isPathInActiveTransaction(
    path: string[],
  ...
  public commitTransaction(transactionId: string): void {
    ...
    if (transaction.status !== TransactionStatus.PENDING) {
      logger.warn(
        `Cannot commit: Transaction ${transactionId} is not pending (${transaction.status})`,
      );
      return;
    }
    ...
    if (!transaction.changesPending) {
      logger.debug(
        `Skipping commit for empty transaction: ${transaction.name} [${transactionId}]`,
      );
    ...
    try {
      ...
      logger.debug(
        `Transaction committed: ${transaction.name} [${transactionId}] with ${transaction.snapshots.length} changes`,
      );
      ...
      logger.error(
        `Error committing transaction ${transaction.name} [${transactionId}]:`,
        error,
      );
  ...
  public rollbackTransaction(transactionId: string): Record<string, any> {
    ...
    if (
      ...
      logger.warn(
        `Cannot rollback: Transaction ${transactionId} is not pending or failed (${transaction.status})`,
      );
      return {};
    }
    ...
    try {
      ...
      logger.debug(
        `Transaction rolled back: ${transaction.name} [${transactionId}] with ${Object.keys(rollbackValues).length} restored values`,
      );
      ...
      logger.error(
        `Error rolling back transaction ${transaction.name} [${transactionId}]:`,
        error,
      );
  ...
  public addChangeListener(
    listener: (transactionId: string, status: TransactionStatus) => void,
  ): () => void {
  ...
  public getTransactionInfo(
    transactionId: string,
  ): (Omit<Transaction, "snapshots"> & { snapshotCount: number }) | null {
    const transaction = this.transactions.get(transactionId);
    if (!transaction) {
      return null;
    }
