...
class SelectiveUpdateManager {
  ...
  private processPendingUpdates(): void {
    ...
    if (optimizedPaths.length > 0) {
      this.logger.debug(`Verarbeite selektive Updates für ${optimizedPaths.length} Pfade`, optimizedPaths);
      // Updates durchführen
...
export class EnhancedStateManager implements StateManager {
  ...
  connect(vueStores: Record<string, any>, legacyState: Record<string, any>): void {
    ...
    try {
      ...
      this.logger.error('Fehler beim Einrichten des Legacy-State-Watchings', error);
    ...
    const watchObject = (obj: any, path: string): void => {
      ...
      for (const key in obj) {
        ...
        try {
          ...
          this.logger.warn(`Konnte keine Beobachtung für ${currentPath} einrichten`, error);
        }
      }
    };
   ...
   */
  private handleStateChange(path: string, value: any, source: 'vue' | 'legacy'): void {
  ...
  private updateLegacyState(path: string, value: any): void {
    try {
      ...
      this.logger.error('Fehler beim Aktualisieren des Legacy-State', { path, error });
      this.healthStatus = false;
    }
  }
  ...
  private updateVueStore(path: string, value: any): void {
    try {
      ...
      if (pathParts.length === 1) {
        ...
        this.logger.warn('Direktes Update des gesamten Stores nicht unterstützt');
        return;
      }
      ...
      if (this.vueStores[storeKey].$state) {
        ...
        let path = pathParts.slice(1); // Store-Namen entfernen
      ...
      this.logger.error('Fehler beim Aktualisieren des Vue-Store', { path, error });
      this.healthStatus = false;
    }
  }
  ...
  setState(path: string, value: any, source: 'legacy' | 'vue' = 'vue'): boolean {
    try {
      this.atomicState.beginTransaction({ path, oldValue: this.getState(path) });
      ...
      this.logger.error('Fehler beim Setzen des Zustands', { path, value, error });
