 ...
 */
export async function initializeOptimizedBridge(
  config: Partial<OptimizedBridgeConfig> = {},
): Promise<OptimizedBridgeAPI> {
  ...
  if (isInitialized && components) {
    logger.warn(
      "Bridge-System bereits initialisiert, bestehende Instanz wird zurückgegeben",
    );
  ...
  try {
    ...
    const selfHealing = mergedConfig.enableSelfHealing
      ? new EnhancedSelfHealing(statusManager, performanceMonitor, {
          enabled: true,
          autoRecovery: true,
    ...
    const stateManager = mergedConfig.enableSelectiveSync
      ? new SelectiveStateManager(statusManager, performanceMonitor, {
          enabled: true,
          enableSelectiveSync: true,
          debounceMs: 50,
          throttleMs: 100,
    ...
    const diagnostics = mergedConfig.enableDiagnostics
      ? new ChatBridgeDiagnostics(performanceMonitor!, statusManager, {
          enabled: true,
    ...
    if (selfHealing) {
      selfHealing.registerComponents(components);
    }
    ...
    logger.error(
      "Fehler bei der Initialisierung des optimierten Bridge-Systems",
      error,
    );
    throw error;
  }
...
function createBridgeAPI(
  components: OptimizedBridgeComponents,
): OptimizedBridgeAPI {
  const {
    eventBus,
    batchedEmitter,
  ...
  return {
    ...
    on: (event, handler, component = "unknown") => {
      if (eventListenerManager) {
        const id = eventListenerManager.registerListener(
          ...
          handler,
          component,
          event.startsWith("vue")
            ? "vue"
            : event.startsWith("vanilla")
              ? "vanilla"
              : "system",
        );
    ...
    getStatus: () => {
      if (diagnostics) {
        return (
          (window as any).chatBridgeDiagnostics?.getStatus() || {
            isInitialized,
          }
        );
      }
