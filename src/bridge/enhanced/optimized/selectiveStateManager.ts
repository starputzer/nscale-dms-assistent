...
const DEFAULT_CONFIG: SelectiveStateManagerConfig = {
  ...
  alwaysSyncPaths: ["auth.user", "auth.token", "ui.theme", "features.toggles"],
...
export class SelectiveStateManager implements StateManager {
  ...
  constructor(
    ...
    this.alwaysSyncMatchers =
      this.config.alwaysSyncPaths.map(createPathMatcher);

    this.logger.info(
      "SelectiveStateManager initialized with configuration",
      this.config,
    );
  }
  ...
  connect(
    ...
    for (const storeKey in vueStores) {
      ...
      this.setupSelectiveWatchers(storeObj, storeKey, "vue", 0);
    }

    // Setup reactive proxies for legacy state
    try {
      for (const stateKey in legacyState) {
        const proxy = this.createSelectiveReactiveProxy(
  ...
  private setupSelectiveWatchers(
    ...
    const isAlwaysSync = this.alwaysSyncMatchers.some((matcher) =>
      matcher(path),
    );
  ...
  private createSelectiveReactiveProxy(obj: any, path: string): any {
    ...
    if (Array.isArray(obj)) {
      return new Proxy(obj, {
        get(target, prop: string | symbol) {
          ...
          if (typeof value === "object" && value !== null) {
            const propPath =
              typeof prop === "symbol" ? path : `${path}.[${prop}]`;
    ...
    return new Proxy(obj, {
      get(target, prop: string | symbol) {
        ...
        if (typeof value === "object" && value !== null) {
          const propPath =
            typeof prop === "symbol" ? path : `${path}.${prop.toString()}`;
          return self.createSelectiveReactiveProxy(value, propPath);
        }
  ...
  private queueUpdate(operation: UpdateOperation): void {
    ...
    const isExcluded = this.excludeMatchers.some((matcher) =>
      matcher(operation.path),
    );
    const isAlwaysSync = this.alwaysSyncMatchers.some((matcher) =>
      matcher(operation.path),
    );
  ...
  private processPendingUpdates(): void {
    ...
    try {
      ...
      // Log batch stats
      this.logger.debug(
        `Processed ${successCount}/${operations.length} state updates in batch #${this.batchUpdateCount}`,
      );
  ...
  private processUpdate(operation: UpdateOperation): boolean {
    try {
      ...
      if (this.config.enableSchemaValidation) {
        ...
        if (!isValid) {
          this.logger.warn(
            `Schema validation failed for ${operation.path}`,
            operation.value,
          );
          return false;
        }
  ...
  private updateVueStore(path: string, value: any): void {
    try {
      ...
      if (this.vueStores[storeKey].$state) {
        ...
        const path = pathParts.slice(1); // Remove store name
