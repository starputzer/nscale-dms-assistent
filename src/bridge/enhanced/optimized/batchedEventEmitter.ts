...
export class BatchedEventEmitter {
  ...
  // Callbacks
  private emitCallback: (type: string, data: any, source: 'vue' | 'vanilla' | 'system') => void;
   ...
   */
  constructor(
    emitCallback: (type: string, data: any, source: 'vue' | 'vanilla' | 'system') => void,
  ...
  private processBatches(): void {
    ...
    const totalEvents = this.vueEvents.length + this.vanillaEvents.length + this.systemEvents.length;
    ...
    try {
      ...
      const queues: ('vue' | 'vanilla' | 'system')[] = this.config.prioritizeLegacyEvents
        ? ['vanilla', 'system', 'vue']
        : ['vue', 'system', 'vanilla'];
      
      // Alle Warteschlangen nacheinander verarbeiten
      for (const queue of queues) {
        const events = queue === 'vue' 
          ? this.vueEvents 
          : queue === 'vanilla' 
            ? this.vanillaEvents 
            : this.systemEvents;
        ...
        this.logger.debug(`Batch mit ${batch.length} Ereignissen aus ${queue} verarbeitet`);
      }
      ...
      this.stats.processingTime.average = this.stats.processingTime.total / this.stats.processingTime.count;
      this.stats.processingTime.max = Math.max(this.stats.processingTime.max, processingTime);
      ...
      if (this.vueEvents.length > 0 || this.vanillaEvents.length > 0 || this.systemEvents.length > 0) {
        this.scheduleBatchProcessing();
      }
    } catch (error) {
  ...
  public flushAllBatches(): void {
    ...
    [...this.systemEvents, ...this.vanillaEvents, ...this.vueEvents].forEach(event => {
      this.emitCallback(event.type, event.data, event.source);
    });
  ...
  public dispose(): void {
    ...
    if (this.config.flushOnUnload) {
      window.removeEventListener('beforeunload', this.flushAllBatches.bind(this));
    }
