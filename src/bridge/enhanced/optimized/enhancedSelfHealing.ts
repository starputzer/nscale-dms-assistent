...
export class EnhancedSelfHealing implements SelfHealing {
  ...
  async performHealthCheck(): Promise<boolean> {
    ...
    try {
      ...
      const healthPercentage =
        totalWeight > 0 ? (weightedHealthScore / totalWeight) * 100 : 100;
      ...
      if (unhealthyChecks.length > 0) {
        this.logger.warn(
          `Health check failed: ${unhealthyChecks.length}/${results.length} checks failed`,
          {
  ...
  private async attemptRecovery(
    ...
    if (this.isRecovering) {
      this.logger.warn(
        "Recovery already in progress, skipping duplicate attempt",
      );
      return false;
    }
    ...
    if (this.recoveryAttempts >= this.config.maxRecoveryAttempts) {
      this.logger.error(
        `Maximum recovery attempts (${this.config.maxRecoveryAttempts}) reached`,
      );
    ...
    this.logger.info(
      `Starting recovery attempt ${this.recoveryAttempts}/${this.config.maxRecoveryAttempts}`,
    );
    ...
    try {
      ...
      this.logger.info(
        `Selected ${strategiesToExecute.length} recovery strategies`,
      );
      ...
      if (allSuccessful) {
        this.logger.info(
          "Recovery successful: All strategies executed successfully",
        );
      ...
      if (allSuccessful) {
        ...
        if (healthCheckResult) {
          this.logger.info(
            "Recovery verified: Health check passed after recovery",
          );
          ...
          this.logger.warn(
            "Recovery verification failed: Health check still failing after recovery",
          );
        }
      }
      ...
      if (!allSuccessful && this.config.retryFailedStrategies) {
        ...
        setTimeout(() => {
          ...
          this.performHealthCheck().catch((error) => {
            this.logger.error(
              "Error during health check after recovery backoff",
              error,
            );
          });
        }, backoffTime);
  ...
  private selectRecoveryStrategies(
    ...
    if (selectedStrategies.length === 0) {
      for (const strategy of this.recoveryStrategies.values()) {
        if (
          strategy.requiredHealthChecks.length === 0 &&
  ...
  private sortStrategiesByDependencies(
    ...
    const visit = (id: string) => {
      if (temp.has(id)) {
        ...
        this.logger.warn(
          `Circular dependency detected in recovery strategies: ${id}`,
        );
        return;
      }
    ...
    if (this.config.strategyExecution.parallel) {
      ...
      for (const strategy of strategies) {
        ...
        if (
          ...
          this.logger.error(
            `Aborting recovery: Critical strategy ${strategy.name} failed`,
          );
          break;
        }
      }
  ...
  private async executeRecoveryStrategy(
    ...
    this.logger.info(
      `Executing recovery strategy: ${strategy.name} (attempt ${strategy.attempts})`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Error executing recovery strategy ${strategy.name}`,
        error,
      );
    }
  ...
  private handleStatusChanged(status: BridgeStatusInfo): void {
    ...
    if (status.state === BridgeErrorState.HEALTHY) {
      ...
      this.logger.debug(
        "Reset recovery attempts counter due to HEALTHY status",
      );
    }
  ...
  getHealthCheckStats(): any {
    ...
    const healthPercentage =
      totalWeight > 0 ? (healthyWeight / totalWeight) * 100 : 100;
    ...
    return {
      ...
      unhealthy: Array.from(this.healthChecks.values()).filter(
        (c) => c.lastResult === false,
      ).length,
  ...
  getRecoveryStats(): any {
    return {
      ...
      strategies: {
        ...
        byComponent: Array.from(this.recoveryStrategies.values()).reduce(
          (acc, strategy) => {
        ...
        critical: Array.from(this.recoveryStrategies.values()).filter(
          (s) => s.critical,
        ).length,
      },
