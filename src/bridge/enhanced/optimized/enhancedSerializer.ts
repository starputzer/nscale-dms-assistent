...
export class EnhancedSerializer {
  ...
  serialize(
    ...
    if (
      ...
      if (
        lastFullTime > 0 &&
        timeSinceLastFull < this.config.performance.fullSerializationInterval
      ) {
        try {
          ...
          if (previousEntry && previousEntry.originalRef) {
            ...
            if (previousObject) {
              ...
              if (Object.keys(diff).length < Object.keys(value).length / 2) {
                ...
                this.logger.debug(
                  `Differential serialization took ${endTime - startTime}ms`,
                  {
                    objectId,
          ...
          this.logger.warn(
            "Error during differential serialization, falling back to full",
            error,
          );
        }
      }
    ...
    try {
      ...
      this.logger.error(
        "Serialization error, falling back to standard JSON",
        error,
      );
    ...
    try {
      ...
      if (parsed && typeof parsed === "object" && parsed.__diff === true) {
        ...
        if (baseObject) {
          ...
          const endTime = performance.now();
          this.logger.debug(
            `Differential deserialization took ${endTime - startTime}ms`,
          );

          return result as T;
        } else {
          this.logger.warn(
            "Base object for differential update not found, doing full deserialization",
          );
        }
      }
      ...
      const endTime = performance.now();
      if (endTime - startTime > 5) {
        // Only log slow deserializations
  ...
  private processValue(
    ...
    for (const typeName in this.config.serializationOptions
      .customTypeHandlers) {
      const handler =
        this.config.serializationOptions.customTypeHandlers[typeName];
  ...
  private processDeserialized(
    ...
    if (value.__circularRef && typeof value.__circularRef === "string") {
      return (
        refMap.get(value.__circularRef) || "[Unresolved Circular Reference]"
      );
    }
    ...
    if (value.__type && typeof value.__type === "string") {
      const handler =
        this.config.serializationOptions.customTypeHandlers[value.__type];
        ...
        if (
          ...
          if (Object.keys(nestedDiff).length > 0) {
            ...
            if (
              ...
              Object.keys(newObj[key]).length / 2
            ) {
              diff[key] = newObj[key];
            } else {
