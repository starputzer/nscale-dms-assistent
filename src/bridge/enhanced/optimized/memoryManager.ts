...
export class MemoryManager {
  ...
  registerSubscription(
    ...
    this.logger.debug(`Subscription registered: ${subscriptionId} for ${eventType}`);
  ...
  private checkSubscriptions(): void {
    ...
    for (const [id, subscription] of this.subscriptionRegistry.entries()) {
      ...
      if (age > this.config.thresholds.subscriptionAgeThreshold) {
        ...
        if (subscription.active && (!subscription.lastUsed || 
            now - subscription.lastUsed > this.config.thresholds.subscriptionAgeThreshold)) {
    ...
    for (const [eventType, count] of Object.entries(eventTypeCounts)) {
      if (count > this.config.thresholds.maxEventListenersPerType) {
        this.logger.warn(`Excessive event listeners detected for type: ${eventType}`, {
          count,
  ...
  private detectComponentLeaks(): void {
    ...
    for (const [id, ref] of Object.entries(this.componentRegistry)) {
      ...
      if (!component) {
        ...
        const componentSubs = Array.from(this.subscriptionRegistry.values())
          .filter(sub => sub.component === id);
        
        if (componentSubs.length > 0) {
          this.logger.warn(`Detected subscriptions for garbage collected component: ${id}`, {
  ...
  private detectPrototypePollution(): void {
    try {
      ...
      const baseObjects = [Object.prototype, Array.prototype, Function.prototype];
      ...
      for (const obj of baseObjects) {
        for (const prop in obj) {
          ...
          if (descriptor && descriptor.configurable) {
            this.logger.warn(`Potential prototype pollution detected: ${prop} on ${obj.constructor.name}`, {
              property: prop,
  ...
  private clearInactiveSubscriptions(): void {
    ...
    for (const [id, subscription] of this.subscriptionRegistry.entries()) {
      ...
      if (!subscription.active || 
          (subscription.lastUsed && now - subscription.lastUsed > inactiveThreshold)) {
  ...
  private checkSubscriptionLimits(): void {
    if (this.subscriptionRegistry.size > this.config.resourceLimits.maxSubscriptionCount) {
    ...
    try {
      ...
      return {
        ...
        growth: {
          ...
          percentage: this.initialMemoryUsage > 0 
  ...
  runFullCheck(): any {
    ...
    return {
      ...
      subscriptions: Array.from(this.subscriptionRegistry.entries())
        .map(([id, sub]) => ({
          id,
