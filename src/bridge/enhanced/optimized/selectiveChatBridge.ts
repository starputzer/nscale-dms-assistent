...
export class SelectiveChatBridge {
  ...
  private connectionStatus = ref<"connected" | "connecting" | "disconnected">(
    "disconnected",
  );
  ...
  public async initialize(): Promise<boolean> {
    try {
      ...
      if (this.memoryManager && this.config.monitorMemory) {
        this.memoryManager.registerComponent("chatBridge", () =>
          this.getMemoryUsageEstimate(),
        );
      }
      ...
      if (isConnected) {
        ...
        this.statusManager.updateStatus({
          ...
          message:
            "SelectiveChatBridge erfolgreich initialisiert und verbunden",
          affectedComponents: [],
        });
        ...
        this.statusManager.updateStatus({
          ...
          message:
            "Verbindungsfehler bei der Initialisierung der SelectiveChatBridge",
          affectedComponents: ["SelectiveChatBridge"],
        });

        this.logger.error(
          "Fehler bei der Initialisierung der SelectiveChatBridge",
        );
        return false;
      }
    } catch (error) {
      this.logger.error(
        "Unerwarteter Fehler bei der Initialisierung der SelectiveChatBridge",
        error,
      );
      this.statusManager.updateStatus({
  ...
  public sendMessage(content: string, sessionId?: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.logger.debug(
          `Nachricht wird gesendet. SessionId: ${targetSessionId}, MessageId: ${messageId}`,
        );
      } catch (error) {
        this.logger.error(
          "Unerwarteter Fehler beim Senden der Nachricht",
          error,
        );
        reject(error);
      }
  ...
  private syncDirtyMessages(): void {
    ...
    try {
      ...
      for (const [
        sessionId,
        messageIds,
      ] of this.pendingMessageUpdates.entries()) {
        // Sitzung prüfen
        ...
        if (
          this.config.prioritizeVisibleMessages &&
        ...
        if (messages.length > 0) {
          ...
          this.logger.debug(
            `${messages.length} Nachrichten für Sitzung ${sessionId} synchronisiert`,
          );
        }
  ...
  private attemptRecovery(): void {
    if (this.reconnectAttempts >= this.config.maxRetryAttempts) {
      this.logger.error(
        `Maximale Anzahl von Wiederverbindungsversuchen (${this.config.maxRetryAttempts}) erreicht`,
      );
    ...
    this.logger.info(
      `Wiederverbindungsversuch ${this.reconnectAttempts}/${this.config.maxRetryAttempts}`,
    );
  ...
  private registerEventListeners(): void {
    ...
    if (this.memoryManager && this.config.monitorMemory) {
      this.memoryManager.registerCleanupCallback("chatBridge", () =>
        this.dispose(),
      );
    }
  }
   ...
   */
  private handleMessagesUpdated(data: {
    ...
    if (
      window.nScaleChat &&
      typeof window.nScaleChat.onMessagesUpdated === "function"
    ) {
      nextTick(() => {
    ...
    this.logger.debug(
      `${data.messages.length} Nachrichten für Sitzung ${data.sessionId} von Vue aktualisiert`,
    );
  }
  ...
  private handleSessionCreated(data: {
    ...
    if (
      window.nScaleChat &&
      typeof window.nScaleChat.onSessionCreated === "function"
    ) {
      nextTick(() => {
  ...
  private handleSessionDeleted(data: { sessionId: string }): void {
    ...
    if (
      window.nScaleChat &&
      typeof window.nScaleChat.onSessionDeleted === "function"
    ) {
      nextTick(() => {
  ...
  private handleSessionsUpdated(data: {
    ...
    if (
      this.config.sessionCache &&
      this.sessions.value.size > this.config.sessionCacheSize
    ) {
    ...
    if (
      window.nScaleChat &&
      typeof window.nScaleChat.onSessionsUpdated === "function"
    ) {
      nextTick(() => {
  ...
  private pruneSessionCache(): void {
    ...
    const sortedSessions = Array.from(this.sessions.value.entries()).sort(
      ([, a], [, b]) => {
  ...
  private handleStatusUpdated(data: {
    ...
    if (
      window.nScaleChat &&
      typeof window.nScaleChat.onStatusUpdated === "function"
    ) {
      nextTick(() => {
  ...
  private handleVueError(data: { action: string; error: string }): void {
    this.logger.error(
      `Fehler von Vue-Komponente: ${data.action} - ${data.error}`,
    );
    ...
    if (this.selfHealing && this.config.autoRecovery) {
      this.selfHealing.handleError(
        new Error(`Vue-Fehler: ${data.action} - ${data.error}`),
      );
    }
  }
    ...
    this.selfHealing.registerRule({
      ...
      remedy: async () => {
        ...
        if (this.streamingMessageId.value) {
          ...
          this.logger.debug(
            `Streaming-Status für Nachricht ${oldStreamingId} zurückgesetzt`,
          );
          return true;
        }
