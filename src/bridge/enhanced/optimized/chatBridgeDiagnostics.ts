...
interface ChatBridgeDiagnosticsConfig {
  ...
  alertThresholds: {                   // Schwellwerte für Warnungen
...
export class ChatBridgeDiagnostics {
  ...
  public recordMetric(
    ...
    if (this.performanceMonitor) {
      this.performanceMonitor.recordMetric(name, value, unit, category, subcategory);
    }
  ...
  private checkThresholds(metric: MetricDataPoint): void {
    ...
    if (metric.name.includes('latency') && 
      ...
      this.createAlert(
        'latency',
        metric.value > this.config.alertThresholds.highLatencyMs * 2 ? 'critical' : 'warning',
    ...
    if (metric.name.includes('memory') && 
      ...
      if (previousMetrics.length > 0) {
        ...
        const percentIncrease = ((metric.value - previousMetric.value) / previousMetric.value) * 100;
        
        if (percentIncrease > this.config.alertThresholds.memorySpikePercent) {
          this.createAlert(
            'memory',
            percentIncrease > this.config.alertThresholds.memorySpikePercent * 2 ? 'critical' : 'warning',
    ...
    if (metric.name === 'bridge.lastActivityMs' && 
        metric.value > this.config.alertThresholds.inactivityTimeoutMs) {
      this.createAlert(
        'inactivity',
        metric.value > this.config.alertThresholds.inactivityTimeoutMs * 2 ? 'critical' : 'warning',
  ...
  public generateReport(): DiagnosticReport {
    ...
    try {
      ...
      const memoryEstimate = this.eventListenerManager ? 
        (this.eventListenerManager as any).getStats()?.memoryEstimate || 0 : 0;
      const listenersCount = this.eventListenerManager ? 
        (this.eventListenerManager as any).getStats()?.activeCount || 0 : 0;
      const eventBatchesCount = this.batchedEventEmitter ? 
        (this.batchedEventEmitter as any).getStats()?.batchedEvents || 0 : 0;
      
      // Leistungs-Metriken
      const messageProcessingMetrics = this.metrics.filter(m => 
        m.name === 'chatBridge.syncMessagesDuration'
      );
      const batchProcessingMetrics = this.metrics.filter(m => 
        m.name === 'batchedEventEmitter.processingTime'
      );
      
      const messageProcessingTimeAvg = messageProcessingMetrics.length > 0 ?
        messageProcessingMetrics.reduce((sum, m) => sum + m.value, 0) / messageProcessingMetrics.length : 0;
      
      const batchProcessingTimeAvg = batchProcessingMetrics.length > 0 ?
        batchProcessingMetrics.reduce((sum, m) => sum + m.value, 0) / batchProcessingMetrics.length : 0;
      
      // Fehlerrate
      const syncOpCount = this.metrics.filter(m => m.name === 'chatBridge.syncOperations').length;
      ...
      if (avgLatency > this.config.alertThresholds.highLatencyMs) {
        recommendations.push('Überprüfe die Netzwerklatenz und optimiere die Event-Batching-Konfiguration');
      }
      
      if (errorRate > this.config.alertThresholds.errorRateThreshold) {
        recommendations.push('Erhöhe die Self-Healing-Kapazität und verbessere die Fehlerbehandlung');
      }
      
      if (memoryEstimate > 5000000) { // 5 MB
        recommendations.push('Überprüfe Memory-Leaks bei Event-Listenern und optimiere die Cache-Größen');
      }
      
      if (messageProcessingTimeAvg > 100) { // 100ms
        recommendations.push('Optimiere die Nachrichtenverarbeitung durch selektivere Synchronisation');
      }
      
      if (activeAlerts.some(a => a.type === 'inactivity')) {
        recommendations.push('Überprüfe Verbindungsprobleme zwischen Vue- und Legacy-Komponenten');
      }
      ...
      const report: DiagnosticReport = {
        ...
        metrics: {
          ...
          performance: {
            ...
            syncOperationsPerSecond: syncOpCount > 0 ? syncOpCount / (messageProcessingMetrics.length || 1) : 0
          },
          errorRate
        },
        alerts: activeAlerts,
      ...
      return {
        ...
        metrics: {
          ...
          performance: { messageProcessingTimeAvg: 0, batchProcessingTimeAvg: 0, syncOperationsPerSecond: 0 },
          errorRate: 1
        },
        alerts: [{
          id: `error_${Date.now()}`,
  ...
  private sampleMetrics(): void {
    try {
      ...
      if (lastPingAge > 30000) { // 30 Sekunden
        this.pingTest(1).catch(() => {/* Ignorieren */});
      }
      ...
      if (lastActivity > 0) {
        ...
        this.recordMetric('bridge.lastActivityMs', inactivityMs, 'ms', 'bridge', 'activity');
        
        // Inaktivitäts-Warnung auflösen, wenn wieder aktiv
        if (inactivityMs < this.config.alertThresholds.inactivityTimeoutMs) {
          const inactivityAlerts = this.alerts.filter(a => 
            a.type === 'inactivity' && !a.resolved
          );
      ...
      this.log('error', 'diagnostics', 'Fehler bei der Metriken-Erfassung', error);
    }
  }
  ...
  private startAutoReporting(): void {
    ...
    this.log('debug', 'diagnostics', 'Automatische Berichterstattung gestartet', {
  ...
  private registerConsoleCommands(): void {
    ...
    (window as any).chatBridgeDiagnostics = {
      ...
      getStatus: () => {
        ...
        return {
          ...
          connectionStatus: (this.chatBridge as any).connectionStatus?.value || 'unknown',
    ...
    this.log('info', 'diagnostics', 'Konsolen-Befehle registriert als window.chatBridgeDiagnostics');
  }
  
  /**
   * Neueste Metriken für jede Kategorie abrufen
   */
  private getLatestMetrics(): Record<string, { value: number, unit: string, timestamp: number }> {
    const result: Record<string, { value: number, unit: string, timestamp: number }> = {};
  ...
  private createDeveloperToolbar(): void {
    ...
    try {
      ...
      setInterval(() => {
        if (this.developerToolbar && this.developerToolbar.style.display !== 'none') {
          this.updateDeveloperToolbar();
        }
      }, 1000);
      ...
      this.log('error', 'diagnostics', 'Fehler beim Erstellen der Entwickler-Symbolleiste', error);
    }
  }
  ...
  private updateDeveloperToolbar(): void {
    ...
    try {
      ...
      this.developerToolbar.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div>
            ...
            | Listener: ${latestMetrics['eventListenerManager.memoryBytes']?.value 
                ? Math.round(latestMetrics['eventListenerManager.memoryBytes'].value / 1024) + 'KB'
                : '-'}
          </div>
      ...
      this.log('error', 'diagnostics', 'Fehler beim Aktualisieren der Entwickler-Symbolleiste', error);
    }
  }
  ...
  public dispose(): void {
    ...
    // UI entfernen
    if (this.developerToolbar && document.body.contains(this.developerToolbar)) {
      document.body.removeChild(this.developerToolbar);
      this.developerToolbar = null;
    }
    
    // Konsolen-Befehle entfernen
    if (this.config.enableConsoleCommands && 'chatBridgeDiagnostics' in window) {
    ...
    this.log('info', 'diagnostics', 'ChatBridgeDiagnostics erfolgreich freigegeben');
  }
}
