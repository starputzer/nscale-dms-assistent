...
export class PerformanceMonitor {
  ...
  private setupPerformanceObservers(): void {
    try {
      ...
      if (PerformanceObserver && this.config.metricsTracking.trackBrowserMetrics) {
  ...
  private handleResourceEntries(entries: PerformanceResourceTiming[]): void {
    ...
    for (const [type, typeEntries] of Object.entries(byType)) {
      ...
      const avgDuration = typeEntries.reduce((sum, e) => sum + e.duration, 0) / typeEntries.length;
      ...
      if (entriesWithSize.length > 0) {
        const avgSize = entriesWithSize.reduce((sum, e) => sum + e.transferSize, 0) / entriesWithSize.length;
  ...
  private sampleMetrics(): void {
    try {
      // Sample frame rate
      if (this.config.metricsTracking.trackFrameRate && this.frameRates.length > 0) {
        const avgFrameRate = this.frameRates.reduce((sum, fps) => sum + fps, 0) / this.frameRates.length;
        ...
        if (avgFrameRate < this.config.bottleneckDetection.thresholds.frameRate) {
          this.detectBottleneck(
            'browser.frameRate',
            avgFrameRate,
            'browser',
      ...
      // Sample memory usage
      if (this.config.metricsTracking.trackMemory && window.performance && window.performance.memory) {
        ...
        if (memoryMetrics && memoryMetrics.length >= 2) {
          ...
          if (memoryGrowth > this.config.bottleneckDetection.thresholds.memoryGrowth) {
            this.detectBottleneck(
   ...
   */
  measure(name: string, startMark: string, endMark: string, category: string = 'default'): number {
    try {
      ...
      if (name in this.config.metricsTracking.customMetrics) {
        ...
        switch (true) {
          ...
          case name.includes('event') && name.includes('processing'):
            threshold = this.config.bottleneckDetection.thresholds.eventProcessingTime;
            break;
            
          case name.includes('dom') && name.includes('operation'):
            threshold = this.config.bottleneckDetection.thresholds.domOperationTime;
            break;
          ...
          case name.includes('state') && name.includes('sync'):
            threshold = this.config.bottleneckDetection.thresholds.stateSyncTime;
            break;
          ...
          case name.includes('bridge') && name.includes('operation'):
            threshold = this.config.bottleneckDetection.thresholds.bridgeOperationTime;
            break;
        }
      }
  ...
  private detectBottleneck(
    ...
    const effectiveCritical = criticalThreshold ?? 
    ...
    if (isIssue) {
      ...
      const detection: BottleneckDetection = {
        ...
        timestamp: Date.now(),
        details: this.formatBottleneckDetails(name, value, effectiveThreshold, higherIsBetter)
      };
  ...
  private formatValue(value: number, metricName: string): string {
    ...
    } else if (metricName.includes('rate') && !metricName.includes('frameRate')) {
  ...
  private notifyIssue(issue: BottleneckDetection): void {
    ...
    if (this.config.visualization.enabled && this.config.visualization.showOnIssues) {
  ...
  private notifyImprovement(
    ...
    if (!this.config.notifications.enabled || !this.config.notifications.notifyOnImprovement) {
      return;
    }
    
    // Log to console if configured
    if (this.config.notifications.logToConsole) {
      console.info(`[Bridge] Performance improved: ${name} is now ${this.formatValue(value, name)}`);
    }
    
    // Update bridge status if previous was critical
    if (previous.severity === 'critical') {
      // Check if we still have any critical issues
      const criticalIssues = Array.from(this.currentIssues.values())
   ...
   */
  getFrameRateStats(): { current: number; average: number; min: number; max: number } {
    if (this.frameRates.length === 0) {
      return { current: 0, average: 0, min: 0, max: 0 };
    }
    
    const current = this.frameRates[this.frameRates.length - 1];
    const average = this.frameRates.reduce((sum, fps) => sum + fps, 0) / this.frameRates.length;
  ...
  getPerformanceOverview(): any {
    return {
      metrics: {
        ...
        categories: Array.from(this.metrics.values())
          .reduce((categories, metrics) => {
      ...
      issues: {
        ...
        critical: Array.from(this.currentIssues.values())
          .filter(issue => issue.severity === 'critical').length,
        high: Array.from(this.currentIssues.values())
  ...
  getSummary(): string {
    ...
    return `Performance monitoring detected ${issues.length} issues: ` +
