 ...
 * @template T Der Rückgabetyp der Operation
 */
export type BridgeOperation<T> = AsyncBridgeOperation<T> | SyncBridgeOperation<T>;
 ...
 * @template T Der Rückgabetyp der Operation
 */
export interface CancellableBridgeOperation<T> extends Promise<BridgeResult<T>> {
...
export interface ChainableBridgeOperation<T, R> {
   ...
   */
  then: <U>(nextOperation: (result: R) => Promise<BridgeResult<U>>) => ChainableBridgeOperation<T, U>;
  
  /**
   * Definiert eine alternative Operation für den Fehlerfall
   * @param recoveryOperation Die Wiederherstellungsoperation
   */
  catch: (recoveryOperation: (error: BridgeError) => Promise<BridgeResult<R>>) => ChainableBridgeOperation<T, R>;
  
  /**
   * Definiert eine finale Operation, die immer ausgeführt wird
   * @param finalOperation Die finale Operation
   */
  finally: (finalOperation: () => Promise<void>) => ChainableBridgeOperation<T, R>;
}
...
export function chain<T>(
  ...
  let recoveryOperation: ((error: BridgeError) => Promise<BridgeResult<T>>) | null = null;
...
export async function withEventHandler<T extends keyof BridgeEventMap>(
  eventBus: {
    on: (eventName: T, handler: (data: BridgeEventMap[T]) => void, options?: any) => Promise<BridgeResult<{ unsubscribe: () => Promise<BridgeResult<void>> }>>;
  },
  eventName: T,
...
export interface PausableOperation<T> {
   ...
   */
  getStatus: () => Promise<BridgeResult<{
  ...
  onStatusChange: (handler: (status: {
    ...
    result?: T;
    error?: BridgeError;
  }) => void) => Promise<BridgeResult<() => Promise<BridgeResult<void>>>>;
}
...
export function makePausable<T>(
  ...
  let state: 'idle' | 'running' | 'paused' | 'completed' | 'cancelled' | 'error' = 'idle';
  let progress = 0;
  ...
  return {
    ...
    async onStatusChange(handler: (status: any) => void): Promise<BridgeResult<() => Promise<BridgeResult<void>>>> {
      try {
