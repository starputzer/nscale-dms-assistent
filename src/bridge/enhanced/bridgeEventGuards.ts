 ...
 */
export function isBridgeInitializedEvent(value: unknown): value is BridgeInitializedEvent {
  ...
  return (
    ...
    typeof event.configuration === 'object' && event.configuration !== null &&
    typeof event.environment === 'object' && event.environment !== null &&
    ...
    typeof event.environment.browserInfo === 'object' && event.environment.browserInfo !== null &&
 ...
 */
export function isBridgeStatusEvent(value: unknown): value is BridgeStatusEvent {
  if (!isEventPayload(value)) return false;
  
  const event = value as Partial<BridgeStatusEvent>;
  const validStatuses = ['initializing', 'active', 'degraded', 'error', 'recovering', 'inactive'];
  
  return (
    typeof event.status === 'string' &&
    validStatuses.includes(event.status) &&
    typeof event.timestamp === 'number' &&
    (event.previousStatus === undefined || typeof event.previousStatus === 'string') &&
    (event.message === undefined || typeof event.message === 'string') &&
    (event.affectedComponents === undefined || Array.isArray(event.affectedComponents)) &&
    (event.recoveryAttempts === undefined || typeof event.recoveryAttempts === 'number')
  );
}
 ...
 */
export function isBridgeHealthCheckEvent(value: unknown): value is BridgeHealthCheckEvent {
  if (!isEventPayload(value)) return false;
  ...
  if (
    typeof event.timestamp !== 'number' ||
    typeof event.components !== 'object' || event.components === null ||
    typeof event.overall !== 'string' || !validOverallStatuses.includes(event.overall)
  ) {
    return false;
  }
  
  // Prüfe die Komponenten-Status-Objekte
  const components = event.components as Record<string, { status: string; details?: string }>;
  ...
  for (const componentName in components) {
    ...
    if (
      ...
      typeof component.status !== 'string' || !validComponentStatuses.includes(component.status) ||
 ...
 */
export function isBridgeHealingEvent(value: unknown): value is BridgeHealingEvent {
  if (!isEventPayload(value)) return false;
 ...
 */
export function isBridgeMemoryEvent(value: unknown): value is BridgeMemoryEvent {
  if (!isEventPayload(value)) return false;
  ...
  return (
    ...
    (event.totalMemory === undefined || typeof event.totalMemory === 'number') &&
    (event.component === undefined || typeof event.component === 'string') &&
    (event.leakSuspect === undefined || typeof event.leakSuspect === 'boolean') &&
    (event.details === undefined || typeof event.details === 'string')
  );
}
 ...
 */
export function isBridgePerformanceEvent(value: unknown): value is BridgePerformanceEvent {
  if (!isEventPayload(value)) return false;
 ...
 */
export function isStateConflictEvent(value: unknown): value is StateConflictEvent {
  if (!isEventPayload(value)) return false;
 ...
 */
export function isLegacySessionEvent(value: unknown): value is LegacySessionEvent {
  if (!isEventPayload(value)) return false;
 ...
 */
export function isLegacyMessageEvent(value: unknown): value is LegacyMessageEvent {
  if (!isEventPayload(value)) return false;
 ...
 */
export function isLegacyStreamingEvent(value: unknown): value is LegacyStreamingEvent {
  if (!isEventPayload(value)) return false;
 ...
 */
export function isLegacyUIStateEvent(value: unknown): value is LegacyUIStateEvent {
  if (!isEventPayload(value)) return false;
  ...
  return (
    typeof event.timestamp === 'number' &&
    typeof event.changes === 'object' && event.changes !== null
  );
}
 ...
 */
export function isVueSendMessageEvent(value: unknown): value is VueSendMessageEvent {
 ...
 */
export function isVueSessionCommandEvent(value: unknown): value is VueSessionCommandEvent {
  if (!isVueCommandEvent(value)) return false;
  
  const event = value as Partial<VueSessionCommandEvent>;
  const validCommands = ['selectSession', 'createSession', 'deleteSession', 'cancelStreaming'];
 ...
 */
export function isVueUpdateSessionEvent(value: unknown): value is VueUpdateSessionEvent {
  if (!isVueCommandEvent(value)) return false;
  ...
  if (
    ...
    (updates.isArchived !== undefined && typeof updates.isArchived !== 'boolean') ||
 ...
 */
export function isVueDeleteMessageEvent(value: unknown): value is VueDeleteMessageEvent {
  if (!isVueCommandEvent(value)) return false;
