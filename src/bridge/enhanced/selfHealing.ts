...
import { SelfHealing, BridgeLogger, BridgeStatusManager, BridgeErrorState } from './types';
...
export class SelfHealingBridge implements SelfHealing {
  ...
  constructor(logger: BridgeLogger, statusManager: BridgeStatusManager, healthCheckInterval: number = 30000) {
    this.logger = logger;
  ...
  addHealthCheck(check: () => boolean): void {
    ...
    this.logger.debug('Gesundheitsprüfung hinzugefügt', { total: this.healthChecks.length });
  }
  ...
  addRecoveryStrategy(strategy: () => Promise<boolean>): void {
    ...
    this.logger.debug('Wiederherstellungsstrategie hinzugefügt', { total: this.recoveryStrategies.length });
  }
  ...
  private setupPeriodicHealthCheck(): void {
    ...
    this.logger.info(`Periodische Gesundheitsprüfung eingerichtet (Intervall: ${this.healthCheckInterval}ms)`);
  }
  ...
  async performHealthCheck(): Promise<boolean> {
    ...
    try {
      ...
      if (unhealthyChecks.length > 0) {
        this.logger.warn(`Gesundheitsprüfung fehlgeschlagen: ${unhealthyChecks.length} von ${this.healthChecks.length} Prüfungen fehlgeschlagen`);
        ...
        this.statusManager.updateStatus({
          ...
          affectedComponents: unhealthyChecks.map(c => `HealthCheck-${c.index}`)
        });
  ...
  private async attemptRecovery(): Promise<boolean> {
    ...
    try {
      ...
      for (let i = 0; i < this.recoveryStrategies.length; i++) {
        ...
        this.logger.debug(`Versuche Wiederherstellungsstrategie ${i + 1} von ${this.recoveryStrategies.length}`);
        
        try {
          const success = await strategy();
          
          if (success) {
            this.logger.info(`Bridge-Wiederherstellung erfolgreich mit Strategie ${i + 1}`);
          ...
          this.logger.error(`Fehler in Wiederherstellungsstrategie ${i + 1}`, error);
        }
      }
      ...
      this.logger.error('Schwerwiegender Fehler während der Bridge-Wiederherstellung', error);
      
      // Kritischen Status melden
      this.statusManager.updateStatus({
        state: BridgeErrorState.CRITICAL_FAILURE,
        message: 'Kritischer Fehler bei der Wiederherstellung. Bitte Seite neu laden.'
      });
  ...
  setHealthCheckInterval(interval: number): void {
    ...
    this.setupPeriodicHealthCheck();
    this.logger.info(`Gesundheitsprüfungs-Intervall auf ${interval}ms aktualisiert`);
  }
