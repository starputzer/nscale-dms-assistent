...
export class ChatBridge {
  ...
  private registerEventHandlers(): void {
    ...
    this.bridge.on('vueChat:messagesUpdated', this.handleVueMessagesUpdated.bind(this));
    this.bridge.on('vueChat:statusUpdated', this.handleVueStatusUpdated.bind(this));
    this.bridge.on('vueChat:sessionsUpdated', this.handleVueSessionsUpdated.bind(this));
    this.bridge.on('vueChat:sessionCreated', this.handleVueSessionCreated.bind(this));
    this.bridge.on('vueChat:sessionDeleted', this.handleVueSessionDeleted.bind(this));
  ...
  public initialize(): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      const timeout = setTimeout(() => {
        this.bridge.log(LogLevel.WARN, 'ChatBridge: Timeout bei der Initialisierung');
        resolve(false);
      }, 3000);
      ...
      const handleInitSuccess = () => {
        ...
        this.bridge.log(LogLevel.INFO, 'ChatBridge: Erfolgreich mit Vue-Komponenten verbunden');
  ...
  private startAutoSync(): void {
    ...
    this.bridge.log(LogLevel.DEBUG, 'ChatBridge: Automatische Synchronisierung gestartet');
  }
  ...
  public stopAutoSync(): void {
    ...
    this.bridge.log(LogLevel.DEBUG, 'ChatBridge: Automatische Synchronisierung gestoppt');
  }
  ...
  public syncState(): void {
    if (!this.isInitialized) {
      ...
      if (this.syncAttempts > this.config.maxSyncAttempts) {
        this.bridge.log(LogLevel.WARN, 'ChatBridge: Maximale Synchronisierungsversuche erreicht');
        return;
      }
  ...
  public sendMessage(content: string, sessionId?: string): void {
    ...
    if (!targetSessionId) {
      this.bridge.log(LogLevel.ERROR, 'ChatBridge: Keine aktive Session zum Senden der Nachricht');
      return;
    }
   ...
   */
  public editMessage(messageId: string, content: string, sessionId?: string): void {
    const targetSessionId = sessionId || this.activeSessionId;
    
    if (!targetSessionId) {
      this.bridge.log(LogLevel.ERROR, 'ChatBridge: Keine aktive Session zum Bearbeiten der Nachricht');
      return;
    }
  ...
  public retryMessage(messageId: string, sessionId?: string): void {
    ...
    if (!targetSessionId) {
      this.bridge.log(LogLevel.ERROR, 'ChatBridge: Keine aktive Session zum erneuten Senden der Nachricht');
      return;
    }
   ...
   */
  private handleVueMessagesUpdated(data: { messages: ChatMessage[], timestamp: number }): void {
      ...
      for (const session of data.sessions) {
        ...
        if (this.sessionCache.size > this.config.sessionCacheSize) {
          ...
          for (const [key, cachedSession] of this.sessionCache.entries()) {
            if (!cachedSession.isPinned && cachedSession.lastActivity < oldestTimestamp) {
              oldestKey = key;
  ...
  private handleVueError(data: { action: string, error: string }): void {
    this.bridge.log(LogLevel.ERROR, `ChatBridge: Fehler bei ${data.action} - ${data.error}`);
