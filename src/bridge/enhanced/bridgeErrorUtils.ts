 ...
 */
export function hasErrorCode(error: BridgeError | null | undefined, code: BridgeErrorCode): boolean {
...
export function withRecovery<T>(
  ...
  return Promise.resolve(recoveryFn())
    .catch(recoveryError => {
 ...
 */
export function getUserFriendlyErrorMessage(error: BridgeError | null | undefined): string {
  if (!error) {
  ...
  const friendlyMessages: Record<BridgeErrorCode, string> = {
    ...
    [BridgeErrorCode.INITIALIZATION_FAILED]: 'Die Initialisierung der Bridge konnte nicht abgeschlossen werden',
    [BridgeErrorCode.COMMUNICATION_ERROR]: 'Kommunikationsfehler zwischen Komponenten',
    [BridgeErrorCode.STATE_SYNC_ERROR]: 'Fehler bei der Synchronisation von Statusdaten',
    ...
    [BridgeErrorCode.EVENT_HANDLING_ERROR]: 'Fehler bei der Verarbeitung eines Events',
    [BridgeErrorCode.MEMORY_MANAGEMENT_ERROR]: 'Fehler bei der Speicherverwaltung',
    [BridgeErrorCode.LISTENER_MANAGEMENT_ERROR]: 'Fehler bei der Verwaltung von Event-Listenern',
    [BridgeErrorCode.COMPONENT_INACTIVE]: 'Die Komponente ist derzeit nicht aktiv',
    ...
    [BridgeErrorCode.LEGACY_INTEGRATION_ERROR]: 'Fehler bei der Integration mit Legacy-Komponenten',
    [BridgeErrorCode.LEGACY_METHOD_UNAVAILABLE]: 'Eine erforderliche Legacy-Methode ist nicht verfügbar',
    [BridgeErrorCode.LEGACY_STATE_INCOMPATIBLE]: 'Inkompatible Legacy-Zustandsdaten',
    [BridgeErrorCode.RECOVERY_FAILED]: 'Wiederherstellung nach einem Fehler fehlgeschlagen',
    [BridgeErrorCode.RETRY_LIMIT_EXCEEDED]: 'Maximale Anzahl an Wiederholungsversuchen überschritten'
  };
