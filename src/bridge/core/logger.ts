...
export class BridgeLogger {
   ...
   * Log an error object
   */
  logError(namespace: string, error: Error | BridgeError | unknown, context?: Record<string, any>): void {
    let message: string;
    let details: any;
  ...
  private isNamespaceEnabled(namespace: string): boolean {
    if (!this.options.enabledNamespaces || this.options.enabledNamespaces.length === 0) {
      return true;
    }
    
    return this.options.enabledNamespaces.some(pattern => 
      pattern === '*' || 
      namespace === pattern || 
  ...
  private isNamespaceDisabled(namespace: string): boolean {
    if (!this.options.disabledNamespaces || this.options.disabledNamespaces.length === 0) {
      return false;
    }
    
    return this.options.disabledNamespaces.some(pattern => 
      pattern === '*' || 
      namespace === pattern || 
      (pattern.endsWith('*') && namespace.startsWith(pattern.slice(0, -1)))
    );
...
export class NamespacedLogger {
   ...
   * Log an error object
   */
  logError(error: Error | BridgeError | unknown, context?: Record<string, any>): void {
