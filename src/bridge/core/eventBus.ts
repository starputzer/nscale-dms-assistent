...
export class BatchedEventBus implements TypedEventEmitter {
  ...
  emit<K extends keyof BridgeEventMap>(
    ...
    if (this.isPaused) {
      if (this.debug) {
        console.debug(
          `[BatchedEventBus] Skipping event ${String(eventType)} because event bus is paused`,
        );
      }
      return;
    }
    ...
    if (this.debug) {
      console.debug(
        `[BatchedEventBus] Queuing event: ${event.type}`,
        event.payload,
      );
    }
  ...
  private unsubscribe(subscriptionId: string): void {
    for (const [eventType, subscriptions] of this.subscriptions.entries()) {
      ...
      if (index !== -1) {
        ...
        if (this.debug) {
          console.debug(
            `[BatchedEventBus] Unsubscribed from "${eventType}" (ID: ${subscriptionId})`,
          );
        }
  ...
  private pauseSubscription(subscriptionId: string): void {
    for (const subscriptions of this.subscriptions.values()) {
      ...
      if (subscription) {
        ...
        if (this.debug) {
          console.debug(
            `[BatchedEventBus] Paused subscription for "${subscription.eventType}" (ID: ${subscriptionId})`,
          );
        }
  ...
  private resumeSubscription(subscriptionId: string): void {
    for (const subscriptions of this.subscriptions.values()) {
      ...
      if (subscription) {
        ...
        if (this.debug) {
          console.debug(
            `[BatchedEventBus] Resumed subscription for "${subscription.eventType}" (ID: ${subscriptionId})`,
          );
        }
  ...
  private processEvent(event: BridgeEvent): void {
    ...
    if (subscriptions.length === 0) {
      ...
      if (this.debug) {
        console.debug(
          `[BatchedEventBus] No subscribers for event "${event.type}"`,
        );
      }
      return;
    }
    ...
    for (let i = 0; i < subscriptions.length; i++) {
      ...
      try {
        ...
        console.error(
          `[BatchedEventBus] Error in event handler for "${event.type}":`,
          error,
        );
      }
    }
