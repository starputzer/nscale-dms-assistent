<template>
  <div class="document-converter">
    ...
    <!-- Conversion View -->
    <div
      v-else-if="currentView === 'conversion'"
      class="document-converter__conversion"
    >
    ...
    <div
      ...
      class="document-converter__results"
    >
      <DocumentPreview
...
async function initialize() {
  try {
    ...
    addConversionLog(
      "error",
      `Fehler bei der Initialisierung: ${error instanceof Error ? error.message : String(error)}`,
    );
  } finally {
...
async function startConversion(documentId: string) {
  try {
    ...
    addConversionLog(
      "error",
      `Konvertierungsfehler: ${error instanceof Error ? error.message : String(error)}`,
    );
  } finally {
...
async function cancelConversion() {
  ...
  if (confirmed) {
    try {
      ...
      addConversionLog(
        "error",
        `Fehler beim Abbrechen: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
...
async function confirmDeleteDocument(documentId: string) {
  const document = store.convertedDocuments.find(
...
function $t(key: string, params?: Record<string, any>) {
  ...
  const translations: Record<string, string> = {
    ...
    "documentConverter.cancelConfirmMessage":
      "Sind Sie sicher, dass Sie die aktuelle Konvertierung abbrechen möchten?",
    ...
    "documentConverter.deleteConfirmMessage":
      'Sind Sie sicher, dass Sie das Dokument "{name}" löschen möchten?',
