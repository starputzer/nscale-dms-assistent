...
// Einfache Mocks für die Composables
const useDocumentConverter = () => {
  ...
  const uploadDocument = async (file) => "doc-" + file.name;
  const convertDocument = async (id, cb) => true;
  const selectDocument = (id) => {};
  const cancelConversion = async (id) => {};
  const clearSelection = () => {};
  const deleteDocument = async (id) => {};
  
  return {
    isInitialized, error, isConverting, documents, selectedDocument,
    isUploading, isLoading, initialize, uploadDocument, convertDocument,
    selectDocument, cancelConversion, clearSelection, deleteDocument
  };
};
...
const useGlobalDialog = () => ({
  error: async (options) => {},
  confirm: async (options) => true
});

// i18n - einfache Implementierung
const useI18n = () => ({
 ...
 */
async function promptDeleteDocument(documentId) {
