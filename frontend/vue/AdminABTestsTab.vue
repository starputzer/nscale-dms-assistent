<template>
  <div class="admin-ab-tests">
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
      ...
      <div v-else-if="!tests || tests.length === 0" class="py-4 text-center text-gray-500">
      ...
      <div class="p-4">
        <div class="mb-4">
          ...
          <p>
            ...
            Control: {{ testResults.participants.control }}, 
            Variant: {{ testResults.participants.variant }}
          </p>
        </div>
          ...
          >
            ...
            <div class="flex justify-between mt-2">
              ...
              <div>
                ...
                <div class="font-medium text-green-600">{{ data.improvement }}</div>
              </div>
            </div>
          </div>
        </div>
  ...
  setup() {
    ...
    const changeTestVariant = async (testId, variant) => {
      try {
        ...
        if (variant !== 'none') {
          ...
          alert(`Sie wurden der Variante '${variant}' für Test '${testId}' zugewiesen. Die Seite wird neu geladen, um die Änderungen anzuwenden.`);
    ...
    const formatMetricValue = (metric, value) => {
      if (metric === 'messageResponseTime') {
        return `${value}ms`;
      } else if (metric === 'sessionDuration' || metric === 'taskCompletionTime') {
        return `${value}s`;
      } else if (metric === 'userSatisfaction' || metric === 'conversionSuccess') {
