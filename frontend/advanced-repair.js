...
(function () {
  ...
  function debugLog(message, data) {
    if (window._repairState.debugMode) {
      ...
      const debugContainer =
        document.querySelector("#repair-debug") || createDebugContainer();
  ...
  function createDebugContainer() {
    ...
    container.style.cssText =
      "position: fixed; bottom: 10px; right: 10px; width: 400px; max-height: 300px; overflow-y: auto; background: rgba(0,0,0,0.7); color: #0f0; font-family: monospace; padding: 10px; border-radius: 4px; z-index: 9999; font-size: 12px; display: none;";

    const header = document.createElement("div");
    header.style.cssText =
      "display: flex; justify-content: space-between; margin-bottom: 5px; border-bottom: 1px solid #0f0; padding-bottom: 5px;";
    header.innerHTML =
      '<span>nScale-Fix Debug-Log</span><button id="repair-debug-toggle" style="background: none; border: none; color: #0f0; cursor: pointer;">Ausblenden</button>';
    ...
    document
      .getElementById("repair-debug-toggle")
  ...
  function runMainRepairLoop() {
    ...
    if (
      window._repairState.repairAttempts < window._repairState.maxRepairAttempts
    ) {
      debugLog(
        `Reparatur-Versuch ${window._repairState.repairAttempts} von ${window._repairState.maxRepairAttempts}`,
      );
  ...
  function repairVueInit() {
    ...
    if (typeof Vue === "undefined") {
      ...
      vueScript.onload = function () {
        ...
        vueDemiScript.src =
          "https://cdn.jsdelivr.net/npm/vue-demi@0.13.11/lib/index.iife.js";
        vueDemiScript.onload = function () {
          ...
          piniaScript.src =
            "https://cdn.jsdelivr.net/npm/pinia@2.0.33/dist/pinia.iife.js";
  ...
  function fixSettingsButton() {
    ...
    const settingsButtons = document.querySelectorAll(
      'button[class*="settings"], button i.fa-cog, button i.fa-gear, header button:nth-child(1)',
    );
    ...
    if (settingsButtons.length > 0) {
      ...
      settingsButtons.forEach((button) => {
        ...
        const targetButton =
          button.tagName === "I" ? button.closest("button") : button;
    ...
    function buttonClickHandler(event) {
      ...
      if (typeof window.toggleSettings === "function") {
        ...
        window.app &&
        typeof window.app.toggleSettings === "function"
      ) {
    ...
    focusableElements.forEach((element) => {
      if (!element._focusRepaired) {
        ...
        if (window.getComputedStyle(element).pointerEvents === "none") {
          debugLog(
            `Korrigiere Element mit pointer-events:none: ${element.tagName}`,
          );
  ...
  function finalRepairAttempt() {
    ...
    if (window.nscale && window.nscale.featureToggles) {
      ...
      localStorage.setItem(
        "featureToggles",
        JSON.stringify(window.nscale.featureToggles),
      );
  ...
  function setupGlobalEventDelegation() {
    ...
    document.body.addEventListener("click", function (event) {
      ...
      if (
        target.classList.contains("admin-tab-button") ||
        target.closest(".admin-tab-button")
      ) {
        const button = target.classList.contains("admin-tab-button")
          ? target
          : target.closest(".admin-tab-button");
        const tabId =
        ...
        if (tabId) {
          ...
          const targetTab = document.querySelector(
            `.admin-tab-content[data-tab="${tabId}"], [class*="admin-tab"][data-tab="${tabId}"]`,
          );
          if (targetTab) {
            targetTab.style.display = "block";
