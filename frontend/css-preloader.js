...
(function() {
  ...
  function validateAndFixStylesheets() {
    ...
    requiredCssFiles.forEach(cssFile => {
      if (!loadedStylesheets.includes(cssFile)) {
        console.warn(`Stylesheet '${cssFile}' fehlt oder wurde nicht korrekt geladen. Lade neu...`);
  ...
  function ensureCssVariables() {
    ...
    if (!nscaleGreen) {
      console.warn('CSS-Variablen nicht korrekt geladen. Füge Inline-Fallback hinzu.');
