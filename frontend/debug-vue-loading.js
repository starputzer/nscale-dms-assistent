...
(function() {
    ...
    function createDebugUI() {
        ...
        header.innerHTML = '<h3 style="margin: 0 0 10px 0; color: #00ff00; text-align: center;">Vue 3 SFC Debug</h3>';
        ...
        featureToggleSection.innerHTML = '<h4 style="margin: 5px 0; color: #00ccff;">Feature Toggles</h4>';
        ...
        mountPointsSection.innerHTML = '<h4 style="margin: 10px 0 5px 0; color: #00ccff;">Mount Points</h4>';
        debugContainer.appendChild(mountPointsSection);
        
        const bridgeSection = document.createElement('div');
        bridgeSection.id = 'bridge-debug';
        bridgeSection.innerHTML = '<h4 style="margin: 10px 0 5px 0; color: #00ccff;">Bridge Status</h4>';
        debugContainer.appendChild(bridgeSection);
        
        const actionsSection = document.createElement('div');
        actionsSection.id = 'actions-debug';
        actionsSection.innerHTML = '<h4 style="margin: 10px 0 5px 0; color: #00ccff;">Actions</h4>';
        ...
        toggleAllButton.style.cssText = 'background: #4CAF50; color: white; border: none; padding: 5px 10px; margin: 5px; border-radius: 3px; cursor: pointer;';
        ...
        reloadButton.style.cssText = 'background: #2196F3; color: white; border: none; padding: 5px 10px; margin: 5px; border-radius: 3px; cursor: pointer;';
        reloadButton.onclick = () => location.reload();
        actionsSection.appendChild(reloadButton);
        
        const closeButton = document.createElement('button');
        closeButton.innerText = 'Close Debug';
        closeButton.style.cssText = 'background: #f44336; color: white; border: none; padding: 5px 10px; margin: 5px; border-radius: 3px; cursor: pointer;';
        ...
        toggleVisButton.style.cssText = 'background: #9c27b0; color: white; border: none; padding: 5px 10px; margin: 5px; border-radius: 3px; cursor: pointer;';
        ...
        logSection.innerHTML = '<h4 style="margin: 10px 0 5px 0; color: #00ccff;">Log</h4>';
    ...
    function checkFeatureToggles() {
        ...
        if (toggleSection) {
            toggleSection.innerHTML = '<h4 style="margin: 5px 0; color: #00ccff;">Feature Toggles</h4>';
            ...
            const sfcFeatures = [
                'useSfcDocConverter', 'useSfcAdmin', 'useSfcChat', 'useSfcSettings',
                'usePiniaAuth', 'usePiniaUI', 'usePiniaSessions', 'usePiniaSettings',
                'useLegacyBridge'
            ];
            
            sfcFeatures.forEach(feature => {
                const isEnabled = featureToggles[feature] === true;
                const isDependent = ['usePiniaAuth', 'usePiniaUI', 'usePiniaSessions', 'usePiniaSettings'].includes(feature);
    ...
    function checkMountPoints() {
        ...
        if (mountSection) {
            mountSection.innerHTML = '<h4 style="margin: 10px 0 5px 0; color: #00ccff;">Mount Points</h4>';
            ...
            createMountButton.style.cssText = 'background: #ff9800; color: white; border: none; padding: 3px 8px; margin: 2px; border-radius: 3px; cursor: pointer; font-size: 11px;';
    ...
    function checkBridgeStatus() {
        ...
        if (bridgeSection) {
            ...
            triggerBridgeButton.style.cssText = 'background: #ff9800; color: white; border: none; padding: 3px 8px; margin: 2px; border-radius: 3px; cursor: pointer; font-size: 11px;';
            ...
            initBridgeButton.style.cssText = 'background: #ff5722; color: white; border: none; padding: 3px 8px; margin: 2px; border-radius: 3px; cursor: pointer; font-size: 11px;';
    ...
    function createMissingMountPoints() {
        ...
        mountPoints.forEach(point => {
            if (!document.getElementById(point.id)) {
                ...
                mountPoint.style.cssText = 'min-height: 50px; border: 1px dashed #ff9800; margin: 10px 0; position: relative;';
    ...
    function toggleFeature(featureName) {
        try {
            ...
            if (featureName === 'useSfcDocConverter') {
                ...
                logDebug(`localStorage.useVueDocConverter set to: ${toggles[featureName]}`);
            }
            if (['useSfcAdmin', 'useSfcChat', 'useSfcSettings'].includes(featureName)) {
    ...
    function initializeBridge() {
        try {
            ...
            window.nscale.featureToggles = window.nscale.featureToggles || JSON.parse(localStorage.getItem('featureToggles') || '{}');
            ...
            window.nscaleAuth = window.nscaleAuth || {
                ...
                getUser: () => ({ id: 'debug-user', email: 'debug@example.com', role: 'admin' })
            };
    ...
    function runDiagnostics() {
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.type === 'childList' && 
                    ['vue-app', 'vue-dms-app', 'doc-converter-mount', 'admin-mount'].includes(mutation.target.id)) {
        ...
        ['vue-app', 'vue-dms-app', 'doc-converter-mount', 'admin-mount'].forEach(id => {
        ...
        localStorage.setItem = function(key, value) {
            if (['featureToggles', 'useVueComponents', 'useVueDocConverter'].includes(key)) {
        ...
        logDebug(`- SFC Features: ${toggles.useSfcAdmin ? '✓' : '✕'} Admin, ${toggles.useSfcDocConverter ? '✓' : '✕'} DocConverter`);
        logDebug(`- localStorage Flags: ${vueComponentsFlag === 'true' ? '✓' : '✕'} Components, ${vueDocConverterFlag === 'true' ? '✓' : '✕'} DocConverter`);
        logDebug(`- Bridge API: ${window.nscaleAuth ? '✓' : '✕'} Auth, ${window.nscaleUI ? '✓' : '✕'} UI`);
    ...
    console.log('Vue 3 SFC Diagnose-Tool wurde initialisiert. Alt+D,E,B,U,G zum Aktivieren.');
})();