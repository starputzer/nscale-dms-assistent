...
(function () {
    ...
    try {
      ...
      mountPoints.forEach((point) => {
        if (!document.getElementById(point.id)) {
          console.log(
            `Mount-Punkt '${point.id}' nicht gefunden, wird erstellt...`,
          );

          // Elternelement finden (oder body als Fallback)
          const parentElement =
            document.getElementById(point.parent) || document.body;
  ...
  function initializeComponents() {
    ...
    if (!setupFeatureFlags()) {
      console.error(
        "Fehler beim Einrichten der Feature-Flags. Die Initialisierung wird fortgesetzt...",
      );
    }
    ...
    if (!setupMountPoints()) {
      console.error(
        "Fehler beim Einrichten der Mount-Punkte. Die Initialisierung wird fortgesetzt...",
      );
    }
    ...
    if (!setupBridge()) {
      console.error(
        "Fehler beim Einrichten der Bridge. Die Initialisierung wird fortgesetzt...",
      );
    }
    ...
    if (!initializeVueApp()) {
      console.error(
        "Fehler bei der Vue-Initialisierung. Die Komponenten wurden möglicherweise nicht geladen.",
      );
    }
  }
  ...
  console.log(
    "Vue 3 SFC Loader Fix wurde installiert. Verwenden Sie window.vueLoaderFix für Debug-Funktionen.",
  );
})();
