...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        ...
        debugInfo.innerHTML = '<h3>Chat Debug-Panel</h3><div id="debug-content"></div>';
        ...
        function updateDebugInfo() {
            if (!window.app) {
                debugContent.innerHTML = '<p style="color: red">App nicht gefunden!</p>';
                return;
            }
            
            // Prüfe, ob Vue-Instanz verfügbar ist
            if (!window.app._instance) {
                debugContent.innerHTML = '<p style="color: yellow">Vue-Instanz noch nicht initialisiert</p>';
                return;
            }
            ...
            const token = state.token?.value || localStorage.getItem('token') || 'Nicht gefunden';
            const userRole = state.userRole?.value || localStorage.getItem('userRole') || 'Nicht gefunden';
        ...
        function directApiCall() {
            ...
            .then(data => {
                showResult(`API-Antwort: ${JSON.stringify(data).substring(0, 100)}...`);
        ...
        function showResult(message, isError = false) {
            ...
            resultElem.style.backgroundColor = isError ? 'rgba(255,0,0,0.1)' : 'rgba(0,255,0,0.1)';
