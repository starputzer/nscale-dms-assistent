...
export function setupSettings(options) {
    ...
    if ("reduceMotion" in settings) {
      ...
      if (settings.reduceMotion !== accessibilitySettings.value.reduceMotion) {
        ...
        if (settings.reduceMotion) {
          console.log(
            "Entferne Event-Listener vor Aktivierung von reduceMotion",
          );
          document.removeEventListener("click", handleOutsideClick);
          document.removeEventListener("keydown", handleEscapeKey);
          ...
          setTimeout(() => {
            if (showSettingsPanel.value) {
              console.log(
                "Füge Event-Listener nach Aktivierung von reduceMotion wieder hinzu",
              );
              document.addEventListener("click", handleOutsideClick);
              document.addEventListener("keydown", handleEscapeKey);
            }
  ...
  const applySimpleLanguageSettings = (useSimpleLanguage) => {
    console.log(
      `Einfache Sprache ${useSimpleLanguage ? "aktiviert" : "deaktiviert"}`,
    );
  ...
  Vue.watch(
    () => accessibilitySettings.value.reduceMotion,
    (newValue, oldValue) => {
  ...
  Vue.watch(
    () => accessibilitySettings.value.simpleLanguage,
    (newValue, oldValue) => {
      if (newValue !== oldValue) {
