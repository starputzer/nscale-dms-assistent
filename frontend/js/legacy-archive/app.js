...
createApp({
  setup() {
    ...
    const logout = () => {
      // Wenn noch eine aktive Session vorhanden ist, erfasse die Dauer für A/B Tests
      if (currentSessionId.value) {
        trackSessionDuration(currentSessionId.value);
      }
    ...
    const loadSession = async (sessionId) => {
      try {
        // Wenn es eine aktuelle Session gibt, erfasse die Sitzungsdauer für A/B Tests
    ...
    const loadSessions = async () => {
      try {
        ...
        if (
          ...
          newSessions.length === sessions.value.length
        ) {
          for (let i = 0; i < sessions.value.length; i++) {
            if (sessions.value[i].title !== newSessions[i].title) {
              titlesChanged = true;
              console.log(
                `Titel für Session ${sessions.value[i].id} hat sich geändert: "${sessions.value[i].title}" -> "${newSessions[i].title}"`,
              );
              break;
            }
          }
    ...
    const updateSessionTitle = async (sessionId) => {
      ...
      try {
        ...
        if (response.data && response.data.new_title) {
          console.log(
            `Session ${sessionId} Titel aktualisiert zu: "${response.data.new_title}"`,
          );
    ...
    window.addEventListener("beforeunload", () => {
      ...
      if (currentSessionId.value) {
        ...
        // A/B Test: Sitzungsdauer erfassen, wenn die Seite geschlossen wird
        trackSessionDuration(currentSessionId.value);
      }
    });
    ...
    onMounted(async () => {
      ...
      initializeABTests();
      console.log("A/B Tests wurden initialisiert");
