...
const ErrorMessages = {
  ...
  TIMEOUT:
    "Die Verbindung zum Server hat zu lange gedauert. Bitte versuchen Sie es später erneut.",
  ...
  STREAM_INTERRUPT:
    "Die Verbindung wurde unterbrochen. Versuche, die Verbindung wiederherzustellen...",
  STREAM_RECONNECT: "Verbindung wiederhergestellt. Setze Streaming fort...",
  STREAM_ERROR:
    "Es ist ein Fehler beim Streaming aufgetreten. Die Antwort könnte unvollständig sein.",
...
export function setupEnhancedChat(options) {
  ...
  const resetStreamTimeout = () => {
    ...
    streamTimeout = setTimeout(() => {
      if (isStreaming.value) {
        console.warn(
          `Stream inaktiv für ${CONFIG.STREAM_TIMEOUT_MS / 1000}s, bisher ${tokenCount} Tokens empfangen`,
        );
      ...
      errorHandler.handleError(error, {
        ...
        handlerOptions: {
          ...
          context: {
            action: "loadSessions",
            message:
              "Fehler beim Laden der aktualisierten Sitzungen nach Stream-Ende",
          },
        },
      });
    }
    ...
    const healthCheckInterval = setInterval(() => {
      ...
      if (isStreaming.value && eventSource.value) {
        ...
        if (timeSinceLastToken > CONFIG.STREAM_HEALTH_CHECK_MS) {
          console.warn(
            `Stream-Gesundheitscheck: Kein Token seit ${Math.round(timeSinceLastToken / 1000)}s`,
          );

          // Wenn der Stream bereits länger inaktiv ist, aber noch nicht das Timeout erreicht hat,
          // zeige eine Warnung an
          if (
            timeSinceLastToken > CONFIG.STREAM_HEALTH_CHECK_MS * 2 &&
            timeSinceLastToken < CONFIG.STREAM_TIMEOUT_MS
          ) {
            // Warnhinweis protokollieren
            errorHandler.handleError(
              new Error(
                `Stream ist seit ${Math.round(timeSinceLastToken / 1000)}s inaktiv`,
              ),
              {
                category: ErrorCategory.NETWORK,
                severity: ErrorSeverity.WARNING,
  ...
  const sendQuestionStream = async () => {
    ...
    if (!question.value.trim()) {
      errorHandler.handleError(new Error("Keine Frage eingegeben"), {
        category: ErrorCategory.APP,
    ...
    if (!currentSessionId.value) {
      errorHandler.handleError(new Error("Keine Session ausgewählt"), {
        category: ErrorCategory.APP,
        severity: ErrorSeverity.WARNING,
        handlerOptions: {
          showUser: true,
          context: {
            ...
            message:
              "Bitte wählen Sie eine Session aus oder erstellen Sie eine neue Session.",
          },
        },
      });
      return;
    ...
    if (!navigator.onLine) {
      ...
      errorHandler.handleError(
        new Error(
          "Offline-Modus: Frage wird gespeichert und gesendet, sobald die Verbindung wiederhergestellt ist.",
        ),
        {
          category: ErrorCategory.NETWORK,
          severity: ErrorSeverity.INFO,
  ...
  const processToken = (token) => {
    ...
    if (
      tokenCount === CONFIG.PARTIAL_TOKEN_THRESHOLD ||
      tokenCount % 50 === 0
    ) {
  ...
  const handleSpecialEventFlags = (eventData) => {
    ...
    if (
      eventData.startsWith("[FINAL_TIMEOUT]") ||
  ...
  const checkResponseQuality = () => {
    ...
    if (elapsedTime > 10000 && tokenCount < 10) {
      console.warn(
        `Niedrige Antwortqualität: ${tokenCount} Tokens in ${Math.round(elapsedTime / 1000)}s`,
      );

      // Warnung protokollieren
      errorHandler.handleError(
        new Error(
          `Langsame Stream-Antwort: ${tokenCount} Tokens in ${Math.round(elapsedTime / 1000)}s`,
        ),
        {
          category: ErrorCategory.NETWORK,
          severity: ErrorSeverity.WARNING,
  ...
  const enqueueOfflineQuestion = (questionText) => {
    ...
    messages.value.push({
      is_user: false,
      message:
        "*Diese Frage wird gesendet, sobald die Verbindung wiederhergestellt ist.*",
      timestamp: Date.now() / 1000,
      offline: true,
    });
  ...
  const processOfflineQueue = async () => {
    ...
    try {
      ...
      console.error(
        "Fehler bei der Verarbeitung der Offline-Warteschlange:",
        error,
      );
    ...
    try {
      ...
      const response = await apiClient.post(
        ...
        },
        {
          ...
          maxRetries: 2,
          retryDelay: 1000,
          headers: window.useSimpleLanguage
        ...
        errorHandler.handleError(error, {
          ...
          handlerOptions: {
            ...
            context: {
              action: "loadSessions",
              message:
                "Fehler beim Laden der aktualisierten Sitzungen nach Antwort",
            },
          },
        });
      }
  ...
  window.addEventListener("online", async () => {
    console.log(
      "Verbindung wiederhergestellt, verarbeite Offline-Warteschlange...",
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    ...
    if (window.app) {
      ...
      console.log(
        "Erweiterte Chat-Funktionalität mit verbesserter Fehlerbehandlung aktiviert",
      );
    }
  }, 800);
});
