...
export function setupChat(options) {
    ...
    const cleanupStream = () => {
        ...
        if (streamPerformance.startTime > 0) {
            ...
            PerformanceMetrics.endMeasure('stream_performance', 'stream_' + Date.now(), {
                avgProcessTime: streamPerformance.processingTimes.length > 0 
                    ? streamPerformance.processingTimes.reduce((sum, time) => sum + time, 0) / streamPerformance.processingTimes.length
                    : 0
            });
    ...
    const sendQuestionStream = async () => {
        ...
        try {
            ...
            // Performance-Messung beenden
            PerformanceMetrics.endMeasure('question_stream', 'question_stream_error', {
                error: error.message,
                tokenCount
            });
        }
    ...
    const sendQuestion = async () => {
        ...
        try {
            ...
            const response = await AsyncOptimization.post('/api/question', {
                question: question.value,
                session_id: currentSessionId.value
            }, requestOptions);
            ...
            // Performance-Messung beenden
            PerformanceMetrics.endMeasure('question_no_stream', 'question_no_stream_complete', {
            ...
            // Performance-Messung beenden
            PerformanceMetrics.endMeasure('question_no_stream', 'question_no_stream_error', {
                error: error.message
            });
        } finally {
            isLoading.value = false;
