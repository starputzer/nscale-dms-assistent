...
const AsyncOptimization = (function() {
    ...
    class RequestQueue {
         ...
         */
        enqueue(request) {
            const requestId = request.id || `${this.id}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            
            // Wenn eine identische Anfrage bereits läuft, diese wiederverwenden
            if (this.pendingRequests.has(requestId)) {
                if (config.debug) {
                    console.log(`[AsyncOptimization] Reusing pending request: ${requestId}`);
                }
        ...
        processNextRequest() {
            ...
            if (config.debug) {
                console.log(`[AsyncOptimization] Processing request: ${id} (${this.processing}/${this.config.concurrentRequests})`);
            }
            ...
            this.executeRequest(request, id, abortController.signal)
                ...
                .catch(error => {
                    ...
                    if (request.retry && request.retryCount < (request.maxRetries || this.config.maxRetries)) {
                        const retryCount = (request.retryCount || 0) + 1;
                        const retryDelay = this.calculateRetryDelay(retryCount, request);
                        
                        if (config.debug) {
                            console.log(`[AsyncOptimization] Retrying request: ${id} (${retryCount}/${request.maxRetries || this.config.maxRetries}) after ${retryDelay}ms`);
                        }
        ...
        async executeRequest(request, requestId, signal) {
            try {
                const { url, method = 'GET', body, headers = {}, responseType = 'json' } = request;
                
                if (config.debug) {
                    console.log(`[AsyncOptimization] Executing ${method} request: ${url}`);
                }
    ...
    function batchRequests(requests, options = {}) {
        ...
        return batches.reduce(async (prevPromise, batch, batchIndex) => {
            ...
            if (config.debug) {
                console.log(`[AsyncOptimization] Processing batch ${batchIndex + 1}/${batches.length} with ${batch.length} requests`);
            }
    ...
    function createRequestBatcher(batchId, batchFn, options = {}) {
        ...
        const processBatch = async () => {
            ...
            try {
                if (config.debug) {
                    console.log(`[AsyncOptimization] Processing batch ${batchId} with ${items.length} items`);
                }
                ...
                if (processedResults instanceof Map) {
                    ...
                    keys.forEach((key, index) => {
                        if (index < results.length) {
                            ...
                            promises.get(key).reject(new Error(`No result found for index: ${index}`));
                        }
                    });
                }
            } catch (error) {
    ...
    function preloadResource(url, options = {}) {
        ...
        if (preloadedResources.size > config.preloadCacheSize) {
            ...
            if (config.debug) {
                console.log(`[AsyncOptimization] Removed oldest preloaded resource: ${oldestKey}`);
            }
        }
                        ...
                        if (stopOnError) {
                            ...
                            if (completed === urls.length) {
                                ...
                                if (Object.keys(results).length === 0) {
                                    reject(new Error(`All ${urls.length} progressive loads failed`));
                                } else {
