...
createApp({
    setup() {
        ...
        const saveCurrentSessionToStorage = AsyncOptimization.debounce((sessionId) => {
            if (sessionId) {
        ...
        const login = async () => {
            try {
                ...
                const response = await AsyncOptimization.post('/api/auth/login', {
                    email: email.value,
        ...
        const resetPassword = async () => {
            try {
                ...
                const response = await AsyncOptimization.post('/api/auth/reset-password', {
                    email: email.value
                });
            ...
            try {
                ...
                for (let i = 0; i < sessions.value.length; i++) {
                    if (sessions.value[i].title !== sessionsData[i].title) {
                        titlesChanged = true;
                        console.log(`Titel für Session ${sessions.value[i].id} hat sich geändert: "${sessions.value[i].title}" -> "${sessionsData[i].title}"`);
                        break;
                    }
                }
        ...
        const formatMessage = DataOptimization.memoize((text) => {
            return marked.parse(text);
        }, (text) => `format_${text.substr(0, 50)}`, 60000); // 1 Minute Cache
        ...
        const updateSessionTitle = AsyncOptimization.debounce(async (sessionId) => {
            ...
            try {
                ...
                if (response && response.new_title) {
                    console.log(`Session ${sessionId} Titel aktualisiert zu: "${response.new_title}"`);
        ...
        const formatMotdContent = DataOptimization.memoize((content) => {
            if (!content) return '';
        ...
        const formatMessageWithSources = DataOptimization.memoize((text) => {
            if (!text) return '';
        ...
        }, (text) => `format_sources_${text?.substr(0, 50)}`, 60000); // 1 Minute Cache
        ...
        onMounted(async () => {
            ...
            PerformanceMetrics.measureScrollPerformance('#chatMessages', 'chat_scroll_performance');
            
            setupAxios();
            
            // Chat-Container-Element für virtualisiertes Rendering vorbereiten
            if (chatMessages.value) {
                messagesVirtualList = DataOptimization.initVirtualList('chat_messages', [], {
