...
const PerformanceMetrics = (function() {
    ...
    function startMeasure(metricName, details = {}) {
        ...
        if (measurements[metricName].length > config.maxMeasurements) {
            measurements[metricName] = measurements[metricName].slice(-config.maxMeasurements);
        }
        
        if (config.debug) {
            console.log(`[PerformanceMetrics] Started measuring '${metricName}'`, details);
        }
    ...
    function endMeasure(metricName, measureId, additionalDetails = {}) {
        if (!measurements[metricName]) {
            console.warn(`[PerformanceMetrics] No measurements found for '${metricName}'`);
            return null;
        }
        
        const endTime = performance.now();
        const measureIndex = measurements[metricName].findIndex(m => m.id === measureId);
        
        if (measureIndex === -1) {
            console.warn(`[PerformanceMetrics] Measurement with ID '${measureId}' not found for '${metricName}'`);
            return null;
        }
        ...
        if (config.debug) {
            console.log(`[PerformanceMetrics] Completed measuring '${metricName}': ${duration.toFixed(2)}ms`, result);
        }
    ...
    function measure(metricName, fn, details = {}) {
        ...
        try {
            ...
            const measurementData = endMeasure(metricName, measureId, additionalDetails);
    ...
    function getStats(metricName, timeWindow = 30000) {
        ...
        const relevantMeasurements = measurements[metricName]
            .filter(m => !m.inProgress && (now - m.endTime) <= timeWindow);
        ...
        if (relevantMeasurements.length === 0) {
            return {
                ...
                p95: 0,
                successRate: 100,
                inProgressCount: measurements[metricName].filter(m => m.inProgress).length
            };
        }
        
        const durations = relevantMeasurements.map(m => m.duration).sort((a, b) => a - b);
        const successCount = relevantMeasurements.filter(m => m.additionalDetails && m.additionalDetails.success !== false).length;
    ...
    function observeMetric(metricName, callback) {
        ...
        return () => {
            observers[metricName] = observers[metricName].filter(cb => cb !== callback);
        };
    }
    ...
    function notifyObservers(metricName, data) {
        ...
        observers[metricName].forEach(callback => {
            try {
                ...
                console.error(`[PerformanceMetrics] Error in observer for '${metricName}':`, error);
            }
        });
    }
        ...
        try {
            ...
            Object.keys(measurements).forEach(metricName => {
                config.avgIntervals.forEach(interval => {
                    allStats[`${metricName}_${interval}ms`] = getStats(metricName, interval);
                });
            });
