...
const TEST_DEFINITIONS = {
  ...
  "chat-interface-v2": {
    ...
    description:
      "Verbesserte Chat-Oberfläche mit optimierter Nachrichtenanzeige",
...
export function assignUserToTest(testId) {
  ...
  const existingAssignments = JSON.parse(
    localStorage.getItem("abTestAssignments") || "{}",
  );
  ...
  console.log(
    `Benutzer wurde für Test "${testId}" der Variante "${assignedVariant}" zugewiesen.`,
  );
 ...
 */
export function trackTestMetric(
  testId,
  metricName,
  value,
  additionalData = {},
  ...
  if (!variant) {
    console.warn(
      `Keine Testvariante für Test "${testId}" gefunden. Metrik wird nicht getrackt.`,
    );
    return;
  }
  ...
  if (!testDefinition.metrics.includes(metricName)) {
    console.warn(
      `Metrik "${metricName}" ist nicht für Test "${testId}" definiert.`,
    );
    return;
  }
  ...
  console.log(
    `Metrik "${metricName}" mit Wert ${value} für Test "${testId}" erfasst.`,
  );
}
...
export function initializeABTests() {
  ...
  Object.keys(TEST_DEFINITIONS).forEach((testId) => {
    ...
    if (!variant) {
      ...
      console.log(
        `Benutzer ist bereits für Test "${testId}" der Variante "${variant}" zugewiesen.`,
      );
