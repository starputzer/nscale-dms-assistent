  ...
  emit(event, data) {
    ...
    this.callbacks[event].forEach(({ callback, id }) => {
      try {
        ...
        console.error(`[Bridge] Fehler im Callback für ${event} (${id || 'anonym'}):`, error);
      }
    });
  },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof originalToggleAdminView === 'function') {
    window.toggleAdminView = () => {
      ...
      const newView = window.vueBridge.getStore('uiState.activeView') === 'chat' ? 'admin' : 'chat';
...
window.vueBridge.on('vue:ready', () => {
  ...
  const initialState = {
    activeView: document.body.classList.contains('activeView-admin') ? 'admin' : 'chat',
    theme: document.body.classList.contains('theme-dark') ? 'dark' : 'light',
