...
class ErrorHandler {
    ...
    setupGlobalHandlers() {
        ...
        window.addEventListener('unhandledrejection', (event) => {
            this.handleError(event.reason || new Error('Unbehandelte Promise-Ablehnung'), {
                category: ErrorCategory.APP,
                severity: ErrorSeverity.ERROR,
                context: {
                    reason: event.reason,
    ...
    handleError(error, options = {}) {
        ...
        if (mergedOptions.showNotification) {
            this.showNotification(this.getUserFriendlyMessage(error, category), severity);
        }
        ...
        if (mergedOptions.retry && typeof mergedOptions.retryCallback === 'function') {
    ...
    logError(error, category, severity, context) {
        ...
        switch (severity) {
            case ErrorSeverity.CRITICAL:
                console.error(`[KRITISCH][${category}][${timestamp}]`, error.message, errorInfo);
                break;
            case ErrorSeverity.ERROR:
                console.error(`[FEHLER][${category}][${timestamp}]`, error.message, errorInfo);
                break;
            case ErrorSeverity.WARNING:
                console.warn(`[WARNUNG][${category}][${timestamp}]`, error.message, errorInfo);
                break;
            case ErrorSeverity.INFO:
                console.info(`[INFO][${category}][${timestamp}]`, error.message, errorInfo);
                break;
            default:
                console.log(`[${severity}][${category}][${timestamp}]`, error.message, errorInfo);
        }
    }
        ...
        switch (category) {
            case ErrorCategory.NETWORK:
                ...
                if (error.message.includes('timeout') || error.message.includes('Timeout')) {
                    userMessage = 'Die Verbindung zum Server hat zu lange gedauert. Bitte überprüfen Sie Ihre Internetverbindung und versuchen Sie es erneut.';
                } else if (error.message.includes('Network Error')) {
                    userMessage = 'Es konnte keine Verbindung zum Server hergestellt werden. Bitte überprüfen Sie Ihre Internetverbindung.';
                } else {
                    userMessage = 'Ein Netzwerkfehler ist aufgetreten. Bitte überprüfen Sie Ihre Internetverbindung und versuchen Sie es erneut.';
                }
                break;
                ...
                if (error.response) {
                    ...
                    if (userMessage === error.message) {
                        switch (error.response.status) {
                            case 400:
                                userMessage = 'Die Anfrage konnte nicht verarbeitet werden. Bitte überprüfen Sie Ihre Eingaben.';
                                break;
                            case 404:
                            ...
                            case 500:
                                ...
                                break;
                            case 503:
                                userMessage = 'Der Service ist derzeit nicht verfügbar. Bitte versuchen Sie es später erneut.';
                                break;
                            default:
                    ...
                    userMessage = 'Ein Fehler bei der Kommunikation mit dem Server ist aufgetreten. Bitte versuchen Sie es später erneut.';
                }
                break;
            ...
            case ErrorCategory.AUTH:
                ...
                if (error.response && error.response.status === 401) {
                    ...
                    userMessage = 'Ein Authentifizierungsfehler ist aufgetreten. Bitte melden Sie sich erneut an.';
                }
                break;
                
            case ErrorCategory.DATA:
                // Datenverarbeitungsfehler
                userMessage = 'Es ist ein Fehler bei der Verarbeitung der Daten aufgetreten. Bitte versuchen Sie es erneut.';
                break;
                
            case ErrorCategory.APP:
                // Anwendungsfehler
                userMessage = 'Es ist ein interner Anwendungsfehler aufgetreten. Bitte aktualisieren Sie die Seite und versuchen Sie es erneut.';
                break;
                
            case ErrorCategory.UNKNOWN:
            default:
                // Unbekannte Fehler
                userMessage = 'Ein unerwarteter Fehler ist aufgetreten. Bitte versuchen Sie es später erneut.';
        }
    ...
    startRetry(error, options) {
        ...
        if (currentAttempt >= options.maxRetries) {
            console.warn(`Maximale Anzahl an Wiederholungsversuchen (${options.maxRetries}) erreicht.`);
            return;
        }
        ...
        }, options.retryDelay * (currentAttempt + 1)); // Exponentielles Backoff
    }
