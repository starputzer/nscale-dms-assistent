...
(function() {
    ...
    function initCharts() {
        ...
        domChart = new Chart(
            document.getElementById('dom-chart'),
            {
                ...commonConfig,
                data: {
                    labels,
    ...
    function checkPerformanceIssues(metrics) {
        ...
        if (metrics.domOps > 100) {
            showNotification(`Hohe DOM-Aktivität: ${Math.round(metrics.domOps)} Ops/s`);
        }
        ...
        if (metrics.memory > 100) {
            showNotification(`Hohe Speichernutzung: ${Math.round(metrics.memory)} MB`);
        }
    }
        ...
        if (window.PerformanceObserver) {
            try {
                const observer = new PerformanceObserver((list) => {
                    ...
                    while (window._networkRequests.length > 0 && window._networkRequests[0] < expireTime) {
                ...
                console.error('PerformanceObserver für Netzwerkanfragen konnte nicht initialisiert werden:', e);
            }
        }
