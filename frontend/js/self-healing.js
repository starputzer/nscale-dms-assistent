...
class SelfHealing {
  ...
  setupConnectionHandlers() {
    ...
    window.addEventListener("online", () => {
      console.log(
        "Verbindung wiederhergestellt, prüfe auf ausstehende Operationen...",
      );
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "visible") {
        console.log(
          "Tab wurde wieder aktiv, prüfe auf Aktualisierungsbedarf...",
        );
  ...
  handleConnectionLost() {
    ...
    errorHandler.handleError(
      new Error(
        "Internetverbindung verloren. Änderungen werden gespeichert und synchronisiert, sobald die Verbindung wiederhergestellt ist.",
      ),
      {
  ...
  handleVisibilityRestore() {
    ...
    if (inactiveTime > 5 * 60 * 1000) {
      // 5 Minuten
      console.log(
        `Seite war ${Math.round(inactiveTime / 1000 / 60)} Minuten inaktiv, aktualisiere Daten...`,
      );
      this.refreshApplicationState();
    }
      ...
      const state = {
        ...
        appState:
          window.app && window.app.$data
            ? this.extractAppState(window.app.$data)
            : null,
      };
  ...
  restoreState(context = "default", index = 0) {
    try {
      ...
      if (
        !this.stateStore.has(context) ||
        this.stateStore.get(context).length <= index
      ) {
        console.warn(
          `Kein gespeicherter Zustand für Kontext '${context}' mit Index ${index} vorhanden`,
        );
        return false;
      }
  ...
  restoreAppState(appState) {
    ...
    if (!app || !app.loadSessions || !app.loadSession) {
      console.warn(
        "App-Funktionen für die Zustandswiederherstellung nicht verfügbar",
      );
      return;
    }
    ...
    app
      ...
      .then(() => {
        ...
        if (
          ...
          app.$data.sessions.some((s) => s.id === appState.currentSessionId)
        ) {
  ...
  refreshApplicationState() {
    ...
    if (
      window.app.$data &&
      window.app.$data.currentSessionId &&
      window.app.loadSession
    ) {
      window.app
        .loadSession(window.app.$data.currentSessionId)
        .catch((error) => {
  ...
  async heal(options) {
    ...
    if (attempts >= maxAttempts) {
      console.warn(
        `Maximale Anzahl an Heilungsversuchen (${maxAttempts}) für Kontext ${contextKey} erreicht`,
      );

      // Callback aufrufen, wenn vorhanden
      if (onFailure) {
        onFailure(
          new Error(`Maximale Anzahl an Heilungsversuchen erreicht`),
          attempts,
        );
      }
    ...
    if (showIndicator) {
      this.showHealingIndicator(
        this.getHealingMessage(strategy, attempts + 1, maxAttempts),
      );
    }
  ...
  async applyRestoreLocalStrategy(context) {
    ...
    const success = this.restoreState(
  ...
  async applyRestoreServerStrategy(context) {
    ...
    if (!context.sessionId) {
      throw new Error(
        "Keine Session-ID für Wiederherstellung vom Server angegeben",
      );
    }
  ...
  async applyReinitializeSessionStrategy() {
    ...
    if (!window.app || !window.app.startNewSession) {
      throw new Error(
        "App-Funktionen für Session-Neuinitialisierung nicht verfügbar",
      );
    }

    // Aktuelle Session-ID speichern
    const currentSessionId = window.app.$data
      ? window.app.$data.currentSessionId
      : null;
  ...
  async applyRerenderUIStrategy() {
    ...
    // Aktuellen Zustand speichern
    const currentState = window.app.$data
      ? this.extractAppState(window.app.$data)
      : null;
