...
(function() {
  ...
  function createTestPanel() {
    ...
    header.style.cssText = `
      ...
      padding-bottom: 5px;
    `;
    header.innerHTML = '<span>Interaktivitätstest</span><button id="test-panel-toggle" style="background: none; border: none; color: #0f0; cursor: pointer;">Schließen</button>';
    ...
    buttons.appendChild(createTestButton('Einstellungsbutton testen', testSettingsButton));
    buttons.appendChild(createTestButton('Admin-Tabs testen', testAdminTabs));
    buttons.appendChild(createTestButton('Chat-Interaktion testen', testChatInteraction));
    buttons.appendChild(createTestButton('Event-Handler prüfen', testEventHandlers));
    buttons.appendChild(createTestButton('Feature-Toggles anzeigen', showFeatureToggles));
    buttons.appendChild(createTestButton('Vue-Mount überprüfen', testVueMount));
    buttons.appendChild(createTestButton('Fehlersuche starten', diagnosticMode));
  ...
  function testSettingsButton() {
    ...
    logToPanel(`✓ ${settingsButtons.length} Einstellungsbutton(s) gefunden`, 'success');
    ...
    settingsButtons.forEach(button => {
      ...
      if (btn && (btn.onclick || btn._repairHandled || btn.getAttribute('onclick'))) {
    ...
    if (!hasEventListeners) {
      logToPanel('⚠️ Keine Event-Listener auf Einstellungsbuttons gefunden', 'warning');
      return false;
    }
    
    // Einstellungsdialog prüfen
    const settingsDialog = document.querySelector('.settings-dialog, .settings-dialog-overlay, .settings-panel');
  ...
  function testAdminTabs() {
    ...
    adminTabButtons.forEach(button => {
      if (button.onclick || button._repairHandled || button.getAttribute('onclick')) {
        hasEventListeners = true;
  ...
  function testChatInteraction() {
    ...
    if (chatForm.onsubmit || chatForm._repairHandled || chatForm.getAttribute('onsubmit')) {
      logToPanel('✓ Chat-Formular hat Event-Listener', 'success');
    } else {
      logToPanel('⚠️ Kein Event-Listener auf Chat-Formular gefunden', 'warning');
    }
    
    // Nachrichtenliste prüfen
    const messageContainer = document.querySelector('.message-container, .messages, [class*="message-list"]');
  ...
  function testEventHandlers() {
    ...
    if (window.nscale && window.nscale.events) {
      ...
      if (typeof window.nscale.events.on === 'function' && typeof window.nscale.events.emit === 'function') {
  ...
  function showFeatureToggles() {
    ...
    if (window.nscale && window.nscale.featureToggles) {
      ...
      try {
        ...
        logToPanel(`❌ Fehler beim Lesen aus localStorage: ${e.message}`, 'error');
      }
    }
  }
  ...
  function testVueMount() {
    ...
    if (mountPoint) {
      ...
      if (mountPoint.children.length > 0) {
        logToPanel(`✓ Mount-Punkt hat ${mountPoint.children.length} Kind(er)`, 'success');
      } else {
  ...
  function diagnosticMode() {
    ...
    const diagnostics = [
      {
        ...
        solution: 'Stellen Sie sicher, dass die Einstellungs-Handler korrekt geladen sind. Sie können die Aktion auch direkt über den "toggleSettings"-Befehl in der Konsole auslösen.'
      },
      {
        test: () => testAdminTabs(),
        message: 'Admin-Tabs reagieren nicht auf Klicks',
        solution: 'Prüfen Sie, ob die Tab-Handler korrekt geladen sind. Sie können die Tabs auch direkt über den "switchAdminTab"-Befehl in der Konsole ändern.'
      },
      {
        test: () => testChatInteraction(),
        message: 'Chat-Interaktion funktioniert nicht',
        solution: 'Stellen Sie sicher, dass das Chat-Formular einen Submit-Handler hat. Sie können Nachrichten auch direkt über den "sendMessage"-Befehl in der Konsole senden.'
      },
      {
        test: () => window.nscale && window.nscale.events,
        message: 'Bridge-Objekt fehlt',
        solution: 'Stellen Sie sicher, dass bridge-fix.js korrekt geladen wurde. Sie können das Bridge-Objekt auch manuell in der Konsole initialisieren.'
      },
      {
        test: () => typeof Vue !== 'undefined',
        message: 'Vue ist nicht definiert',
        solution: 'Stellen Sie sicher, dass die Vue-Bibliothek korrekt geladen wurde. Prüfen Sie die Netzwerkanfragen auf Fehler beim Laden von Vue.'
      },
      {
        test: () => document.getElementById('vue-dms-app'),
        message: 'Vue-Mount-Punkt fehlt',
        solution: 'Stellen Sie sicher, dass das Element mit der ID "vue-dms-app" im DOM vorhanden ist.'
      },
      {
        test: () => window.nscale && window.nscale.featureToggles && window.nscale.featureToggles.useSfcAdmin,
        message: 'SFC-Admin-Feature ist deaktiviert',
        solution: 'Aktivieren Sie das Feature über localStorage oder verwenden Sie den Befehl "window.nscale.debug.toggleFeature(\'useSfcAdmin\', true)" in der Konsole.'
      }
    ];
    ...
    if (foundProblems === 0) {
      logToPanel('✓ Keine bekannten Probleme gefunden. Alle Tests bestanden!', 'success');
    } else {
      logToPanel(`❌ ${foundProblems} Problem(e) gefunden. Bitte die vorgeschlagenen Lösungen anwenden.`, 'error');
    }
    
    // DOM-Tiefe überprüfen
    const maxDepth = calculateDOMDepth(document.body);
    if (maxDepth > 20) {
      logToPanel(`⚠️ Die DOM-Tiefe von ${maxDepth} ist sehr hoch, was Performance-Probleme verursachen könnte.`, 'warning');
    } else {
      logToPanel(`✓ DOM-Tiefe: ${maxDepth} ist im normalen Bereich.`, 'success');
    }
  ...
  function calculateDOMDepth(element, currentDepth = 0) {
    ...
    for (let i = 0; i < element.children.length; i++) {
      const childDepth = calculateDOMDepth(element.children[i], currentDepth + 1);
  ...
  document.addEventListener('keydown', function(e) {
    if (e.altKey && e.key === 't') {
      const panel = document.getElementById('interactivity-test-panel') || createTestPanel();
  ...
  setTimeout(() => {
    createTestPanel();
    logToPanel('Interaktivitätstest geladen. Drücke Alt+T zum Ein-/Ausblenden.', 'info');
  }, 1000);
  
  console.log('Interaktivitätstest-Skript geladen. Drücke Alt+T zum Anzeigen des Test-Panels.');
})();