...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(event) {
        ...
        const settingsButton = closest(target, 'button[class*="settings"], button:has(i.fa-cog), button:has(i.fa-gear)');
        if (settingsButton || target.matches('i.fa-cog, i.fa-gear')) {
            ...
            if (settingsDialog) {
                ...
                console.log('Einstellungsdialog umgeschaltet auf: ' + (settingsVisible ? 'sichtbar' : 'versteckt'));
            }
            return;
        }
        
        // Schließen-Button im Einstellungsdialog
        const closeSettingsButton = closest(target, '.settings-dialog-header .close-button, .settings-dialog-header button:has(i.fa-times)');
        if (closeSettingsButton || (target.matches('i.fa-times') && closest(target, '.settings-dialog-header'))) {
        ...
        // 2. Admin-Tab-Handler
        const adminTabButton = closest(target, '.admin-tab-button, button[class*="admin-tab"]');
        if (adminTabButton || (target.matches('span') && closest(target, '.admin-tab-button'))) {
            ...
            if (button) {
                ...
                if (tabId.includes('benutzer') || tabId.includes('user')) tabId = 'users';
                if (tabId.includes('feedback')) tabId = 'feedback';
                if (tabId.includes('motd')) tabId = 'motd';
                if (tabId.includes('dokument') || tabId.includes('doc')) tabId = 'doc-converter';
                
                console.log('Admin-Tab-ID: ' + tabId);
                currentAdminTab = tabId;
                
                // Alle Tabs ausblenden und aktuellen Tab einblenden
                const allTabButtons = document.querySelectorAll('.admin-tab-button, button[class*="admin-tab"]');
                allTabButtons.forEach(btn => btn.classList.remove('active'));
                button.classList.add('active');
                
                const allTabContents = document.querySelectorAll('[class*="admin-tab-content"], [data-tab]');
        ...
        const feedbackButton = closest(target, '.feedback-button, button[class*="feedback"]');
        if (feedbackButton || (target.matches('i.fa-thumbs-up, i.fa-thumbs-down') && closest(target, '.feedback-button'))) {
        ...
        const closeFeedbackButton = closest(target, '.feedback-dialog-header .close-button, .feedback-dialog-header button:has(i.fa-times)');
        if (closeFeedbackButton || (target.matches('i.fa-times') && closest(target, '.feedback-dialog-header'))) {
        ...
        const adminToggleButton = closest(target, 'button:has(i.fa-tools), button:has(i.fa-comments)');
        if (adminToggleButton || (target.matches('i.fa-tools, i.fa-comments') && closest(target, 'button'))) {
            console.log('Admin-Toggle geklickt (direkter Handler)');
            event.preventDefault();
            
            // Chat-View und Admin-View umschalten
            const chatView = document.querySelector('[v-if="activeView === \'chat\'"]');
            const adminView = document.querySelector('[v-if="activeView === \'admin\'"]');
            ...
            if (chatView && adminView) {
                ...
                console.log('View umgeschaltet auf: ' + (isChatActive ? 'Admin' : 'Chat'));
    ...
    document.addEventListener('submit', function(event) {
        ...
        if (form.querySelector('input[type="text"]') && form.querySelector('button[type="submit"]')) {
            ...
            if (message) {
                ...
                const messagesContainer = document.querySelector('.flex-1.overflow-y-auto.p-4.space-y-4, .messages');
    ...
    document.addEventListener('keydown', function(event) {
        ...
        if (event.key === 'Escape') {
            ...
            const dialogs = document.querySelectorAll('.settings-dialog-overlay, .feedback-dialog-overlay');
        ...
        if (event.ctrlKey && event.shiftKey && event.key === 'A') {
            const adminToggleButton = document.querySelector('button:has(i.fa-tools), button:has(i.fa-comments)');
        ...
        if (event.ctrlKey && event.shiftKey && event.key === 'S') {
            const settingsButton = document.querySelector('button:has(i.fa-cog), button:has(i.fa-gear)');
            if (settingsButton) {
    ...
    window.toggleSettings = window.toggleSettings || function() {
        ...
        if (settingsDialog) {
            settingsDialog.style.display = settingsVisible ? 'flex' : 'none';
            console.log('Einstellungen umgeschaltet auf: ' + (settingsVisible ? 'sichtbar' : 'versteckt'));
        }
    };
    
    // Methode zum Wechseln des Admin-Tabs
    window.switchAdminTab = window.switchAdminTab || function(tabId) {
        const tabButton = document.querySelector(`.admin-tab-button:has(span:contains("${tabId}")), .admin-tab-button[data-tab="${tabId}"]`);
        if (tabButton) {
            tabButton.click();
            return true;
        }
    ...
    window.sendMessage = window.sendMessage || function(text) {
