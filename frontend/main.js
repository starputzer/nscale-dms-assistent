      ...
      import("./vue/app")
        .then(({ default: App }) => {
          ...
          if (appElement) {
            ...
            console.warn(
              "Vue-App konnte nicht initialisiert werden: #vue-app Element nicht gefunden",
            );
          }
        })
        .catch((error) => {
...
function initDocConverter() {
  ...
  if (useVueDocConverter && isDocConverterPage) {
    try {
      import("./vue/doc-converter-app")
        .then(({ default: DocConverterApp }) => {
        ...
        .catch((error) => {
          ...
          document
            .getElementById("legacy-doc-converter")
            ?.classList.remove("hidden");
        });
    } catch (error) {
      console.error(
        "Fehler bei der Dokumentenkonverter-Initialisierung:",
        error,
      );
      // Fallback zur Legacy-Version aktivieren
      document
