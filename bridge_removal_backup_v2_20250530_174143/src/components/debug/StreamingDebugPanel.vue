    ...
    <div v-if="isExpanded" class="panel-content">
      <div class="debug-section">
        ...
        <div class="status-row">
          ...
          <span
            class="status-value"
            :class="{ 'status-error': renderingIssue }"
          >
            {{ renderingStatus }}
          </span>
        </div>
      ...
      <div class="debug-section">
        ...
        <div class="status-row">
          ...
          <span
            class="status-value"
            :class="{ 'status-error': !hasMessageList }"
          >
        ...
        <div class="status-row">
          ...
          <span
            class="status-value"
            :class="{
              'status-error': visibleMessageCount === 0 && messageCount > 0,
            }"
          >
            {{ visibleMessageCount }}
          </span>
        ...
        <div class="status-row">
          ...
          <span
            class="status-value"
            :class="{ 'status-error': cssIssues.length > 0 }"
          >
...
const isStreaming = computed(() => {
  ...
  const streamingMsg = sessionsStore.allCurrentMessages.find(
    (m) => m.isStreaming,
  );
...
const renderingStatus = computed(() => {
  ...
  if (messageCount.value > 0 && visibleMessageCount.value === 0)
    return "No Messages Rendered";
...
const refreshDiagnostics = async () => {
  ...
  if (
    ...
    typeof window.EmergencyChatFixer.runDiagnostics === "function"
  ) {
    try {
...
const checkDOMManually = () => {
  ...
  const messageList = document.querySelector(
    ".message-list-container, .n-message-list",
  );
  hasMessageList.value = !!messageList;

  // Count visible messages
  if (messageList) {
    const messages = messageList.querySelectorAll(
      ".message-item, .n-message-item",
    );
    ...
    if (styles.visibility === "hidden")
      cssIssuesFound.push("visibility-hidden");
...
const forceMessageDisplay = () => {
  if (
    window.EmergencyChatFixer &&
    typeof window.EmergencyChatFixer.forceMessageDisplay === "function"
  ) {
...
const activateEmergencyMode = () => {
  if (
    window.EmergencyChatFixer &&
    typeof window.EmergencyChatFixer.activate === "function"
  ) {
