...
async function convertJsToTs() {
  ...
  try {
    await fs.copyFile("./frontend/js/app.js", "./frontend/js/app.ts");
    ...
    const applyColorThemePosition = appTsContent.indexOf(
...
`;
      ...
      appTsContent =
        appTsContent.slice(0, applyColorThemePosition) +
...
`;
  ...
  try {
    ...
    console.error(
      "Fehler beim Erstellen der Umgebungskonfigurationsdateien:",
      error,
    );
  }
}
...
async function main() {
  ...
  try {
    ...
    console.log(
      "Führe die folgenden Befehle aus, um die neue Build-Umgebung zu testen:",
    );
