...
program
  .option(
    "--reportsDir <dir>",
    "Directory containing report archives",
    "test-archives",
  )
  .option(
    "--outputDir <dir>",
      ...
      if (reportDate >= startDate && reportDate <= endDate) {
        ...
        if (fs.existsSync(reportFilePath)) {
          try {
            ...
            console.error(
              `Error parsing report file ${reportFilePath}:`,
              error.message,
            );
          }
        }
...
function analyzeCategoryTrends(reports) {
  ...
  reports.forEach((report) => {
    ...
    Object.entries(data.categories).forEach(([category, metrics]) => {
      ...
      categories[category].passRate.push(
        metrics.total > 0 ? (metrics.passed / metrics.total) * 100 : 0,
      );
    });
  });
        ...
        if (suite.assertionResults) {
          suite.assertionResults.forEach((test) => {
            ...
            if (!testResults[testId].results[dateStr]) {
              testResults[testId].results[dateStr] =
...
function analyzePerformanceTrends(reports) {
  ...
  reports.forEach((report) => {
    ...
    Object.entries(data.categories).forEach(([category, metrics]) => {
      ...
      performanceTrends.categories[category].duration.push(
        metrics.duration || 0,
      );
      ...
      const categoryAvgDuration =
        metrics.duration && metrics.total
          ? metrics.duration / metrics.total
          : 0;
      performanceTrends.categories[category].averageDuration.push(
        categoryAvgDuration,
      );
    });
  });
...
function detectSlowTests(reports) {
  ...
  const slowTests = Object.values(testTimings)
    ...
    .map((test) => {
      const totalDuration = test.durations.reduce(
  ...
  const htmlContent = `
    ...
    <body>
      ...
      <div class="grid">
        ...
        ${Object.values(performanceTrends.categories)
          .map(
            (category) => `
          <div class="chart-container">
