...
async function main() {
  ...
  if (options.function) {
    ...
    results = results.concat(
      await searchForFunction(options.function, searchDir),
    );
  }
  ...
  if (options.pattern) {
    ...
    results = results.concat(
      await searchForPattern(options.pattern, searchDir),
    );
  }
  ...
  if (uniqueResults.length === 0) {
    console.log(
      "\x1b[32m%s\x1b[0m",
      "✅ No redundancies found. You can proceed with creating a new implementation.",
    );
  } else {
    console.log(
      "\x1b[33m%s\x1b[0m",
      "⚠️  Potential redundancies found. Please review the following results before creating new code:\n",
    );
...
async function searchForType(typeName, searchDir) {
  ...
  for (const pattern of typePatterns) {
    const { stdout } = await exec(
      `grep -r "${pattern}" --include="*.ts" ${searchDir} || true`,
    );
  ...
  const { stdout: importStdout } = await exec(
    `grep -r "import.*{.*${typeName}" --include="*.ts" ${searchDir} || true`,
  );
...
async function searchForFunction(functionName, searchDir) {
  ...
  const { stdout: importStdout } = await exec(
    `grep -r "import.*{.*${functionName}" --include="*.ts" ${searchDir} || true`,
  );
  if (importStdout.trim()) {
    const matches = parseGrepOutput(importStdout);
    results = results.concat(
...
async function searchForFile(fileName, searchDir) {
  const { stdout } = await exec(
    `find ${searchDir} -type f -name "*${fileName}*.ts" -o -name "*${fileName}*.tsx" || true`,
  );
  ...
  if (stdout.trim()) {
    ...
    for (const file of files) {
      ...
      const { stdout: filePreview } = await exec(
...
function getPriorityForType(line, typeName) {
  ...
  if (
    line.includes(`extends ${typeName}`) ||
    line.includes(`implements ${typeName}`)
  ) {
    return 2;
  }
...
function getFilenameSimilarity(filename, searchTerm) {
  ...
  const matchingWords = searchWords.filter((word) =>
    filenameWords.some((fword) =>
      fword.toLowerCase().includes(word.toLowerCase()),
    ),
  );
...
function displayResults(results) {
  ...
  for (const result of results) {
    ...
    console.log(
      `  ${searchTypeIcon} Line ${result.lineNumber}: ${highlightText(result.line, result.searchTerm)}`,
    );
