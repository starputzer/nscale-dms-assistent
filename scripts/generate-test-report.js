...
// Define CLI options
program
  .option('--input <dir>', 'Input directory containing test results', 'all-artifacts')
          ...
          suite.assertionResults
            ...
            .forEach(test => {
              combinedData.failedTests.push({
                ...
                suite: suite.name,
                name: test.title,
                errorMessage: test.failureMessages ? test.failureMessages[0] : null,
              });
            });
        }
      });
        ...
        spec.tests.forEach(test => {
          if (test.results) {
            test.results.forEach(result => {
              ...
              } else if (result.status === 'skipped' && specStatus !== 'failed') {
                specStatus = 'skipped';
              }
            });
          }
...
function processPerformanceReport(filePath) {
  try {
    ...
    console.error(`Error processing Performance report ${filePath}:`, e.message);
    return false;
  }
}
    ...
    <body>
      ...
      ${combinedData.failedTests.length > 0 ? `
        ...
        <table>
          ...
          ${combinedData.failedTests.map(test => `
            <tr>
              <td>${test.category}</td>
              <td>${test.suite}</td>
...
function processTestResults() {
  ...
  const performanceReports = findTestResults(options.input, /performance.*\.json$/);
