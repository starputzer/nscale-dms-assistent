...
(function() {
    ...
    function loadFallbackImplementation() {
        ...
        try {
            ...
            // Verschiedene mögliche Pfade zum Fallback-Script
            const fallbackPaths = [
                '/static/js/doc-converter-fallback.js',
                '/static/js/vue/doc-converter-nomodule.js',
                ...
                '/frontend/vue/standalone/doc-converter-nomodule.js',
                '/api/static/js/doc-converter-fallback.js',
                '/api/static/js/vue/doc-converter-nomodule.js',
                '/api/static/vue/standalone/doc-converter-nomodule.js',
                './js/doc-converter-fallback.js',
                './js/vue/doc-converter-nomodule.js',
                './vue/standalone/doc-converter-nomodule.js'
            ];
            
            // Versuche, die Skripte zu laden (starting with the first)
            let loadedScript = false;
            let scriptIndex = 0;
            
            function tryNextScript() {
                if (scriptIndex >= fallbackPaths.length) {
                    console.error('Alle Fallback-Skripte fehlgeschlagen, zeige Minimal-UI');
                    showMinimalUI();
                    return;
                }
                
                const path = fallbackPaths[scriptIndex];
                console.log(`Versuche, Fallback-Skript zu laden: ${path}`);
                
                const script = document.createElement('script');
                script.src = path;
                
                script.onload = function() {
                    console.log(`Fallback-Skript erfolgreich geladen: ${path}`);
                    loadedScript = true;
                    
                    // Prüfe nach kurzer Verzögerung, ob die Funktion verfügbar ist
                    setTimeout(function() {
                        if (typeof window.initializeClassicDocConverter === 'function') {
                            console.log('initializeClassicDocConverter Funktion gefunden, aktiviere');
                            window.initializeClassicDocConverter();
                        } else {
                            console.warn(`Skript ${path} geladen, aber initializeClassicDocConverter nicht gefunden`);
                            // Versuche das nächste Skript, wenn die Funktion nicht vorhanden ist
                            scriptIndex++;
                            tryNextScript();
                        }
                    }, 100);
                ...
                script.onerror = function() {
                    console.error(`Fehler beim Laden von ${path}`);
                    scriptIndex++;
                    tryNextScript();
                };
                ...
                if (document.body) {
                    ...
                    // Falls document.body noch nicht existiert, verzögern
                    console.warn('document.body nicht verfügbar, verzögere das Einfügen des Fallback-Scripts');
        ...
        try {
            ...
            if (featureSettings.useNewUI && featureSettings.vueDocConverter) {
                ...
                try {
                    ...
                    // Versuche verschiedene mögliche Pfade für die Vue-Komponente
                    const possibleScriptPaths = [
                        ...
                        './js/vue/doc-converter.js',
                        './vue/standalone/doc-converter.js'
                    ];
                    
                    // Verwende den ersten Pfad standardmäßig
                    converterScript.src = possibleScriptPaths[0];
                    ...
                    if (document.body) {
                        ...
                        // Falls document.body noch nicht existiert, verzögern
                        console.warn('document.body nicht verfügbar, verzögere das Einfügen des Scripts');
                        document.addEventListener('DOMContentLoaded', function() {
                            document.body.appendChild(converterScript);
                        });
                    }
