...
(function() {
    ...
    function tryNextUrl(urls, index, callback) {
        ...
        // Für maximale Kompatibilität auch das API-Verzeichnis probieren
        const apiUrl = url.replace('/frontend/', '/api/static/').replace('/static/', '/api/static/');
        if (!document.querySelector(`script[src="${apiUrl}"]`) && apiUrl !== url) {
