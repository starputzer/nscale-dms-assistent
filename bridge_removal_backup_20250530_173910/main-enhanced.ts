...
async function preInitSessionRecovery() {
  ...
  try {
    ...
    console.error(
      "[MAIN-ENHANCED] Fehler bei Session-Wiederherstellung:",
      error,
    );
    return null;
  }
...
async function initializeLegacyCompatibility(app: any) {
  ...
  const { initializeSourceReferenceAdapter } = await import(
    "./utils/sourceReferenceAdapter"
  );

  // Überprüfen, ob Bridge-Modus aktiv ist
  const useLegacyBridge =
    new URLSearchParams(window.location.search).get("useBridge") === "true";
  ...
  const { useSourceReferences } = await import(
    "@/composables/useSourceReferences"
  );
  ...
  if (typeof (window as any).isSourceReferencesVisible !== "function") {
    (window as any).isSourceReferencesVisible = (message: any) => {
      try {
        return (
          emergencyComposable?.isSourceReferencesVisible?.(message) ?? false
        );
      } catch (error) {
        console.error(
          "Notfall-Fehlerbehandlung für isSourceReferencesVisible:",
          error,
        );
        return false;
      }
...
async function initializeApp() {
  try {
    console.log(
      "[MAIN-ENHANCED] Starte App-Initialisierung mit Auth-Persistenz",
    );
    ...
    const { mockServiceProvider } = await import(
      "./plugins/mockServiceProvider"
    );
    ...
    if (authStore.isAuthenticated) {
      ...
      if (lastRoute && lastRoute !== router.currentRoute.value.fullPath) {
        console.log(
          "[MAIN-ENHANCED] Navigiere zur letzten gespeicherten Route:",
          lastRoute,
        );
        try {
          ...
          console.warn(
            "[MAIN-ENHANCED] Konnte nicht zur letzten Route navigieren:",
            error,
          );
        }
      }
    ...
    // Pure Mode Indicator
    setTimeout(async () => {
      const { showPureModeIndicator } = await import(
        "./utils/pureModeIndicator"
      );
    ...
    console.log(
      "✅ [MAIN-ENHANCED] App erfolgreich mit Auth-Persistenz initialisiert",
    );
  } catch (error) {
    console.error(
      "❌ [MAIN-ENHANCED] Kritischer Fehler bei App-Initialisierung:",
      error,
    );
