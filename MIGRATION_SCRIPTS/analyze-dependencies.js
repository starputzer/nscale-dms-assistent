...
const legacyDirs = ["../app/frontend/js", "../app/api/frontend/js"];
...
function isLegacyImport(importPath, legacyPaths) {
  ...
  return legacyPaths.some((legacyPath) => {
    ...
    return (
      normalizedImport.includes(relativeLegacyPath) ||
...
function analyzeDependencies() {
  ...
  console.log(
    `Insgesamt: ${dependencies.legacyImportCount} Legacy-Import-Statements`,
  );

  // Dateien gruppieren nach Anzahl der Legacy-Importe
  const sortedFiles = Object.entries(dependencies.details).sort(
    (a, b) => b[1].legacyImports.length - a[1].legacyImports.length,
  );
  ...
  // In Datei speichern
  fs.writeFileSync(
    "legacy-dependencies-report.json",
...
function generateMigrationRecommendations(dependencies) {
  ...
  fs.writeFileSync(
    "migration-recommendations.json",
  ...
  console.log(
    `- Hohe Priorität: ${recommendations.highPriority.length} Dateien`,
  );
  console.log(
    `- Mittlere Priorität: ${recommendations.mediumPriority.length} Dateien`,
  );
  console.log(
    `- Niedrige Priorität: ${recommendations.lowPriority.length} Dateien`,
  );
}
