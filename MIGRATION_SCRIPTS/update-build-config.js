...
function updateViteConfig() {
  ...
  try {
    ...
    if (content.includes('// LEGACY START') && content.includes('// LEGACY END')) {
  ...
  build: {
    ...
    if (content.includes('build: {')) {
      ...
      const insertPos = content.indexOf('resolve:') > 0 
        ? content.indexOf('resolve:') 
        : content.indexOf('plugins:');
      
      if (insertPos > 0) {
        content = content.slice(0, insertPos) + optimizedBuildConfig + '\n  ' + content.slice(insertPos);
        updated = true;
      }
    }
  ...
  optimizeDeps: {
    include: [
      ...
      const insertPos = content.indexOf('build:') > 0 
      ...
      if (insertPos > 0) {
        content = content.slice(0, insertPos) + optimizeDepsConfig + '\n  ' + content.slice(insertPos);
        updated = true;
      }
    }
    ...
    if (updated) {
      fs.writeFileSync(`${configFiles.viteConfig}.bak`, fs.readFileSync(configFiles.viteConfig));
...
function updatePackageJson() {
  ...
  try {
    ...
    // Backup erstellen
    fs.writeFileSync(`${configFiles.packageJson}.bak`, JSON.stringify(pkg, null, 2));
    ...
    for (const scriptName in pkg.scripts) {
      if (legacyScriptPatterns.some(pattern => scriptName.includes(pattern))) {
...
function updateBuildConfigurations() {
  ...
  console.log('\n📋 Zusammenfassung:');
  console.log(`- Vite-Konfiguration: ${viteUpdated ? '✅ Aktualisiert' : '⚠️ Keine Änderungen'}`);
  console.log(`- package.json: ${packageUpdated ? '✅ Aktualisiert' : '⚠️ Keine Änderungen'}`);
