...
function findAllFiles(dir, excludes, extensions) {
  ...
  try {
    ...
    console.error(`Fehler beim Lesen des Verzeichnisses ${dir}:`, readErr.message);
  }
...
function isFileReferenced(filePath) {
  ...
  try {
    ...
    const result = exec(`git grep -l "${fileName}" -- "*.js" "*.ts" "*.vue" "*.json" "*.html"`);
...
function findUnusedFiles() {
  ...
  fs.writeFileSync('unused-files-report.json', JSON.stringify(results, null, 2));
